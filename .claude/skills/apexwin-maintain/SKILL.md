...
- 🚨 **E5-窗（2026-09-12 維護軌·U38 事故後補·必讀）**：**0 筆輸出只證明「那個窗裡沒有」，不證明「這件事沒做」。**
  U38（08-30 開）宣稱競技場兩張房卡未包 `HL.dom.pressable`，證據是 `sed -n '43,70p' arena.js|grep -c pressable`＝0 與 `sed -n '94,120p'`＝0（兩個**工廠函式體**）。
  兩個 0 都是真的——而 `pressable` 在**呼叫點**（`arena.js:41` 的 `roomCard`），結構上就在那兩個窗**之外**；
  缺陷其實早在 **07-18（U8·`aa1d99d`）** 就修好，本卡**開錯並在佇列裡當了 13 天的假阻塞**（理由寫「卡首屏 #118」，讀起來太合理所以沒人回頭量）。
  ⇒ 這是 CLAUDE.md §4 形狀⑦ 的鏡像：不是**鎖空綠**（性質沒成立而全綠），是**審計空紅**（性質已成立而判它沒成立），成因同構＝**射程排除了真相所在的位置**。
  **開卡前三個必答（任一答不出來就不得入列）**：
  ① **我的窗有沒有涵蓋「呼叫點／消費端／包裝層」？**——修法常常不在定義處。凡「某節點缺某屬性/某包裝」的宣稱，一律**同時**查：誰生產它、誰消費它、出口交出去的是哪一個。
  ② **我用的是鄰近性還是呼叫圖？**——「前後 N 行內沒有 X」對 wrapper-at-call-site 這種形狀**結構上就答不了**（本輪複跑通用版鄰近尺，21 個站點仍恰好重演同一個假陽性）。
  ③ **有沒有第二份台帳講相反的話？**——`intel/arena-inspection-2026-09-07.md:1203` 把同一件事列在**無問題項**，與 DEBT 對立了 5 天而兩邊互不查對。開卡前 grep 一次 `intel/*.md` 的同名表面。
  **另**：卡若長期掛「blocked/byte-blocked」，**每次輪到該維度時要重驗阻塞前提是否還成立**（U38 的阻塞前提「要加 36B」本身就建立在假缺陷上；而它真正缺的那一半＝常駐鎖，住在不出貨的 `tests/`，**從來就不需要首屏位元組**）。
...
- **db/ 新鮮度＝必用可重現計算，禁止目測（2026-07-30 M1 修正）**：舊述「`last_verified` 是否大面積過期」有三個病根——① `platforms.json` 用的欄位是 `next_due`/`last_investigated`（非 `last_verified`）；② 「大面積」無門檻＝每輪目測、連 6 輪誤報「未大面積 stale」（實際 07-28 曾 81% 逾期，該響的警報從未響）；③ 未排除**已停運/停輪替站**（`popularity_note` 含 `DEFUNCT/已停運/死站` 或 `refresh_interval_days>=180`＝刻意 park，如 mega-frenzy 設 365d/next_due 2027，非逾期）。**改用此 node 一行實測逾期率**（對 `platforms.json`，排除 defunct）：
  ...
  ```
  **門檻**：live 逾期率 **>30%**、或任一 **tier-1/2 站逾期 >7 天** → **判「新鮮度警報 ON」**，在 journal 記實測數字 + 逾期清單，並在 CONTROL 船長指令區點名對應軌加速重驗（不代跑）。低於門檻仍**必須在 journal 記下實測百分比**（不可只寫「未大面積 stale」＝那正是被誤報的空話）。
- **games-catalog / providers 新鮮度＝呼叫 `intel/tools/catalog-freshness.js`，禁止自寫規則（2026-09-12 維護軌·G6 判準重寫）**：舊判準（`status==='candidate' && 距今 > stale_days`）於 2026-09-12 被遊戲軌**逐筆分類**當場拆穿：它叫出的 30 個名字是 LOW 15／LOW-MED 11／NONE 1／UNASSESSED 2／MED 1 ⇒ **29/30 早就下過「非建置候選」的定論**，而那句定論講的是**我方 `built[]` 的覆蓋關係、不隨日曆腐壞**（拿日曆去催＝把同一句話重打一遍＝`ban_busywork_heartbeat` 要擋的事；08-05~08-14 四輪 G6「重驗」有大半是這樣過去的）。同時它寫死 `status==='candidate'` ⇒ **`specd`（建置管線頂端）被結構性排除**，而四筆 specd 有三筆同樣逾期（`outsourced-2` 9.2d＝我方宣告的第一順位建置候選、`space-knight` 10.2d、`bubble-up` 7.2d），**警報一聲都不出**；第三個盲點是 `if(!c.last_verified)continue` 把「完全沒驗過」靜默跳過。⇒ **最大聲的 29 筆無關緊要，唯一該催的 4 筆它看不見**（與同日平台軌「收銀台的尺只量得到儲值側」、維護軌 U38「審計空紅」同構＝**射程排除了真相所在的位置**）。**新判準分兩個獨立的軸**：**軸 A（日曆）**只收「內容取決於外面世界」的條目（`specd`／`upcoming_watch`／尚未下定論的 `candidate`；缺 `last_verified` 視為無限舊；`novelty` 缺漏或看不懂一律 **fail-open** 進軸 A）；**軸 B（版次）**給已下定論的那一大群——它們不進日曆，但定論日期早於 `built[].fidelity_verified` 最大值（＝我方最後一次上線新遊戲）時列出來重算，**且只對仍是 `candidate` 的列開火**（`status==='built'` 的列是紀錄不是待辦＝`g6_hygiene` 早就裁定的已知假陽性）。**跑這個，不要自己重寫**：
  ```bash
  node intel/tools/catalog-freshness.js          # 人讀報告；--json 給後續工具吃
  ```
  **門檻**：**軸 A stale > 0**（且非遊戲軌本週明文延後者）、或 provider stale 佔比 >50% → 在 journal 記實測數字並在 CONTROL 船長指令區點名遊戲軌（**不代跑**——這是遊戲軌的研究節奏，維護軌只提報）。**軸 B 有列＝提報「定論可能過期」，不是催新鮮度。**低於門檻仍**必須在 journal 記下軸 A／軸 B／provider 三個實測數字**。⚠️ **這把尺由常駐鎖 `platform/catalog-freshness-ruler` 守著**（`prototype/tests/checks-platform.js`，15 條負向擾動）：任何人想「順手改一下判準」都會當場轉紅——包括把 specd 的豁免拿掉、把 fail-open 反轉成 fail-closed、把軸 B 退化成第二條日曆。
