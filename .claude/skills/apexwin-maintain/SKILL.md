...
- **games-catalog / providers 新鮮度＝必用可重現計算，且必排除 built-status candidate（2026-09-06 M-freshness 修正·同 M1 排除 defunct 的病根）**：舊述「`games-catalog.json` 有 entry `last_verified` > `stale_days` → 警報」**沒有可重現命令、也沒排除已復刻的 candidate** ⇒ naive 掃 candidates 得 13/52 stale，但其中 **8 筆 `status!=='candidate'`（已 built，復刻時未回填 `last_verified`）＝佇列指向已完成的工作**（同 M1 platforms 家族「指標指向已 park/已做完的事」）。遊戲軌早於 `db/games-catalog.json:g6_hygiene_2026_08_10_2200` 就裁定 **actionable 指標＝`status==='candidate' && stale>stale_days`**，但本 SKILL 的健檢從未同步 ⇒ 每輪照舊會誤報。**改用此 node 一行實測**（於 repo 根；無 heredoc/無反引號＝§10.2 安全）：
  ```bash
  node -e 'const g=require("./intel/db/games-catalog.json"),p=require("./intel/db/providers.json"),n=new Date(),D=7*864e5;let ca=0,cb=0,pr=0;const al=[];for(const c of (g.candidates||[])){if(!c.last_verified)continue;if(n-new Date(c.last_verified)>D){if(c.status==="candidate"){ca++;al.push(c.slug)}else cb++}}for(const it of (p.providers||[])){if(it.last_verified&&n-new Date(it.last_verified)>D)pr++}console.log("actionable candidate stale (status=candidate,>7d): "+ca+(al.length?" ["+al.join(",")+"]":""));console.log("built-status candidate stale (excluded per g6_hygiene): "+cb);console.log("provider stale >7d: "+pr+"/"+(p.providers||[]).length)'
  ```
  **門檻**：**actionable** candidate stale（`status==='candidate'`）> 0 且非遊戲軌本週明文延後者、或 provider stale 佔比顯著（>50%）→ 在 journal 記實測數字並在 CONTROL 船長指令區點名遊戲軌（**不代跑**——這是遊戲軌的研究節奏，維護軌只提報）。**built-status candidate 的 stale 一律不計入警報**（已知假陽性）。低於門檻仍**必須在 journal 記下三個實測數字**。
