...
- [ ] **base 元件的隱含情境假設不要外洩**。`.ax-btn-primary` base 是 `width:100%`（為手機遊戲面板全寬 CTA 設計）——**一旦內嵌到橫向 flex（橫幅/工具列/inline）就會撐成一整條**。橫向情境要用 `width:auto; white-space:nowrap; flex:0 0 auto`（或加 modifier `--inline`），並讓文字區 `flex:1; min-width:0`。
...
- [ ] **組字/動態字串**（「最高 5×　RTP 98.5%」「開出 0.4×」「第 N 次」）walker 無法逐節點翻——盡量拆成可翻的固定片語 + 純數字節點，或明知留中（記到對應 debt，如 U19）。
...
- **修法方向**：橫幅內按鈕用 `width:auto; white-space:nowrap; flex:0 0 auto`（或新增 `.ax-btn-primary--inline` modifier），`.ax-tny-banner__l{flex:1; min-width:0}`。以後任何把 `.ax-btn-primary` 放進橫向容器都要走這條。
