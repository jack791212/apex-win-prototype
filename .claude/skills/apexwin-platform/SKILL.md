...
- 去重：已在佇列/已 ✅/在 CONTROL.avoid 的剔除。
  ...
  - ⚠️ **開卡時要把來源模組 evidence 裡的「阻塞事實」一併抄進卡**（2026-08-15 20:00 窗實例）：#94 的不變量寫「rtp 值必須與
    `games-catalog.json` 的 `gate_log` 對得上」，但 `gate_log` 是**自由散文**、且 `intel/` 不被前端服務 ⇒ **那條鎖沒有錨點可以鎖**。
    而該模組 08-07 的 evidence **早就寫著**「各遊戲的 RTP 只以顯示字串存在＝不可查詢」——阻塞條件本來就在台帳裡，開卡時沒讀進去。
    ⇒ 結果是**實作輪必須重新發現一次**，並當場多開一張前置卡（#98）。**開卡前掃一次來源模組 evidence 裡的「不可／零命中／只以…存在」句式**，
    有的話要嘛寫進卡的依賴、要嘛先開成前置卡，別讓它變成實作輪的意外。
