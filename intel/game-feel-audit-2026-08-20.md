...
**進度（2026-08-20 前景 Wave 1 + G7）**：✅ 18 條已修 ｜ 🏗️ 1 條部分修（keno 已修、dice-duel 未動） ｜ 其餘 ⬜ 待做。修法一律附常駐結構鎖（`node prototype/tests/run.js` 201 項全綠、負向擾動 15/15 全被抓）。另：Plinko 倒飛（`03bbaf4`）與 Plinko 併發＋#103 RTP 溢出（本輪）不在下表——它們是船長直接目視回報、非本次巡檢產出。
