...
/* ⭐ ③ **認領型**（2026-09-15 平台軌 20:00 窗補·第三種形狀，而它正是本輪漏掉的那一筆）：
 *   「已由 **#175（⬜待批准）** 認領，不重複開卡」——語意與②完全相同（缺口寄在別張卡上），
 *   只是動詞不在表上。本輪審〈功能／PWA 離線〉時，那一格的 evidence 逐字寫著 PRECACHE
 *   「仍恰 4 筆、可執行資產 0 筆」，而 **#175 已於 2026-09-14 落地**（sw.js 改成 install 當下解析
 *   index.html、逐筆容錯快取 94 支 script）⇒ 台帳整整高報了一個自己已經關閉的缺口，
 *   而 sweep **雙向都回報 0 筆**。實測全庫有 **12 筆**這種引用（認領 ×10／承接 ×1／負責 ×1），
 *   其中 **3 筆的卡已完成**（#175 與 `活動` 的兩筆 #82）。
 *   ⇒ CLAUDE.md §4 形狀⑦-(h)「尺的量程漏了一段」的第三次重演：**這把尺量的是動詞，不是語意**，
 *   而動詞表是列舉式的。列舉式的表只能靠「每次發現漏網就補一格」維持——所以每補一次都要記下來。 */
function citedCards(ev) {
  var ids = [], kind = {};
  [[/開卡\s*\*{0,2}#(\d{1,3})/g, "開卡"],
   [/(?:掛在|寫進既有|寫進|併入|已開的|改由|轉由|收在)\s*\*{0,2}#(\d{1,3})/g, "委派"],
   [/\*{0,2}#(\d{1,3})\*{0,2}\s*(?:底下追蹤|底下)/g, "委派"],
   /* ⚠️ 否定形要排掉：首版只打動詞，於是「依 **#82** 先例（同一筆**未認領**記載三次即應升格為卡）」
    *   被讀成「#82 認領了這個缺口」——實測 3 筆告警裡有 2 筆是這樣來的（誤報率 2/3）。
    *   ⇒ 動詞前面若是 未／尚未／沒／不 一律不算。**列舉式的表除了會漏，也會多。** */
   [/\*{0,2}#(\d{1,3})\*{0,2}[^。；\n]{0,14}?(?:認領|認養|接手|承接|負責)/g, "認領",
    function (whole) { return !/(?:未|尚未|沒|不)(?:認領|認養|接手|承接|負責)/.test(whole); }]
  ].forEach(function (pair) {
    var re = pair[0], ok = pair[2], m;
    re.lastIndex = 0;
    while ((m = re.exec(ev))) {
