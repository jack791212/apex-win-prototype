...
## 🔄 2026-09-05 刷新（平台軌 08:00 窗 · tier-1 到期 · 本輪取材鏡頭＝**資料**分類 + 新開維度 12）

> 本輪與 Stake 同批深挖（兩站皆 08-15 後到期、priority 90/96）。**維度 5–12 逐項作答**（清單缺一項就永遠看不到，見 sourcing-methods 第 5 條的教訓）。

### ⭐ 維度 12（本輪新開）：玩家的自我統計面 ＝ **本站有，而 Stake 沒有**
- BC.Game 的帳戶頁把 **Transactions / Bill / Game history** 分成三個分頁；評測記載玩家可「檢視完整遊戲歷史、追蹤輸贏、並用**內建分析工具**監看自己的活動」。
- 第三方追蹤器的描述進一步指出其歷史逐筆帶 **遊戲型別／押注額／結果／日期**，並有**圖表化的分析儀表板**，可**依時間區間或遊戲型別產生自訂報表**。
- ⇒ **期間軸在這裡是「篩選 + 出報表」的一層**，不是第七張新報表——與 09-02 從 Stake 取到的結論（「期間軸應掛在篩選那一層」）**獨立收斂到同一個形制**。這對 #151 是第二個外部座標。
- **匯出（維度 9）**：原生匯出**來源未載**；生態上是第三方（mybets.gg 的 Chrome 擴充抓 betslip）在補。⇒ 與 Stake 的「原生匯出很完整、閱讀外包給第三方」**恰好相反**：BC.Game 是**讀得懂但拿不走**，Stake 是**拿得走但讀不懂**。兩個標竿站各只做了一半。

### 維度 11：帳戶安全與隱私的自助面 ＝ **本庫目前最完整的正面形制**
- **2FA**：支援 Google Authenticator／Authy 等標準 app，且**不只擋登入——提款等敏感動作也要輸碼**；另有 **passkey 登入**。
- **登入活動面**：評測明載玩家可「定期檢視自己的**登入紀錄與作用中的 session**，並**登出不認得的裝置**」。
  ⇒ 這正是 09-04 20:00 窗記下的那個組合缺口（Thrill＝有 2FA 但無登入活動面）的**正面對照**，也是 #125 目前最完整的一份形制來源（Punkz 只有「新裝置 email 告警」一項）。
- **提款目的地白名單／改密碼後提款鎖**：來源未載。**站內可見度自控（隱身/隱藏統計）**：來源未載。

### 維度 5：玩家保護
限額可自 profile settings 直接設定；**自我排除自 6 個月起**、**7 天 cooling-off**、以及**永久關閉帳戶**三段並列。
⇒ 與 Stake 的「僅 indefinite（最短 6 個月）」相比，BC.Game 提供的是**分段式**選項——#96 的期間設計可對照這一側。

### 維度 6：社群治理／共決 — **無**（未見玩家對平台本身投票的機制；社群面是聊天室/rain/論壇）
### 維度 7：在地化 — 本輪未取到形制（多語支援存在，語言與貨幣是否同一軸、切換器位置**來源未載**）
### 維度 8：創作者/工作室做為目的地 — **無**（8,000+ 遊戲庫有供應商名單，未見供應商目錄頁或工作室檔案頁）
### 維度 10：活動的收尾面 — **來源未載**（任務/Lucky Spin/Coco Find 皆為進行中的鉤子；賽後往期成績、得獎名單、告知方式逐項無記載）

### 留存/經濟（reconfirm，非本輪重點）
VIP **140+ 級**；**返水自 VIP 14 起解鎖**，公式＝`總押注 × 莊家優勢 × VIP 百分比`、**每次遊戲階段後即時入帳、零流水要求**。
⇒ 與 ApexWin #60（返水改以莊家優勢計價）**同一個公式形狀**——這是我方 07 月自審導出的設計，本輪首次取到外部同構佐證。

### 對 ApexWin 的意義（本輪）
1. **維度 12 的缺口在我方是「有數字、沒有面」**：見 `platform-modules.json` 新增的「玩家自我統計面」格與本輪開的卡。
2. **返水公式已有外部同構**：#60 不再是孤例，可在 `db/platform-modules.json` 的金流/功能格補上這條佐證。
3. **#125 的形制來源自此有三站**（Punkz 告警／Thrill 反面／BC.Game 完整三件套：2FA+敏感動作二次驗證+session 清單）。

**來源**：CCN／cryptoslate／worldpokerdeals／btcgambling／bitcoin.com 的 2026 BC.Game 評測、BC.GAME Help Center（reglog）、bccasino.org 責任博弈頁、aorda／mybets.gg 的 bet-tracker 說明。**只記共識與形制，不引用絕對數字。**
