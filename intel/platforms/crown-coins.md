...
## 🔄 2026-08-25 刷新（平台軌·14:00 窗 · tier-2 到期 · 換「VIP 階級待遇」維度）

**刻意換維度而非重掃上次那 9 項**——08-11 已逐項對照過一次，同一份清單抄第三遍就是 busywork。
本輪問的是「**階級給的是什麼待遇**」（前兩輪只記了階數與 coinback%）。

- **⚠️ 一處本庫記載更正**：VIP **不是五階，是 6 階**——`Entry / Bronze / Silver / Gold / Diamond / Emerald`
  （前輪記「VIP 五階、coinback 2–6%」，階數少記一階；coinback 區間無誤）。
- **日登階梯數值刷新**：第 1 天 5,000 CC → 第 7 天 40,000 CC + 1 SC → **第 30 天累計 412,000 CC + 7.8 SC**。
  ⇒ 與本檔既記的「30 天階梯、里程碑 8/15/22/30」形制一致，數量級亦同 ⇒ 無新增量。
- **四項階級待遇 → ApexWin 逐項對照，全數已有出口**：

| Crown Coins 階級待遇（實查） | ApexWin 現況（機械證據） | 判定 |
|---|---|---|
| 更大的每日登入獎 | `core/rewards.js` LADDER 遞增 + 里程碑 8/15/22/30 + #84 連簽容錯 | 已覆蓋（更完整） |
| 月獎 + 生日獎 | `core/reload.js` 日/週/**月**三週期齊備；生日獎 `grep birthday` 仍 **0** | 月獎已覆蓋；生日維持**刻意不開卡**（需生日欄位＝近 KYC、價值低） |
| **更快的兌獎處理** | `core/service-level.js` `HL.sla` 逐段位時效+額度，且 `sla/monotone-service` 鎖住「隨段位單調不變差」、`sla/live-not-looser` 鎖住真站不寬鬆 | 已覆蓋（**且已上鎖**） |
| **商城高階品項解鎖** | `core/shop.js` #123 受眾閘已落地（`HL.release.matches` + `isGoalAudience` 裁決「該藏 vs 該鎖」） | 已覆蓋 |

⇒ **本輪對 ApexWin 增量＝0（本平台首次零淨新）**。08-11 那輪並非零增量（產出 #88 酬賞負載軸），
