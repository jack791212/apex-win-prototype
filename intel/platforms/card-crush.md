...
## 2026-08-07 複查（首挖後 28 天 · 平台軌 08:00 窗）

四項招牌（卡牌對戰／稀有度四階／重複卡升階／ELO 榜）**全數複驗成立**，但查出三筆首挖漏記的機制：

### ⭐ ① 社群彩池＝可選加注的「資格軸」（本輪最強新素材 → 開卡 #78）
- 機制：slot 每轉**額外付 0.1 MC** 才取得四級彩池（**Mini / Minor / Major / Grand**）的**中獎資格**；官方保證池 **≥50,000 MC**（評測實測期間見 **108,000+ MC**）。
- ApexWin 對照（grep 實證 `core/jackpot.js`）：`onBet()` 從**每一筆下注隱形抽成**（假站 mega/major/mini＝0.5%/0.3%/0.2%、真站 0.15%/0.1%/0.05%），命中判定對**所有**下注無條件生效。
  ⇒ 玩家 **① 不知道自己在付 ② 不能選擇不付 ③ 也沒有「因為付了所以有資格」的對價感**。三級 vs 四級只是數量差異，真正缺的是**資格軸**。
- 這同時是**透明度**議題（#72 說明中心的天然條目）與**負責任博弈**議題（#67 的家族：把隱形成本變成明示選擇）。

### ② 賽季制卡組會過期 → **刻意不開卡**（反向結論）
- 事實：「若你在 Season 1 或 2 註冊，你的 Card Collection 已不能再用於對戰」；Season 3 起改為每週發卡。
- 不開卡理由：ApexWin 的 #45 成就是**終身**、#46 賽季獨立計算，且 **jackpotter（2026-08-06 取材）才剛佐證「XP 與段位永不重置」是被當成行銷賣點的**。讓收藏資產過期屬玩家敵意設計，與本站已對齊且更優的方向相反。

### ③ 忠誠點來自「每日自動對戰」→ 部分已對齊，只留一條給 #65
- 事實：七段忠誠（**Iron → Bronze → Gold → Platinum → Ruby → Diamond → Black Diamond**），Club Points 由**每日自動舉辦的 10 場 Card Battles** 結果餵養（玩家無須手動報名）。
- 已對齊：ApexWin `HL.tournament.record` 掛在中央結算點＝自動入榜本就成立。
- 仍缺的那一層：**進度來自「勝負」而非「押注量」**。#65 `HL.progressSrc` 現有三筆來源（wager／deposit／checkin）**全是量型**，勝負型（skill-based）來源類別尚不存在 ⇒ 寫進 #65 的後續來源候選，不另開卡。

### 自我更正一處首挖的過度推論
- 07-10 記「Mystery Coins 一律購買取得」→ 實則**每日 Quests 與 Mystery Drops 也會發免費 MC**（惟 Quests 需先花 MC 遊玩＝仍是購買驅動的循環）。改記為「主要途徑為購買，另有小額免費發放」。

## 排程
- 首挖回填：`status: done`、`last_investigated: 2026-07-10`、`next_due: 2026-08-09`（tier3 +30 天）。
- 本輪複查：`last_investigated: 2026-08-07`、`next_due: 2026-09-06`、`last_verified: 2026-08-07`、`confidence: medium`。
