...
## 🔄 2026-08-25 刷新（平台軌·14:00 窗 · tier-2 到期 · 換「玩家保護/責任博弈」+「帳號安全」兩維度）

**為何換維度**：08-11 那輪已把 Promo Points／opt-in／非投注來源／VIP 八階四項訊號逐一 grep 覆核完畢，
再掃同一份清單＝busywork。本輪改用 2026-08-16 才補進取材清單的「**玩家保護**」維度
（該維度補進來之前，35 份 dossier 對責任博弈的命中數是 0），並順帶開「**帳號安全**」這一格。

### ⭐ 本輪結論與預期相反：限額面是 **ApexWin 更完整**

| 1xBet 2026 載明（實查） | ApexWin 現況（機械證據） | 判定 |
|---|---|---|
| 存款限額（帳戶設定可調） | `responsible.js` `deposit-daily/weekly/monthly` 三週期，`rg/deposit-gate`＋`rg/deposit-period-rollover` 鎖著「跨日只清日桶」 | 已覆蓋（**更細**） |
| 強制工作階段限額 | `time-daily`（每日遊玩時間上限，`IDLE_CAP_MS` 防掛機灌時數） | 已覆蓋 |
| 現實檢查（間隔可設，輸贏皆顯示，可選繼續/停止） | `maybeRealityCheck()` + `setRealityCheck(on, everyMin)` | 已覆蓋 |
| 自我排除 | `registerPause` 冷靜 24h/7d/30d + 排除 6m/1y/5y/永久，`rg/self-exclusion-monotone` 鎖住「只能加長、不可提前解除」 | 已覆蓋（**更完整**） |
| 活動紀錄（時間·注額·結果） | `core/betlog.js` 277 行、`HL.betlog.record` | 已覆蓋 |
| — （1xBet 未載）| **「調降立即生效、調升須等 24h」的不對稱冷卻**（`rg/raise-cooldown`，本檔明載為「最重要的不變量」） | ApexWin **淨多一項** |

### 1xBet 淨多的兩項 —— 兩項都**刻意不開新卡**（去重理由寫明）

1. **鎖定特定遊戲品類**（1xBet 可在使用者面板鎖某類遊戲）。ApexWin 的暫停是**全站粒度**，品類粒度為 0。
   但 `evaluate(limits, st, amount, now, coolUntil, axis)` **已有 `axis` 參數**（儲值軸即由它切出），
   且 `HL.gameAxes` 已有分群軸 ⇒ 這是「**加一筆 spec**」而非容器缺口，
   照 SKILL「擴充性優先」的判準不值得單獨開卡。記此處備查，若哪天 RG 面板要做品類頁再併入。
2. **裝置/session 清單 + 2FA + 五層帳號保護** ⇒ **已是 #125**。
   去重依 SKILL 的判準（比對**來源模組 + 出口形狀**而非卡名）：來源模組同為「資安／帳戶安全自助中心」、
   出口形狀同為「可註冊的安全項目清單」、首批註冊者同一批（2FA／登入活動／裝置清單／登出所有裝置）
   ⇒ 實質重複，**不開雙胞胎**。本輪的貢獻是給 #125 補上**第一個非 Stake 的對手形制**
   （#125 原本只有 Stake Help Center 一個外部對照）。

### ⭐ 本輪唯一淨新不在上述兩維度，而在**通知層**（⇒ 開卡 #128）

1xBet 的三項描述共享同一個前提——**通知是玩家可分類、可設定的**：
「可設投注速率告警門檻（5 分鐘 5 注／單日 $2,500）」／「現實檢查間隔可設」／「跨所有已登入裝置的工作階段監控」。
反查 ApexWin：`grep -c "notify.add(" src/**` ＝ **37 個呼叫點**，而 `add(n)` 收的是 `{ ic, title, text }`
——**沒有任何分類維度**（無 `cat`/`kind`/`aud`），`HL.notify` 表面 ＝ `{ add, open, unreadCount, markAllRead, refreshBadge }`，
`grep -riE "notifyPref|通知偏好|unsubscribe|退訂"` 全 `src` ＝ **0**。
⇒ 37 個生產者共用一條**扁平**串流，玩家一顆開關都沒有。詳見 #128。

⇒ **本輪對 ApexWin 淨新 1 項（#128）**；`saturation_watch` 因此**維持在 1/2、計數不推進**
（本輪並非零增量——只是增量落在別的維度上）。**這正是「換維度」的價值**：
