...
- **調研日期**：2026-09-05（刷新）／ 2026-07-17（刷新）／ 2026-07-10（刷新）／ 2026-07-03（刷新）／ 初調 2026-06-26
- **tier**：1（全球頂級 · 7 天刷新）
- **regions**：global
...
### ⭐ 維度 12（本輪新開）：玩家的自我統計面 ＝ **本站沒有，而且缺口被一整個第三方生態填掉了**
- 站方給的是 `stake.com/my-bets/archive`（也有 `stake.us/...`）的 **bet archive：逐期 JSON 檔**，內容是「你下過的每一注」、可自行驗證真偽。
- **但站上不提供任何「讀懂它」的表面**——圍繞它長出的是一整圈第三方工具：
  - **StakeStats** 的 archive organizer/downloader（免訂閱即可用）
  - **StakeArchiveDownloader**（GitHub 腳本：把每一份 archive 抓下來、依日期改名）
  - **Stake Analyzer／Stake Archive** 的線上檢視器（全文搜尋、依遊戲篩、盈虧分析）
  - **Spindex P&L Calculator**（吃 Deposits／Withdrawals 兩份 CSV，算精確盈虧、月 ROI、**逐幣別分解**）
- ⇒ **這一條是本輪最重要的形制**：Stake 在維度 9（拿不拿得出去）**滿分**，在維度 12（看不看得懂自己）**零分**，而市場用第三方把後者補起來了。
  **「可攜性」與「可理解性」是兩件事**——維度 9 只問了前者，這正是本輪必須把維度 12 獨立開一格的理由。
  對照組 BC.Game 恰好相反（有內建分析面板、原生匯出來源未載）⇒ **兩個標竿站各做一半，沒有一家兩者都有** ⇒ 對 ApexWin 是低成本差異化位置。
- **維度 9 補記（修正 09-02 的記載口徑）**：09-02 記「Deposits／Withdrawals 分頁各按一次 Export CSV、注單是另一套 JSON」——本輪複驗**成立且更完整**：那兩份 CSV 正是 Spindex 的輸入格式，而 JSON archive 是**分期檔**（故才需要 downloader 腳本逐期抓）。⇒ 「連標竿站都沒有一份對帳單」這句話**再次成立**，且成因更清楚：**它連「一份」都不是，是逐期多份**。

### 維度 5：玩家保護（本輪取到一組**來源互相矛盾**，依紀律記為未定）
- **限額**：存款／損失／單注限額齊備；**調降/設定即時生效，調升或移除要過 24 小時冷卻**（這一條多來源一致＝共識）。
- **自我排除期間**：**官方 Help Center** 寫的是「**Indefinite（不定期），自申請日起最短 6 個月**」，且**不會自動復啟**，需書面申請後經內部「return-to-play review」；而**評測站**寫「**6 個月至 5 年**可選」。⇒ **一手與二手不一致，記為未定**（比照 09-04 Chancer 2FA 的處理）。**對 #96 的意義**：不可據評測那句去做「多段可選期間」，官方形制更像「單一不定期 + 最短期 + 人工復啟」。
- 站方**得在玩家未申請時逕行自我排除**（有成癮跡象時）——這是**平台側的強制路徑**，我方 `HL.rg` 目前只有玩家自願側。
- StakeSmart 專區集中放置說明與外部求助資源連結。

### 維度 6：社群治理／共決 — **無**（未見對平台本身的玩家投票）
### 維度 7：在地化 — 本輪未取到新形制（沿用既有記載）
### 維度 8：創作者/工作室做為目的地 — 本輪未取到新形制（沿用既有記載）
### 維度 10：活動的收尾面 — **來源未載**（本輪四份來源逐項追問賽後往期成績/得獎名單皆無記載，與 09-04 兩站同型）
### 維度 11：帳戶安全自助面 — 沿用既有記載（Stake 一直是 #125/#127/#137 幾乎唯一的形制來源；本輪未取到新增項）

### 對 ApexWin 的意義（本輪）
1. **維度 12 是 Stake 的空白，也是我方的空白，但空白的形狀相反**：Stake 是「原始資料完整、無分析面」；ApexWin 是「**分析面所需的數字每一注都在累積、卻沒有出口也沒有面**」（`HL_ACHIEVE` 的 6 欄終身統計只被拿來算徽章進度條）。⇒ 我方補這一格的成本，比 Stake 低得多——**資料早就在**。
2. **匯出檔的身分（#159）再獲佐證**：Stake 的 archive 之所以需要外部腳本「依日期改名」，正是因為**檔案本身說不出自己是哪一期**。與我方 `apexwin-<id>.csv` 恆定檔名是**同一個病**。
3. **#96 的期間設計**：官方形制＝不定期＋最短 6 個月＋人工復啟；評測的「6 個月–5 年」不採信。

**來源**：Stake Help Center（self-exclusion）、stake.com/policies/self-exclusion、stake.us/responsible-gaming、askgamblers／igamingfuture 2026 評測、GitHub stakearchivedownloader、stakestats.net／stakeanalyzer.live／stakearchive.com／spindex.net 工具頁。**只記形制與共識。**
