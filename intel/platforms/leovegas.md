...
## 🔄 刷新 2026-09-11（tier-2 · `next_due` 09-09 到期票 · 逾期 2 天，因三軌 09-09~09-11 排程重置而順延）

**定位 reconfirm**：續為「King of Mobile」（MGM 旗下歐洲持牌）。核心（真金/真人荷官/KYC/體育投注）仍屬 `avoid`，無定位變動。

**淨新訊號＝0 條（連續第二輪，據實記載，不硬湊）**

本輪 WebSearch 回來的可用訊號全部是聯盟站改寫的常青內容（遊戲數 2,000+、供應商名單 NetEnt/Microgaming/Evolution/Play'n GO、
VIP 五階 Bronze→Diamond 的 5–20% 返現 + 每 €1 得 1 comp point、App「更緊湊更好讀」）——
逐條比對後**全數落在既有記載之內**（VIP 分階＝#63 服務水準軸已覆蓋；comp point＝#65 進度來源註冊表已覆蓋；
App 改版＝**純視覺、無可對照形制**且排程軌拿不到 preview，08-24 20:00 窗已機械證實）。

**維度 22（本輪新開）的提問結果＝來源未載**：LeoVegas 是**持牌營運方**，其後台（營運動作軌跡／簽核／角色）
在前台與聯盟評測中**結構上不可見**。⇒ 本維度對「營運方型」平台的取材出口是 **B2B 平台供應商**（同 08-26 那輪對「後台」分類的處置）。
本輪的對照組因此取自 Spinlab 的後台模組拆解，見 `sourcing-methods.md` 維度 22。

⇒ 連續兩輪淨新訊號 0（08-26 為 0、本輪為 0）。**依 08-26 自己留下的建議執行**：
`refresh_interval_days` 由 **14 → 21**，把配額讓給還沒被任何維度掃過的表面。
（不降 tier、不移出清單——它仍是「行動優先 + 歐洲合規」這一支的唯一標竿。）

**既有點子狀態**：「必掉落限時 Jackpot」(M)、「行動導航低摩擦審查」(S) 仍未做且仍有效（後者需 preview，續受阻）。

**下次到期**：2026-10-02（+21 天）。
