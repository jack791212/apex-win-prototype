...
## 🔄 2026-09-05 複查刷新（平台軌 20:00 窗｜逾期 1 天）

⚠️ **來源強度**：本輪**全為 WebSearch 二手摘要**（igamingfuture／next.io／wsn／thelines／livescore／sweepschaser／sportsgambler／dimers 交叉），**未直取官網**。段位中間名稱未取得者一律不臆造。

### 四筆新事實（前兩輪皆未記）

1. **營運方＝Mamba Limited（Spinfinite 的營運方）**，上線 2026-03。⇒ 補上本站的公司歸屬，之前兩輪只有「2026-03 上線的新 sweepstakes」。
2. **遊戲數 500+ → 670+**，slot 陣容點名 NetEnt／BGaming。
3. ⭐ **VIP ＝ 7 級（Blue → Diamond）、註冊即自動入會，進度貨幣為 Stars，來源三條：missions／gameplay／promotions**。
   - 前輪（08-05）只記到「每日神秘禮**可能掉出** VIP Stars」，本輪補齊全貌：**Stars 既是 VIP 進度、也是可以被當成獎品掉出來的東西**。
4. **3,000 GC 歡迎金在 email 驗證前即發**（『immediately, even before email verification』）＝FTUE 門檻比前輪記載更低。

### ⭐ 第 3 點的對照結論：這是 #65 家族的**反方向**佐證（記錄，**刻意不開卡**）

- #65 `HL.progressSrc` 解的是「**非投注行為也能算進度**」（deposit／checkin 成為進度來源）。
- Zonko 展示的是**另一個方向**：「**進度貨幣本身被當成獎品直接派發**」（每日神秘禮／促銷可以掉 Stars）。
- **ApexWin 實況**：`HL.vip.addWager` 只吃結算流水，加上 #65 註冊來源 ⇒ **沒有「把 VIP 經驗當獎品發出去」這個出口**。
- **為什麼刻意不開卡**：那等於多一條送幣（送進度）通道，與 §11「真站經濟收斂」的方向**相反**；
  #65 落地時已把真站非投注來源一律設 `xpPerLive: 0`，此處若開卡等於自打嘴巴。⇒ **只記錄形制，不進佇列**。

### 08-05 兩筆判斷的複驗

| 08-05 記載 | 2026-09-05 複驗 |
|---|---|
| 8 日軌是 **back-loaded**（權重壓第 6–8 天），非 07-02 誤記的 front-loaded | ✅ 仍成立（本輪二手續載「for the first eight days, you can also claim extra daily rewards」＋200% 首購結構未變）；**07-02 的自我更正維持有效** |
| 反向結論：我方 `rewards.js` LADDER 30 天單調遞增＋里程碑壓 8/15/22/30 天＝**已比它更強** | ✅ 仍成立（本輪未見任何超越我方的簽到結構） |

### 🕳️ 仍成立、且**仍未開卡**的那一個缺口（本輪據實再記一次）

- **「有邊界的 FTUE 多日養成軌」**：Zonko 是 8 天走完即結束；我方 `core/onboarding.js` 是 **6 小時啟用窗口**、`rewards.js` 是**永久 streak**——**兩者都不是「有邊界的多日新手軌」**。
- **2026-07-02 首次記為本站最強缺口 ⇒ 至今 65 天未開卡**（08-05 那輪的理由是卡額被 #70/#71 佔滿；本輪卡額給了 #171）。
- ⇒ **這條紀錄本身就是證據**：一個缺口被連續三輪確認成立、卻連續三輪讓路給當輪的其他卡。下一個平台輪若無新指派，這是**現成的、無需再調研的**開卡對象。

### 本輪處置

- 本站**無新卡**；上面那一條缺口維持記錄狀態，並在此標明「已連續三輪讓路」以免下一輪又重新發現一次。
- 回填：`last_investigated` 2026-08-05 → **2026-09-05**、`next_due` → **2026-10-05**、`last_verified` → 2026-09-05。
