...
1. **RTP 證明**：確認回收率在宣告 RTP ±0.5% 內、每個下注格/型別都驗。暗地賠 <95% 或 >100% = FAIL。**不接受手拍賠付表上線**。
    - **⚠️ raw 蒙地卡羅均值對重尾遊戲不可靠（2026-07-29 消化船長 G3·Plinko 實例）**：當單一稀有桶/結果主導變異（如 Plinko 16-high 頂桶 25031×、機率 1/65536），即使 1e7 樣本，raw MC 均值的 CI95 都可能達 ±數 pp——`fidelity_min_rtp_sims: 1000000` 這種固定回合數會「連 1pp 的錯都測不出」，卻報出假的 100.29%/102.4%（純抽樣雜訊，非真 RTP）。**正確作法（依遊戲擇一）**：
        - **(a) 精確解析式**（結果空間可窮舉/有封閉式者，最嚴謹）：若結果機率有解析式（如 Plinko 桶 k＝二項 C(n,k)/2^n、輪盤 37 格窮舉、骰寶 216 窮舉），直接算 `RTP = Σ p·payout`＝**零抽樣誤差**。再用中等 N 蒙地卡羅做**分布吻合檢查**（實測結果頻率 vs 理論機率，最大偏差應隨 N→0）佐證「玩的那份數學真的產生該分布」。
        - **(b) CI 收斂法**（無解析式的高波動 slot/crash）：先跑 1M 量樣本標準差 SD，再由 `N ≥ (1.96·SD / 0.005)²` 決定正式樣本數（目標 CI95 半寬 ≤ `fidelity_rtp_ci95_max_pp` 0.5pp）；`fidelity_min_rtp_sims` 僅為**下限**、非充分條件。報 gate_log 時附 CI95。
    - 純數學區必 `module.exports`，node 驗證器 require 的**就是玩家玩的同一份**（browser==node 交叉）。
...
14. **🆕 買入型入口的 RTP 必須與宣告 RTP 一致**（2026-07-28 健檢後新增·血淚條款）：任何「花 N× 直接買特色」的入口（bonus-buy / X-iter / feature-buy）與任何 side bet，都必須**獨立量測其自身的 RTP**：`買入RTP = E[買入路徑總倍數] / 買入價`，須落在宣告 RTP **±0.5pp** 內，且**絕不可 >100%**（可套利）或顯著 <宣告值（玩家暗虧陷阱）。
    - **驗法**：`node -e` require 該遊戲 module.exports，以**至少 2 個獨立種子**各跑 ≥2M 局的「強制進特色」路徑量 `E[倍數]`，再除以買入價。買入價本身必須由**程式常數**（如 `CFG.buyX`）同時驅動「按鈕文字」與「扣款」，禁止兩處各自硬編（會 drift）。
