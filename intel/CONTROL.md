...
build_lock: false         # 單一寫入鎖。空閒為 false；寫入型 routine 進場「上鎖」、收尾清回 false；進場見非 false 就讓路。
                          #   目的：避免多個排程並行寫同一個 prototype/ 造成「觸發卻未收尾」的孤兒改動。
                          #   （E7）上鎖採 claim-token：值可能是 `<前綴>-<時分秒>-<亂數>`（c-/e-/i-/r-）而非裸 true——
                          #     firing 寫入自己的 token→停頓→重讀，token 仍在才進場，防兩 firing 同時 heal 同一把 stale 鎖並行進場。
...
↳ (2026-07-17 打磨) E2 落地：原本累積於此的 103 筆例行心跳（2026-06-28 起，~145KB）已全數搬到 [loop-journal.md](loop-journal.md)。此區今後只放「對上方待處理指令的回覆」；例行心跳請看 loop-journal.md（最新在上）。
