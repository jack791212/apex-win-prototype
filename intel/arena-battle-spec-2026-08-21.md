...
- ✅ **§5 #7／#9／#10**（`eac6bd0`，2026-09-11 前景）：同一個病的三個切面——**房卡上會變的東西，只有一部分被列進每秒刷新的清單**。
  `updateCard` 拆成 `cardsOf(id)`（走 `document`、拿到**所有表面**上的同一張卡）→ `paintCard(card, r)`；刷新清單補上賞金池與場次進度；
  `tick` 新增「不在競技場頁」分支（照樣逐房刷 + 結構變動時 `sweepGhostCards` 清鬼房）。
  ⚠️ footer 多一個節點的同時，玩家數從位置選擇子 `.ax-rc-done span:last-child` 改成具名 `.ax-rc-seats`——否則新節點會靜默頂替它。
  新鎖 `games/arena/room-card-live-on-every-surface`（每條性質配一個反向錨），負向擾動 **11/11**；preview 實測大廳倒數會動、賞金池原地更新、場次 9/20→10/20、鬼房被清。
- ✅ **§5 #8**（`dc34965` 系列，2026-09-07）：`canJoin` 不重算（滿房仍寫「加入」）——已收斂成 `ctaSig` 指紋比對，鎖 `games/arena/room-cta-not-stale`。
...
- ⬜ **§2 缺的狀態**：ROOM_OPEN／SEAT_FILLING／SPECTATE〔EXPIRED 已改 🟡半：**真站押金有進無出那個 P0 已於 2026-09-15 `4bc8abd` 修掉**，仍缺「通知中心留一筆」——見上方 S15 列〕
- ✅ **§4.4 結算卡**（已落地 2026-09-15 前景 `4c28e38`）：兩欄制（分數／派彩）、平手裁決可見化、再戰一局印原價、比分矩陣
  ...
  ✅ **落地（2026-09-15 前景，`4c28e38`）**：
    · 新純函式 `CORE.rankFor(mode, entries, {order, winnerIdx, tieRoll})`——**有權威 order 就照它排，一格都不重算**；
      伺服器路徑沒有完整 order、只有 `winnerIdx` ⇒ 排完把權威贏家提到首位；兩者都沒才退回 `rankBy`。
    · `finishLocal` 傳 `{order, winnerIdx, tieRoll}`，伺服器路徑傳 `{winnerIdx}` ⇒ **兩條路徑的名次都與付錢同一份**。
    · 名次表加表頭列，量的欄名向 `BM.displayMetricLabel` 求值（terminal 會寫「本輪增量」）；另加**逐席派彩欄**
      （我那一列印權威 `net`，確保與卡面大字逐字一致）。
    · 平手行印出 `roll=0.xxxxxx`（舊版只寫「由可驗證公平抽籤裁決」而不給值＝還是要玩家相信我們）；走 `i18n.fmt`。
    · 「再戰一局（同設定）NT$X」印原價；贏敗不共用文案（贏＝綠底「收下 +NT$X」／敗＝中性「關閉」）。
    · 比分矩陣 N 席 × N 輪：`rec.rounds` 是**逐輪累計**⇒ 格子取「本輪 − 上一輪」（直接印累計就是每列單調遞增、
      看不出哪一輪爆分）；末欄標「決勝」。樣式全部**內聯**——`vsslot.js` 是延遲載入的，但 `components.css` 是 eager。
    · 新鎖 `games/arena/settle-card-single-verdict`（行為級跑 `rankFor`／`payOf` 三條路＋逐字釘兩個呼叫點＋反向錨）。
...
| **`S15 EXPIRED`🟡半**〔2026-09-15 前景 `4bc8abd`：**真站那一半是 P0，已修**。
舊版 `tick()` 開頭只有一行 `if (isLive()) return;`，而它**同時管了兩件事**：
① 別產生假活動（它的本意，注解只寫了這一件）、② 玩家**自己那間房**的生命週期。
實測（玩家真實路徑）：真站開一間賞金房 → 精靈逐字寫「Demo · 不扣真錢」而餘額 200,000 → 149,000（真扣 51,000）；
卡片 ⏱ 永遠停在 1:00:00；跑 7,200 次 tick 後房間還在、餘額逐位不動 ⇒ **押金有進無出**。
正向對照：假站 3600→3564→…→房間結束、149,000→191,600。
**修法**：`live` 只閘「假活動」（假挑戰者、自動生成假房），倒數與到期退款照常；
另加「無房才早退」避開真站每秒寫一次狀態（**那一行必須帶 `!rooms.length`**，光看 `live` 就是把缺陷裝回去）。
`chargeTag()` 讓真站**說實話**而不是把標籤藏起來；結算卡分得出「打完」與「到期」並印出退了多少。
新鎖 `games/arena/live-room-still-expires`（**行為級**：把 `tick` 函式體抽出來注入假 HL 真的跑）。
負向擾動 **13/13 CAUGHT**，其中 **L0＝直接還原成 HEAD（原始缺陷本人）也轉紅**。
⚠️ **仍缺的那一半**：規格要的「通知中心留一筆」未做——現在只有彈窗，錯過就沒了。
⚠️ **同輪發現、刻意不在本卡射程**：`arena.js` 有 **0 個** `HL.ledger` 呼叫却在三處動餘額
（開房 −押金−開房費、結束 +賞金池／+net）⇒ **2% 平台開房費是平台收入卻完全不進 GGR**。
加帳本型別會動到 `derived()` 與營運儀表板的數學，屬獨立一張卡；**半做比不做糟**。〕 | 「房間已過期，已退款 NT$X」（通知中心留一筆） | 重新開房 | →`S0` |
