...
## 6.6 第三波（2026-09-08 · 船長裁決「先做零首屏那批」）

首屏餘裕只剩 157B ⇒ 本波**只碰不吃首屏的檔**（`views/vsslot.js` 延遲載入、`tests/`、i18n 語言包）。
四條全是「玩家看得到的說謊」：

| | 缺陷 | 修法 |
|---|---|---|
| ① | **平手被渲染成互相矛盾的兩句**：名次徽章走 `order.indexOf`（平手時由**建立順序**決定）寫出 `#2/3`，而同一席的差距文字寫「並列第一」 | 改**競技排名**（比我嚴格好的人數 +1，比較子仍向 `BM.better` 求）⇒ 同分必同名次；領先高亮給**每一個**榜首平手席（原本只有 `leaderIndex` 那一個＝平手時謊稱有人獨走） |
| ② | **平手完全不可見**：`BM.tieAtTop` 有定義、有測項，全 repo **零呼叫** | 對戰中寫「並列第一（N 人）」；結算卡在榜首平手時寫「🤝 榜首平手 N 人 · 由可驗證公平抽籤裁決」——那個籤本來就是 `HL.fair` 抽的，只是玩家看不到 |
| ③ | **轉輪期間顯示的是上一輪的值**（terminal 連主數字都是） | `spinning` 期間本輪那一行改「本輪進行中…」、terminal 的主數字欄名標成「上一輪增量」；起轉當下就刷 |
| ④ | **結算卡名次表自寫排名量**（`room.mode === "terminal" ? o.last : o.total`）＝第 6 個表面，而防它的鎖只認舊字面 | 改問 `BM.metricOf`；並補一行「名次依「總分／本輪增量」排」讓那一欄有欄名 |
| ⑤ | **i18n 零容忍棘輪的射程漏掉 `core/battle-mode.js`**（它自己託管 selftest spec ⇒ 落進 `SPEC_HOSTS` 整檔排除）⇒ 模式名與勝負條件實測 EN/zh-Hans 命中數**皆為 0**，而那把「零容忍」的尺全綠 | 補 11 條字典（三語）＋立**不依賴那把尺**的鎖：直接把宣告值從 `battle-mode.js` 原始碼撈出來逐條查字典，並帶反向錨防自己塌成空集合 |

⑤ 的射程口徑是平台軌的檔（正解＝#122 逐宣告判別），**我刻意不改那把尺**，改用不依賴它的鎖。

### 鎖與擾動
改寫 `games/arena/in-play-standings`（同一組不變量、新形狀，未放寬）＋新鎖 `games/arena/battle-mode-i18n`。
**負向擾動 15/15 CAUGHT**（三波合計 **68/68**）。
⭐ **第五條被擾動抓出來的空綠鎖**：`S1` —— 我把斷言寫成「`BM.better` **或** `BM.rankBy` 都算過」＝自己放寬，
於是「退回 `rankBy(...).indexOf(e)+1`」照樣全綠，而那正是缺陷本身。
⇒ 改成釘競技排名的形狀＋反向錨 `hasNot(rs, "indexOf(e)")`。
另 `S10` 也踩到同族：`vs.indexOf("spinning = false;")` 會先命中**宣告** `var spinning = false;` ⇒ 改釘「與揭曉那一拍相鄰」。

### 第三波實測（preview，這次連結算卡都看到了）
- 4 人房開局全 0：四席**全部** `#1/4` ＋「並列第一（4 人）」＋四席都有 `is-lead`（修前是 `#1/4`…`#4/4` ＋ 一句「領先」三句「並列第一」）✅
- 揭曉後非平手：`#1/2 領先`／`#2/2 距第一 NT$600`，只有一席 `is-lead` ✅
- 本輪那一行在轉輪時「本輪進行中…」、揭曉後變「本輪 +NT$1,100 / +NT$200」✅
- **結算卡**：「🏆 你贏了！+NT$500」＋新增的「名次依「總分」排」＋名次表 8,000／5,000 ✅
- 金流逐筆對得上：28,560 →（escrow −500）28,060 →（派彩 +1,000）29,060＝淨 +500，與生涯 `profit +500` 一致 ✅
- 純邏輯 live 覆驗：`tieAtTop` 全 0 terminal＝3、兩席同分 normal＝2；競技排名 normal `[1,1,3]`／crazy `[2,2,1]` ✅
- 零 console error ✅
