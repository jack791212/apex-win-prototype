...
> 最後更新：**2026-09-14**（前景連續輪：#160 首屏解鎖→#174 提款到帳時間→#175 離線骨架→#178 第二波 CTA→#181 網址作為地址→競技場 §5 四張→手感 #11；同輪重建三軌排程註冊（重置後消失的只有「註冊」本身）、§10 五個規模數字全數校正、手感進度行改以稽核檔表格為單一真相）。舊：2026-09-07（競技場全面檢修輪：修「17 天不能玩」的配對總崩＋房卡 CTA 說謊＋棄局的帳；§4 家族形狀⑥⑦、§9 headless 配方的射程邊界、§10 快照）。
...
- **BACKLOG**：編號卡到 **#182**。待做（已批准）：#87 獎勵逐片到期、#88 簽到酬賞負載軸、#93 導覽入口註冊表 `HL.nav`、#103 已結案、#107 促銷受眾述詞、#110 已完成。**待你批准**：#104 限量挑戰伺服器名額仲裁、#105 推薦歸因見證者（含後端 SQL，2026-08-19 裁決**暫緩**）。
...
- **規模**（🔄 2026-09-14 前景重新數過——**五個數字全部過期**；它們列在這裡就是要給人拿來判斷的，順手重量）：`prototype/src/core/` **74** 檔、`prototype/src/views/` **36** 個 view 檔、首屏 **95** 支 script；自我檢測 **390** 項（`node prototype/tests/run.js`）。`sw.js` CACHE 現 **v312**。〔舊記載：core 69／views 34／首屏 91／385 項／v301〕
- **RTP 登記**：單值 **19** 款（新增 abyssal-surge 96.52%＝**首款以精確解析式而非 MC 定案**的遊戲：線賠逐列舉／框收集閉式求和／免費幾何和，零抽樣誤差；MC 60M×2 僅作實作核對。⚠️ 兩種子**非獨立複現**——mulberry32 是計數器式單週期(2^32)，不同 seed 只是同一條環的不同起點）＋ **參數化 1 款**（plinko，9 種 rows×risk；#103 裁決 (c)＝正式承認參數化、逐設定揭露，**不得混進單值 API**）。
- **遊戲**：**26 款**登錄可玩（最新：**深淵氣湧 Abyssal Surge**，2026-09-13 遊戲軌＝隔 23 天的第一款新遊戲）。品類齊備：SLOT／TABLE 6 款／CRASH-INSTANT 12 款／GAME-SHOW／特殊。
...
- ✅ **首屏餘裕 48,907 bytes（2026-09-14 遊戲軌·#190 落地後）** —— **新遊戲上架封鎖已解除**。#189 把容器補完（樣式可隨遊戲延遲載入），但只有三款新遊戲用它；**#190 把其餘 17 個清單列／19 個獨有前綴全數遷出**：`components.css` **232,367 → 183,450 bytes（−48,917／−21%）**、首屏餘裕 **322 → 48,907**。**每款新遊戲的 CSS 首屏成本 ~3,300 → 0** 至此對**全部 20 款**成立。1,825 條規則中 **1,824 條逐位元組不變**；`@keyframes` 一律不搬（跨款共用）。**刻意沒搬且不是遗漏的三個**：`instant-crash-mines.js`（無獨有前綴）、`table-baccarat.js`（`ax-bacc` 與輪盤共用）、**`.ax-tbl__`**（`core/table.js` 畫的桌遊引擎外殼、六款共用）。⭐ **搬之前必讀兩條教訓**：① 判「這個前綴是不是該款獨有」**要掃全 `src/`，不是只掃 `views/`**（`.ax-tbl__` 住在 `core/table.js`，舊判準會把它判成「輪盤獨有」⇒ 搬走就是弄壞另五款桌遊，而鎖全綠）；② **CSS 勝負關係會被搬家翻轉而不改任何一個字元**——延遲樣式檔後載入，同特異度時「後寫者勝」會反過來（`.ax-pump__go` 舊世界被 `.ax-btn-primary` 蓋過＝**死碼**，搬家後活了）。兩條分別由鎖 `platform/lazy-css-rides-with-game`（(c) 已擴到全 src/＋(c2)(c3)）與 **新鎖 `games/lazy-css-no-cascade-flip`** 守著。全文 → [intel/lazy-css-migration-2026-09-14.md](intel/lazy-css-migration-2026-09-14.md)。
