...
- ✅ **首屏餘裕 14,803 bytes**（2026-09-11 前景 #160 `ce5ff0e` 折掉 481 個「同一句中文打兩遍」的 `t(k,k)`）⇒ **#118 不再是阻塞點**，所有標著「卡在 #118」的卡都可動工（#132／#133／#134／#137／#138／#139／#141／#150／#154／#157／#158…）；#118 仍該做（拆 `slot.js` 再省 24.3KB）但不再是唯一解法。新鎖 `platform/i18n-no-duplicated-default-arg` 守「不得再打兩遍」與「單引數呼叫的檔 helper 必須補得出預設值」兩條方向相反的不變量。
