...
> 最後更新：2026-09-07（競技場全面檢修輪：修「17 天不能玩」的配對總崩＋房卡 CTA 說謊＋棄局的帳；§4 家族形狀⑥⑦、§9 headless 配方的射程邊界、§10 快照）。
...
- **規模**：`prototype/src/core/` 69 檔、`prototype/src/views/` 34 個 view 檔、首屏 91 支 script；自我檢測 **385 項**（`node prototype/tests/run.js`）。`sw.js` CACHE 現 **v301**。
...
- **2026-08-20 遊戲手感巡檢**（78 條存活/69 CONFIRMED）→ [intel/game-feel-audit-2026-08-20.md](intel/game-feel-audit-2026-08-20.md)，已修 22 條（家族 A 回合鎖／B 換頁不停 autobet／C 極速模式承諾未實現／D+E 桌遊分階段結算一處通吃 6 款）、⬜ 53 條待做。**〔2026-09-12 22:00 遊戲軌更正：該檔進度行的權威值為 ✅ **66**／🏗️ 7／⬜ **5**（餘 #6／#11／#36／#50／#53）——本行「已修 22／⬜ 53」是 08-21 的舊快照、三週未更新〕**
...
3. 動 `prototype/` 就 bump `prototype/sw.js` 的 `CACHE` 版號（現 v305），否則 preview 會被 SW 餵舊檔。
