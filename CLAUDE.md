...
> 最後更新：**2026-09-14**（遊戲軌 22:00 窗：**第 28 款遊戲星鑄 Star Forge 上架**＝首款「符號等級演進」；同輪校正 §10 的**首屏餘裕**〔48,907 → 實測 10,465，差 38KB〕與 **RTP 登記款數**〔19 → 21〕兩個過期讀數。前一批為前景連續輪：#160 首屏解鎖→#174 提款到帳時間→#175 離線骨架→#178 第二波 CTA→#181 網址作為地址→競技場 §5 四張→手感 #11；同輪重建三軌排程註冊（重置後消失的只有「註冊」本身）、§10 五個規模數字全數校正、手感進度行改以稽核檔表格為單一真相）。舊：2026-09-07（競技場全面檢修輪：修「17 天不能玩」的配對總崩＋房卡 CTA 說謊＋棄局的帳；§4 家族形狀⑥⑦、§9 headless 配方的射程邊界、§10 快照）。
...
- **自適應**（⚠️ 2026-08-12 維護軌·12:00 窗複測更正：5 項 2026-07-10 痛點已解 4，僅「斷點分散」未收斂——「台帳承諾與現況不符」家族·比照 §4 baccarat/roulette 過時記載之更正）：~~≤720px 側欄（唯一主導覽）`display:none` 且無替代 → 手機無法切主要分頁~~〔✅ 已解：R1〈commit `8981686`, 2026-07-10〉手機主導覽改左側抽屜 `ax-drawer`，平台軌 P2 台帳「導覽殼層」已 partial→present〕；~~9 個雜亂斷點仍在（560/480/720/1280/1024/860/760/721/520＝9 distinct·收斂需 preview）~~〔✅ 2026-09-08 維護軌·12:00 窗複測更正（同「台帳承諾與現況不符」家族）：`520` 已於 R4 移除並上鎖，現為 **8 個 distinct**（`560/480/720/1280/1024/860/760/721`）＝R4 canonical 5 階（480/560/720/1024/1280）＋3 刻意例外（721=720 min-width 互補側／760=Slots Battle 盤面／860=直播間），由 fail-closed 鎖 `platform/breakpoint-ladder-canonical`〈`prototype/tests/checks-platform.js:153`〉守住＝任何新漂移斷點（500/520/600/900…）當場紅、新刻意例外須在白名單登記。⇒ **斷點分散痛點已收斂+上鎖**，非「未收斂待 preview」；且與此並列的 `--ax-bp-*` 死碼／`100vh`→`dvh`／`env(safe-area)` 三項早已✅ ⇒ 自適應 5 大痛點現 5/5 皆解〕；~~`--ax-bp-*` token 是死碼~~〔✅ 已解：`grep -rn "ax-bp" prototype/src` 零命中＝死 token 已移除〕；~~用 `100vh` 非 `dvh`~~〔✅ 已解：18 處 `dvh`＋僅存 3 處 `100vh` 皆為 `min-height:100vh;min-height:100dvh` 漸進回退正確樣式〕；~~無 `env(safe-area)`~~〔✅ 已解：4 處 `env(safe-area)`〕。
...
- **賠付表／玩法說明 `HL.slotPaytable`（core/slot-paytable.js，2026-09-15 遊戲軌立·G5③）＝延遲 slot 的說明面唯一容器**。**不在首屏**：由 `lazy-games.js` 清單的 `dep` 帶著走（同 `core/table-tier.js` 形制）⇒ 七款 slot 的賠付表**首屏成本 0**。用法＝該 view 寫一個 `ptSpec()` 建構式，render 裡 `HL.slotPaytable.titleRow(el("h2",…), "<id>", ptSpec)`；容器不認得任何遊戲 id／賠率數字／規則，**加一款＝加一個 spec，容器一行不改**。RTP 不由 spec 提供（容器自己向 `HL.gameRtp` 求＝#98 單一真相）。
  ⭐ **鐵律：spec 裡不准重打任何一個賠率數字**——一律由該款自己匯出的 `PAY`／`SYMBASE`／校準鈕（`G`／`payScale`）求值。理由是這個家族最典型的形狀：說明面與數學區各存一份 ⇒ 日後調賠付的人只會去改數學區（那裡才影響 RTP），**玩家看到的賠率會靜默變成過期的**，而畫面正常、RTP 測項全綠、零錯誤訊息。由鎖 `games/slot-paytable-numbers-are-derived` 用**活見證者**守著（擾動該款賠付常數 ⇒ 表上**每一個**倍率都必須跟著動；判「有沒有東西變了」擋不住「九列寫死只留一列還是算的」，本輪負向擾動實測）。
  ⚠️ **三個寫 spec 時會踩到的坑**：① **登記必須發生在 render 內**（`titleRow` 代勞），寫在模組頂層會被 `platform/fsdeps-boot-registration-guard` ⑤ 判為「開機出站註冊被延後」。② **散文裡不要出現「兌現／cashout」**——`platform/game-axes-pace-rubric` 靠這幾個字判「有沒有局中兌現控制」且**刻意不剝字串**（它要找的正是按鈕上那行字）⇒ 會開出假紅。③ **那個欄位走 `text:` 不吃 markdown**：寫 `**粗體**` 玩家會看到字面的兩個星號（2026-09-15 線上實測，測項抓不到）。
...
- **離場鉤 `HL.shell.onExit(fn)`（2026-08-21 立｜2026-09-07 大修）**：view 被換掉時要「結束自己」的帳。`HL.instant.stopAll()` 解的是計時器，這個解的是**錢**——對戰的 escrow 曾因「用底部導覽換頁」而被靜默沒收（不記敗局、無提示）。需要據實了結的 view 自己註冊；**一次性**（跑完清空清單）。
...
- ⭐ **一條反覆出現的缺陷型態：「修一半而看不出來」**（2026-08-20/21 共踩四次，立鎖時務必自問）：① 活面板登記簿在註冊時順手 purge 把自己刪掉（層① 死了但層② 會補上，畫面全對）② 參數化 RTP 的不變量只擋一個方向（反向呼叫就能造出第二份真相）③ 排名規則放在延遲載入的模組（取不到就靜默退回舊行為）④ 彈分壽命 JS 與 CSS 各寫一套（只有在特定速度下才露出）。⑤ **可切換性被條款面反鎖**（2026-09-07 平台軌·#176）：`#85` 把錦標賽的計分軸/分組做成可切換資料，而玩家唯一的說明面上一行 KV 正確求值、下一行散文寫死流水軸 ⇒ **換軸就自我矛盾，於是那個容器至今 0 個使用者**（`startNew` 帶 spec 的呼叫點＝0），而所有既有測項全綠（含 `#85` 自己的零回歸測項——因為沒人換過軸）。⑥ 🚨 **修法本身引進的缺陷，落在「驗證配方刻意繞過的那一段路」上**（2026-09-07 前景·船長提報「競技場配對系統壞了」）：08-21 為修「換頁沒收賭注」而加的離場鉤，因為 `main.js` 是 `mountView(def.render(s), …)`（**參數先被求值**）而**被它自己那一次掛載開火** ⇒ vsslot 的 `clearTimers()` 把配對相位剛排的計時器清掉 ⇒ **Slots Battle 整個玩法壞了 17 天**，期間 225 項測項全綠。當時立的鎖只守「有沒有註冊鉤子／有沒有跑 runExit」＝守寫法不守存活；而 §9 的 headless 配方（直接 `h.appendChild(g.render())`）**正是為了繞過 mountView 而生的** ⇒ 缺陷剛好躲在被繞過的那一段。⇒ **加一條自問：「我這次的驗證路徑，跟玩家真正走的路徑，差在哪幾段？差掉的那幾段有鎖嗎？」** 全文見 [intel/arena-repair-2026-09-07.md](intel/arena-repair-2026-09-07.md)。
  ⑦ **斷言認的是概念，還是某一種寫法**（2026-09-07 同一輪抓到**四種漏法**，全是我自己寫的鎖、全是負向擾動或巡檢抓出來的）：(a) **認寫法**——`/filledHere >= room.players/` 擋不住 `if (false && filledHere >= …)`；(b) **被短路**——`/HL.arenaStats.record\(/` 擋不住 `void 0 && HL.arenaStats.record(…)`；(c) **巢狀洩漏**——`/stale\(myEpoch\)/.test(body(vs,"runRound"))` 讀到了**巢狀在 runRound 裡的 `d()`** 的閘，於是拿掉 runRound 自己的閘照樣全綠；(d) **守到一個會被換掉的識別字**——存活閘寫 `!body.contains(root)` 而 `root` 是**模組級變數**（下一次 render 會換成新的、attached 的節點）⇒ 閘與斷言雙雙空綠。⇒ 守「到得了」要釘**函式標頭之後的第一個敘述句**＋**逐字的守衛形狀**＋**把判定抽成純函式在 node 直接跑**；守「per-render」要用**世代（epoch）**而不是模組級節點參照。（**2026-09-08 平台軌·#180 補第五種**：(e) **字面在檔內、求值沒發生**——`flat.indexOf("if (HL.liveStats) HL.liveStats.record(")` 擋不住 `var s = "if (HL.liveStats) HL.liveStats.record(";`：逐字守衛的字面確實在檔內、還落在會寫餘額的那個函式體裡，於是**三條斷言全綠**而求值一次都沒發生。⇒ 凡「逐字釘形狀」的鎖，掃描前除了 `stripComments` 還要**剝字串字面量**〔`stripStringLiterals`，見 `tests/checks-platform.js`〕；註解與字串是同一類東西——**都是不會被求值的字**。）（**2026-09-13 遊戲軌·#189 補第六、七種——兩條都是「行為級」鎖，卻仍然空綠**：(f) **探針認的是位置，不是身分**——沙箱驗「程式與樣式都到齊才換手」時，只結算**佇列最前面那個**，於是把 `Promise.all([js,css])` 改成「先叫 css、再回傳 js」就騙過去了（此時最前面那個是 css，不換手本來就對）＝§4 形狀⑦「認寫法」的位置變體；(g) **只記下載了什麼，不記它以什麼身分被載**——沙箱把注入節點一律記成 URL，於是把 `.css` 當 `<script>` 注入照樣全綠，**而瀏覽器根本不會套用它**（畫面沒有樣式、零錯誤訊息）。⇒ 行為級沙箱要**依身分（是不是 script／是不是 `<link rel=stylesheet>`）**結算與斷言，不要依佇列順序；「載到了」與「生效了」是兩件事，鎖必須分別守。這兩條是**負向擾動 13 條裡唯二漏掉的**，且**漏的正好是那條鎖號稱要守的性質**。）（**2026-09-14 遊戲軌·#190 補第八、九種——這次兩條都出在「鎖自己」而不是產品碼**：(h) **尺的量程漏了一段，而防空心的保險也架在同一段量程裡**——`platform/lazy-css-rides-with-game` 判「該款獨有前綴」只掃 `src/views/`，而 `.ax-tbl__` 是 `core/table.js` 畫的桌遊引擎外殼（六款共用）⇒ 它會**要求**把共用外殼搬進輪盤私有檔＝**另五款當場沒有籌碼列，而鎖全綠**；而 `exclusiveChecked >= 1` 這條防空心數的是「量程內」的前綴數，量程漏掉整個 `core/` 時讀數完全正常。這與 `platform/rg-bet-gate-coverage` 2026-09-02 修過的是同一種病 ⇒ **立鎖時要問「這把尺的量程，涵蓋得到這個概念會出現的所有地方嗎」**，並補一條「量程必須涵蓋 X 以外」的反向不變量。(i) **反空綠的錨，本身也只認寫法不認概念**——為了防 (h) 的 fail-open 而加的錨，第一版是「拿合成字串去打 `stripComments` 自己」，那只證明**這個函式會剥註解**，**沒證明掃描端用了它**：把掃描端的 `stripComments` 拿掉照樣全綠（P6 實測 MISSED）。改用**活見證者**（某個「只有算進註解才會被誤判成共用」的真前綴，必須確實落在受檢清單裡）後 12/12 CAUGHT。⇒ **錨要釘在「這個性質有沒有在真實掃描中成立」，不是「這個 helper 會不會做這件事」。**另同輪記一條產品側的鏡像：**CSS 的勝負關係會被「搬家」翻轉而不改任何一個字元**（延遲樣式檔後載入，同特異度時後寫者勝會反過來；`.ax-pump__go` 舊世界是死碼、搬家後活了）⇒ 凡「只搬位置」的遷移，都要先問**那條規則所在元素上還有哪些 class、它們有沒有宣告同一個屬性**。全文見 [intel/lazy-css-migration-2026-09-14.md](intel/lazy-css-migration-2026-09-14.md)。）
  🚨 **⑧ 空綠跑到了「證明鎖不空綠」的那個工具上（2026-09-15 平台軌·#185，本輪親身踩到）**：
  負向擾動是本專案唯一用來證明一條鎖不是空綠的手段，而它每輪都是手刻的一次性腳本。
  本輪第一批擾動跑到一半被外層 timeout 砍掉 ⇒ **它的「還原」那一步沒跑到**，變異就留在 `checks-platform.js` 裡；
  第二批 harness 啟動時把**那個已被污染的檔**讀成 `orig`，於是 **基線本來就是紅的**——而判定只問「本鎖有沒有紅」，
  **七條擾動全部印出 `CAUGHT`，七條全部不可信**，輸出看起來完全正常。
  ⇒ **擾動 harness 必做三件事**：① 開跑前先量基線、**非全綠就中止**；② 每條擾動前後還原並**逐位比對**；
  ③ 結束印 `restored identical=true`，false 視同紅。另：一次跑十幾條容易被 timeout 砍，要能 `--case` 單跑。（已開成 **#201**。）
  **一個會說謊的驗證工具比沒有工具更糟**——它讓整輪的卡片、journal 與這份交接檔都建立在假讀數上。
  同輪的第二條（**列舉式的判準既會漏也會多**）：`intel/tools/ledger-card-sweep.js` 用**動詞表**認「這筆 evidence 引用了哪張卡」，
  而 evidence 寫「已由 **#175** 認領」時它一筆也看不見（全庫 12 筆這種寫法），於是台帳**高報了一個十天前已關閉的缺口而工具雙向回報 0**。
  補上動詞後當場冒出 3 筆，逐筆讀完 **2 筆是誤報**（「依 #82 先例（同一筆**未認領**記載三次即應升格）」被讀成「#82 認領」）⇒ 還要加否定形排除。
  ⇒ **凡列舉式的表，補一格就記一次理由；補完必須同時量「漏」與「多」兩個方向。**
  ⭐ **「策略是資料」的例外：有一格必須由程式贏過資料（2026-09-14 前景·#178 第三波）**——
  本專案一貫把抑制策略做成資料（`SUP_BY_PAUSE`），好處是新增一種暫停＝加一筆策略、述詞一行不改。
  但「自我排除已啟動」那一則確認訊息不行：它若被自己的閘吃掉，玩家按下按鈕會**什麼都看不到**，
  而他永遠不會知道要回來檢查。⇒ **凡「錯了就無法補救」的那一格，把例外寫進程式，不要仰賴「資料表剛好沒列它」**；
  而鎖要用**毒化過的資料表**當合成輸入實際跑一次，證明程式那一層真的贏得過資料那一層。
  同輪的第二條：**白名單型的分類要兩個方向一起守**——執行期 fail-closed（沒自陳就當成最嚴的那一類）
  ＋鎖要求**每一個呼叫點都必須自陳**；只有前者會默默擋錯東西，只有後者則忘了標就漏出去。
...
- - **⚠️ 放置區遊戲的必經掛鉤（#180，2026-09-08 平台軌）**：凡走 `registry.json` 上架的遊戲，**結算時一定要 `if (HL.liveStats) HL.liveStats.record(id, bet, win)`**（沒中傳 0）。少了它，遊戲照玩、餘額照動、零錯誤訊息，但那一注對 `record()` 下游 **21 個子系統**（VIP／任務／返水／彩金／限時賽／成就／季票／公會／商城／挑戰／抽獎券／`HL.betlog` 注單／`HL.ledger` 帳本〔GGR/NGR/RTP 會靜默少算〕／`HL.heat` 熱度牆／**`HL.rg.record` 玩家自設限額**）全部不存在。**這條由 `platform/placement-games-feed-central-hook` 守著**（`prototype/games/` 目前唯一的測項）：它掃 `registry.json` 上每一款＋Dev Kit 範例，要求逐字守衛形狀、且該呼叫與**餘額寫入落在同一個函式體**。⇒ **在 registry.json 加一行之前，先確認那支 game.js 有這一行**，否則自我檢測會轉紅。
...
- 🆕 **排程輪其實看得見畫面（2026-09-12 遊戲軌·22:00 窗實測，更正一個 10 輪份的誤解）**：排程輪起不了 dev server 是真的（`preview_start {name}` 連 10 次被拒，訊息逐字「Dev servers can't be started from unattended sessions」），但**那不等於看不到畫面**——`preview_start` 的 **`{url}` 形式開的是瀏覽器分頁、不需要任何 dev server**，而本站**本來就部署在 GitHub Pages**。⇒ 排程輪可以對**線上站**跑**玩家真實路徑**（`?demo=1` 讓 `HL.auth.backend()` 為 false ⇒ 過得了登入 gate，再走 `HL.router.goGame()`＝**正是下一條 headless 配方繞過的那一段**）。**完整配方＋實測射程邊界＋但書見 [intel/scheduled-visual-verification-2026-09-12.md](intel/scheduled-visual-verification-2026-09-12.md)。** 三個要點：① **JS／`setTimeout`／DOM／節點識別都是真實速度、完全可量**（`setTimeout(100)` 實測 108ms，**未被夾到 1s**）；② **`rAF` 與 CSS transition 只在截圖那一刻推進一格**——截圖就是影格幫浦（20 秒線性位移靜置 42 秒仍凍在 x=63/600，只在 4 次截圖時各跳一格）⇒ **「節奏/時序做對沒」可驗，「補間動畫長什麼樣」仍不可靠**；③ **截圖會成功**（舊記載「截圖常逾時」在此環境不成立）。⚠️ 這條路量的是**線上站＝HEAD 不是工作區** ⇒ 驗自己的改動要**先 push**（Pages 1–2 分鐘；驗前先 `unregister()` 所有 SW ＋ 清 `caches` ＋ 用 `fetch('…?cb='+Date.now())` 確認新版真的到了）。⚠️ **用它必須附一個「正向對照」**（同品類已知做對的那一款、同一把量具量一次）：若每一款都量到「1 拍」，證明的是儀器壞了而不是遊戲有缺陷。
- **驗證強度**：
...
- **⚠️ 遊戲 view 的 headless 驗證配方（2026-08-20 前景實測·更正一個長期誤解）**：三軌長期認為「headless 完全驗不了 UI，所以保真閘第 10/11 項只能自我宣稱」——**只有一半成立**。
  ...
  - 🚨 **這個配方的射程邊界（2026-09-07 血的教訓，務必先讀）**：它**刻意繞過 `renderApp` 與 `mountView`**（那正是它能躲過登入 gate 的原因）⇒ **凡是缺陷住在「掛載／離場／換頁／同頁重繪」這條路上，用這個配方一定驗不到**。競技場對戰就是這樣壞了 17 天而測項全綠（見 §4 形狀⑥）。要驗那條路必須：① 走真實路徑（`HL.router.go(...)`，用 `?demo=1` 讓 `HL.auth.backend()` 為 false 就過得了 gate），或 ② 立源碼鎖把順序釘死。**每次用這個配方，先問「玩家真正走的路徑跟我這條差在哪幾段」。**
  - ⚠️ **Chrome intensive throttling（2026-09-07 實測）**：分頁隱藏**超過 5 分鐘**後 `setTimeout` 被降到**約每分鐘一次**（不是 ≥1s）⇒ 一場 10 輪對戰要跑十幾分鐘，看起來像「卡住了」。判「卡住 vs 只是慢」要先量一次 `setTimeout` 延遲。播無聲音訊解節流的招在此環境**無效**。

---

## 10. 現況快照（2026-09-07 競技場檢修輪後更新）

- **BACKLOG**：編號卡到 **#201**（2026-09-15 平台軌 20:00 窗開 #200 通知身分沒被存下來／#201 負向擾動 harness 要有保險）。待做（已批准）：#87 獎勵逐片到期、#88 簽到酬賞負載軸、#93 導覽入口註冊表 `HL.nav`、#103 已結案、#107 促銷受眾述詞、#110 已完成。**待你批准**：#104 限量挑戰伺服器名額仲裁、#105 推薦歸因見證者（含後端 SQL，2026-08-19 裁決**暫緩**）。
...
- **規模**（🔄 2026-09-14 遊戲軌·22:00 窗重量）：`prototype/src/core/` **74** 檔、`prototype/src/views/` **37** 個 view 檔、首屏 **95** 支 script；自我檢測 **409** 項 fast（`node prototype/tests/run.js`）＋ **16** 項 deep（`--deep --group games`）。`sw.js` CACHE 現 **v319**（2026-09-15 遊戲軌 22:00 窗 bump ×2：G5③ 賠付表七款＋說明面星號修正＝出貨位元組）。⚠️ `core/` 現 **75** 檔（新增 `core/slot-paytable.js`，**不在首屏**）。〔舊記載：views 36／390 項／v312；406 項／v316；408 項／v317〕
- **RTP 登記**：單值 **22** 款（🔄 2026-09-15 遊戲軌·10:00 窗：**shadow-ritual 終於可以登記**——它從 2026-08-02 起被 `platform/game-rtp-no-false-claim` 刻意擋在表外（當時實測全回合 1132%，登記就是把假數字鑄成 API），本輪重平衡過閘後登記 **97.07%**，那把鎖也換成了反方向的形狀「必須登記且畫面值同源」）（`node -e "console.log(require('./prototype/src/data/game-rtp.js').ids().length)"` 可重算；⚠️ 舊記載 19 於 emerald-sprite 上架時漏更＝**兩款的落差**，本輪一併校正。最新 star-forge 96.5%＝**首款以「分層估計」定版**的遊戲：RTP 拆成 base局期望＋觸發率×E[免費整段] 三量獨立量測再合成，CI95 ±0.125pp；校準鈕單一且「RTP 對它嚴格線性」已常駐化成鎖。前一款 abyssal-surge 96.52%＝**首款以精確解析式而非 MC 定案**的遊戲：線賠逐列舉／框收集閉式求和／免費幾何和，零抽樣誤差；MC 60M×2 僅作實作核對。⚠️ 兩種子**非獨立複現**——mulberry32 是計數器式單週期(2^32)，不同 seed 只是同一條環的不同起點）＋ **參數化 1 款**（plinko，9 種 rows×risk；#103 裁決 (c)＝正式承認參數化、逐設定揭露，**不得混進單值 API**）。
- **遊戲**：**28 款**登錄可玩（最新：**星鑄 Star Forge ⚒️**，2026-09-14 遊戲軌 22:00 窗＝首款「**符號等級演進**」＝盤面上相連同階符號合併成高一階的另一個符號，9 級階梯、位置乘數 ×2→×128、base 局過熱炸彈、三種買入 90×/138×/587×）。品類齊備：SLOT／TABLE 6 款／CRASH-INSTANT 12 款／GAME-SHOW／特殊。
  ⭐ **建置它時抓到的通則（值得帶到下一個階梯型機制）**：照規格的「固定成群門檻 4」寫出來，**9 級階梯走不到 T6 以上**（20,000 局實測 T6↑ 0 次、💣 0 顆、T9 0 次）⇒ 賠付表第 7–9 階、炸彈、頂階全是「寫在說明面而程式永遠走不到」的裝飾，**而 RTP／最大贏額／無 NaN／連鎖收斂全部照樣綠**。⇒ **凡「階梯／等級／收集／解鎖」型機制，都要問一次「最高那幾級程式真的走得到嗎」並把答案做成常駐鎖**（本款＝`games/star-forge/tier-ladder-reachable`，它不驗數值、驗每一階有沒有被真的升出來過）。全文 → [intel/star-forge-tier-ladder-2026-09-14.md](intel/star-forge-tier-ladder-2026-09-14.md)。
- **2026-08-20 遊戲手感巡檢**（78 條存活/69 CONFIRMED）→ [intel/game-feel-audit-2026-08-20.md](intel/game-feel-audit-2026-08-20.md)。**進度（2026-09-15 遊戲軌·16:00 窗重新數過表格狀態欄）：✅ 69／🏗️ 7／⬜ 2（餘 #50／#53）**。⚠️ **這一行歷來是本檔漂移最快的一行**（曾停在「已修 22／⬜ 53」三週）⇒ **以那支檔的表格為單一真相**，這裡只是鏡射。✅ **#6 已於 2026-09-15 10:00 窗結案**（遊戲軌隨暗影儀式 RTP 重平衡一併做：`_enterPending` 把 mode/背景/符號池的翻轉延到 spin 邊界＋新增 `featureIn` 轉場拍＋鎖 `games/shadow-ritual/mode-flip-at-spin-boundary`；⚠️ 那一輪**沒回頭改稽核表的狀態欄**，直到 16:00 窗才據實更正 ⇒ 又一次「台帳承諾與現況不符」)。〔沿革·它教的那件事仍然成立：**#6 經 2026-09-14 前景查證，不是表現層問題而是 RTP 變更**（真正中途翻轉的是 `drawSym` 的**符號池**，而那已寫進被驗證過的 RTP 模型；「宣告 5×5 而盤面 4 列」則是**文案說謊**，連爆補位讀的是 `g[0].length`）⇒ **屬遊戲軌保真閘（需 ≥ 1M MC、CI95 ≤ 0.5pp），不得在前景輪只修表現層**——最後也確實是由遊戲軌連同 3M×2 種子的 RTP 戰役一起結掉的。〕
...
- ⚠️ **首屏餘裕：不要再從這一行抄數字了——它現在有程式出口（#197，2026-09-15 平台軌·14:00 窗）**。一行可重算、也可回到過去任何一天重算：`node intel/tools/ledger-probe.js --category 擴充性`（加 `--ref <sha>` 對某個 commit 重算）。**2026-09-15 14:00 實測：餘裕 1,227 bytes／首屏 1,637,173 bytes／94 支本地 script**（門檻 1600KB／120 支）。⚠️ **口徑**：本尺數 94 支，而 `index.html` 有 95 個 `<script src=`——差的那一支是外部 CDN 的 `supabase-js`（不吃我們的位元組）；**兩個數都對，只是口徑不同**。讀數為 LF 正規化＝git blob＝Pages 服務的位元組。  ⭐ **為什麼要開這個出口**：這把尺（`platform/first-screen-budget`）一直都在而且是對的，但 `firstScreenMeasure()` 過去是 `checks-platform.js` 的**模組級區域函式、零出口** ⇒ 每個引用者只能抄一個當場就開始過期的數。**本行自己就是最好的證據**：48,907（過期 38KB）→ 11,281 → 10,465（**後兩個是同一輪寫下的兩個數**，差 816＝星鑄六張註冊表，一個在註冊前一個在註冊後）→ 今天實測 **1,227**（09-15 前景競技場兩輪又吃掉 9,238B）。⇒ **任何人要動 eager 檔，跑那行指令當場重量，不要相信任何文件裡的數字（包括這一行）。**  〔舊記載（保留沿革）：首屏餘裕 10,465 bytes（2026-09-14 遊戲軌·22:00 窗實測，用的就是 `platform/first-screen-budget` 那把尺）** —— 舊記載 48,907 **已過期 38KB**，任何人照它規劃會以為空間是實際的四倍多。差額 **35,362 bytes** 來自同日前景兩輪：`core/offline.js`（6,610B·#175 離線骨架）與 `core/route.js`（6,285B·#181 網址作為地址）**兩支新的 eager 檔**，加上既有 eager 檔的增長；那兩輪沒有回頭更新本行。可重算：`git show <ref>:p…〕
- ✅ **（沿革）#190 落地當下的首屏解封**：#189 把容器補完（樣式可隨遊戲延遲載入），但只有三款新遊戲用它；**#190 把其餘 17 個清單列／19 個獨有前綴全數遷出**：`components.css` **232,367 → 183,450 bytes（−48,917／−21%）**、首屏餘裕 **322 → 48,907**。**每款新遊戲的 CSS 首屏成本 ~3,300 → 0** 至此對**全部 20 款**成立。1,825 條規則中 **1,824 條逐位元組不變**；`@keyframes` 一律不搬（跨款共用）。**刻意沒搬且不是遗漏的三個**：`instant-crash-mines.js`（無獨有前綴）、`table-baccarat.js`（`ax-bacc` 與輪盤共用）、**`.ax-tbl__`**（`core/table.js` 畫的桌遊引擎外殼、六款共用）。⭐ **搬之前必讀兩條教訓**：① 判「這個前綴是不是該款獨有」**要掃全 `src/`，不是只掃 `views/`**（`.ax-tbl__` 住在 `core/table.js`，舊判準會把它判成「輪盤獨有」⇒ 搬走就是弄壞另五款桌遊，而鎖全綠）；② **CSS 勝負關係會被搬家翻轉而不改任何一個字元**——延遲樣式檔後載入，同特異度時「後寫者勝」會反過來（`.ax-pump__go` 舊世界被 `.ax-btn-primary` 蓋過＝**死碼**，搬家後活了）。兩條分別由鎖 `platform/lazy-css-rides-with-game`（(c) 已擴到全 src/＋(c2)(c3)）與 **新鎖 `games/lazy-css-no-cascade-flip`** 守著。全文 → [intel/lazy-css-migration-2026-09-14.md](intel/lazy-css-migration-2026-09-14.md)。
  〔舊記載（已不適用，保留沿革）：**首屏餘裕 2,229 bytes**〔**2026-09-13 遊戲軌·10:00 窗更正**：進場實測僅剩 **33 bytes**（09-12 的 2,072 被兩天內的三軌支出吃光），而**一款新 slot 的首屏成本 ≈ 3,650**（CSS ~3,300 ＋ manifest/rtp/edge 三張註冊表 ~350）⇒ **任何新遊戲都上不了架**。⭐ 根因不是「又超支了」，而是 **`#80`／`#110` 的延遲載入容器只搬了一半**——程式離開首屏，**各遊戲專用樣式仍整包留在 `components.css`**（依 class 前綴歸戶合計約 **33.5KB**，玩家沒點開也下載，**而首屏讀數看起來完全正常**）。**#189** 把容器補完：`core/lazy-load.js` 的 `load(src)` 依副檔名分派（`.css` → `<link rel=stylesheet>`，**走同一份載入態表**）、`lazy-games.js` 一列可帶 `css:`、**程式與樣式都到齊才換手**（否則真 render 會在樣式抵達前先畫一次）、樣式失敗不讓遊戲變「載入失敗」。餘裕 **33 → 2,229** 且同時多了一整款遊戲；**每款新遊戲的 CSS 首屏成本 ~3,300 → 0**。鎖 `platform/lazy-css-rides-with-game`（檔案存在／**行為級**都到齊才換手／**帶自己樣式者其獨有前綴不得留在 components.css**）。**其餘 25 款尚未遷移（約 30KB 仍在首屏）＝卡 #190**，兩個陷阱先讀：共用前綴（`.ax-inst__`）不可搬、四款 slot 共用的 keyframes（`ax-dbn-win`／`ax-pir-pop`）要一起處理。全文 → [intel/lazy-game-container-2026-09-13.md](intel/lazy-game-container-2026-09-13.md)〕。〔**2026-09-12 遊戲軌·16:00 窗更正並擴充**：09-11 之後三軌又花掉約 3.5KB，進場實測只剩 **289 bytes**＝連「在 `lazy-games.js` 清單上加一列新遊戲」（~310B）都做不到 ⇒ 本輪把該清單 23 列裡逐列重複的 7 個欄位收成預設表（`CARD_DEFAULTS`＋`fillDefaults`，展開於註冊前、**顯式值一律勝出**），餘裕 **289 → 2,072**、**每款新遊戲的邊際成本 ~310 → ~90 bytes**。零行為變更證在 vm 沙箱**註冊後**的 12 個卡面欄位逐款逐欄 0 差異；鎖 `games/lazy-manifest-defaults`（其「顯式值勝出」一條**在 live 資料上沒有見證者**，故以合成輸入打純函式）〕。〔舊記載：**3,808 bytes**（2026-09-11 前景 #160 `ce5ff0e` 折掉 481 個「同一句中文打兩遍」的 `t(k,k)`＝回收 14,634B、餘裕一度到 14,803B；同日三筆支出用掉約 11KB：競技場 `eac6bd0` 約 2.3KB、遊戲軌手感 #8 `d7cc61b` +3,236B〔`slot.js` 收斂節拍表〕、平台軌 #182 +5,488B〔`core/ops-audit.js` 營運軌跡，**eager 且不可延遲**〕）⇒ **兩天內已用掉解鎖額度約六成**，動 eager 檔前請先量⇒ **#118 不再是阻塞點**，所有標著「卡在 #118」的卡都可動工（#132／#133／#134／#137／#138／#139／#141／#150／#154／#157／#158…）；#118 仍該做（拆 `slot.js` 再省 24.3KB）但不再是唯一解法。〕新鎖 `platform/i18n-no-duplicated-default-arg` 守「不得再打兩遍」與「單引數呼叫的檔 helper 必須補得出預設值」兩條方向相反的不變量。
  〈舊記載：首屏餘裕只剩 72 bytes（2026-09-07 競技場檢修兩波在三支 eager 檔淨增約 1.6KB **程式碼**；長註解已全數搬進 intel 與鎖）⇒ **任何 eager 改動都會撞 `platform/first-screen-budget`**，#118 佇列實質全阻。競技場尚有一條已定位的 a11y 缺陷（大廳整表重繪偷走鍵盤焦點）因此刻意不修。
...
- 🧾 **2026-09-11 平台軌·後台輪（#182 · 營運操作軌跡 `HL.opsAudit`）**：營運面 9 個會改變世界的開關（切站別／金流模式／**真金牌照**／餘額歸零／排行榜歸零／清帳本／假站三個演出旋鈕）在此之前**按下去不留任何一個字**，而「🧹 重置本機帳本」銷毀的正是唯一能記錄它自己的那本帳。落地＝動作型別登記簿（容器零內建／fail-closed／唯讀純值副本／環形上界 200／**獨立 storage key `HL_OPSAUDIT`** 受 `HL.site.ns()` 前綴）＋ 8 個寫入面接線 ＋ 儀表板軌跡區 ＋ 三語；`actor` 向 `HL.rbac` 求值＝**#117 的第二個消費者**（在此之前它只有 `core/reports.js` 一個，而 `ops` 角色 label 逐字是「營運（⚙ 工具面板）」——被它命名的那個面板從來沒問過它）。新鎖 `platform/ops-writes-leave-a-trace`、**負向擾動 16/16**。⭐ **發現的方式值得記住**：「後台」這一格的常規讀數只掃 `views/ops-dashboard.js`（恆為 2、連十二輪逐位相同），而真正的開關在 `core/demo-tools.js` ⇒ **一個「零漂移」的讀數，量的是隔壁那支檔**。**刻意不做** four-eyes／角色簽核（純前端做不成權威）⇒ 台帳判 partial。
