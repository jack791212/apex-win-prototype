...
- **規模**（🔄 2026-09-14 遊戲軌·22:00 窗重量）：`prototype/src/core/` **74** 檔、`prototype/src/views/` **37** 個 view 檔、首屏 **95** 支 script；自我檢測 **406** 項 fast（`node prototype/tests/run.js`）＋ **16** 項 deep（`--deep --group games`）。`sw.js` CACHE 現 **v316**（本輪未 bump＝零出貨位元組變更：只動 `prototype/tests/` 與 `intel/`，sw 不快取 tests）。〔舊記載：views 36／390 項／v312〕
...
- **2026-08-20 遊戲手感巡檢**（78 條存活/69 CONFIRMED）→ [intel/game-feel-audit-2026-08-20.md](intel/game-feel-audit-2026-08-20.md)。**進度（2026-09-14 前景重新數過表格狀態欄）：✅ 67／🏗️ 7／⬜ 4（餘 #6／#36／#50／#53）**。⚠️ **這一行歷來是本檔漂移最快的一行**（曾停在「已修 22／⬜ 53」三週）⇒ **以那支檔的表格為單一真相**，這裡只是鏡射。🔴 **#6（暗影儀式免費遊戲中途翻轉）經 2026-09-14 前景查證，不是表現層問題而是 RTP 變更**（真正中途翻轉的是 `drawSym` 的**符號池**，而那已寫進被驗證過的 RTP 模型；「宣告 5×5 而盤面 4 列」則是**文案說謊**，連爆補位讀的是 `g[0].length`）⇒ **屬遊戲軌保真閘（需 ≥ 1M MC、CI95 ≤ 0.5pp），不得在前景輪只修表現層**。
...
1. `node prototype/tests/run.js` 必須全綠（現 **406** 項）。**改動讓既有鎖變紅時，改成「守新形狀下的同一組不變量」，不要放寬。**
2. 修完一條缺陷就**立一條常駐鎖**，並用**負向擾動**證明它真的會紅（把修好的性質逐一破壞、確認被**對應的那一條**抓到）。
3. 動 `prototype/` 就 bump `prototype/sw.js` 的 `CACHE` 版號（現 **v316**），否則 preview 會被 SW 餵舊檔。
