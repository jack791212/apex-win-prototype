...
- **規模**（🔄 2026-09-14 前景重新數過——**五個數字全部過期**；它們列在這裡就是要給人拿來判斷的，順手重量）：`prototype/src/core/` **74** 檔、`prototype/src/views/` **36** 個 view 檔、首屏 **95** 支 script；自我檢測 **389** 項（`node prototype/tests/run.js`）。`sw.js` CACHE 現 **v312**。〔舊記載：core 69／views 34／首屏 91／385 項／v301〕
...
1. `node prototype/tests/run.js` 必須全綠（現 **389** 項）。**改動讓既有鎖變紅時，改成「守新形狀下的同一組不變量」，不要放寬。**
