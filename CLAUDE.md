...
- **BACKLOG**：編號卡到 **#199**。待做（已批准）：#87 獎勵逐片到期、#88 簽到酬賞負載軸、#93 導覽入口註冊表 `HL.nav`、#103 已結案、#107 促銷受眾述詞、#110 已完成。**待你批准**：#104 限量挑戰伺服器名額仲裁、#105 推薦歸因見證者（含後端 SQL，2026-08-19 裁決**暫緩**）。
...
- **規模**（🔄 2026-09-14 遊戲軌·22:00 窗重量）：`prototype/src/core/` **74** 檔、`prototype/src/views/` **37** 個 view 檔、首屏 **95** 支 script；自我檢測 **407** 項 fast（`node prototype/tests/run.js`）＋ **16** 項 deep（`--deep --group games`）。`sw.js` CACHE 現 **v317**（2026-09-15 遊戲軌 16:00 窗 bump：#36 動了 `views/table-moneywheel.js` 與 `styles/game-money-wheel.css` ＝出貨位元組）。〔舊記載：views 36／390 項／v312；406 項／v316〕
...
1. `node prototype/tests/run.js` 必須全綠（現 **407** 項）。**改動讓既有鎖變紅時，改成「守新形狀下的同一組不變量」，不要放寬。**
