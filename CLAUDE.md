...
- **BACKLOG**：編號卡到 **#195**。待做（已批准）：#87 獎勵逐片到期、#88 簽到酬賞負載軸、#93 導覽入口註冊表 `HL.nav`、#103 已結案、#107 促銷受眾述詞、#110 已完成。**待你批准**：#104 限量挑戰伺服器名額仲裁、#105 推薦歸因見證者（含後端 SQL，2026-08-19 裁決**暫緩**）。
...
- **規模**（🔄 2026-09-14 遊戲軌·22:00 窗重量）：`prototype/src/core/` **74** 檔、`prototype/src/views/` **37** 個 view 檔、首屏 **95** 支 script；自我檢測 **402** 項 fast（`node prototype/tests/run.js`）＋ **16** 項 deep（`--deep --group games`）。`sw.js` CACHE 現 **v314**。〔舊記載：views 36／390 項／v312〕
...
- ⚠️ **首屏餘裕 10,465 bytes（2026-09-14 遊戲軌·22:00 窗實測，用的就是 `platform/first-screen-budget` 那把尺）** —— 舊記載 48,907 **已過期 38KB**，任何人照它規劃會以為空間是實際的四倍多。差額 **35,362 bytes** 來自同日前景兩輪：`core/offline.js`（6,610B·#175 離線骨架）與 `core/route.js`（6,285B·#181 網址作為地址）**兩支新的 eager 檔**，加上既有 eager 檔的增長；那兩輪沒有回頭更新本行。可重算：`git show <ref>:prototype/index.html` 取 script/link 清單逐檔 LF 正規化加總，門檻 1600KB。**新遊戲上架封鎖仍是解除的**（星鑄的程式與樣式都在延遲載入路徑上，只有六張註冊表吃了 816B），但**動 eager 檔前務必當場重量**。
...
1. `node prototype/tests/run.js` 必須全綠（現 **390** 項）。**改動讓既有鎖變紅時，改成「守新形狀下的同一組不變量」，不要放寬。**
2. 修完一條缺陷就**立一條常駐鎖**，並用**負向擾動**證明它真的會紅（把修好的性質逐一破壞、確認被**對應的那一條**抓到）。
3. 動 `prototype/` 就 bump `prototype/sw.js` 的 `CACHE` 版號（現 **v312**），否則 preview 會被 SW 餵舊檔。
