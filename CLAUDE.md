...
- **規模**（🔄 2026-09-14 前景重新數過——**五個數字全部過期**；它們列在這裡就是要給人拿來判斷的，順手重量）：`prototype/src/core/` **74** 檔、`prototype/src/views/` **36** 個 view 檔、首屏 **95** 支 script；自我檢測 **389** 項（`node prototype/tests/run.js`）。`sw.js` CACHE 現 **v311**。〔舊記載：core 69／views 34／首屏 91／385 項／v301〕
...
3. 動 `prototype/` 就 bump `prototype/sw.js` 的 `CACHE` 版號（現 **v311**），否則 preview 會被 SW 餵舊檔。
