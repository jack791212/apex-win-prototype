...
- **BACKLOG**：編號卡到 **#180**。待做（已批准）：#87 獎勵逐片到期、#88 簽到酬賞負載軸、#93 導覽入口註冊表 `HL.nav`、#103 已結案、#107 促銷受眾述詞、#110 已完成。**待你批准**：#104 限量挑戰伺服器名額仲裁、#105 推薦歸因見證者（含後端 SQL，2026-08-19 裁決**暫緩**）。
...
- **規模**：`prototype/src/core/` 69 檔、`prototype/src/views/` 34 個 view 檔、首屏 92 支 script；自我檢測 **357 項**（`node prototype/tests/run.js`）。`sw.js` CACHE 現 **v282**。
...
- ✅ **首屏餘裕 12,532 bytes**（2026-09-11 前景 #160 `ce5ff0e` 折掉 481 個「同一句中文打兩遍」的 `t(k,k)`＝回收 14,634B、餘裕一度到 14,803B；同日競技場 `eac6bd0` 用掉約 2.3KB）⇒ **#118 不再是阻塞點**，所有標著「卡在 #118」的卡都可動工（#132／#133／#134／#137／#138／#139／#141／#150／#154／#157／#158…）；#118 仍該做（拆 `slot.js` 再省 24.3KB）但不再是唯一解法。新鎖 `platform/i18n-no-duplicated-default-arg` 守「不得再打兩遍」與「單引數呼叫的檔 helper 必須補得出預設值」兩條方向相反的不變量。
