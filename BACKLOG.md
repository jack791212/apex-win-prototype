...
- **2026-09-01（平台軌 · **08:00 窗** · 建置輪＝台帳輪替審「**金流**」6 模組＋查獲**潛伏的錢缺陷：全站有兩套遊戲識別詞彙，而管錢的那一端用精確比對**＋立常駐鎖 `platform/settle-vocab-scoped-bonus-interlock`＋開卡 #154·claim `p-080950-b4e1`·純 `tests/`＋`intel/`＋`BACKLOG`·**零首屏位元組**·`sw` 不 bump）**
    - **① 進場**：`build_lock` 乾淨 `false`（維護軌 09-01 00:00 窗 `m-000930-7c4a` 已釋放）→ claim `p-080950-b4e1`（帶心跳 08:09→08:26→08:34→收尾）→ **當下即單檔 commit**（`1b9465a`，依 CLAUDE.md §10 對 08-20 那次「claim 只存在工作區就閃退＝等於沒 claim」的處置）→ 重讀確認 token 仍在＝claim 成立·**未奪鎖**。dark 11.1h（`last_platform_run_at` 08-31T21:05）<24h 非 catchup；`lead_track: games` 本可讓路，但 08-31 20:00 窗已明文把本窗指向台帳下一格與 #153 ⇒ **做而不讓路**。
    - **② 台帳自我查核**：`node intel/tools/ledger-card-sweep.js` 雙向皆 **0 筆**待人工確認（正向 0／反向 0；宣告來自台帳的卡 47 張、可比對 33 張）⇒ 無「缺口被自己的卡關閉了卻沒回填」。
    - **③ ⭐ 真產出＝台帳審金流時撿到一個潛伏的錢缺陷（既有兩條鎖都看不到的第三層）**。同一條中央結算掛鉤上已有兩條鎖：`platform/central-hook-fanout-roster`（下游**收不收到** `game`）與 `platform/central-hook-game-arg-consumed`（下游**讀不讀** `game`）——**兩條都綠**，而**沒有任何一條問「讀到的那個值，在它要查的那本字典裡查得到嗎」**。實測全站有**兩套詞彙**：結算端流通的是顯示名或舊 slug，登錄表用的是正式 slug id。
      · **機械事實**（新探針 `registry-probe.settleVocab()`，可複跑）：結算詞彙 **29 個字面量鍵／52 個呼叫點**（direct 15 鍵＝各 view 直呼；engine 14 鍵＝經 `betPanel`／`betArea` 轉手），對權威登錄表（沙箱實跑 `HL.games.all()`＝**65 筆 id**）求解 ⇒ **22 解析成功／7 失敗**：`暗影儀式`×7（應為 `shadow-ritual`）、`小雞過馬路`×5（`chicken-cross`）、`roulette`×1（`european-roulette`）、`Slots Battle`×3、`賞金局 · 翻牌`×2、`賞金局 · 踩地雷`×2、`跟注·百家樂`×2（後四者登錄表根本無對應遊戲）。
      · **為什麼是錢**：`core/wager-scope.js` 的 `typeOf()` 走精確 `HL.games.byId(id)`，查不到回 `unknown`；`weightFor()` 對未列出類別取 `w.rest`、**沒有 `rest` 就 0**；而出貨的 `standard` preset（`{slot:1, original:1, special:1, table:0.1, live:0.1}`）**正好沒有 `rest`** ⇒ 一旦有紅利宣告 `scope`，**押在旗艦 slot 上的流水對那筆紅利貢獻 0**，而 `standard` 的賣點正是「slot 全額、桌遊一折」＝**與意圖完全相反**，且畫面、帳本、其他每一條鎖全都是綠的。
      · **今天還沒出事的唯一原因**：全站 `HL.bonus.add` **零筆**宣告 `scope`（實測）⇒ `weightFor` 的零回歸錨點直接回 1。**潛伏**，#89 的功能一被用上就變活缺陷。
    - **④ ⭐ 本輪最值得記住的一件事＝同一個落差在另一個消費端早就被發現過，而那次的局部繞過吃掉了全域修法的動機**。`core/heat.js` 的 `matchGame()` 自己寫了一個雙向 `indexOf` fuzzy 比對器繞過去，檔內註解原句是「名稱字串各遊戲不一致，**找不到不致命**」——在熱度那裡確實不致命。⇒ **無害的消費端把落差局部繞過、管錢的消費端維持精確查表，而繞過的那一版沒有回頭把「落差本身」記下來**。這是 CLAUDE.md §4「修一半而看不出來」家族的一個新變形（**第一個明確實例：局部繞過使全域缺陷失去被發現的機會**），已寫進 #154 卡與探針檔頭。
    - **⑤ 落地物＝互鎖，刻意不是「修好」**。正確修法動不了：改結算端傳 id 會讓 `edge`／`betlog`／`achievements`／`heat` 的 localStorage 歷史鍵漂移（既有玩家的注單/徽章/熱度靜默歸零），也動到 `HL.fair.floatOr("roulette")` 的公平串流命名與保真閘白名單；改 `core/wager-scope.js` 加解析層則是**首屏 eager** 檔而 [P-FS] 餘裕 **27B** ⇒ 卡在 #118。⇒ 立常駐鎖 **`platform/settle-vocab-scoped-bonus-interlock`**：**不符集合不得成長**（棘輪＋雙向基線防腐：既要「不得新增」，也要「修好了就必須移出基線」）＋**只要還有不符，就不許有任何宣告 `scope` 的紅利上線**；詞彙全部對上時互鎖**自動釋放**（判準抽成純函式並**打 fixture** 驗三個分支皆有鑑別力——比照 08-31「由構造成立的性質拿真檔斷言＝空心鎖」那條教訓）。⇒ 本卡未落地期間缺陷不會變活，但也**擋住 #89 的功能被使用**：這是刻意的取捨，已寫在卡上。
    - **⑥ ⚠️ 一個關於「驗證器自己」的事故，比本輪的鎖更值得寫下來**。負向擾動第一次是在前景跑的，**被 2 分鐘 timeout SIGTERM 殺掉，`finally` 因此沒有執行**，某一例的擾動（打壞 engine 側掃描）**留在磁碟上**；接著改到背景重跑，而它的「進場快照」把**那個已被破壞的檔案**當成乾淨基準 ⇒ 那一輪 **9/10 CAUGHT 的結論完全無效**（基線本身就紅，每一例都會報紅）。**當場作廢重做**，並把驗證器加了三道防線：快照落磁碟（可 `--restore` 復原）／**每輪先跑一次基線、不綠就中止**／每例還原後**逐位比對驗證還原成功**。⭐ **教訓＝「負向擾動」這個工具本身也會有『修一半而看不出來』的形狀：它報 CAUGHT 的前提是基線為綠，而那個前提從來沒有被斷言過。** 此後擾動驗證器一律先驗基線。
    - **⑦ 驗證**：node **318→319 全綠**（新增恰為本輪那一條鎖）；**負向擾動 11 例**（見下輪報告區的數字）——擾動集合刻意包含「**把解析器改成照抄 `heat.matchGame` 的子字串 fuzzy**」一例，因為那是後手最可能順手做的修法，而它會讓 `roulette` 被 `european-roulette` **誤中**而顯得已修好（反向 `dice` vs `dice-duel` 則會誤中另一款遊戲）。
    - **⑧ 台帳審金流 6 模組**（全部回填 `last_audited=09-01`）：收銀台 partial（`HL.payment|payMethod|payments.register|pspRegistry` **0 命中**＝連八輪／`FIAT_METHODS` 3＋`CRYPTO_COINS` 3）／錢包帳本 present（`ledger.record(` raw **10 行／7 檔**、扣 `reports.js:582` 字串述詞＝真插樁 **9／6**，連四輪逐位相同）／提款審核 absent（raw 4／實質 **0**，維持不開卡＝`CONTROL.avoid`）／P2P partial（`canTrade(` **1 處**＝連七輪）／獎金流水 partial（三缺口逐位未變＋本輪新第四面）／資金分倉 absent（`vault|保險庫|分倉|金庫` **0 命中**）。**新增一筆與覆蓋率正交的觀察（記在錢包帳本格）**：`ledger.record()` 的 `byGame` 分桶**沒有 catch-all 桶**，而 bonus 側有 `|| "其他紅利"` ⇒ 結構上 `Σ byGame.bet` 可小於 `totals.bet` 而不留痕跡；**今天沒有真的漏**（唯一 bet/win 插樁點 `live-stats.js:22`，50 個呼叫端全數傳了非空 game）⇒ 據實記「結構上可漏、當前未漏」，`present` 不下修。⇒ 接 08-29 的通則再加一問：**這個分桶的總和有沒有義務等於總額？**
    - **⑨ #153 本輪未做，理由據實**：它是 08-31 20:00 窗指派的實作候選，但本輪台帳輪替**在指定的那一格裡撿到一個潛伏的錢缺陷**，而 `max_implement_per_run: 1` ＋ `quality_over_quantity: true` ⇒ 一輪只徹底做一張。#153（探針列舉器判準改行為式）是**量測精度**改良、無時間壓力；#154 這條是**錢**且互鎖必須先上。⇒ #153 順延，**下輪平台軌 09-01 20:00 首選**。
    - **⑩ [P-FS]**：落地後首屏 **1638373／1638400 bytes、90 支 script＝與進場逐位相同**（餘裕仍 **27B**）＝**連續第十個「逐位未動」數據點**；本輪只動 `prototype/tests/`（不出貨）＋`intel/`＋`BACKLOG`，一支首屏 src 都沒碰；`sw` 依 08-26 判準**不 bump**（無任何檔會送到瀏覽器）。
    - **⑪ counters**：`platform_cards_opened` +1（#154）；`platforms_researched` **不加**（本輪未深挖平台、走主題式加深取材，且取材沒生出新缺口，據實不虛報）；`platform_cards_implemented` **不加**（落地物是鎖與探針，非佇列上的卡）。孤兒四筆（`prototype/games/registry.json`(M)／`prototype/src/styles/tokens.css`(M)／`prototype/games/slot-engine/`(??)／兩個 `Game assets/`(??)）依 §7 一位元組未碰。
    - **⑫ ⚠️ 驗證誠實聲明（連續第 N 輪）**：排程輪起不了 dev server（工具明文拒絕 unattended session）⇒ **無 preview、無目視**。本輪產出全在 `tests/`＋`intel/`＋`BACKLOG`，**零使用者可見表面** ⇒ 本輪不新增待目視項；既有待目視清單不變（大廳分群軸頁籤列／經濟旋鈕面板／#72 說明中心面板，桌機與 ≤720px 兩態）。
    - **⑬ 下輪平台軌 09-01 20:00**：台帳輪替下一格＝**功能**（18 模組·08-29 最舊）；實作候選＝**#153**（順延自本輪，S·純 `tests/`·零首屏）；調研：`platforms.json` **09-02 兩站到期**（thrill／mega-dice）可留給 09-02 窗。**#118 未解前一律不動首屏 src。**
