...
- **2026-09-05（平台軌 · **14:00 窗** · 台帳輪替審「**擴充性**」8→**9** 模組（新增一格）＋到期複查 tier-3 兩站 Legendz／CapySpin＋上一條**零首屏位元組**的止血鎖）**
    - **① 進場量測**：首屏 **1,638,181／1,638,400＝餘裕 219B、91 script**（與維護軌 12:20 收尾逐位相符）；`ledger-card-sweep` 正／反向皆 **0 筆**；`node` 進場 **338 全綠**。`lead_track: games` 本可讓路，但前手 08:00 窗明文指派本輪三項（台帳→擴充性／到期平台／續作）⇒ **做而不讓路**。
    - **② 取材（兩站到期複查，但本輪有一個該記下的限制）**：`WebFetch` 對兩站官網與 9 家評測站**全數 403**（legendz.com／playusa／casino.org／gamingtoday／next.io／sweepscasinos…）⇒ 本輪結論**一律為 WebSearch 二手摘要**，已在兩份 dossier 與 `platforms.json` 逐句標明，未見的畫面細節一律不臆造。到期 5→**3 筆**。
    - **③ ⭐ 本輪最重要的發現：取材與台帳輪替從兩邊指到同一格，而那一格本來不存在**。Legendz 上線 **Legendz Hub／「My Hub」**，把 Daily Drops／Missions／Badges／Tournaments／Leaderboards **五件事收進單一儀表板**，理由逐字是「不必在頁面之間跳來跳去」。回頭量自己：**我方這個表面比對手大**——`layout/app-shell.js` 的 `HUB_GROUPS`＝**4 分類 × 18 條 × 4,898 bytes**、涉 **18 個擁有者命名空間**、**34 個 API 引用**。⇒ **產品面不缺，缺的是形制**：`when`／`order`／`enabled` 在該區塊命中數**皆 0**（對照組：同形制的 `HL.support` 登記表有 `when()`），且全庫 **24 個 `HL.<ns>.register` 擴充點沒有一個是它**。**台帳盲點第 8 例**（前七：玩家保護／帳戶安全／Feature Flags／營運自動化／大廳版位／中央結算擴充形制／玩家自我統計面）。
    - **④ ⭐ 它也是首屏預算的一個具體泵浦**：每一張獎勵類卡（#45／#46／#47／#67／#74…）的落地步驟都含「在 app-shell.js 的 hub 加一項入口」⇒ **首屏檔隨功能數線性長大**，而 13 張卡正卡在這個預算上（餘裕 219B）。反過來：把 18 條搬離 app-shell **是目前唯一一條「做功能反而讓首屏變小」的路**（淨值需實測、不先宣稱）。
    - **⑤ 處置：不開雙胞胎卡**。出口形狀（登記表＋`enabled`／`order`／`badge`）、宿主檔（`layout/app-shell.js`）、要治的病（硬寫入口清單）三者與 **#93 `HL.nav`** 完全相同 ⇒ 依 SKILL 第 3 步去重規則**擴充 #93 範圍**（`surface` ∈ `side`／`bottom`／**`hub`**＋新不變量 (e) 擁有者自註冊、(f) `when()` 述詞），**本輪開卡 0 張**。#93 仍需 preview（三個表面目視）⇒ 不在 headless 輪落地。
    - **⑥ 本輪落地：一條零首屏位元組的止血鎖** `platform/rewards-hub-entry-owners`（住 `prototype/tests/`）——(a) 量程錨；(b) 每個 `HL.<ns>.<fn>(` 的 `<fn>` 必須在擁有者的**公開匯出物件**裡；(c) 這種形狀的入口清單全庫唯一一份；(d) 條目數棘輪 ≤ 18。`node` 338→**339 全綠**。
    - **⑦ ⭐ 負向擾動又一次拓到我自己（「出口 vs 提及」家族再一例）**：(b) 初版寫成「這個名字有沒有在擁有者檔裡出現過」，而擾動 P2（把 `core/shop.js` 的 `function points()` 連同匯出鍵一起改名）**穿了過去**——因為 `status()` 回傳物件裡剛好有一個**不相干的** `points: points()` 也長成 `points:`。改為括號配對抽出 `HL.<ns> = {…}` **匯出物件本身**後才真的抓到。**擾動 6/6 CAUGHT**（P1 呼叫端改名／P2 擁有者端改名／P3 加第 19 條／P4 第二份清單／P5 `HUB_GROUPS` 改名／P6 幽靈命名空間）、**BASELINE=0、RESTORED=0**；其中 P1／P5／P6 **只有本鎖轉紅**，P2 併觸 `platform/shop-registry-and-audience-gate`、P3／P4 併觸 `platform/i18n-attr-ratchet`（擾動新增了帶中文的節點，屬合理連動）。
    - **⑧ 兩筆「併入既有卡」當場寫進卡體（落實 capyspin.md 自己 08-06 記下的教訓）**：CapySpin 的 **Lucky Jackpot 由任務點數推高共享池**→ #78 「注資來源軌」候補維度（**刻意不做**：與 §11 真站送幣收斂方向相反）；**提款優先序做為 VIP 權益** → #63 第八個外部佐證。那條教訓本身是：「併入 #N」只寫在 dossier 建議區就等於蒸發（曾讓一條結論躺 31 天）。
    - **⑨ 其餘取材對照（皆無淨新卡）**：Legendz 帳內自助自我排除（6 月／1 年／3 年／終身・**二手待證**、#96 不得單憑此定稿）＋**2FA**（#125 +1）；CapySpin 自我排除**須 email 客服** ⇒ 我方「帳內即時自助」領先的**第四個外部反面佐證**（6 段 VIP／15%–25% 返水皆在 #29／#60 既有家族內）。兩站的**社群共決皆為「無」**、**玩家自我統計面皆未見**（維度 12 首輪外部對照：對手也沒做，#170 是領先項而非追趕項）。
    - **⑩ 下輪（平台 20:00）**：台帳輪替→**前端UI/UX**；到期餘 **3 筆**（zonko／kaasino／gokong·皆 tier-3）；最高價值續作仍＝**等 #169 裁決**（#93 擴充後反而是唯一一條可能回收首屏位元組的卡，**但需 preview**）。