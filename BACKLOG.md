...
63. ✅完成（2026-08-05 平台軌 14:00 窗建置輪） **VIP 分階提領 SLA + 分階額度（VIP 從「送錢軸」延伸到「服務水準軸」· 容器優先）** — S–M — 來源：**Dorados 2026-08-04 刷新**——其五階 VIP 決定的不是給多少錢，而是 **① 兌獎處理速度（L1 72h → 42h → L5 24h）② 每月兌獎上限（30,000 → 60,000 Gems）**；刻意**不分階**的是每日上限（全階 2,500 Gems）。同源訊號＝業界已把「兌獎速度」當獨立競爭軸橫評（sportsgambler「Fastest Redemptions」專題）。
    ...
    - ➕ **第八個外部佐證（2026-09-05 平台軌·CapySpin 到期複查）**：CapySpin 的 Platinum 段位權益明列 **「提款優先序」（withdrawal priority）** 與即時 rakeback 並列 ⇒ 「把提領服務水準當成 VIP 權益軌」的形狀與本卡逐項同構，本卡設計無需修改，僅上調佐證強度。⚠️ **落實 `capyspin.md` 08-06 自己記下的流程教訓**（「併入既有卡 #N」只寫在 dossier 就等於蒸發，曾讓一條結論躺 31 天）：本筆**當場寫進卡體**。
...
113. 🟦已批准待做 **競技場剩餘工作（規格已寫完、逐條有證據，照波次吃）** — M–L（分批，勿一輪吞） — 來源：**2026-08-21 前景競技場輪**（船長原話「競技場是一個很大的重點」）：5 份外部同類平台研究 + 4 批稽核 → 逐批敵對複驗 ⇒ 32 條存活、29 CONFIRMED。
    ...
    - 🚨 **2026-09-07 前景檢修輪（船長提報「配對系統壞了、沒辦法玩」）**：上面那批的「換頁沒收賭注」修法**本身**把整個玩法弄壞了 17 天（離場鉤被它自己那一次掛載開火 ⇒ 配對永遠停在「配對中…」）。已修並立鎖，全文見 [intel/arena-repair-2026-09-07.md](intel/arena-repair-2026-09-07.md)。**同輪連帶結案本卡的 ③**（`canJoin` 不重算）：可加入性收斂成 `joinability()`／`battleCta()`／`cardAction()` 單一出口、`updateCard` 每秒連按鈕一起重建、`vsslot.render` 以純判定 `noSeatFor` 擋滿房進場。另修「切語系/登出沒收在途賭注」「在途賭注從帳上消失」「逃單不留痕」「勝負已定後離場贏了也被沒收」「房卡模式名硬寫第 5 份（顯示成 1v1v1Crazy）」。
    - **仍待做（依價值）**：① **會員模式 F5 後戰績每列「敗 · −NT$ NaN」**（伺服器 payload 缺前端讀的欄位、全 repo 無 normalize；同一個根因也讓回放 10 輪縮成 1 輪、席位標籤全空）② 賞金池原地更新漏掉（挑戰次數在爬、賞金池凍結）③ ~~`canJoin` 不重算 ⇒ 滿房仍寫「加入 NT$X」、點下去付錢進滿房~~〔✅ **2026-09-07 前景已修**，鎖 `games/arena/room-cta-not-stale`〕④ 大廳「🔥 熱門玩家擂台」是凍結快照（倒數不動、已 splice 的鬼房還在賣；**要接 `HL.ticker`，不要新開 setInterval**）⑤ 房卡無 `done/plays` 進度（房間會在 ⏱ 還剩十幾分鐘時憑空消失）⑥ 進行中/結算兩套淨利公式（差一個開房費）⑦ `isBusyView` 與 `VIEWS[].isGame` 兩份真相（結算模態會蓋住直播房）⑧ 空狀態文案不分頁籤 ⑨ 四個缺的狀態：ROOM_OPEN（房間開著等人＋5:00 加入窗）／SEAT_FILLING／SPECTATE 觀戰／EXPIRED 逾時退款⑩ 結算卡兩欄制（本局分數／派彩）＋平手裁決可見化（現在已由 `HL.fair` 裁決但畫面沒說）＋「再戰一局」印原價 ⑪ 跳過本輪演出（Enter／點畫面）。
...
78. 🟦已批准待做 **彩金參與＝可選加注的「資格軸」（把隱形抽水變成明示選擇）** — S–M — 來源：**Card Crush 2026-08-07 複查**——其社群彩池是 **opt-in 側注**：slot 每轉**多付 0.1 MC** 才取得四級彩池（Mini/Minor/Major/Grand）的中獎**資格**，官方保證池 ≥50,000 MC（實測見 108,000+）。
    ...
    - ➕ **候補維度「注資來源軸」（2026-09-05 平台軌·CapySpin 到期複查併入本卡，刻意不另立卡）**：CapySpin 的 **Lucky Jackpot 由每日免費任務的 Task Points 一併推高池**＝「**非押注型行為也能成為共享池的注資來源**」，而我方 `HL.jackpot` 只吃 `onBet`。⚠️ **現在不做**：真站 JP seed 已依 §11 改為自籌 0，新增一條「不押注也能長池」的注資管道與經濟收斂方向相反 ⇒ 僅登記為本卡的第二軸候補，待真金經濟數值重調定案後再評估。
...
82. ✅完成（2026-09-12 平台軌 **08:00 窗**當輪落地 · `65bbc86`） **支付通道註冊表（收銀台的 provider/地區/幣別/限額述詞層 · 骨架先行、預設全關）** — S–M — 來源：**platform-modules「支付/收銀台」stowable_note**——該設計自種子日即寫明「支付方式為 provider 插件註冊表（enabled/地區/幣別/限額），關通道 toggle，加通道＝註冊 adapter」，但**歷經 07-30／08-05／08-10 三次審計、三次都被記下卻從未被認領**＝08-06 記載的「**處置管道沒有帳可查**」家族第 8 筆。
    ...
    - **✅ 落地（2026-09-12 平台軌 08:00 窗）**：新容器 `prototype/src/core/cashier.js`＝`HL.cashier`（`register/all/get/kinds/count` + `KINDS/FLOWS`；述詞欄位 `id/kind/name/icon/code/net/enabled/flows/regions/currencies/networks/min/max/order`），**容器零內建**（通道字面一個都不在容器裡）、**fail-closed**（id/kind 不合規或重複一個字都寫不進去）、**預設全關**（沒明講 `enabled:true` 就是關的）、**讀出去是唯讀純值副本**（JSON 深拷貝，外部改了不回寫表）。首批六筆註冊在 `layout/app-shell.js`（＝容器的第一批內容也是第一個消費者）。
      - ⭐ **實作當下查獲卡上沒寫的那一半，而它比卡上寫的更嚴重**：本卡（與台帳連十輪的機械複驗）盯的是 `FIAT_METHODS`／`CRYPTO_COINS` 兩個陣列＝**儲值側**；而 **`renderWd()` 另外自帶第三份清單**——硬寫 `[["fiat","法幣（銀行）"],["crypto","加密貨幣"]]` 兩格 ＋ 硬寫 placeholder `提款地址（USDT-TRC20）` ⇒ **玩家儲值進得了 BTC／ETH，卻只提得出 USDT-TRC20**，而且「哪些通道能提款」在資料形狀上無法表達。這條結構上落在兩個量測窗之外，所以**十輪「逐位未動」每一輪都是真的、也每一輪都不完整**（同 09-12 00:00 U38 的「射程排除了真相所在的位置」，只是現形在台帳的尺上）。⇒ 新增 `flows` 述詞（信用卡／超商代碼**只進不出**；銀行轉帳與三幣種進出皆可），**儲值側與提款側改讀同一張表**，提款幣別由註冊表產生。
      - **零回歸（node 重播首批註冊逐項比對）**：儲值·法幣＝`信用卡/超商代碼/銀行轉帳`、儲值·加密＝`USDT·TRC20/BTC·Bitcoin/ETH·ERC20`，**與改版前逐位相同**（順序、標籤、圖示皆未動）。提款側為**刻意的行為改變**：`fiat` 只剩銀行轉帳（原本那一格本來就只有銀行帳戶）、`crypto` 由一種變三種。
      - **額度依卡執行「委派而非新增」**：`min/max` 只做為述詞存在、**首批一筆都沒填**；常駐鎖 (B6) 反向擋住收銀台讀 `sel.min/max` 自己做閘，並以 `HL.sla.check(` 為反向錨確保既有額度閘沒被順手拆掉。
      - **鎖**：新增 `platform/cashier-registry`（node **363 項全綠**）——(A) 行為半**真的 require 進來跑**容器（零內建／fail-closed／預設全關／關掉的 `get` 不到／flows 分流／唯讀副本／地區述詞）；(B) 形狀半釘住 UI 端（舊兩陣列不得長回來／儲值與提款**兩個**消費者各自逐函式體釘死／renderWd 不得硬寫 `USDT-TRC20`／不得長出第二套限額／空清單連標題都不畫／`index.html` 掛載且排在註冊者之前）。掃描一律 `stripComments`＋`stripStringLiterals`。
      - ⭐ **負向擾動 20/20 CAUGHT，但首批是 15/16——MISSED 的那一條又是我自己的鎖**：(B4) 第一版只寫「`renderWd` 函式體內要出現 `HL.cashier.`」，於是把 kind 切換列改回硬寫 `["fiat","crypto"]` 之後，**巢狀在 `drawForm` 裡的幣別查詢仍在**、斷言照樣命中＝**正向全綠而第二份清單長回來了**（CLAUDE.md §4 形狀⑦(c) 巢狀洩漏的第二次應驗）。改成**兩份清單各釘一條**（`HL.cashier.kinds(` 與 `HL.cashier.all({…flow:"withdraw"})`）＋反向擋住「kind 字串陣列字面量」這個原形，重跑 P10／P10b **雙雙 CAUGHT**。
      - **首屏代價（據實）**：新檔 2,502B ＋ `app-shell.js` 淨增 ＋ `index.html` 一行，扣掉本輪回收的 **531B 失效註解**（`index.html` 自己宣告已失效的 `selftest` 擺放理由 ×6 句）⇒ **餘裕 3,808 → 289 bytes**、scripts 91→92。⚠️ **這把 #160 剛解開的額度用掉九成**，後續任何 eager 改動再度堵死 ⇒ 已開 **#184**（index.html 註解 11.2KB 的回收），而 **#118 仍是結構解法**。
      - **已知限制**：排程輪 `preview_start` 被環境拒絕 ⇒ **無 preview 目視**，收銀台/提款面實際渲染與三語譯文屬 **UNVERIFIED**；可證的是 node 行為測項、逐字守衛鎖與擾動。`enabled` 仍只能改碼（無營運出口）⇒ 台帳該格維持 `partial`。
...
93. 🟦已批准待做 **導覽入口註冊表 `HL.nav`：三個導覽表面的項目全是硬寫，本模組承諾的 registry 從未兌現** — S–M — 來源：**platform-modules「前端UI/UX / 導覽殼層」2026-08-14 複審**（本輪查獲，非外部平台）。
    ...
    - ⭐ **範圍擴充（2026-09-05 平台軌·14:00 窗「擴充性」輪替審計 + Legendz 到期複查，兩邊指到同一格）**：`surface` 由 `side`/`bottom` **擴為 `side`/`bottom`/`hub`**，把**福利中心 hub 的 18 條入口**一併納管。**為什麼是擴充本卡而不是開新卡**：出口形狀（登記表 + `enabled`/`order`/`badge`）、宿主檔（`layout/app-shell.js`）、要治的病（硬寫入口清單）三者完全相同 ⇒ 依 SKILL 第 3 步去重規則「同來源模組／同出口形狀＝以較早那張為主體落地」，**不開雙胞胎**。
      - **機械現況（本輪實測）**：`HUB_GROUPS`＝**4 分類 × 18 條 × 4,898 bytes**，涵蓋 **18 個擁有者命名空間**、**34 個擁有者 API 引用**；`when`／`order`／`enabled` 三個欄位在該區塊命中數**皆為 0**；`else ui.comingSoon(...)` 退路 **18 個**。對照組：同形制的 `HL.support` 登記表**有** `when()`。
      - **外部佐證（本卡首次有對手形制可對照）**：Legendz 2026-02-06 上線的 **Legendz Hub／站上顯示「My Hub」**，把 Daily Drops／Missions／Badges／Tournaments／Leaderboards 收進單一儀表板，二手來源逐字理由是「不必在頁面之間跳來跳去」⇒ **聚合中樞是一級產品表面**。我方產品面**比它大**（18 條 vs 5 格），缺的只有形制。
      - **新增不變量 (e)**：hub 條目的 `ic`/`label`/`badge`/`open` 由**擁有者模組自己註冊**（比照 `HL.support` 的「誰擁有規則誰負責解釋」），不得再由 `app-shell.js` 代寫；**(f)** 帶 `when()` 述詞，不適用情境不渲染（避免入口擠壓，同 #93/#94 既有原則）。
      - **落地即可回收首屏位元組**：18 條搬離 `app-shell.js` 後該檔淨減（現況 45,187 bytes，`HUB_GROUPS` 佔 4,898）——這是**目前唯一一條「做功能反而讓首屏變小」的路**，對 [P-FS] 219 bytes 餘裕與卡在 #118 的 13 張卡有直接助益。⚠️ 但擁有者那側要各自加註冊碼，**淨值需實測**、不可先宣稱。
      - **已先上止血鎖（2026-09-05，零首屏位元組）**：`platform/rewards-hub-entry-owners`（住 `prototype/tests/`）——(a) 量程錨；(b) 每個 `HL.<ns>.<fn>(` 的 `<fn>` 必須在擁有者的**公開匯出物件**裡（括號配對抓 `HL.<ns> = {…}`，不是「名字在檔裡出現過」）；(c) 這種形狀的入口清單全庫唯一一份；(d) 條目數棘輪 ≤ 18。**負向擾動 6/6 CAUGHT**。
...
118. 🟦已批准待做 **把三個「被 view 綁住的共享引擎」抽成 core：`HL.slotEngine`／`HL.gameFrame`／`HL.fgBoard`（解鎖 #112 剩下的 64.2KB）** — M — 來源：**#112 實作當輪的機械輸出**（`node intel/tools/first-screen-deps.js` 第 ④ 節，非人工推理）。
    ...
    - ⭐ **2026-09-08 前景實測：卡頭那個「64.2KB」是帳面數字，真正的淨省是 24.3KB，而且只來自一支檔**（船長裁決「做 #118 把空間騰出來」後，照卡上自己的閘「動手前先量」執行）：
      · `views/game-frame.js` 13KB — 唯一全域 `HL.gameFrame`，**沒有任何 view 殘餘** ⇒ 搬進 `core/` 是**純位移、淨省 0**；而它有 **64 個同步消費點**（bounty／chicken／instant-cases…各延遲遊戲皆 `HL.gameFrame.wrap(...)` 取回傳值）⇒ **零收益、最高風險 ⇒ 本卡不動它**。
      · `views/fgboard.js` 7.7KB — 唯一全域 `HL.fgBoard`，同理**沒有 view 殘餘、淨省 0** ⇒ **不動**。
      · `views/slot.js` 44.1KB（45,161B／691 行）＝**唯一有殘餘的那支**。逐段量：純數學 CORE（`HL.shadowRitual`）11,126B ／ 共享渲染原語 8,468B ／ **畫面本體（HUD・轉輪動畫・買入・賠付表・render）24,900B** ／ 出口宣告 664B。⇒ 引擎留首屏約 20.3KB、**view 可延後 24,900B＝24.3KB＝淨省**（過卡上 ≥10KB 的閘）。首屏 1,638,243 → 約 1,613,343，餘裕 **157B → 約 25KB**，九～十張卡一次解開。
      · **真正的工作量已機械定位**（卡上原本只說「這是本卡真正的工作量所在」，沒說是哪些）：`HL.slotEngine` 對外只有 6 個成員（`FG_LEVEL`／`makeGrid`／`drawSym`／`evaluate`／`tumble`／`symEl`），而 `fgboard.js` 只用其中 4 個（`makeGrid`／`drawSym`／`evaluate`／`symEl`）。畫面側**直接呼叫**（不經命名空間）引擎側識別字 26 個 ⇒ 拆檔後這些呼叫點要改走命名空間；另有 11 個名字被判「引擎讀畫面側」，但那是**分界線畫在 271 行的粗切造成的**（`tumbleAnimate`／`freshState`／`bal`／`alive` 等其實是畫面側、`symEl` 才是引擎側）⇒ **分界要手畫，不能照行號切**。
      ⇒ **建議把本卡的範圍收斂成「只拆 slot.js」**，`game-frame`／`fgboard` 的搬移改列為純架構整理（零位元組效益）另案處理。
...
149. ⬜待批准 **挑戰引擎每一局都收得到 `game`，而它從來沒有看過一眼——「這個促銷只在指定遊戲算」在架構上做不出來，可是簽名看起來是完整的** — S（加法式零回歸；卡在 #118，見下方首屏成本） — 來源：**台帳「活動/促銷框架」相鄰面 + 2026-08-30 取材五站共識**（`intel/platforms/gamdom.md`／`duelbits.md` 08-30 刷新）
    ...
    - **對手形制＝五站共識（不是單站新聞）**（2026-08-30 取材）：
      ...
      · **Stake.us**（2026-09-06 平台軌 20:00 窗補·第六站）— **Originals Challenge**：每週從 Originals 挑**指定一款**遊戲設一個目標倍數，5,000 SC 由**全體達標者均分**；另有掛在**單款 slot**上的專屬挑戰（例 Sea Shanty）
      · **Roobet**（同輪補·第七站）— **Roo's Specials**：在**指定遊戲**裡找特定倍數才解鎖獎池（評測逐字稱之為站台的 "metagame"）
      ⇒ 七站共識、跨社交博彩/crypto/受監管三種商業模式；**且 Stake.us 那條同時是 #83「達標即均分」型分配的第二條佐證**。
...
160. ✅完成（2026-09-11 前景·`ce5ff0e`）**首屏有 14.3KB 是同一句中文被打了兩遍——而首屏餘裕只剩 14 bytes，四張卡因此排在 #118 後面** — M（純機械折疊，零行為改變；**落地後首屏餘裕 14B → 約 14.6KB**）
    - ✅ **落地（2026-09-11 前景，`ce5ff0e`）**：折疊 **481** 個呼叫點、**30** 支 helper。首屏 **1,638,231 → 1,623,597 bytes**（門檻 1,638,400），**餘裕 169B → 14,803B**。
      實際射程比卡上寫的大：改成全 `src/`（首屏外只有 `views/tournament.js` 1 個點——正因為少才更沒理由留一個洞，見 #119 檔頭教訓）。
      helper 採 `function t(k, d) { d = d || k; … }` 而非卡上建議的 `t(k)`：**後者會讓未來任何一行 `t(key, fallback)` 靜默失效**（d 被忽略），而那正是 #129 合法形態。
      代價 30×13B，換到兩種呼叫形狀永遠都成立。
    - ✅ **驗收**：`node prototype/tests/run.js` 355→**356 全綠**；i18n 五面棘輪 **逐位元組不退**（dom/data/attr/fb 的 totals 改前改後完全相同）；
      preview 實測切 EN 畫面真的翻（「Event ends in」「A server-wide challenge — beat the dragon back…」）、切回繁中復原、**全站無 `undefined`**、零 console error。
    - ✅ **新常駐鎖 `platform/i18n-no-duplicated-default-arg`**（兩條方向相反的不變量）：A）全 src/ dup 呼叫點恆為 0；B）凡有單引數 `t("中文")` 的檔，helper 必須在**核可形狀白名單**內。
      B 是折疊**自己造出來的**風險（舊 helper 被單引數呼叫 ⇒ 無 i18n 分支回 `undefined`）——只守 A 不守 B 就是 CLAUDE.md §4「修一半」。
      白名單逐字列三種等價寫法（repo 裡都活著：`t(k,d)` 補預設 30 支、`t(k)` 1 支、`t(zh)` 1 支），且**每條都必須有真實使用者**（死條目會轉紅，防白名單腐爛）。
      含 9 條形狀探針（`k!==d`／`tt(`／`obj.t(`／註解／**字串字面量**／純 ASCII…）與兩條反向錨。**負向擾動 8/8 CAUGHT**，每例只有本鎖轉紅，基線先驗過是乾淨的。
    - ➡️ **解凍的卡**：所有標著「卡在 #118」的卡現在都有 14.8KB 可用（#132／#133／#134／#137／#138／#139／#141／#150／#154／#157／#158…）。
      **#118 仍該做**（拆 `slot.js` 可再省 24.3KB），但它不再是唯一解法、也不再是阻塞點。
...
168. ✅完成 **決定整個 backlog 排序的那份報告，量的是首屏的 9.8%——十張以上的卡一律寫「等 #118」，而 #118 的量程只有 4.8%，最大那一類 42.6% 至今零卡** — S（`intel/tools/first-screen-deps.js` 新增 `census()` ＋ `tests/checks-platform.js` 新增常駐鎖，**runtime 零位元組**）　`(2026-09-04 平台軌·20:00 窗 · 同窗開卡+落地)`
    - **它壞在哪（不是分析錯，是報告只說了 10% 的故事）**：`intel/tools/first-screen-deps.js` 的**預設 `scope` 是 `views/`**——首屏 90 支 script 裡的 **6 支／130.8KB＝9.8%**。它結尾印的那句「**可回收 KB 總量＝0.0KB**」自 2026-08-24 起被連續多輪引用為「首屏動不了」的**事實前置**，於是 BACKLOG 上 **10 張以上的卡**（#125／#127／#128／#131／#134／#137／#139／#141／#146／#148／#151／#152／#163）一律寫「**必須排在 #118 之後**」。
    - ⭐ **首次全庫普查的結果（本輪 `census()`，一律 `scope=all`、LF 正規化位元組＝與預算尺同一把尺）**：
      📌 `boot-registration-blocked` **24 支 569.2KB（42.6%）**／⛔ `first-screen-bound` 22 支 296.4KB（22.2%）／🔗 `shared-dep-blocked` 22 支 279.9KB（20.9%）／🟡 `needs-methods` 18 支 167.1KB（12.5%）／✅ `safe-to-lazy` 3 支 15.0KB（1.1%＝三個載入器自己，載入器不能延遲載入自己）。
      ⇒ **#118 的量程（views/ 的 shared-dep）＝64.7KB＝首屏 4.8%**（與 #118 卡上自記的 64.2KB 交叉相符），**而最大那一類 569.2KB 一張卡都沒有**。整個佇列的排序依據，看的是 10% 的樣本卻被當成全部。
    - ⭐ **這是 CLAUDE.md §4「尺的量程漏了一段」的第三個實例，而三個的共同點是「讀數看起來完全正常」**：① `rg-bet-gate-coverage` 的 `readdir` 漏了 `layout/`（09-02 修）② 首屏尺量工作區 CRLF 而非部署位元組（#165，09-04 08:00 修）③ 本卡：報告的預設量程是 views/，而結論被當成全首屏。**漏掉的那一段從來不出現在輸出裡**，所以三次都不是靠看報告發現的。
    - **落地**：① `census()`（**無論使用者要求哪個 scope 都印**，因此「只看 views 就下全首屏結論」在報告裡不再可能），順帶就地算出 #118 的量程佔比（不寫死數字＝不會隨檔案變動而說謊）；② 常駐鎖 **`platform/first-screen-blocker-census`**：(a) **涵蓋率**——每支首屏 `.js` 都必須落在恰好一個 verdict 類別，唯一豁免是逐筆寫得出理由的 `EXEMPT`（今天只有 `main.js`＝首屏渲染路徑本體）；(b) **加總＝另一把獨立的尺**——普查位元組＋豁免位元組必須**逐位等於**測項自己量的首屏 JS 合計（不是向工具要的數字）；(c) **量程涵蓋率**——首屏 script 住在哪些目錄、各幾支，必須與普查的分類數**逐位對得上**；(d) 防空綠（類別數 ≥3、總量 >1MB）。
    - ⚠️ **負向擾動 P1 首測 MISSED，抓到的是我自己的鎖在自我指涉**：初版 (c) 寫成 `fsDeps.censusDirs.forEach(...)`——**向被檢查的工具詢問「應該檢查哪些目錄」**⇒ 把 `CENSUS_DIRS` 砍成 `["views"]` 時**兩邊一起縮**、本鎖照樣全綠（而報告已退回 views-only 的錯誤故事）。**自我指涉的不變量可以靠同時縮小「被檢查者」與「期望值」來滿足**＝§4 家族的又一形狀。修法＝期望值一律從 `index.html` 現實推導（實測 core 69／layout 7／data 7／views 6 支）。
    - **驗證**：`node prototype/tests/run.js` 335→**336 全綠**；**負向擾動 5/5 CAUGHT**（P1 CENSUS_DIRS 縮成 views-only／P2 最大一類不入表／P3 測項 EXEMPT 清空／P4 `unclassified` 一律回空／P5 `classifiedBytes` 只累 views/），**每例只有本鎖轉紅（fails=1）**，先驗基線非髒（BASELINE 336/0）、還原後回 336/0。
    - **成本**：**runtime 零位元組**（只動 `intel/tools/`＋`prototype/tests/`）⇒ 首屏 1,637,793／餘裕 **607B 全程未變**、90 支 script 不變、**sw 不 bump**。自我檢測時間 +11s（普查多跑一輪全庫；`_maskCache` 已在同 process 內攤銷）。
    - **它為誰鋪路**：#169（下一張）＋上面那 13 張卡的排序從此有機械依據；`census()` 也是 #169 落地後「到底省下多少」的驗收尺。

169. ⬜待批准 **首屏被自己的擴充性形制釘死了：24 支檔、569.2KB 動不了的唯一理由是「它們在載入當下把自己寫進別人的登記簿」——而 repo 裡已經有一個容器把這件事做對了** — L（新協定 + 五個面板登記簿接線 + 逐支遷移；**每遷一支省 8–63KB**）— 來源：**#168 的全庫普查（機械輸出，非人工推理）＋ `intel/tools/first-screen-deps.js` 的 `REG_DEFERRABLE` 判準**
    - **問題**：普查最大一類 📌 `boot-registration-blocked`＝**24 支／569.2KB／首屏 42.6%**，而它們**不是開機必須執行**，是**開機時把自己寫進別人的登記簿**：`HL.econCfg.register`（最多）、`HL.support.register`、`HL.promoCal.register`、`HL.achievements.register`、`HL.reports.register`、`HL.gameAxes.register`。延後它們＝**那一格永遠是空的、不報錯、console 全乾淨、既有鎖全綠，只有玩家看得到少了東西**。
    - ⭐ **反諷之處**：**「容器先於內容」是本專案的擴充性主形制**（#109 `HL.reports`／`HL.dock`／`HL.gameAxes`／`HL.support`…），而**它現行的實作方式（執行期推入式註冊）正是把首屏釘死的那根釘子**。⇒ 這張卡不是「再做一個容器」，是**把註冊本身從「執行期推入」升級為「宣告式資料 + 首次使用時才載入本體」**。
    - ⭐ **repo 內已有做對的先例（所以這不是新發明，是推廣）**：`data/lazy-games.js` 的 MANIFEST **開機註冊 meta+stubRender**、view 檔載入時**以同 id 覆蓋換手**，漂移由 `platform/lazy-games-manifest` 鎖住。工具的 `REG_DEFERRABLE` 已經把「容器備有內容遲到協定」寫成**可查的三條判準**：①容器開機先以別處的 meta 註冊 stub ②內容檔載入時同 id 覆蓋 ③有機械鎖盯住 stub 與真值不漂移。**目前這張白名單只有 `games` 一筆。**
    - 🔴 **先抄進卡的阻塞事實（本輪逐筆查證，別讓實作輪重新發現一次）**：
      (a) **不是每個出站呼叫都是登記簿**：`core/happyhour.js:102`／`core/rakeboost.js:527` 出站的是 `HL.notify.add`＝**開機宣告行為**，延後＝那則通知永不發生 ⇒ 這類要走「補發」而非「stub 換手」。
      (b) **累積側不能等**：`core/live-stats.js:54` 在**每一注**同步呼叫 `HL.cashback.record` ⇒ 未載入就是**靜默少記玩家的返現**。⇒ 需要 **record-and-replay（遲到掛鉤）**：事件先進佇列，模組載入後重播。這也是 `HL.liveStats.record` 這個中央掛鉤第一次需要「訂閱者可以遲到」的語意（與 #152 同源）。
      (c) **看似安全的退回其實會改變數值**：`core/progress.js:43` 對 `HL.sla.bonusReqFor` 有 fallback「退回舊制常數」⇒ 延後 `service-level.js` 不會當機，但**紅利流水倍數會靜默變成另一個值**。⇒ 這類消費點必須先有**能同步回值的 stub**（工具的 `needs-methods` 就是在指這件事）。
      (d) **`HL._selftestQ`**：24 支檔全部在載入時把自己的瀏覽器端測項推進佇列 ⇒ 延後＝Demo 面板的自我檢測**靜默少幾項**。⇒ 需要「開檢測面板前先把宣告過的模組全部載入」這一條，且要有鎖（否則兩端測項數不同步，正是 `platform/selftest-registration-order` 在防的那個家族）。
    - **建議切法（一輪一支、每輪都能量出省了多少）**：先做**協定 + 面板類登記簿**（`econCfg`／`support`／`reports`＝消費者一律是「開面板」⇒ 最安全），首批候選按風險排序：`core/cashback.js`(8.1KB·但受 (b) 限制)、`core/onboarding.js`(11.5KB)、`core/faucet.js`(9.1KB)、`core/edge.js`(18.0KB)、`core/service-level.js`(37.9KB·受 (c) 限制)、`core/reports.js`(42.9KB)、`core/responsible.js`(63.1KB·**最大但 sharedRisks 35，排最後**)。
    - **為什麼標 ⬜待批准而非 🟦已批准**：它**改變全站 core 模組的載入語意**（不是新增一個面板），且 (b)/(c) 兩條要新增跨模組協定；依 #118 卡上同一條理由，**這種改動要在有可靠 preview 的輪做**，headless 輪只證得出換手、證不了「看起來對」。⇒ 請船長裁決：**(i) 照上面切法逐輪推進**／**(ii) 只做面板類三個登記簿（不動累積側）**／**(iii) 先擴預算門檻**（不建議：那是把尺調鬆而不是把東西搬走）。
    - **擴充性槓桿**：做完之後「登記簿式擴充性」不再與首屏預算衝突 ⇒ 上面 13 張卡（含 #125／#127／#137 三個 absent 的資安模組、以及 09-04 14:00 窗建議的 `HL.eventResults` 活動結果面登記簿）全部解閘；每一個未來的新容器也不再需要「把內容檔留在首屏」。

170. ⬜待批准 **每一注都在幫玩家記 6 個終身數字，而那 6 個數字全站沒有任何讀者——玩家看得到 25 條進度條，讀不到自己押了多少** — M（`achievements` 補一個唯讀出口 + 第七張玩家報表 + 口徑宣告；**不需要新容器**）— 來源：**新開取材維度 12「玩家的自我統計面」（36 份 dossier 真實命中 0/36）＋ Stake／BC.Game 兩站各做一半的形制分歧**
    - **問題（機械事實，可複跑）**：中央掛鉤 `core/live-stats.js:56` 每一注呼叫 `HL.achievements.record(game, bet, win)`，`HL_ACHIEVE` 累積 **6 欄終身統計**（`bets／wagered／wins／bestWin／bestMult／games`，站別隔離）。但 `core/achievements.js:179` 的公開 API 是 `{register, record, sync, status, open, ids}`——**`stats()` 不在上面**；而 `status()` 只把每一欄折成 `prog`（**0–1 的進度比例**）餵給徽章進度條。⇒ **那 6 個數字全站零讀者，玩家沒有任何一條路讀到它們。** 唯一叫「統計」的表面 `HL.liveStats`（📈 實時統計）是 `var data = fresh()`＝**本工作階段限定、重載歸零**（面板自陳「本瀏覽器工作階段」＝誠實，但也就沒有終身面）。
    - ⭐ **為什麼這一格值得做而不是「反正沒人要」**：外部兩個標竿站**各只做一半**——**Stake** 把 `my-bets/archive` 的逐期 JSON 給得很完整卻**零內建分析**（閱讀被 StakeStats／StakeArchiveDownloader／Stake Analyzer／Spindex 一整圈第三方接走）；**BC.Game** 有**內建圖表化分析**與「依時間區間/遊戲型別出自訂報表」，但**原生匯出來源未載**。⇒ 沒有一家兩者都有，而**我方的資料早就在**（缺的是出口與面，不是資料）⇒ 這是目前成本最低的差異化位置之一。
    - 🔴 **先抄進卡的阻塞事實（本輪逐筆查證，別讓實作輪重新發現一次）**：
      (a) **口徑必須隨數字一起出，否則就是再造一個「兩份真相」**：本輪已查獲並修掉一個現成的例子——`HL.vip.status().wager` 是 **edge 加權**（`HL.edge.weighted` 現行 1.00×–1.80×）且自 #65 起**可含儲值/簽到來源**的**經驗值**，而 `reports.js:501` 一直把它標成「VIP 累積押注」並 `money()` 成 NT$；徽章門檻用的 `HL_ACHIEVE.wagered` 才是**原始押注額**。**同一段人生兩個「累積押注」，差距最高 80%。**（本輪已修並立 `platform/vip-xp-label-single-truth`。）⇒ 新報表**每一欄都必須明寫是哪個量**。
      (b) **`stats()` 有一半是即時讀來的、不是存下來的**：`achievements.js:49` 的 `stats()` 回傳值裡 `vipRank` 讀 `HL.vip.status().index`、`streak` 讀 `HL.rewards.status().streak`、`variety` 由 `games` 物件的鍵數當場算 ⇒ **唯讀出口要明確界定回的是「終身統計」還是「終身統計＋即時忠誠維度」**，兩者混在一起匯出會讓 CSV 的欄位語意不一致。
      (c) **它與 #138（本機存檔清冊）是同一筆資料**：`HL_ACHIEVE` 是「這台瀏覽器上存了什麼」的一員，且目前**沒有上界**（不在 `platform/retention-bound-queryable` 的兩個上界之列）。兩卡若各做各的清冊會長出第二份真相 ⇒ **#170 的出口要能被 #138 的清冊直接引用**。
      (d) **期間軸不屬於本卡**：BC.Game 的形制是「先篩（時間區間/遊戲型別）再出報表」，而期間軸是 **#151** 的事；本卡先做**終身**那一欄，期間切片等 #151 落地後掛上去（**兩卡共用同一個來源，不各自再抄一份統計**）。
    - **建議切法（擴充性優先·容器已在）**：① `HL.achievements` 補唯讀出口（回終身統計 + 明確欄位描述子）；② 以 `HL.reports.register({ aud: "player" })` 掛第七張報表「我的終身統計」——**顯示與 CSV 匯出一次到位、受眾閘與匯出原語都不必重寫**；③ 每欄帶口徑述詞（原始額／加權額／涵蓋期間）；④ 徽章牆可順帶把進度條旁的「x / 目標」實數補上（同一個出口，零新資料源）。
    - **⚠️ 首屏預算**：`core/achievements.js`／`core/reports.js` 皆為首屏 eager 檔，本輪收尾餘裕 **219B**（#118／#169 未解前，本卡的 ②③ 大概率撞閘）⇒ **建議排在 #169 裁決之後**；但 ① 的唯讀出口極小、可先落地供 #138 引用。
    - **擴充性槓桿**：做完之後「玩家自己的數字」有了單一出口，#138（清冊）／#151（期間軸）／#159（匯出檔身分）三張卡都直接消費它，不必各自再開一份統計來源。
171. ⬜待批准 **有兩條設定會改變整個站的樣子，而玩家要改它們得先進一款遊戲——而且已經有三張卡在等一個沒人負責的「放偏好的地方」** — M — 來源：**GoKong 2026-09-05 到期複查（直取 casinolandia：把「Player Customization」＝語言＋幣別選項寫成獨立段落）× Stake 站層 `/settings/preferences` 專頁（本地幣別顯示就設在那裡、25 種法幣）× 台帳「前端UI/UX」分類輪替新增一格「玩家偏好面」判 `weak`**；同輪新開取材維度 **13「玩家偏好的安放處」**
    - **問題（機械事實，全可複跑）**：`core/game-settings.js` 的 `HL.gset` 是全站唯一玩家偏好持久層，`DEF` 恰 **4 條**（`fast`／`anim`／`hotkeys`／`fiatView`）。
      **編輯面全庫只有一份**——`grep -l "HL.gset.set("` ＝ `views/game-frame.js` 一支，掛在**遊戲外框 ⚙**（`:96`）與 **PiP ⚙**（`:149`）兩顆鈕上，**兩顆都在遊戲裡**。
      而那 4 條裡有 **2 條的生效範圍是整個站**：
      (a) `fiatView`（金額顯示幣別）被 `core/dom.js` 的 `money()` 消費 ＝ **全站金額格式化器**，實測 **67 支檔／384 個呼叫點**，改完還 `HL.app.refresh()` **全站重繪**；
      (b) `anim`（介面動效）被掛成 `<html>.ax-anim-off` **全域 kill-switch**（`components.css:2985`，與 `prefers-reduced-motion` 同一套規則），並由 `core/reveal.js` 讀取（揭曉層不只在遊戲裡用）。
      ⇒ **玩家在大廳想關掉動效、或把金額換成別的幣別顯示，必須先隨便進一款遊戲、開它的齒輪、再退出來。**
    - ⭐ **為什麼這張卡的價值不在那兩條設定，而在容器**：佇列裡**已經有三張卡各自需要「一個放偏好的地方」**，而**沒有任何一張擁有那個容器**——
      **#128**（通知分類軸與玩家偏好；卡體自陳「偏好入口位置**待 #118／#93**」）、**#148**（外觀/主題模式；`data-theme` 每次開機寫上去、CSS 零讀者，切換 UI 出口 0）、本卡。
      三個功能各自等一個沒人負責的容器 ⇒ 這正是本軌「**擴充性優先：先做容器再填功能**」要攔的形狀。
    - 🔴 **先抄進卡的阻塞事實（本輪逐筆查證過，別讓實作輪重新發現一次）**：
      (a) **入口位置卡在同一個瓶頸**：站層偏好面的入口要落在 `layout/app-shell.js`（側欄／抽屜／福利中心 hub），而它是**首屏 eager 檔**、本輪實測餘裕 **219 bytes** ⇒ **依賴 #93 `HL.nav`（入口登記簿）與 #118／#169（首屏預算）**。**面本身可以是延遲 route（零首屏成本），入口不行。**
      (b) **`set(k, v)` 對未知 key 靜默 return**（`if (!(k in DEF)) return;`）⇒ 打錯字的一列會**畫得出來、按得下去、什麼都不會發生**，測項與 console 一律沉默。**本輪已上鎖止血**（見下方 ④），實作時**不要把這條鎖放寬**。
      (c) **`fast` 與 `hotkeys` 刻意留在遊戲層**：兩者的消費端全在 `core/instant.js`／`core/table.js`／`views/*`＝真的只在遊戲裡有意義 ⇒ 站層面**不該**把 4 條一起搬走，要**按 `scope` 分**（見下方切法）。這一條若沒寫下來，實作輪很容易「全部搬去站層」反而讓遊戲內少了快捷。
      (d) **`anim` 與 `prefers-reduced-motion` 已經是同一套規則**（`components.css:2975` 與 `:2985` 兩塊內容相同）⇒ 站層面呈現時要**照實說「你的系統已要求減少動態」**，不要做出「開關顯示為開、實際被系統覆蓋」的第三份真相。
      (e) **偏好持久化目前走原生 `localStorage`（`ax:gset`）、不走 `HL.dom.lsSet`** ⇒ **不受 `HL.site.ns()` 站別隔離**（真站/假站共用，與語言/側欄/收藏同一派）。這是**刻意的**（UI 偏好兩站共用），實作輪**別順手加站別前綴**——會讓玩家切站後偏好莫名重置。
    - **建議切法（擴充性優先 · 容器先於內容 · 加法式零回歸）**：
      ① **`HL.prefs` 偏好登記簿**（新的延遲/小型 core 檔）：`register({ key, owner, group, kind:'toggle'|'select', label, desc, scope:'site'|'game', options?, when?() })`，沿 #93／#109／#54 既有形制——**擁有者自己註冊自己那一條**，不再由某支檔硬寫全部清單。
      ② **`HL.gset` 的 4 條改為由擁有者註冊**（`game-settings.js` 註冊 `fast`／`hotkeys` 為 `scope:'game'`、`anim`／`fiatView` 為 `scope:'site'`），`DEF` 仍是唯一的旗標真相（**不要變成兩份**）。
      ③ **兩個渲染面、同一份登記簿**：遊戲外框 ⚙ 只渲染 `scope` 含 `game` 者 + 站層生效者的唯讀提示；**站層偏好面**（延遲 route）渲染全部並依 `group` 分組。
      ④ **#128／#148 只需各自 `register` 一條**（通知分類開關、主題選擇）＝那兩張卡的「偏好面」部分自動有家。
      ⑤ **`scope` 讓缺口變成宣告可查**：本輪那條止血鎖的棘輪（站層生效卻只在遊戲內可編輯 ≤2）在 ① 落地後應改為「**任何 `scope:'site'` 的偏好都必須至少有一個遊戲外的編輯入口**」——守新形狀下的同一組不變量。
    - **刻意排除（連同理由，別在實作輪順手加回來）**：① **音效開關**——全庫 `AudioContext`／`new Audio(`／`volume` 命中 **0**＝這個站目前沒有聲音，加開關就是死 flag（`game-settings.js` 檔頭自己立的紀律）。② **版面密度／字級**——屬 DEBT 的自適應斷點收斂家族（9 個 distinct `@media`），需 preview 目視回歸，不進本卡。③ **主題明暗**＝#148，本卡只提供它的**貨架**。
    - **本輪已落地的那一半（零首屏位元組，住 `prototype/tests/`）**：常駐鎖 `platform/gset-preference-surface` ——(a) 旗標清單以 `DEF` 物件本身為錨（不是「檔案裡提到過的字」）；(b) 每個旗標都必須有編輯列（**死 flag 有機械出口了**）；(c) 反向：每個被當 key 傳進去的字面值都必須是真旗標（**打錯字的靜默列會紅**）；(d) 編輯面**唯一一份**；(e) **棘輪：站層生效卻只在遊戲內可編輯的條數 ≤2**（現況 2＝`anim`／`fiatView`，別再長第三條）；(f) 兩道防空心（抽取器至少抽到 4 個 key、消費端掃描至少 10 筆）。負向擾動 **6/6 CAUGHT**。
    - **（2026-09-08 平台軌 14:00 窗回填·擴充性輪替順帶查獲，不另開卡）**：本卡要的那個「站層偏好出口」，**入口其實已經存在、而且是死巷**——幣別下拉裡有一項「**錢包設定**」（`layout/app-shell.js:61`），點下去只呼叫 `ui.comingSoon("錢包設定")`（「此功能在本 Demo 中為示意」）。本輪對全 `src` 的 `ui.comingSoon` 做了機械普查＝**27 個呼叫點**，其中 **25 個是擁有者守衛的防禦分支**（擁有者皆 eager ⇒ 實務不可達）、**1 個是合法用途**（`views/lobby.js` 把標記即將推出的遊戲卡導向它）⇒ **全站真死巷就剩這一個**。而它的位置剛好對：`fiatView`（把餘額顯示成法幣）本來就該住在幣別下拉旁邊。⇒ **實作提示**：本卡的站層面板可以直接接上這顆既有的鈕，**不必新增任何導覽入口**（同 #72 接說明中心的作法），也就不必等 #93／#118 給位置——**這讓本卡的入口依賴從「卡在 #93／#118」降級為「只卡首屏位元組」**。
    - **工作量**：M（登記簿 S、兩面渲染 S–M、四條遷移 S、**入口卡在 #93／#118** ⇒ 這一段不是本卡能自解的）。

172. ⬜待批准 **我們有一條 30 天的永久簽到階梯，卻沒有任何一段「只屬於新手的前 N 天」——而那條被記了三次、讓路了三輪的缺口，今天第四次確認仍然成立** — M（新的有邊界多日軌 + 交棒語意；**受首屏預算硬阻塞，見下方 (a)**）— 來源：**Zonko 2026-07-02 首記 → 08-06／09-05 兩度複驗成立 → 本輪（09-05 20:00 窗明文指派「下一個平台輪若無新指派，這是無需再調研就能開的卡」）正式立卡**
    - **問題（機械事實，全可複跑）**：ApexWin 有兩套「多天」機制，**沒有一套是「有邊界的新手軌」**：
      (i) `core/rewards.js` 的 `LADDER` ＝ **30 天逐日遞增**（`LADDER_LEN` 30）＋ `MILESTONES` **{8:3000, 15:8000, 22:15000, 30:50000}** ⇒ **永久 streak**，斷了重來、走完 plateau，**對新手與三個月老玩家完全相同**；
      (ii) `core/onboarding.js` ＝ **6 小時啟用窗口**（`WINDOW_MS = 6*3600000`、`REWARD = 500`），任務＝首注＋簽到，**逾期不補發** ⇒ 是「首日轉化鉤子」，**不是多日軌**。
      ⇒ 一個註冊第 1 天的玩家，看到的階梯與一個第 400 天的玩家**是同一條**；而 6 小時窗口一過就永遠不再出現。
    - **對手形制（Zonko，三輪複驗一致）**：**8-Day Powerboost**＝首購啟動後把剩餘獎勵拆成**接下來 8 天逐日派發**、**必須每天登入才領得到全額**、**8 天走完即結束**（之後交棒給常態機制）。
      ⭐ **2026-07-02 的自我更正仍然有效且是本卡的設計要點**：它是 **back-loaded**——獎勵權重刻意壓在**第 6–8 天**，理由是「那正是多數玩家原本會流失的時點」。**不是把糖砸在第一天。**
    - 🔴 **先抄進卡的阻塞事實（本輪逐筆查證過，別讓實作輪重新發現一次）**：
      (a) **首屏預算是硬阻塞，而且比 #171 更硬**：`core/onboarding.js`（11,774B）、`core/rewards.js`（35,290B）、`core/rakeboost.js`（39,458B）**三支全部是首屏 eager 檔**（`index.html:95` 等），而本輪實測首屏餘裕 **219 bytes**（1,638,181／1,638,400，與 09-05 兩輪逐位相符）。
          ⇒ **本卡在 #169／#93／#118 任一條退還位元組之前不可能落地**，除非做成**淨零位元組**（例如把新軌的表塞進既有 `LADDER` 旁、共用既有 render）——但那樣就失去「有邊界」的語意。**請把它排在退還位元組的那張卡之後。**
      (b) **帳齡只有一份真相，不准再刻第二份**：`HL.rakeboost.newcomerTs()`（`core/rakeboost.js`）是全站唯一的「首次見到這位玩家」時間戳，`#107` 的 `HL.release.audienceCtx()` 的 newcomer 維度已經**向它求值**，檔頭明文寫著「多一份帳齡＝兩個新手期定義會分岔」。
          ⇒ 新手軌的「第幾天」**必須**由它推導，**不得**新增 `HL_FTUE_START` 之類的第二個錨；也**不得**沿用 `onboarding.js` 的 `s.start`（那是「進入主 shell 的時刻」，語意是窗口不是帳齡）。
      (c) **交棒語意必須寫死，否則會雙重派發**：新手軌與 `rewards.js` 的 30 天階梯**在前 8 天是重疊的**（第 8 天恰好是 `MILESTONES` 的第一個尖峰 3,000）。
          ⇒ 要嘛新手軌**只加不換**（明示為額外禮包）、要嘛前 8 天**由新手軌接管**而階梯從第 9 天起算——**兩者擇一寫進卡的驗收，不要留給實作輪判斷**。
      (d) **`HL.bonus.add` 一律要帶 `source`**（`HL.ledger` 靠它分類送幣成本，見 CLAUDE.md §4），且真站送幣值需另行收斂（§11 長尾清單已列 onboarding）。
    - **建議切法（擴充性優先 · 容器先於內容）**：把「有邊界的多日軌」做成**可註冊的軌道表**（`register({ id, days, rewardAt(day), anchor:"newcomerTs", onFinish })`）而不是硬寫 8 天——這樣 `#34` 連登階梯、季票的入門軌、未來任何「前 N 天」都是加一筆 spec。
    - **驗收（headless 可證）**：① 第 0/1/8/9 天的 `status()` 逐日快照；② 「必須每天登入」的漏領語意（漏一天＝那天的份額**不補發**，與 Zonko 一致）；③ 交棒不重疊的不變量（同一天不得同時由兩條軌發錢）；④ 帳齡只讀 `newcomerTs()` 的反向鎖（新增第二個錨即轉紅）。

173. ⬜待批准 **VIP 面板告訴玩家「你這一階享優先客服／專屬客戶經理」，而全站沒有任何一個地方可以聯絡到人——連 AI 助手答不出來時，都只是把玩家送回他剛剛沒找到答案的說明中心** — M（`HL.requests` 請求佇列容器 + 玩家端出口 + 營運端收件匣；**面可延遲、入口受 #93 阻塞**）— 來源：**新開取材維度 14「自助走到盡頭之後：人工通道與它的服務承諾」**（38 份 dossier 真實命中 0）× **Card Crush 2026-09-06 到期複查**（自我排除與 KYC 都要寄信給客服＝人工通道是**必經路徑**）× **Kaasino**（Prime 頂階＝專屬客戶經理＝人工通道是**分階權益**）× 台帳「後台」分類輪替新增一格「玩家請求佇列」判 `absent`
    - **問題（機械事實，全可複跑）**：站上有**兩處**明確對玩家提到客服，且其中一處是**賣點**：
      (i) `core/service-level.js` 的 `#63` 服務水準軸把「**客服層級**」註冊成一條分階權益維度——`id:"support-level"`、`kind:"info"`、`better:"higher"`、`byTier` demo `[1,1,2,2,3]`／live `[1,1,1,2,3]`、`fmt` 逐字產出 **標準客服／優先客服／專屬客戶經理**；它還經 `HL.econCfg` 進了**經濟旋鈕描述表**（`id:"sla"`），面板文案寫「…**與客服層級皆隨段位提升**」。**全檔 9 處客服字樣。**
      (ii) `core/responsible.js:922` 的自我排除說明寫「…**客服也無法代為解除**」——**這句話本身預設了有一個客服**。
      **而全站沒有任何聯絡得到人的出口**：`mailto:` 全 `src` **0 命中**、無工單／聯絡表單／請求佇列；`grep -rniE "ticket|工單|contact"` 在 `src/**/*.js` 的命中**全是註解與無關字詞**。
      唯一的問答表面 **AI Luna**（`layout/ai-concierge.js:38`）在罐頭 KB（7 條）與 `HL.support` 都答不出來時，回傳的終局字串是「…**完整規則可從側欄「•••更多」開啟說明中心查詢**」——**而說明中心正是他剛剛沒找到答案的地方**（同一個迴圈，0 個外部出口）。
      ⇒ **段位越高「客服層級」越高，但玩家永遠到不了那個人。** 這是 CLAUDE.md §4「修一半而看不出來」的一個新變體：**承諾在、通道不在**，而畫面完全正常。
    - ⭐ **為什麼這格是「後台」分類裡唯一一個純前端現在就能做的**：該分類八格裡 `會員管理`／`營運自動化規則` 判 absent 的**真正前置都是後端權威 + 角色閘**（§11，已連十輪維持不開卡）；
      而人工通道的**兩端都在我們手上**——玩家端＝一個送得出請求的出口，營運端＝`HL.opsBoard`（`views/ops-dashboard.js`，全站唯一營運面，寫入面**恰 2 處**、**無收件匣**）。
      ⇒ 它同時**餵飽了那個一直沒有 action 可執行的營運面**：`營運自動化規則` 那格連四輪的不開卡理由逐字是「先做規則引擎會做出一個**沒有 action 可執行**的容器」——**請求佇列正是第一個真的 action 來源**。
    - 🔴 **先抄進卡的阻塞事實**：
      (a) **入口位置卡在同一個瓶頸**：玩家端出口要落在 `layout/app-shell.js`（側欄／福利中心 hub）或 `HL.support` 的說明中心底部，而 `app-shell.js` 是**首屏 eager 檔**、本輪實測餘裕 **219 bytes** ⇒ 與 **#93／#118／#128／#148／#171** 同一個瓶頸。**請與 #93 一起看。**
      (b) **AI Luna 是第二個消費者，不接就是修一半**：通道做出來而 `ai-concierge.js:38` 的終局退路沒改，玩家最常撞到死路的那個表面**照樣是死路**。**本輪已上鎖止血**（見下方 ④ (d)）。
      (c) **不要做成假的**：Demo 無後端 ⇒ 請求只能落在本機（`HL.dom.lsSet`，受 `HL.site.ns()` 站別隔離）＋ 營運端收件匣讀同一份。**必須明示「這是 Demo，沒有人在另一端」**，否則就是拿一個承諾去補另一個承諾。真站接 Supabase 是後續的事（`docs/supabase-phase*.sql` 的形制可沿用）。
      (d) **`#96` 自我排除刻意不需要人**（`HL.rg` 純自助、即時生效、不可撤銷）——**本卡不得把它改成要送請求**。Card Crush 那條是**我們做得比它好**的地方，取材價值在於它讓我們回頭發現承諾與通道的落差，不是抄它。
    - **建議切法（擴充性優先 · 容器先於內容 · 加法式零回歸）**：
      ① **`HL.requests` 請求登記簿**：`registerKind({ id, label, owner, fields, sla?(tier), when?() })` ——**擁有者自己註冊自己的請求型別**（沿 `#72 HL.support`／`#109 HL.reports`／`#54 HL.release` 既有形制）。首批註冊者：帳戶問題／金流疑義／回報 BUG／申訴。
      ② **請求本身有狀態**（`open / answered / closed`）＋ 時效由 `HL.sla` 的 `support-level` **求值而來**（**唯一真相仍在 #63 的表**，不要在這裡再寫一份時效）。
      ③ **營運端收件匣**掛進 `HL.opsBoard`（它已是唯一營運面）＝第 3 個寫入面；
      ④ **止血鎖已先落地**（本輪，零首屏位元組，住 `prototype/tests/`）：`platform/human-channel-promise-ratchet` —— (a) 承諾必須真的還在（否則棘輪該退役而不是空綠）、(b) 通道落地前承諾面**棘輪 ≤2 支檔**、(c) **反向**：通道一落地就必須回填收緊為 0（T54「過期的哨比沒有哨更誤導」）、(d) **第二個消費者**：AI Luna 的死路只有一份，通道落地時必須同時改它。
    - **驗收（headless 可證）**：① 註冊一個新請求型別＝零改動出現在玩家端與收件匣兩側；② 請求時效與 `HL.sla` 同源（改 `#63` 的表，這裡跟著變）；③ 上述四條棘輪在通道落地後全部回填收緊且仍以**負向擾動**證明會紅。
174. ✅完成（2026-09-14 前景·`68d4831`）**提款面板告訴玩家「預計到帳時間 48 小時內」，然後在同一個 tick 把錢扣掉、記成一筆已完成的歷史、跳出「已提款」——那 48 小時從來沒有發生過** — M｜來源：platform-modules 台帳「金流」新增格〈交易狀態面／到帳事件〉absent ＋ **取材維度 15**（2026-09-06 新開）＋ bet365／Rollbit 兩站同輪到期、方向相反、指到同一格
    - ✅ **落地（2026-09-14 前景，`68d4831`）**：那 48 小時現在真的存在。`pushDemoTxn` 給 **withdraw 這一筆**寫上到帳時刻 `eta`，
      小時數只向 **#63 的 `wd-sla-hours`** 求值（卡上 (b)：禁止在 view 內另寫一份）。實測 `eta − ts` 恰為 **48 小時**、與面板印的同一個數字。
    - ✅ **狀態是「算」出來的，不是存下來的**——這一點**刻意沒照舊鎖的字面要求做**（它寫「必須帶 status 與 eta」）：
      存旗標就需要有人定期去翻它，而**玩家沒打開錢包的時候沒有任何人會翻** ⇒ 那個 `status` 會停在「處理中」而時間早就過了，畫面看起來卻完全正常（§4）。
      新增純函式 `txnStatus(t, now)`，同卡上自己指定的先例 `core/responsible.js` 的 `effective(lim, now)`。理由已寫進鎖的訊息裡。
    - ✅ **沒有 eta 的列一律 done，而且不補時刻**：那包含 #174 之前的舊紀錄與**會員模式的伺服器記帳**（`walletTxn` 只回 `balance`）——
      我們當時並不知道它何時到帳，**不回頭替歷史編造一個時刻**。這也正好落實卡上 (d)「會員側只顯示 eta、不做狀態機」。
    - ✅ **承諾不再被下一個動作否證**：兩條 toast（Demo／會員）從「已提款」改成「**提款已送出** … · 預計 N 小時內到帳」，
      N 走共用出口 `etaSuffix()`（同樣只向 #63 的表求值，取不到就整段不說）。
    - ✅ **卡上 (e)**：提款面板明示「Demo 站的到帳時序是模擬的：時間到了，紀錄會自己變成已到帳，**不經人工審核**。」
      ⇒ 同時守住卡上 🔴 邊界與 `core/service-level.js` 檔頭那條「人工狀態機仍不做」，說明中心「不是送人工審核」那句話維持為真。
    - ✅ **preview 走玩家真實路徑實測**（錢包 → 提款 → 確認）：toast 逐字「提款已送出 NT$ 1,000（銀行 · Demo） · 預計 48 小時內到帳」；
      紀錄列先顯示「🕒 處理中 · 預計 9/14 16:18 到帳」，把 `eta` 撥到過去後**自己**變成「✅ 已於 … 到帳」（沒有任何 sweeper）；
      切 EN 那一行真的翻成「🕒 In transit · arriving around 9/14 16:18」、切回繁中復原。
      ⚠️ 到帳那一行走 `HL.i18n.fmt` 而不是自己串字串——**這是 #175 當輪剛踩過的坑**：`HL.i18n.t` 是 passthrough，
      真正翻譯的 DOM walker 比對**整個文字節點**，自己串出來的節點含日期，補進字典也永遠查不到（假條目）。
    - ✅ **既有棘輪 `platform/withdraw-eta-promise-ratchet` 已依它自己的指示回填收緊**（原本寫「時序機制落地時必須回填」）：
      (c) 段從「偵測到 eta 就轉紅」改成「機制必須維持正確」——eta 必須向表求值／限定 withdraw／**pushDemoTxn 內不得出現任何像小時數的字面量**
      （允許集合僅 `{0, 50, 3600000}`）／不得**存** status；(c-2) 把 `txnStatus` 抽出來在 node 直接跑六個案例（含 `now === eta` 邊界、無 eta 的舊紀錄、儲值、null）；
      (c-3) `txnRow` 必須消費 `txnStatus` 且必須走 `fmt`；(c-4) 不得再出現「已提款」；(c-5) 不得長出人工審核語意 ＋ 說明中心那句話的反向錨。
      ⚠️ 回填時自己先踩了一個：在**剝掉字串字面量**之後去找 `"wd-sla-hours"`，那當然永遠找不到 ⇒ 拆成兩把尺
      （只剝註解的證明「參數是哪個維度」、連字串一起剝的證明「呼叫真的在程式位置」），兩把一起才擋得住 §4 形狀⑦(e) 的字串頂替。
    - ✅ **驗收**：`node prototype/tests/run.js` **386 全綠**；**負向擾動 13/13 CAUGHT**
      （不寫 eta／硬寫 48／不限定 withdraw／存 status／`txnStatus` 永遠回 done／邊界寫成 `>`／舊紀錄判 pending／`txnRow` 不問狀態／不走 fmt／toast 改回「已提款」／長出審核語意／**把守衛塞進字串而實際不呼叫**／說明中心那句話被拿掉）。
    - ⬜ **本卡沒做的那一段**：卡上 (d) 提到的 `wallet_txns.status`（phase8 SQL）**未做**——會員模式那一側仍只顯示 eta、不做狀態機，
      因為純前端狀態不得偽裝成伺服器狀態。要真的讓會員側也有時間線，需要後端，**需另裁決**。
    - **問題（機械事實，全可複跑）**：
      (i) **承諾在**：`core/service-level.js:148` 註冊 `wd-sla-hours`（label「提領處理時效」、`kind:"info"`、`better:"lower"`、`byTier` demo `[48,36,24,12,6]`／live `[72,60,48,36,24]`），`layout/app-shell.js:284-288` 把它印成「**預計到帳時間 <N> 小時內**」；說明中心 `sla/withdraw`（`service-level.js:598`）另寫「**實際到帳時間亦依段位不同**」＝第二處承諾。
      (ii) **行為否證它**：`app-shell.js:322` 在玩家按下「確認提款」的**同一個 tick** 就 `HL.state.set({balance: nb})` + `pushDemoTxn("withdraw", …)` + toast「**已提款**」。**「到帳」在全庫不是一個事件**——無回呼、無排程、無到帳時刻欄位。
      (iii) **紀錄裡沒有地方可以表達「在路上」**：`walletTxns` 逐筆形狀恰 `{kind, amount, bal, ts}`（寫入端 `app-shell.js:104-108` × 讀出端 `txnRow` `:112-122`，兩端逐欄相符）；`TXN_KINDS`(:98) 三型別只有 `{label, sign, tone, ic}`。全 `prototype/src/**/*.js` **交易語境**的 `pending|processing|審核中|處理中|待處理` **實質 0 命中**（唯二 `pending` ＝ `core/i18n.js:39` 語言包載入旗標、`core/responsible.js:211-227` 限額調升冷卻，皆非交易狀態）。
      (iv) **承諾沒有第二個消費者**：`HL.sla.valueOf(` 全庫**恰 1 個呼叫點**＝那個印字的面板本身。`wd-sla-hours` 是 `HL.sla` 六個註冊維度中**唯一一個 `kind:"info"` 且無任何非顯示消費者**的——三個 `cap` 由 `HL.sla.check()` 真擋、`bonus-wager-mult` 由 `bonusWagerMult()`→`bonusReqFor()` 真算、`support-level` 的通道缺席已由 **#173** 立案。
      ⇒ 這是 CLAUDE.md §4「修一半而看不出來」的**承諾面版本**，與 #173 同一天被抓到、同一個形狀（**承諾在、機制不在**），差別是**這一條更嚴重：#173 是承諾未兌現，本條是承諾被我們自己的下一個動作當場否證**。畫面全對、`node` 全綠、console 乾淨。
    - 🔴 **邊界（這是本卡最容易被誤殺的一點，先抄進來免得實作輪重新吵一次）**：本卡**不是** `CONTROL.avoid` 的「提款審核佇列」。
      `core/service-level.js:35-38` 的檔頭**已明文**把「提款→待審→核准的**人工**狀態機」列為刻意不做、不開卡——**那個判斷成立且維持**（人工裁量＝牌照／後端／角色閘／§11）。
      但同一句話**順手也擋掉了一件不需要牌照的事**：**無人工介入的自動時序**（送出 → 依時效自動到帳 → 狀態可見）。**(b) 這半邊從未被單獨評估過，卻連九輪被 (a) 的 avoid 一起帶走**（已在台帳「提款審核佇列」格 evidence 更正）。
      ⇒ 本卡**不得**引入任何「送人工審核／等客服核准」語意；說明中心 `sla/withdraw` 現有的「不是送人工審核」那句話**必須維持為真**。
    - 🔴 **其他先抄進卡的阻塞事實**：
      (a) **首屏瓶頸**：寫入端 `pushDemoTxn`／讀出端 `txnRow`／提款按鈕全在 `layout/app-shell.js`（**首屏 eager**，2026-09-06 14:00 窗實測餘裕 **219 bytes**）⇒ 與 `#93／#118／#128／#148／#169／#171／#172／#173` 同一個瓶頸，**必須排在「退還位元組」的那張卡之後**。這是第八張堵在此處的卡。
      (b) **時效的唯一真相是 `#63` 的表**：`eta` 一律向 `HL.sla.valueOf("wd-sla-hours")` 求值，**禁止在 view 內另寫一份小時數**（§4 第二份真相；`#107`／`#173` 已立同樣的紀律）。
      (c) **時序機制不必新造**：`core/responsible.js:211-227` 的 `{value, pending:{value, at}}` ＋ 到期自動生效，已是在庫的同形制先例（限額調升冷卻）。抄它，不要再發明第二種延時語意。
      (d) **會員模式那一側是伺服器記帳**：`app-shell.js:315-321` 走 `HL.api.walletTxn()`，回傳只有 `balance`。純前端狀態不得偽裝成伺服器狀態 ⇒ **會員模式本卡只做「顯示 eta」不做「狀態機」**，或在 phase8 SQL 補 `wallet_txns.status`（**本卡不含後端，需另裁決**）。
      (e) **不要做成假的**：Demo 站必須明示「這是 Demo 的模擬時序」，否則就是拿一個承諾去蓋另一個承諾（同 #173 (c)）。
    - **建議切法（擴充性優先 · 容器先於內容 · 加法式零回歸）**：
      ① **交易長出狀態維度**：`TXN_KINDS` 每個型別宣告 `states`（預設 `["done"]`＝**既有三型別零回歸**）與終態；逐筆 txn 加 `status` + `eta`（**缺欄位一律視為 `done`** ⇒ 舊 `walletTxns` 存檔不需遷移）。
      ② **`withdraw` 是第一個宣告非平凡狀態的型別**：`sent → arriving → done`，`eta = now + wd-sla-hours`（唯一真相見 (b)），到期自動轉終態（機制抄 (c)）。
      ③ **`txnRow` 多一個狀態徽章＋一行「預計 <時刻> 到帳」**——**狀態面完全收在錢包彈窗「歷史」分頁內，不需要新的導覽入口** ⇒ **本卡不吃 `HL.nav`／#93／#118 的入口瓶頸**（與 #128／#148／#171／#172／#173 不同，這是它唯一比較好落地的地方）。
      ④ **對照組（落地時照抄，不要各自發明）**：bet365 的三態逐字為 Pending／Processing／Completed 且出口與交易紀錄同一個地方；Rollbit 的狀態面是**專屬 Withdrawals 頁**＋鏈上 TXID。我方採 bet365 形制（同一個地方）＝符合 ③ 的零入口成本。
    - **驗收（headless 可證，不需 preview）**：① 新增一個宣告 `states` 的交易型別＝玩家端徽章與 eta 兩處零改動出現；② 舊 `walletTxns`（無 `status` 欄）讀進來一律呈現終態＝零回歸；③ `eta` 與 `HL.sla.valueOf("wd-sla-hours")` **同源**（改 `#63` 的表，這裡跟著變＝負向擾動要能抓到有人另寫一份）；④ 說明中心 `sla/withdraw` 的「不是送人工審核」仍為真；⑤ **本輪先落地的止血鎖 `platform/withdraw-eta-promise-ratchet` 必須在本卡落地後回填收緊**（見分析師日誌 09-06 14:00 窗 ④）。




175. ✅完成（2026-09-14 前景·`2141188`）**我們把「離線可用」寫進 sw.js 檔頭、寫進 index.html 註解、在 #14 打了 ✅、在台帳判了 8 輪 present——而離線骨架裡一行程式碼都沒有；更糟的是，我們每輪用來證明它還活著的那個數字（`CACHE` bump），正是每天把它清空 3 次以上的那個動作** — M（`PRECACHE` 由 4 筆擴為整份殼 + 逐筆容錯 + 安裝入口；**需一個有 preview 的輪次實測安裝/離線行為**）｜來源：**新開取材維度 16「裝機與離線：安裝提示的時機 × 離線時還剩下什麼」**（39 份 dossier 真實命中 0）× **Slotegrator 2026-07-17 為 Casino Builder 推出 PWA 功能**（B2B 平台商把「PWA 安裝 + service worker 快取遊戲資產 + 安裝提示時機可控（例：首儲後才跳）」當成賣給營運方的**平台能力**）× 台帳「功能」分類輪替：「PWA/離線 Service Worker」**present → partial**（8 輪來第一次不是只看版號）
    - ✅ **落地（2026-09-14 前景，`2141188`）**：`install` 不再手抄清單——**當下抓 `./index.html` 自己解析**出所有本地 `<script src>`／`<link href>`，
      逐筆各自 `catch` 放進快取。`addAll` 完全消失。實測 install 後快取 **99 個檔**（＝獨立推導的 oracle：92 script ＋ 3 css ＋ `./` ＋ index ＋ manifest ＋ icon），零重複。
    - ✅ **真的斷網驗過（不是宣稱）**：preview 起站 → SW 安裝完成、快取 100 筆 → **停掉 dev server**（未快取路徑實測回 `504 offline`＝伺服器確認已死）
      → 冷重載 → **大廳完整畫出來**（`#app` `aria-busy=false`、67 款遊戲登錄、5,311 字內容、零 console error）。截圖存於本輪對話。
      ⇒ 卡上驗收 ① 成立。② 與 ③ 由 probe 三情境證明（見下）。
    - ✅ **玩家可見產物（卡上的 ②）**：新增 `core/offline.js`＝`HL.offline`，把「離線可用性」變成**量測**而不是宣稱。
      清單的唯一真相是**這份文件自己**（`script[src]`／`link[href]`，快照取 `DOMContentLoaded` ⇒ 延遲載入的遊戲程式與樣式不算在殼裡）。
      在說明中心註冊一條「斷網的時候，這個站還開得起來嗎？」，`body` 是**函式**＝每次展開讀當下真值。實測回「開站需要的 98 個檔案全部都有離線副本（100%）」。
      文案刻意**不含**任何「離線可用」式承諾字樣，只陳述量到的數字；沒補滿時直說「斷網重開會是一片空白——不是『部分功能』，是整個介面畫不出來」。EN／zh-Hans 各補 6 條。
    - ✅ **鎖改寫成行為級**（原本是掃 `PRECACHE` 陣列裡有幾個 `.js`）：自解析之後那個陣列恆為 4 筆、0 個可執行資產
      ⇒ **舊鎖不會轉紅，它會安靜地變成空綠**（能力改變了而尺沒改）。新版由 `tests/sw-install-probe.js` **在 vm 沙箱裡真的把 install 跑一遍**，三個情境：
      `normal`（整份殼都要進快取）／`one404`（弄壞一支，其餘**仍須全進**＝證明「全有全無」已消失）／`newfile`（在 html 插一支 sw.js 沒聽過的新 script，**必須自動被收**＝證明沒有第二份清單）。
      ⚠️ probe 獨立成一支、由鎖 `execFileSync` **同步 spawn**，是因為 harness 嚴格同步而 install 全程是 Promise——不自刻「同步 Promise」墊片，墊片有 bug 時「沒抓到」與「結果被吞了」在輸出上完全同形。
    - ✅ **順手補掉一個空綠**：承諾閘 `missing===0 || claim===0` 在 #175 之後**兩邊都真**（能力到位、全站零承諾字串）⇒ 永遠綠、從未被考驗。
      依 #120 健檢②-b 的紀律抽成純函式 `claimGate` 並用合成輸入造 witness（`(3,1)` 必須擋、`(0,1)` 必須放、`(3,0)` 不該紅），另加兩條掃描器反向錨。
    - ✅ **驗收**：`node prototype/tests/run.js` **386 全綠**；**負向擾動 11/11 CAUGHT**（改回 `addAll`／只認 `<script>`／拿掉逐筆 catch／手抄第二份清單／不呼叫 `waitUntil`／只回前 10 筆／**probe 死掉**／`claimGate` 反過來／`AVAIL_CLAIM` 永不匹配／`#app` 塞靜態內容）。
    - ⬜ **本卡沒做的那一段（誠實記下）**：建議切法的 **③ 安裝入口容器化**（`beforeinstallprompt` 存起來、安裝提示時機做成可註冊規則）**未動**——
      它是另一個題目（安裝那一半），與「離線開不開得起來」正交。全庫 `beforeinstallprompt` 仍是 0 命中，卡 (v) 的敘述仍然成立。
    - **問題（機械事實，全可複跑；命令見每條括號）**：
      (i) **承諾在，而且只寫給我們自己看**：`prototype/sw.js:2-5` 檔頭逐字「離線可用」「install：預快取最小 app shell（含 index.html）**讓首次離線也能開**」；`prototype/index.html:8` 註解「PWA（#14）：可安裝 + **離線載入**」；BACKLOG **#14 ✅**「可安裝 + 離線載入」；台帳該格 **present ×8 輪**。⚠️ **玩家可見表面沒有這個承諾**——全庫唯一玩家看得見的 `離線` 字樣是 `core/meta.js:156`「進度離線保留」＝**持久性**主張（localStorage 為真，不在本卡射程）。⇒ 這是**台帳高報自己**，不是對玩家說謊；價值在於它讓一個真能力連續 8 輪被當成已完成。
      (ii) **骨架裡 0 行程式碼**：`PRECACHE` 恰 **4 筆**＝`./`／`./index.html`／`./manifest.webmanifest`／`./icon.svg`，**可執行資產（.js/.css）0 筆**。而 `index.html` 的 `<div id="app" aria-busy="true">` **內容為空**，整個介面由 **90 支本地 `<script src>` + 3 支 css ＝ 93 個 code 檔**在執行期畫出來（`grep -c '<script[^>]*src="\.' prototype/index.html`）。⇒ **離線首次啟動＝拿得到殼、拿不到任何程式碼**，`#app` 永遠停在 `aria-busy="true"`。
      (iii) **失敗是靜默的**：離線缺檔在 `sw.js` fetch handler 走 `new Response("", { status: 504 })`；導航類退回 `index.html`（於是又一次白畫面）。無錯誤面、無提示。
      (iv) ⭐ **稽核指標就是破壞行為**：`activate` 的語意是「刪掉所有 key !== 當前 `CACHE` 的快取」⇒ **每一次 bump 都把離線語料整個清空**，自動補回來的只有那 4 筆（0 個可執行）。而台帳這一格 8 輪的 evidence 量的**全是 bump 次數**（「六天 27 次」「兩日 53 次」「三日 13 次」）並以此證明「紀律持續運作」。實測近 7 日 `sw.js` **24 次** commit（`git log --since=... --oneline -- prototype/sw.js | wc -l`）＝**平均一天清空 3.4 次**。⇒ 玩家的離線語料非空的機率 ≈「他在最近一次 bump（中位數幾小時前）之後有沒有上線過」。
      (v) **安裝那一半也只是預設值**：`beforeinstallprompt` 全庫 **0 命中**、無站內安裝入口、無時機控制；`manifest.webmanifest` 是 `display:standalone`／`start_url:"./?demo=1"` ⇒ 玩家把它裝上桌面之後，**第一次離線開啟看到的就是白畫面**。
      ⇒ CLAUDE.md §4「修一半而看不出來」的**量尺版本**（家族第 12 例）：前幾例是「第二端不存在」「收到了卻不讀」「承諾被下一個動作否證」；**這一例是稽核指標與破壞行為是同一件事**，所以每一輪的複查都會回報「健康」。
    - 🟢 **為什麼這件事在本專案特別值得做（不是為 PWA 而 PWA）**：Demo 站是**純前端 + localStorage、零後端依賴**（`?demo=1` 明文「不連 Supabase」）⇒ **整站在離線下本來就能完整運作**，缺的只有「檔案在不在本機」。這與真金/會員模式不同，**不需要牌照、不需要後端**。對照組 Slotegrator 的賣點逐字是「service workers cache game assets, allowing interface browsing even with poor connectivity」——我方比它更有條件做到「離線也能真的玩」，而不只是「瀏覽介面」。
    - 🔴 **先抄進卡的阻塞事實與陷阱（別讓實作輪重新發現）**：
      (a) **整批原子失敗會被吞掉**：現行 `caches.open(CACHE).then(c => c.addAll(PRECACHE).catch(function () {}))`——`addAll` 是**全有全無**，清單裡任何一個路徑 404，整份 precache 就**靜默 no-op**，而畫面、console 與所有既有測項全綠。⇒ 落地時**必須**改成逐筆（或分批）各自 catch。**同窗新立的鎖已把這條寫成反向錨**：PRECACHE 一旦出現可執行資產而 silent catch 還在，測項當場轉紅。
      (b) **不得半修**：半份 precache 是最壞狀態（離線 boot 一半、其餘 504 靜默）＝比完全沒有更難診斷。鎖已強制「0 或 ≥ 整份殼（93）」。
      (c) **清單的唯一真相是 `index.html`**：**禁止在 sw.js 手抄第二份檔案清單**（那是 §4「第二份真相」的教科書形狀——新增一支 script 時沒人會想到回頭改 sw.js，而離線語料會安靜地少一個檔）。可行做法：建置期由 `index.html` 產生清單注入 sw.js，並立「兩份清單必須逐筆相等」的鎖；純前端無建置工具 ⇒ 次佳解是 sw.js `install` 時 `fetch("./index.html")` 自己解析出 script/link 清單（**離線首裝時網路仍在，install 本來就在線上跑**）。**建議採後者**：零建置工具、零第二份真相。
      (d) **首屏成本 0**：sw.js **不在** `index.html` 的 `<script src>` 清單裡（瀏覽器另外抓）⇒ 改它**不吃**首屏預算的 219 bytes ⇒ **本卡不卡 #118**（這是目前佇列裡少數幾張不堵在那個瓶頸上的卡）。
      (e) **1.6MB 的一次性預抓**：整份殼實測 **1,638,181 bytes**。install 時抓 93 個檔對行動網路是一次可觀的支出 ⇒ 建議 **install 只 precache「開得起來的最小集合」**（index.html + 3 css + boot 序列必要的 script），其餘沿用既有 network-first 執行期快取；**但「最小集合」必須是機械算出來的、不是人挑的**（否則又是第二份真相）。⇒ 若無法機械界定，就整份抓，並在 (c) 的自解析方案裡分批 catch。
      (f) **驗證需要 preview**：SW 的 install/activate/離線行為 **headless 驗不到**（§9：preview 面板隱藏時不合成影格、SW 生命週期需真瀏覽器；且改 sw.js 後 preview 本身會被舊 SW 餵舊檔，需先清 SW/caches）。⇒ **本卡不適合無人值守的排程輪落地**，與 #93 同類。
    - **建議切法（擴充性優先 · 容器先於內容）**：
      ① **離線骨架清單自解析**（見 (c)）：install 時取 `./index.html`、解析出 script/link 清單、逐筆 `cache.put` 各自容錯；失敗筆數記進一個可查詢的旗標。
      ② **「離線可用性」成為一個可回答的狀態**，而不是一個宣稱：一個純函式回答「目前快取涵蓋殼的幾 %」，供說明中心/設定面顯示（**這是本卡唯一玩家可見的產物，也是 (d) 反向錨解除的前提**）。
      ③ **安裝入口容器化**：接 `beforeinstallprompt` 存起來，安裝提示的**時機**做成一條可註冊的規則（對照 Slotegrator 的「首儲後才跳」），預設不改變現行行為（＝不主動跳）。
      ④ **bump 與離線語料脫鉤**（可選，需另評估）：`activate` 目前刪掉所有舊 key。若改為「保留舊快取直到新版 precache 完成」，離線能力就不再被每次 bump 清空——但要一併評估陳舊檔混用風險，**本卡不預設採用，只把選項寫下來**。
    - **驗收**：① 離線（DevTools offline）冷啟動能看到大廳而不是白畫面；② 任意刪掉快取中一個檔，install 仍成功且只少那一個檔（證明 (a) 已解）；③ 新增一支 `<script>` 到 `index.html` **不需要**改 sw.js（證明 (c) 沒有第二份真相）；④ 同窗新立的鎖 `platform/offline-shell-precache-ratchet` 依其 (b)(c) 兩條**必須被回填收緊**（棘輪從「0 或 ≥93」改成「必須涵蓋殼且逐筆容錯」）。
    - **下游/相鄰**：#128（通知偏好）與 ② 的可用性狀態同屬「站層狀態要有地方顯示」⇒ 若 #171 `HL.prefs` 先落地，② 直接註冊一條即可，**不需要新容器**。

176. 🏗️進行中（2026-09-07 平台軌 08:00 窗落地 A 半條款面；**2026-09-12 平台軌 20:00 窗落地 B 半的資格閘＋逐遊戲權重＋第四條計分軸**；**僅餘上限帽與條款的第二個出口**）
    - ✅ **B 半已落地的三件（2026-09-12 · commit `e655e3a`）**：
      · **資格閘＝權重 0，與 #89 `HL.wagerScope` 同一張表**——`core/tournament.js` 新增 `weigh()` 為**唯一**的資格與權重出口（`scopeWeight` → `HL.wagerScope.weightFor`），**不自刻第二份權重表**。Pragmatic 的「eligible games」與 Stake 的「live dealer 約 50%」在形狀上是同一條軸的兩端；分開做就會立刻長出兩份真相。
      · **`NAMES` 由字串陣列改為 `{ n, s }` 描述子**——名字與它的資格範圍住同一筆 ⇒ 「名字說 Slots、引擎沒在濾」**在結構上寫不出來**。三個預設賽事因此有了真的範圍：`週末 Slots 衝刺賽→slotOnly`／`Originals 大亂鬥→originalsOnly`／`深夜極速賽→standard`（桌遊·真人一成）。⭐ 這也是對「可切換性 0 使用者」（形狀⑤）的**正面回答**：要讓容器有人用，最可靠的做法是把它接進**預設路徑**，而不是等人來呼叫。
      · **`scoreAxis` 第四軸 `sumMult`（倍數總和）**，對標 Drops & Wins 逐字的 sum of win multipliers；與 `bestMult` 同一條紅線（`bet<=0` 或 `win<=0` 不得產生倍數）。
      · 條款面同步：`SCOPE_RULE`（逐 preset 一句整句片語）＋「合格遊戲」KV（值向 `HL.wagerScope.labelOf` 求值）＋`status().scope` 為唯一出口；i18n EN／zh-Hans 各 +7 條。
      · **#89 `HL.wagerScope` 的消費者 1 → 2**（`core/progress.js` 紅利流水 ＋ `core/tournament.js`）——上一輪把「全庫只有 1 個消費者」記成缺口，本輪還了它一個。
    - ⭐ **倍數軸為什麼只受閘、不受縮放**：倍數是比值，`bet` 與 `win` 同縮會抵銷；而先取整再相除會把小額倍數整個抹掉（`bet=3／win=7` 在 10% 權重下會變成 `0/1`＝0×）。⇒ `weigh()` 只縮 `unit==="coin"` 的軸，倍數軸的權重只作為資格閘。這條有負向擾動（P3）盯著。
    - ⬜ **仍餘兩件（維持 🏗️）**：
      · **上限帽（單筆獎項絕對上界）＝本輪刻意不做**。它今天沒有任何使用者，而**這張卡診斷的正是「零使用者的容器」**；在同一輪裡新增第五個零使用者旋鈕，只會花掉 eager 位元組而沒有見證者。等到真的有一場需要宣告上界的賽事再做。
      · **條款的第二個出口仍不存在**：說明中心（#72）的註冊者裡仍無錦標賽（`support.register` 於 tournament 兩檔仍 0 命中）⇒ 離開賽事頁就查不到規則。BitStarz「每一則促銷條款公開、**每次版式相同**」是本輪取到的第三筆獨立佐證。
    - **驗證**：`node` 368 → **369 全綠**（新增行為級鎖 1 條、改寫 1 條）；**負向擾動 20 例全數 CAUGHT**（另有 3 例是我自己的 harness 無效擾動、1 例錨點打錯字，見分析師日誌）；`sw` v288 → **v289**；首屏 **2,072 → 980 bytes 餘裕**（92 支不變）。⚠️ **無目視**：`preview_start` 第 11 次被環境拒絕（無人值守 session）。
    - 〔原卡文如下〕 🏗️進行中（2026-09-07 平台軌 08:00 窗 · **條款面那半已落地**，資格閘與權重那半待批准） **旗艦錦標賽的玩法說明把「憑什麼排名／哪些遊戲算／怎麼分」寫死成散文，而 #85 早就把這三件事做成可切換的資料——於是那個可切換性至今 0 個使用者，而賽事名稱今天就在對玩家宣告一個引擎裡不存在的遊戲範圍** — S–M（**兩半分開**：條款面求值那半住**延遲載入**的 `views/tournament.js`＝**零首屏位元組、已落地**；資格閘＋逐遊戲權重＋上限帽那半住 eager `core/tournament.js`／`core/score-axis.js`＝**吃首屏、卡在 #118**）｜來源：**新開取材維度 17「活動的條款面／加入之前公布了什麼」**（前 16 條維度對「條款」的真實命中數＝0）× platform-modules 台帳「活動」分類輪替新增格〈**活動條款面／規則的單一真相**〉判 `weak`（台帳盲點第 13 例）× **Pragmatic Play Drops & Wins（一手官方頁）＋ Stake Daily Races（二手）兩筆獨立佐證、從相反方向壓同一格**
    - **機械事實（可複跑，全 `prototype/src` 靜態分析）**：
      · `views/tournament.js` 的玩法 modal 是**玩家唯一的條款面**（`HL.ui.rules` 全庫僅 5 個呼叫點，活動類只有這一個）。它上一行 KV **已經正確地讀 `st.axis.label`**（#85 落地時做對了一半），而下一行散文寫死「限時賽期內，於任一遊戲完成的**有效押注**（含跟注）即累積積分。」⇒ **`axis` 一換成 `bestMult`，同一個彈窗裡「最高倍數」與「有效押注即累積積分」並列**。
      · 付獎百分比 `25%／14%／9%／1.5%／1.16%` 是**手抄的第二份真相**：真相 `SPLIT`（`core/tournament.js:28-32`，30 筆、合計恰 1.0）**唯一讀者是 `prizeFor`**；外部只有 2 處讀 `SPLIT.length`（`layout/app-shell.js:541`／`views/tournament.js:122`），**沒有任何人讀那些百分比**。
      · `NAMES` 5 個賽事名裡 **2 個宣告了遊戲範圍**（「週末 **Slots** 衝刺賽」「**Originals** 大亂鬥」，`freshEvent()` 隨機取一個），而 `record(bet, win, game)` 的 `game` **只被拿去當 `groupKey` 的分組鍵、從不過濾**（`eligible|allowedGames|onlyGames|qualif` 於該檔 **0 命中**）⇒ **在一場叫「Originals 大亂鬥」的賽事裡用百家樂刷分照樣進榜、照樣領獎。**
      · **#85 買下的可切換性至今 0 個使用者**：`startNew`／`settleAndCycle` **帶 spec 的呼叫點＝0**（唯一呼叫點 `views/tournament.js:177` 不帶引數）。
      · **權重那一軸完全缺**：`HL.wagerScope`（#89）已是逐類別權重表（恆在 [0,1]、真站不寬鬆、fail-open），但全庫**只有 1 個消費者**（`core/progress.js` 的紅利流水）；`tournament.js`／`score-axis.js` 對 `wagerScope` **0 命中**。
      · **條款沒有第二個出口**：說明中心（#72）11 個註冊者裡**沒有錦標賽**（`support.register` 於 `core/tournament.js` 與 `views/tournament.js` 皆 0 命中）⇒ 離開賽事頁就查不到規則。
      · **上限帽概念全缺**：`cap|maxPrize|上限帽` 於 tournament／score-axis **0 命中**。
    - ⭐ **為什麼這是 CLAUDE.md §4「修一半而看不出來」家族的新形狀＝「可切換性被條款面反鎖」**：前幾例是「容器做好了、接線沒補完」（`HL.dock` 零外部註冊者／`data-theme` 零 CSS 消費端）或「收到了卻不讀」（#149）。這一例**容器做好了、接線也接了一半（KV 求值）、而剩下那半是散文** ⇒ 後果不是畫面壞掉，是**沒人敢用那個容器**（換賽制就會產生自我矛盾的文案）。於是每一個既有驗證面都是綠的：`node` 全綠、console 乾淨、中文下畫面完全正常，`#85` 的零回歸測項也全綠（因為**沒有人換過軸**）。
    - **對手形制（維度 17 首輪即取到兩筆獨立佐證）**：
      · **Pragmatic Play「Drops & Wins」Season 7**（**一手** `pragmaticplay.com/en/drops-and-wins/`）——逐字 `the industry's largest **provider-funded** prize pool`（誰出獎）、`Daily leaderboards are ranked based on the **sum of win multipliers** accumulated during the tournament`（計分方法逐字公布）、`up to 100,000x their bet – **capped at €100,000**`（**相對倍數上界 × 絕對金額上界並存**）。⇒ 「倍數**總和**」是我方 `scoreAxis` 三軸（turnover／bestWin／bestMult）**沒有的第四軸**，而它只是一筆 `define()`。二手補充：€25m 池、每月 60+ 支合格 slot **會輪替**、opt-in 每週一次且**入場點在遊戲裡的彈窗**（已回填 #136 的設計依據）。
      · **Stake「Daily Races」**（二手評測；官方頁 403）——`points based on wager amount (**multiplier varies by game**)`，且 **live dealer 只計約 50% 權重**、付獎深 **前 5,000 名**、**不需 opt-in**（下第一注即入榜）。⇒ **合格遊戲的權重是公布出來的條款**，而我方早就有那張表（#89）卻沒接。
      ⇒ 兩站一供應商一營運方、一從「誰出獎與上限」一從「算幾成」，**壓同一格**：條款要在投入之前說清楚。
    - **範圍（兩半，刻意分開）**：
      · **A. 條款面向引擎求值（0 首屏位元組 · 本輪已落地）**：① `views/tournament.js` 新增 `AXIS_RULE`（逐軸**整句片語**，覆蓋 `score-axis.js` 的每一個軸）＋ `GROUP_RULE`（`groupBy==="game"` 時追加，說明「各自一份榜、各自一份獎池」）；② `core/tournament.js` 的 `NAMES` 去掉兩個範圍宣告（`週末 Slots 衝刺賽→週末衝刺賽`／`Originals 大亂鬥→全站大亂鬥`，**淨 −11 bytes 首屏**）；③ i18n 三語同步（3 條新規則句 ＋ **5 個賽事名**——實測原本 5 個名字**一個都沒進字典**，旗艦活動的名字對英文玩家一直是繁中）；④ 常駐鎖 `platform/tournament-terms-single-truth`。
      · **B. 資格閘 ＋ 逐遊戲權重 ＋ 上限帽（吃首屏 · 卡在 #118 · ⬜待批准）**：① `freshEvent` 的 spec 加**選用** `scope`（未宣告＝今日行為逐位相同），`record()` 讀它**已經收到**的 `game` 做資格過濾；② 權重**向 `#89 HL.wagerScope` 求值**，**不得在 tournament 自刻第二份權重表**；③ `scoreAxis` 新增 `sumMult` 軸（需與 `bestMult` 同樣擋 `bet<=0`／`win<=0`，否則無限大倍數）；④ `prizeFor` 加**選用**絕對上界；⑤ 名稱可恢復範圍宣告——**且屆時必須恢復**，因為新鎖的反向錨會在 `record()` 長出資格閘的那一刻**主動轉紅**逼我們回填。
    - 🔴 **首屏成本（B 半卡在 #118 的原因，先寫下來別讓實作輪重新發現）**：`core/tournament.js`（`index.html:88`）與 `core/score-axis.js`（`:85`）**都是首屏 eager**，而本輪收尾實測餘裕 **230 bytes**（A 半已把 219 → 230）。`views/tournament.js` 是 **#112 第三批延遲載入**（`index.html:193`）⇒ **A 半的位元組完全免費**，這是本卡與 #171–#175 最大的不同。
    - 🚧 **反重複界線（四條）**：
      · **不是 #149 的雙胞胎**——#149 治的是 `challenges.record(game,…)` **收到引數卻讀取次數 0**（挑戰引擎）；本卡治的是**條款面與引擎各說一套**（錦標賽），而且本卡的 A 半**根本不動引擎**。兩者是**同族不同層**：#149 在引擎層，本卡在條款層；#149 落地後本卡 B 半的 `scope` 應與它共用同一份「遊戲 id 唯一真相 `HL.games`」的紀律。
      · **不是 #83（分配軸）的雙胞胎**——#83 買「達標即均分」這**新的分配法**；本卡 A 半只要求**現有的分配法被正確地說出來**（付獎百分比與 `SPLIT` 對得上）。#83 落地時，本鎖的 (c) 項會逼它同步改那句話＝**本鎖是 #83 的護欄，不是它的競品**。
      · **不是 #64（遊戲專屬任務）的雙胞胎**——#64 造「把玩家導去某一款」的**新工具**；本卡 B 半只是讓**既有的錦標賽**能宣告範圍。
      · **不是 #167（賽果可讀）的雙胞胎**——#167 是活動的**收尾面**（跑完之後長什麼樣，維度 10）；本卡是**條款面**（加入之前被告知什麼，維度 17）。兩者是鏡像，台帳刻意分成兩個模組。
    - **驗收（headless 可證，不需 preview）**：① `AXIS_RULE` 覆蓋 `score-axis.js` 每一個軸且**無兩軸共用同一句**；② 規則句的付獎深度與逐名次百分比與 `SPLIT` **逐項相符**、`SPLIT` 合計恰 1.0；③ 賽事名稱在**沒有資格閘**的前提下不含任何範圍詞；④ **反向錨**：`record()` 長出資格閘 ⇒ 該項主動轉紅（承 T54「過期的哨比沒有哨更誤導」）；⑤ 規則句與賽事名稱**全數有 EN／zh-Hans**（語言包延遲載入＝補字典不吃首屏）。**負向擾動 8 例逐一驗過**（見分析師日誌 09-07 08:00 窗 ⑦，含**首測 MISSED 一例＝我自己的鎖漏收 `GROUP_RULE`**）。
    - **下游/相鄰**：B 半落地後「每週換一款指定遊戲的錦標賽」＝`startNew({ name, scope, axis })` 一行，**不需要新容器**（#49 排程軸 + #52 opt-in 已在位）；#136（遊戲內活動掛件）取得「加入點在遊戲裡」的對手依據；#146（創作者軸）取得「誰出獎／供應商聯網促銷」的第二條設計依據（**本輪刻意不開第三張卡**——那條軸要等同仁自製遊戲數量上來才成立）。

177. ✅完成 🚨 **競技場對戰整個玩法壞了 17 天，而 225 項自我檢測全綠——因為修法本身的缺陷剛好落在「驗證配方刻意繞過的那一段路」上** — S（改 4 支檔、淨 −938 bytes 首屏）｜來源：**船長直接提報**（2026-09-07：「競技場配對系統壞了 沒辦法玩 請全面檢修」）
    - **全文＝事故報告 + 修法 + 制度性後果**：[intel/arena-repair-2026-09-07.md](intel/arena-repair-2026-09-07.md)
    - **P0 根因**：`main.js` 寫 `mountView(def.render(s), …)`＝**參數先被求值** ⇒ 新 view 的 `render()` 已把離場鉤 push 進 `exitFns`，接著 `mountView` 的 `runExit()` 就把**剛註冊的那一個**開火；vsslot 的鉤子是 `clearTimers()` ⇒ `phaseSearching()` 排的 `later(phaseFound, …)` 一掛載就被清掉 ⇒ **配對永遠停在「配對中…等待玩家加入」**（preview 實測 60s 不動、零 console error）。
    - **為什麼測項與手測都沒抓到**：① 08-21 立的鎖只守「有沒有註冊鉤子／有沒有跑 runExit／是不是一次性」＝**守寫法，不守存活**；② 08-21 的 preview 驗證照 §9 配方直接 `h.appendChild(g.render())`，**那個配方的存在意義就是繞過 mountView** ⇒ 缺陷躲在被繞過的那一段。⇒ 已寫成 CLAUDE.md §4 家族**第 6 種形狀**，並在 §9 配方加上「射程邊界」警告。
    - **修法（由構造上排除）**：`mountView(build, backTo, opts)` **只收工廠函式**（傳節點直接 throw），清理（`stopAll` → `runExit`/`dropExit` → 清 DOM）一律先於 `build()`；`renderApp` 不再自行 `runExit`（開火時機收斂一處）；同頁重繪走 `renderApp({rerender:true})` → `dropExit()`＝丟掉不開火；登出路徑補 `runExit("signed-out")`；`vsslot.later()` 自帶存活閘。
    - **同輪連帶修掉（皆有 preview 實測或 node 鎖）**：房卡 CTA 每秒說謊（滿房寫「加入」／空房寫「觀戰」，收斂成 `joinability`／`battleCta`／`cardAction`，`updateCard` 連按鈕一起重建）｜滿房進場會擠掉最後一位在座玩家（`noSeatFor` 純判定守門）｜切語系／登出沒收在途賭注｜在途賭注從帳上消失（`room._escrow` ＋ `escrowTake` 冪等）｜逃單不留痕（棄局現在會記一筆生涯敗局）｜勝負已定後離場贏了也被沒收（`pendingSettle`）｜房卡模式名硬寫第 5 份（顯示成「1v1v1Crazy」，改問 `HL.battleMode.labelOf`）。
    - **鎖與驗證**：新鎖 `games/arena/room-cta-not-stale`、`games/vsslot/forfeit-is-honest`；改寫 `games/arena/exit-hook-settles-escrow`、`games/arena/battle-single-charge`。`node` 345→**346 全綠**、**負向擾動 34/34 CAUGHT**。⭐ **擾動過程抓出兩條自己的空綠鎖**（`/filledHere >= room.players/` 擋不住 `if (false && …)`；`/HL.arenaStats.record\(/` 擋不住 `void 0 && …`）⇒ CLAUDE.md §4 補**第 7 種形狀「斷言認的是概念還是寫法」**。
    - **首屏**：三支 eager 檔（`app-shell.js`／`main.js`／`arena.js`）的長註解改為指標到 intel＋鎖 id ⇒ **淨 −938 bytes**，`platform/first-screen-budget` 綠。`sw` v272→**v273**。
    - **UNVERIFIED（誠實標記）**：高潮四拍與結算卡的**目視**未驗——面板隱藏時不合成影格，且 **Chrome 對隱藏 >5 分鐘的分頁把 `setTimeout` 降到約每分鐘一次**（實測），一場 10 輪要跑十幾分鐘。該段僅有 node 鎖覆蓋，**請船長在真機開一場確認結算卡觀感**。

178. ✅完成（第一波 2026-09-13 `b684f60`／第二波 CTA 面 2026-09-14 `496d390`／**第三波 行銷通訊面 2026-09-14 `9baeeae`**）**玩家按下「永久自我排除」，畫面說「立即鎖定此帳戶，沒有任何解除方式」——然後他照樣可以轉每日轉盤、領抽獎、搶紅包雨，福利中心還繼續對他寫「N 項可領取」** — M（**第一波已於 2026-09-13 08:00 窗落地**：送幣單一出口 + 容器外 faucet 兩個閘已接上；第二波＝CTA／行銷通訊面，見下方「仍缺」）
    - **一句話**：這個鎖的**機械射程恆等於兩條逐筆交易閘**（`HL.rg.check` 下注／`HL.rg.checkDeposit` 儲值），而承諾面說的是「**帳戶**已鎖定」。全站另外 18 個把錢送進玩家帳戶的點、以及轉盤／抽獎／紅包雨這些**隨機結果表面**，在排除期間**一個閘都沒有**。
    - **機械事實（全 `prototype/src`、已剝註解、可複跑；本輪實測）**：
      · 承諾面：`core/responsible.js` `confirmExclude` 逐字「永久自我排除將**立即鎖定此帳戶**，且沒有任何解除方式。」；擋注／擋儲值 toast「自我排除進行中，**帳戶已鎖定**」。
      · 射程：`HL.rg.check(` **25** 個呼叫點、`HL.rg.checkDeposit(` **1** 個呼叫點——就這兩條。
      · 送幣側：`HL.bonus.add(` **19 筆／17 檔**，其中問過暫停狀態的 **＝0 檔**（achievements／cashback／challenges／guild／luckyspin／meta／onboarding／raffle／rain／redeem／referral／reload／rewards／safetynet／season／shop／tournament）。
      · 容器外還有**第 18 個送幣點**：`core/faucet.js` 走 `HL.state.set({ balance: … + RELIEF })` 直入餘額（不經 `HL.bonus.add`），同樣無閘。
      · 全站讀得到暫停狀態的地方**只有 1 處**：`layout/app-shell.js:580` 的 `HL.rg.status()`，而它產出的是福利中心 hub 的**一行副標題字串**「自我排除進行中」——**不是閘**。
      · 於是 `layout/dock-growth.js` 與 hub 各項 `sub()` 仍照常對已排除的玩家寫「N 項可領取／前往領取」。
    - **⭐ 為什麼 #96 自己那幾條鎖全綠（CLAUDE.md §4 家族第 ② 種：不變量只擋它知道的方向）**：`rg/self-exclusion-gate` 的斷言逐字是「下注與儲值**兩軸**都擋」——它證的是**它知道的那兩軸**，從不問「這個站實際有幾軸」；而 `platform/rg-bet-gate-coverage` 的豁免表裡就寫著 faucet／progress／rewards「餘額只增不減＝送幣，不是押注」⇒ **送幣那一側被兩把尺同時豁免，於是沒有任何一把尺在量它**。
    - **外部形制（取材維度 18 首輪執行·一手＋二手互證，且兩者都從「射程」回答而非「工具清單」）**：ESPN BET／Hollywood Casino 說明中心（**一手** `espnbet.zendesk.com`）逐字 "you will **not be able to login** to your account"／"all open bets will be cancelled"／"you will **not receive any email or texts** from us other than to confirm the self-exclusion process"／"**All bonuses and promotions will also be forfeited**"，Ohio 版另載「不得下注或儲值 ＋ 不寄行銷素材 ＋ 只留提款申請」。⇒ 業界形制是**五面全關**（登入／下注·儲值／行銷通訊／紅利促銷／只留提款），**我方關的是第 2 面**。合規側綜述（二手·ACGCS／BettingUSA）另載 "cease all direct marketing … including **bonus alerts**"／"removed from marketing lists"／"prevent … **claiming any winnings**"；casino.guru 責任博弈論壇有現實案例「自我排除後仍收到紅利 email」⇒ **不是理論風險**。
    - **出口形狀（容器先於內容·此為本卡重點）**：`HL.rg` 長出一個**可被任何表面求值的抑制述詞** `suppressed(surface)`；表面用 `register` 自陳自己屬哪一類（`grant` 送幣／`chance` 隨機獎勵／`comms` 行銷通訊／`cta` 招攬文案），**抑制策略依類別決定、不依檔名**。第一個接線點就是送幣的單一出口 **`HL.bonus.add`**——一處接上即一次覆蓋 17 檔；`core/faucet.js` 因在容器外需單獨接一行。⇒ **未來新增一個獎勵表面＝加一筆註冊，閘與面板皆零改動。**
    - **抑制動作本身也必須是資料（三選一，不得寫死）**：`forfeit` 沒收／`hold` 暫停累積／`defer` 期滿後發放。法域與站別（demo/live）決定採哪一種；本專案建議 demo 走 `hold`（可逆、便於檢視）、真站留 `forfeit` 位。
    - ---
    - **✅ 第一波（2026-09-13 08:00 窗 · 平台軌 · commit `b684f60` · 線上複驗通過）**：
      · **策略是資料**：`core/responsible.js` 新增 `SUP_BY_PAUSE = { cool: {}, exclude: { grant: 1, chance: 1 } }` ＋ 述詞 `HL.rg.suppressed(kind)`。**冷靜期刻意不抑制送幣**——它的承諾面逐字只說「期間將暫停下注，時間到自動解除」，射程超過承諾與不足於承諾是同一種病。新增一種暫停＝加一筆策略，述詞一行不改。
      · **送幣單一出口接閘**：`core/progress.js` 的 `badd()`（＝`HL.bonus.add`）在寫入前求值 ⇒ **一處覆蓋 17 檔 19 個送幣點**（achievements／cashback／challenges／guild／luckyspin／meta／onboarding／raffle／rain／redeem／referral／reload／rewards／safetynet／season／shop／tournament）。
      · **容器外第 18 點接閘**：`core/faucet.js` 的閘放在 `eligible()` 而**不是** `claim()` ⇒ 藥丸與領取鈕一併收起，不留「按了沒反應」的鈕。
      · **不靜默吞掉**：被抑制的授予留一筆通知中心紀錄（非行銷素材，是「為什麼沒發」的說明）＋當場 toast；三語各補一條。
      · **鎖**：既有 `platform/rg-pause-scope-census` **改寫而非放寬**（CLAUDE.md §10.1）——(b) 棘輪基準 17 逐位保留但語意改為「全部委託給單一出口」、(d) 消費者集合由 1 筆改釘 3 筆、(e) faucet 條由「還沒接閘」改釘「閘必須在 `eligible()` 裡」，**新增 (f) 委託對象必須真的問**（逐字守衛形狀＋剝字串版＋閘必須排在 `save(KEY_B` 之前＋抑制分支必須留通知與 toast）與 **(g) 行為級**（把真正那份策略表與那一行述詞抽出來注入假 `status()` 直接跑：自我排除抑制 grant/chance、冷靜期不抑制、未暫停不抑制、未登記類別不抑制、未登記暫停種類 fail-closed）。
      · **首屏代價**：+947 bytes（餘裕 980 → **33**）。語言包非首屏 ⇒ i18n 三條零成本。
    - ✅ **第二波 CTA 面落地（2026-09-14 前景，`496d390`）**：政策表加 `cta`（`SUP_BY_PAUSE.exclude = { grant, chance, cta }`），
      **同時**接上第一個消費者——不留沒人用的死條目。
      · **福利中心 hub**：17 個項目裡會寫「N 項可領取／今日可轉／可領 NT$X／待退 NT$X／輸入領獎金」的 **10 個**自陳 `sup:"cta"`，
        求值收斂到**單一出口 `hubSub(it)`**（不逐項判斷——那樣新增一個項目就會漏一個）。
      · **成長面板**（`layout/dock-growth.js`）：季票與公會的 CTA 退回中性入口（面板本身仍進得去，玩家有權看見自己有什麼；
        被擋的是「去領」這個動作與說法），「可領 N」改成一句說明。**資料指紋也加進抑制狀態**——漏了它，排除生效的那一刻面板不會重繪、文案會停在舊的。
      · ⚠️ 刻意**不**把數字改寫成 0：獎勵確實還在那裡，只是這段期間拿不到；寫 0 是另一個謊（這條也寫進鎖）。
    - ✅ **preview 實測**：啟動永久自我排除後，hub 那 10 項全部變成「自我排除中 · 暫不開放」，而純資訊項目
      （活動日曆／抽獎券數／徽章數／點數／金磚）**原樣保留**；成長面板「可領獎勵」變「排除中 · 不可領取」、CTA 變「開啟季票」。
    - ✅ **既有鎖 `platform/rg-pause-scope-census` 擴到四筆消費者**並補 (d-2)：政策表與消費者**雙向**釘死
      （只加政策沒人問＝死規則；只有人問而政策沒列＝`suppressed("cta")` 恆 false＝閘是假的、畫面完全正常）。
    - 🚨 **本輪最重要的產出其實是鎖的一個漏法**：第一版斷言寫成 `/function hubSub\(/ && /hubSub\(it\)/`，
      而**這兩條都被 `function hubSub(it) {` 那一行自己滿足** ⇒ 負向擾動 X2（拿掉閘的內容）、X4（拿掉呼叫點）、
      X6（`claimCell` 同型）**三例全部穿過去**。這是「鎖空綠」的**第十種**漏法：**斷言被它要檢查的東西的「定義處」自己滿足**。
      改法：① 呼叫點釘在**消費者的函式體**裡（`fnBody(src,"openRewardsHub").indexOf("hubSub(")`）；
      ② 把 `hubSub`／`claimCell` 抽出來在 node **直接跑**，餵合成輸入證明它真的依狀態改變輸出
      （抑制時不得回原文／未抑制時不得改寫／沒標記的項目不得被波及／`HL.rg` 缺席時不得丟例外）。
      修完後**負向擾動 8/8 CAUGHT**。
    - ✅ **第三波：行銷通訊面（`comms`）落地（2026-09-14 前景，`9baeeae`）**——業界五面裡的第 3 面。
      · **容器**：`HL.notify.add(n)` 長出類別欄位 `kind`（`rg`／`account`／`reward`／`comms`），
        閘接在**單一出口** ⇒ 一處覆蓋 **23 檔 39 個生產者**（卡上寫 37，實測 39）。
      · **預設 fail-closed**：沒自陳的當成 `comms`。忘了標→在排除期間被擋住（開發者當場看得到），
        而不是溜出去（畫面完全正常、沒人會發現）。另一方向由鎖守：**每一個呼叫點都必須自陳**，漏一個當場紅。
      · **業界唯一的例外寫進程式而不是仰賴「政策表剛好沒列它」**：`rg` 硬性放行
        （ESPN BET 逐字「除了確認自我排除流程以外，不會再收到任何訊息」）。
        少了它，一旦有人把 `rg` 寫進政策表，玩家按下「永久自我排除」會**什麼都看不到**
        ——而那是唯一無法補救的一格。鎖以**毒化政策表**的合成輸入實際跑一次來證它。
      · **notify 之外還有兩條投遞管道**（只擋通知中心就是修一半）：
        聚天室 `RainBot` 「紅包雨來了！點上方領取！」與 Happy Hour 的 toast，各自接上同一個述詞。
        （前者尤其荒謬：第一波早就讓他**領不到**，還在叫他來拿。）
      · **不得變黑洞**：擋下的則數會累計，通知中心逐字寫「自我排除期間已為你擋下 N 則促銷通知
        （獎勵沒有消失，只是這段期間不再向你招攬）」。句子嵌了數字 ⇒ 走 `HL.i18n.fmt`
        （`HL.i18n.t` 是 passthrough，補進字典也永遠查不到，同 #175 踩過的坑），三語各補一條。
      · **分類裡唯一的判斷題**：`progress.js` 的「紅利即將到期·請盡快完成流水」判為 **comms**
        ——它唯一可以照做的方式就是破戒下注；而損失本身仍由「紅利已逾期」（`account`）如實告知。
        這一條是可以被推翻的判斷，改成 `account` 只需改一個字（鎖會因為 comms 計數 6→5 而紅，請一併更新）。
      · 鎖：`platform/rg-pause-scope-census` **再擴一次**（(b) 棘輮 17→16、(d) 消費者 4→7、
        (d-2) 政策鍵加 `comms`、**新增 (h) 通知容器普查·(h-2) 閘的位置·(h-3) 行為級·(i) 兩條管道**）。
        (h-3) 把 `notify.js` 真正那三段與 `responsible.js` 真正那份策略表**接在一起跑**
        ⇒ 兩支檔的契約不一致時會紅，而不是各自全綠。**負向擾動 15/15 CAUGHT**。
      · 首屏代價 **3,389 bytes**（餘裕 12,763 → 9,374）；語言包非首屏 ⇒ i18n 零成本。
    - ⚠️ **本卡射程到此為止**：業界五面中我方已關 **第 2＋3＋4 面**（下注·儲值／行銷通訊／紅利促銷）。
      仍未做的兩項**刻意不在本卡射程內**：**登入面**（純前端 Demo 下的意義要另行判斷）與
      **抑制動作三選一**（`forfeit`／`hold`／`defer`；現行一律 `forfeit`，`hold`/`defer` 需一份待發放佇列）。
    - **⬜ 仍缺（第二波，按價值排序）**：
      · **CTA 面**：`layout/dock-growth.js` 與福利中心 hub 的各項 `sub()` 仍對已排除的玩家寫「N 項可領取／前往領取」——**東西領不到了，但招攬文案還在**。這是 `cta` 類別的第一個消費者。
      · **行銷通訊面**（`comms`）：業界形制五面全關中的第 3 面，我方現在關的是第 2＋4 面。
      · **抑制動作三選一**（`forfeit`／`hold`／`defer`）：第一波一律 `forfeit`（不發、不記成本、留通知）。`hold`／`defer` 需要一份「待發放佇列」，屬獨立工作量。
      · **登入面**：業界第 1 面（`not be able to login`）。純前端 Demo 下的意義與做法要另行判斷，**不在本卡射程內**。
    - ⚠️ **落地時最容易做錯的一件事＝把承諾換成黑洞**：被抑制的授予**必須留一筆可查紀錄**（`HL.ledger` 已是現成出口，CLAUDE.md §4「任何新送幣務必在授予當下記帳」的反面同樣成立）。否則就是把「說謊的承諾」換成「看不見的沒收」——那比現況更糟。
    - **驗收（每條都要有負向擾動）**：(a) **零回歸錨**：未暫停時每一個送幣點的行為與 HEAD 逐位相同；(b) 暫停生效時 `HL.bonus.add` 的 17 個呼叫點**全部**被同一條述詞攔下（不得逐點手寫＝那就是 25 個 `rg.check` 的重演）；(c) **反向錨**：任一表面繞過述詞直接動餘額／直接 `bonus.add` 時，`platform/rg-pause-scope-census` 必須紅；(d) `suppressed()` 拋例外時 **fail-closed＝視為抑制**（玩家保護寧可誤擋不可誤放，與 `promo-cal` 的 `audienceOk`／#158 的 (d) 同紀律）；(e) 被抑制的授予在 `HL.ledger` 留痕，且 `HL.opsBoard` 看得到。
    - 🚧 **反重複界線（SKILL 第 3 步：比對來源模組＋出口形狀，不比卡名）**：
      (a) **不是 #158 的雙胞胎**——#158 是**下注側**「執行點有 25 個、要收斂成 1 個」（容器 `HL.bet.attempt`）；本卡是**送幣／獎勵側**「執行點有 **0** 個」（容器 `HL.bonus.add` ＋ 抑制述詞）。**同一個家族的兩側，出口是兩個不同的中央點**（#158 自己已寫明「閘不能掛在中央結算點，因為錢早就扣掉了」——送幣側剛好相反：授予的單一出口就在授予當下）。#158 先落地不會使本卡消失，反之亦然；兩張都落地後「誰該去問那個鎖」在兩側各只有一處。
      (b) **不是 #150 的雙胞胎**——#150 問**限額的表達能力**（能不能只關掉一類遊戲），本卡問**鎖的射程**（關掉幾個表面）。#150 落地後本卡更必要（範圍判斷若也只長在下注側，送幣側依舊是零）。
      (c) **不是 #107／#54 受眾述詞的雙胞胎**——那是「這個促銷該給誰看」（正向篩選受眾），本卡是「這個玩家已經要求不要再被招攬」（負向一票否決）。但本卡的 `comms`／`cta` 類別**應向受眾述詞求值的同一個出口報到**，不得長出第二份真相。
    - **首屏成本（阻塞事實·開卡時已寫進卡）**：17 個送幣檔（`luckyspin`／`raffle`／`rain`／`rewards`／`shop`／`redeem`／`season`／`guild`／`meta`／`reload`／`cashback`／`referral`／`challenges`／`achievements`／`onboarding`／`safetynet`／`tournament`）與 `progress.js`（`HL.bonus.add` 的定義處）、`app-shell.js`、`dock-growth.js` **全在 `index.html` 的首屏 `<script>` 清單上**；本輪實測餘裕 **72 bytes**（1,638,328／1,638,400）⇒ **必須排在 #118／#169 之後**，與 #93／#128／#148／#160／#171／#173／#174／#158 同一個閘。
    - **本輪已落地的那一半（止血·零首屏位元組）**：常駐鎖 **`platform/rg-pause-scope-census`**（住 `prototype/tests/checks-platform.js`）把射程變成**雙向棘輪**——未問暫停狀態的送幣檔釘定 **17**：變大＝新增未接閘的送幣／抽獎表面（轉紅並指名該檔），變小＝已接上閘（要求把基準調低）。另含 (a) 量程錨＋三個隨機表面必在量程內、(c) **防空綠的正向對照**（同一把 `HL.rg.<member>` 抽取尺必須在逐筆交易閘那側數到 ≥23 檔，否則 (b) 會面不改色地全綠）、(d) 暫停狀態消費者集合釘死為 1 處、(e) **量程外錨**（`faucet.js` 直入餘額的寫法）。負向擾動 **8/8 CAUGHT**。
    - **來源**：**platform-modules 台帳「資安」本輪輪替審計**（8→**9** 模組，新增〈暫停狀態的全站射程／促銷與獎勵面的閘〉判 `weak`）＋ **新開取材維度 18「自我約束的射程：一個鎖宣告關掉什麼，實際關掉了哪些表面」**（維度 5 只問工具本身的參數，18 份 dossier 對「射程」的真實命中數＝0）＋ 外部形制對照（ESPN BET 一手／ACGCS·BettingUSA 二手／casino.guru 現實案例）。


179. ✅完成（2026-09-08 平台軌 08:00 窗當輪落地） **注單頁的每一列都亮著「驗算 →」，而沒有任何一列是按下去會有結果的** — M（動 `core/fair.js` + `core/betlog.js` 兩支首屏 eager 檔；本輪用**註解壓縮**自籌位元組，收尾**淨還回 86 bytes**）
    - **一句話**：當期的列，伺服器種子依承諾**本來就還不能給**；過去的列，種子在輪換的那一刻**被永久銷毀**。兩者相加＝那顆按鈕對所有人都是死路，而站上有**三處**在說「你事後算得出來」。
    - **機械事實（全 `prototype/src`、可複跑；進場版）**：
      · `serverSeed` 全 `src` **只存在於 `core/fair.js` 的 `HL_FAIR` blob**，且**只有一個**（當期）。
      · `rotate()` 就地覆寫 `o.serverSeed = randHex(32)`，舊值**只以回傳值存在**、repo 內無第二個寫入點。
      · `rotate()` 全 `src` **恰 1 個呼叫點**（公平性面板的鈕），唯一消費者 `revealModal` 把 64 字十六進位**畫進一個彈窗**——無複製鈕、無持久化 ⇒ **關掉即永久消失**。
      · 注單每列欄位恰 `{id,ts,game,bet,win,cs,ne}`＝**沒有任何欄位說得出自己屬於哪一個承諾期**。
      · 驗算鈕條件逐字 `isPF(r.game) && r.ne != null && r.cs`＝**從不問鑰匙在不在**；而 `verify()` 沒有 serverSeed 就回 `null`（點進去是空欄位 + 紅字錯誤）。
    - **承諾面（我們自己說的三句）**：`fairnessModal`「輪換種子會揭露原始伺服器種子，**即可回頭驗證每一注**」／`revealModal`「把…貼進驗證器，即可重算先前每一注是否吻合」／說明中心 `fair/how`「平台事後改不了、**你事後算得出來**」。三句成立的唯一路徑是玩家**手抄 64 個十六進位字元**，而列上沒有欄位告訴他該配哪一期。
    - **為什麼一路全綠（§4「修一半」家族第 ⑧ 種形狀）**：**兩個半邊各自都對**——密碼學那半逐項正確、紀錄那半也落地了——缺的是**銜接**：鑰匙的生命週期與列的身分**沒有任何一方負責**，於是兩邊都能宣稱自己完整。更難堪的是 #51 的立卡理由逐字是「承諾雜湊能驗算，但**要驗哪一局**查不到」：**它修好的那一半，正好把下一個問句留在原地**。
    - **修法（容器先於內容）**：
      · **(A) 鑰匙留得住**：`HL_FAIR` 新增 `epochs[]`（已揭露種子期台帳，`EPOCH_CAP = 20`），`rotate()` **在覆寫之前**存檔；新出口 `HL.fair.seedOf(hash)`（**吃前綴**）／`epochs()`／`EPOCH_CAP`。**`seedOf` 對當期恆回 `null`**——當期不是遺失、是承諾本身；任何表面要判「這列驗不驗得動」一律問它。
      · **(B) 紀錄自己說得出屬於哪一期**：每列多存 `sh`＝承諾雜湊前綴 16 字（`info()` 本來就在算這個雜湊，**非新成本**）；`COLS` 多一欄「承諾雜湊」／CSV `server_seed_hash` ⇒ **匯出也帶得走期身分**（否則拿到檔案一樣驗不了）。
      · **(C) 驗算欄改三態**：拿得到鑰匙 → 「驗算 →」且點下去**自動帶入種子＋客戶端種子＋nonce**（玩家零手抄）；當期 → 據實寫「**待輪換**」；其餘 → 「—」。
      · **(D) i18n**：EN／zh-Hans 三語同步（新增 5 條、汰換 2 條過期條目）；語言包延遲載入 ⇒ **零首屏位元組**。
    - **鎖與驗證**：新鎖 **`platform/fair-epoch-key-survives-rotation`**（**行為級**：node 樁環境**真跑** `fair.js`，驗當期不外洩／輪換後憑舊承諾雜湊與 16 字前綴都查得回／`sha256hex(查回的種子)===當初承諾`／上界擠掉最舊那期後據實查不到）＋ **`platform/betlog-verify-gate-asks-for-the-key`**（期身分是欄位且進得了 CSV／`record()` 有寫入／`seedOf` 求值早於 `can` 判定／`can` 逐字 `!!key`／點擊帶 `serverSeed: key`／當期顯示「待輪換」／**防空綠正向對照**：`HL.fair` 出口真的轉發 `seedOf` 與 `EPOCH_CAP`）。`EPOCH_CAP` 另由既有鎖 **`platform/retention-bound-queryable` 的反向掃描當場抓到**並登記進 `RETENTION_ROSTER`（第 3 筆；前兩筆丟的是**資料**，這一筆丟的是**鑰匙**）。`node` 349→**351 全綠**。
    - ⭐ **負向擾動：首版 12/13，MISSED 的那一條是我自己的鎖被短路**——把守衛寫成 `(… && HL.fair.seedOf && false) ? seedOf(r.sh) : (pf ? "x" : null)`：`seedOf(` 的字面還在、順序還在、`can` 還是逐字 `!!key`，**求值整條被繞過、鑰匙由字串頂替**，而三條斷言全綠。＝CLAUDE.md §4 形狀⑦(b)「被短路」的第二次現形。**已依該條處方改成釘那一個敘述句的逐字守衛形狀**，第二批 4/4 CAUGHT（含恆假算式 `(1===2)`、字串頂替、放寬守衛不問 `sh`）⇒ **合計 17/17**。
    - **殘餘（誠實標記，未當成已解）**：(a) `EPOCH_CAP=20` 仍是會丟鑰匙的上界，第 21 次輪換擠掉最舊那期；(b) Demo 站的伺服器種子存在玩家自己的瀏覽器裡＝純前端下只是形制演練，正式版須由伺服器簽發保管（`ax-demo-tag` 已明示、本輪未改）；(c) 沒有「種子期一覽」的獨立表面；(d) **本輪無 preview 目視**（`preview_start` 在排程輪被環境拒絕）⇒「畫面有沒有真的長出三態」屬 **UNVERIFIED**。
    - **來源**：**platform-modules 台帳「資料」本輪輪替審計**（6→**7** 模組，新增〈可查證期／紀錄的承諾期身分〉判 `partial`）＋**新開取材維度 19**（事後可查證性）＋對照組 Stake.us／Stake seed history（Settings→Fairness 保存 seed pair；揭露掛在**那一列 bet 的 Fairness／Details 分頁**）／1xBet（預設涵蓋期 last three months、Request History 綁篩選）。逐條事實見 [intel/data-verifiability-2026-09-08.md](intel/data-verifiability-2026-09-08.md)。



180. ✅完成（2026-09-08 平台軌 14:00 窗當輪落地） **同仁自製遊戲是「可插拔」的整個重點，而我們交給他們的契約裡，沒有那個必經的掛鉤** — M（只動 `prototype/games/**` 與 `prototype/tests/`＝**首屏逐位未動**）
    - **一句話**：平台有**兩條**遊戲註冊路徑。自己寫的那條 **20/20** 都餵了中央結算掛鉤；同仁那條 **0/1**。而那不是同仁的疏忽——**我們發出去的契約就是這樣寫的**。
    - **機械事實（全部剝註解後計，可複跑）**：
      · 路徑①`src/views/*.js` 註冊型 **20 支**：**18 支**直接 `HL.liveStats.record(`，另 2 支（`instant-cases`／`instant-games`）由引擎 `core/instant.js` 代呼 ⇒ **20/20**。
      · 路徑②`games/registry.json` 放置區：`games/mina/lucky-seven/game.js` **0 命中**（只做 `HL.state.set({balance})`）；另有一支 5 週未提交的沙箱橋接 WIP `games/slot-engine/poc-spin/game.js` 同樣 **0**。
      · 交給外部作者的**三份契約，命中數皆為 0**：`games/README.md` 服務表 **10 列**（無它）且「下注/派彩（Demo 寫法）」**直接示範只改餘額**；`dev-kit/hl-stub.js` 模擬器**沒有這個出口**（⇒ 同仁連寫都寫不出來＝真正的上游）；範例 `dev-kit/game.js` 不呼叫。
      · `core/live-stats.js` 的 `record()` 體內 **22 個 `HL.*` 命名空間**＝**21 個被餵的子系統** ＋ `HL.edge`（計算用）。
    - **後果（誠實計量）**：放置區的每一注對 VIP 流水／每日任務／返水／累積彩金／限時賽／成就／季票／公會／商城點數／挑戰／抽獎券／`HL.betlog` 注單（#179 才剛讓每列可驗算）／`HL.ledger` 營運帳本（GGR/NGR/RTP 因此**靜默少算**）／`HL.heat` 熱度牆（本專案 06-26 對標 Roobet Live RTP 的落地點子）／**`HL.rg.record`＝玩家自己設的損失與時間限額**全部不存在——**而遊戲本身零症狀**（畫面在動、餘額在變、無錯誤訊息、353 項測項全綠）＝§4「修一半而看不出來」。
    - **誠實邊界（刻意不把它講得比事實大）**：(a) 今天放置區只有 **1 款示範遊戲**，所以**對玩家的實害很小**；本卡的價值在於**在團隊放大這條路之前**把契約接對——每一個未來的同仁都是複製那份範例。(b) **`Math.random` 不構成對玩家的違諾**：玩家看得到的公平承諾（`core/fair.js:165`）**逐字自限於「Originals 與桌遊（Dice／Limbo／Plinko／Towers／Hilo／百家樂／輪盤）」**，沒有涵蓋放置區。
    - **落地（全數，非半修）**：
      · **(A) 模擬器補上出口**：`dev-kit/hl-stub.js` 新增 `HL.liveStats.record/tape`，並把回報畫成畫面下方一條回報條（顯示這一注的注額／派彩）。
      · **(B) 把「沒有症狀」變成有症狀**：stub 包住 `HL.state.set`——**餘額動過而 2.5 秒寬限窗內沒收到 `record()`** ⇒ 回報條轉 ⚠️ ＋ `console.warn`。用寬限窗而非同一個 tick，是因為多數遊戲先扣注、動畫跑完才派彩結算。
      · **(C) 契約寫進三處**：`games/README.md` 服務表加一列並改寫「下注/派彩」配方（附「為什麼最後那一行不能省」）、`dev-kit/README.md` 加「⭐ 一行必做」段、範例 `dev-kit/game.js` 實際呼叫。
      · **(D) 兩款放置區遊戲補上呼叫**：`lucky-seven`；沙箱橋接 `poc-spin` 補在 `onSettle` 的**冪等早退之後**（重送同一 `betId` 不重複回報）——那裡正是沙箱路徑的錢包權威點。
      · **(E) `DEVKIT_VERSION` 1.0.0 → 1.1.0**（加功能）＋重跑 `pack-devkit.ps1`（§5 維護流程）。
    - **鎖**：新增 **`platform/placement-games-feed-central-hook`**（住 `prototype/tests/checks-platform.js`）——掃 `registry.json` 上**每一款**＋Dev Kit 範例，要求 (a) 逐字守衛 `if (HL.liveStats) HL.liveStats.record(`、(b) 該呼叫與**餘額寫入落在同一個函式體**、(c) stub 出口與未回報偵測都在、(d) 兩份 README 都指名它且標「必做」，並以 (e) `record()` 下游命名空間 **≥12** 當**反向錨**（掛鉤被掏空時本鎖要紅，而不是對著空殼全綠）。⭐ 這是 `prototype/games/` **有史以來第一條**測項——先前 352 項全部只掃 `src/` 與 `tests/`，`registry.json` 只在兩處註解裡被提到、**從未被任何斷言讀過**。
...
181. ✅完成（2026-09-14 前景·09c5564）**這個站有 36 個去處、0 個地址：5 顆「🔗 分享戰績」的訊息點名了遊戲，連結卻回不到那款遊戲；裝成 App 之後第一次按返回就直接離開** — M（`HL.route` 去處↔位址雙向編解碼登記簿 + `pushState`/`popstate` 接線 + 分享側一處改寫；**宿主 `main.js`／`core/ui.js` 皆首屏 eager ⇒ 卡 #118／#169**，且**必須排在有 preview 的輪**）— 來源：**platform-modules 台帳「前端UI/UX」本輪輪替審計**（9→**10** 模組，新增〈網址作為地址（可分享／可回到／可深連）〉判 `absent`）＋**新開取材維度 21**（一個去處有沒有地址）
    ...
    - **機械事實（`prototype/src` **120 支 js**，剝註解＋剝字串後計，全可複跑）**：
      ...
      · 對照的去處數量：`HL.router.go` **34 個呼叫點**＋`goGame` **1**、`main.js` VIEWS 登錄表 **12 筆**、`HL.games.register` **24 筆** ⇒ **約 36 個去處、0 個地址**。
      · URL 只被當**入站**地址用（`?demo=1`＠`core/config.js:14`、`?ref=CODE`＠`core/referral.js:244`），**從不被寫出**。唯一的 URL 建構子是 `origin + pathname`，**3 個消費者**（`core/auth.js:34`／`core/referral.js:38`／`core/ui.js:102` `shareUrl()`）組出**同一個字串＝大廳大門**。
    - **三個後果（逐一實測，非推理）**：
      · **① 分享**：`HL.ui.resultBlock` 的 `share:` 出口 **5 個**（arena／bounty×2／slot／vsslot）＋ `core/live-stats.js` 那顆不帶 url 的 `share.text` ⇒ **6 顆按鈕**。訊息由 `shareResult` 組成、逐字**點名了遊戲**（「我在 ApexWin 玩「X」贏得 Y！一起來試手氣 👉」），而 👉 後面那條 URL **回不到 X**；`copyFallback` 的 toast 還把整份剪貼簿內容稱作「已複製**戰績連結**」。
      · **② 返回鍵／重整**：`manifest.webmanifest` 是 `display: standalone`（**Android 恆有返回鍵**）而 history 裡**一筆都沒有** ⇒ **第一次按返回就離開 App**；Demo 的 `view` 與 `balance` **都不持久化**（`core/persistence.js` 只寫 currency/wallet 到後端、`core/app-state.js` 無 localStorage）⇒ **F5 或一次誤觸返回＝回到大廳、Demo 餘額歸回預設**。全庫 `pagehide` 只有 `core/ledger.js:214` 一處（帳本 flush）＝**離場鉤不在那條路上**。
      · **③ 它在畫面上被賣過（本輪已修）**：`views/arena.js` 建房表單「🔒 私密房間 Private」副標逐字寫「**僅分享連結可加入**」、房間資訊 modal 寫「僅限分享連結加入」——而 `joinability()` 的 `canJoin: … && !priv` 是**硬 false**、連結機制不存在 ⇒ **這個旗標的機械效果只有「沒有人能加入」那一半**。
    - ⭐ **最難堪的一點（§4「修一半而看不出來」家族的新實例）**：**同一張表單的隔壁三列（Shared／Team／Sponsored）早就被前手輪次逐一改成「示意」並在檔內寫下理由**（`arena.js` 那段 🤝 註解逐字寫「收了錢什麼都沒發生 … 不再收費、也不再承諾」）。**修隔壁那一列的那一輪，沒有回頭看這一列。** ⇒ 同一張表、同一種病、只治了當時看得見的那幾行。
    - **誠實邊界（刻意不誇大）**：(a) **Demo 站重整歸零本來就是原型的設計** ⇒ 今天對玩家的實害小；真正的代價在**承諾側**與**可分享性**。(b) `?ref=` 證明我們**已經**會把 URL 當入站地址用 ⇒ 這不是缺一整類能力，是**第二個方向沒做**。(c) 對照組（Track360 深連結詞條·**一手直取**）刻意**未**宣稱「營運方必須支援 per-game URL」，只把它寫成聯盟通路的事實前置——我方照抄這個口徑，**不把它寫成合規要求**。
    - **範圍（容器先於內容 · 可漸進）**：① `HL.route.register({ view, encode(state)→seg, decode(seg)→patch, title? })`，由各 view／遊戲**自己註冊**（比照 `HL.gameAxes`／`HL.support`／`HL.reports` 既有形制，**不由 `main.js` 代寫一張大表**）；② `HL.router.go` 落地時多一次 `pushState`，**開機解析與 `popstate` 走同一個 `decode`** ⇒ 前進／返回／重整／分享**四件事共用一份真相**；③ `shareUrl()` 改為向 `HL.route.encode(HL.state.get())` 求值 ⇒ **6 顆分享鈕一處通吃、call site 一個都不用改**；④ 未註冊的 view 退化為現行行為（無位址）＝**不需要一次遷完 12 個 view**。
    - **⚠️ 必寫成測項的不變量**：(a) **encode∘decode 恆等**（對每個已註冊 view 的代表性 state 往返一次逐位相同）；(b) **decode 對垃圾輸入必須退回大廳而非丟例外**（網址是外來輸入，且 `?g=` 可能指向已下架遊戲）；(c) **`?ref=` 與 `?demo=1` 的既有入站語意逐位不變**（`?ref=` 只在首次寫入的冪等性不得被新解析器破壞）；(d) **一款遊戲的位址不得寫死在 `main.js`**（反向鎖：不得長出第二張大表）；(e) **深連結進入延遲載入的遊戲**必須先 `await HL.lazyGames.load(id)` 才 render（否則會落到 `renderGameView` 的 `console.warn` 退回大廳＝靜默失敗）。
    - 🔴 **先抄進卡的阻塞事實（別讓實作輪重新發現一次）**：
      (a) **首屏**：宿主 `main.js`／`core/ui.js` **皆 eager**，本輪收尾餘裕 **169 bytes** ⇒ 與 #93／#128／#148／#158／#160／#171／#173／#174／#178 同一個閘（**這是第十張**）。
      (b) 🚨 **驗證射程**：缺陷會住在「**開機解析／掛載／換頁／同頁重繪**」這條路上，而 CLAUDE.md §9 的 headless 配方（直接 `h.appendChild(g.render())`）**正是為了繞過 `renderApp`／`mountView` 而生的** ⇒ **這張卡在 headless 排程輪驗不了它最容易壞的那一段**（§4 形狀⑥＝Slots Battle 壞了 17 天的同一條路）。**必須排在有 preview 的輪**，與 #93／#169／#175 同批。
      (c) **`HL.shell.holdView` 的交互**：對戰有在途 escrow 時 `refresh()` 被禁止重掛 ⇒ `popstate` 若在對戰中觸發，**必須先問 `viewHeld()`**，否則就是把「同頁重繪＝免費重骰」那條 blocker 換一個入口重新開門。
      (d) **真站/假站軸**：`?demo=1` 決定 `HL.auth.backend()`，而站別旗標 `HL_SITE_MODE` 走 localStorage **不在 URL 上** ⇒ 分享出去的連結**不帶站別**，這是對的（別順手加上去＝會讓收件人被切站）。
...
182. ✅完成（2026-09-11 · `4f68dcf`） **營運端按下去的每一個開關，沒有任何一個地方記得他按過：9 個會改變世界的旋鈕零軌跡，而「🧹 重置本機帳本」銷毀的正是唯一能記錄它自己的那本帳** — M（`HL.opsAudit` 動作型別登記簿 + 9 個寫入面接線 + 儀表板軌跡區 + 一條鎖）— 來源：**platform-modules 台帳「後台」本輪輪替審計**（9→**10** 模組，新增〈營運操作軌跡與寫入端問責〉）＋**新開取材維度 22**（營運端按下去之後，誰知道他按了）
...
> 🤖 **以下由自我進化引擎「平台軌」自動開卡**（2026-09-12 **08:00 窗** · 來源：`intel/db/platform-modules.json` **金流** 分類全審〔7 模組，`last_audited` 09-06＝全庫最舊〕＋ 實作 #82 當下在同一支檔裡量到的兩件事）。

183. 🟦已批准待做 **這個站有三份「幣別」，它們互不相認：一份零消費者、一份跟我們自己寫的 money-mode 契約相反，而 USDT 儲值進得來、錢包裡沒有那一格** — S–M（`core/money.js` + `layout/app-shell.js` 幣別列 + 一條鎖；**首屏檔**，見下方阻塞事實）— 來源：**platform-modules「金流/錢包・帳本」2026-09-12 輪替審**（實作 #82 時在同一支檔裡撞到）
    - **一句話**：`money.js` 寫下了「各模式允許的幣別」，然後沒有任何人問它；真正在決定錢包顯示什麼的是一份 mock 資料，而那份資料從不問現在是不是休閒模式。
    - **機械事實（三份清單，全可複跑）**：
      · ① `core/money.js:26` `currencies()` ＝ 休閒 `["COIN"]`／真金 `["TWD","USD","BTC","ETH","USDT"]`，檔頭註解逐字「各模式允許的幣別」。全 `src/` 對 `.currencies(` 的**呼叫點 0 個**（只在 `money.js:39` 被掛上出口）⇒ **宣告了卻沒有任何消費者**。
      · ② 錢包幣別切換器的真相是 `data/mock-data.js:94` 的 `HL.mock.currencies`（TWD/USD/BTC/ETH/**LTC/TRX/XRP**…），由 `layout/app-shell.js:34,47`／`views/game-frame.js:33,61`／`core/dom.js:177` **五處**消費，**渲染時完全不問 money-mode**。
      · ③ 兩份都**沒有 USDT**——而 USDT 是收銀台第一順位的儲值幣（#82 首批註冊、`flows` 含 deposit/withdraw）。
    - **玩家看得到的後果（兩個方向都錯）**：(a) **休閒模式（遊戲幣）下錢包照樣列出 7 種加密幣餘額**，與 `modeLabel()` 同一支檔宣告的「休閒模式 · 遊戲幣」直接矛盾；(b) **儲值得進 USDT，錢包裡沒有 USDT 那一格**；反過來 LTC/TRX/XRP 有餘額格卻**沒有任何儲值通道**。
    - **為什麼一路全綠**：沒有任何一條鎖問過「這三份清單要不要一致」。`money.js` 那份是**死的宣告**（零消費者＝任何斷言打它都只是打一個沒人跑的函式），而 mock 那份因為「是 mock 資料」從來沒被當成契約看。⇒ 同 `wd-sla-hours`（#174）／`HL.rbac`（#182）**「宣告了卻沒有消費者」家族第 3 筆**。
    - **範圍（純前端 · 容器先於內容）**：① 幣別的**單一真相**收斂成一份可註冊的表（`code/name/ic/color/rate/modes[]`），`money.currencies()` 改為**向它求值**而不是自己另寫一個字面量陣列；② 錢包切換器、`game-frame` 兩處、`dom.js` 格式化**全部**改讀同一份並**套 money-mode 過濾**；③ USDT 補進表（它已經是儲值通道了）；LTC/TRX/XRP 要嘛補通道、要嘛依 money-mode 收起來——**二選一，不得繼續兩頭都對不上**。
    - ⚠️ **必寫成測項的不變量**：(a) **零回歸**——真金模式下錢包列出的幣別集合與現制**逐位相同**（除了刻意新增的 USDT，須單獨列出並說明）；(b) **`currencies()` 必須有非 0 個消費者**（反向鎖：零消費者就是這張卡沒落地）；(c) **休閒模式下不得出現任何 `modes` 不含 casual 的幣別**（正反兩向各一條）；(d) **每一個 `flows` 含 deposit 的收銀台通道，其 `code` 必須在幣別表內**（跨表恆等，這條才擋得住「下一次又只改一邊」）；(e) 不得出現第四份清單（grep 釘死 `mock.currencies` 只剩資料宣告處一個定義點）。
    - **阻塞事實（開卡時就寫進來，別讓實作輪重新發現一次）**：`app-shell.js`／`dom.js`／`game-frame.js`／`money.js` **全部是首屏 eager**；本輪 #82 收尾後**首屏餘裕僅 289 bytes** ⇒ 這張卡**現在做不進去**，除非先落地 **#118**（拆 `slot.js`，可再省 24.3KB）或 **#184**。

184. ⬜待批准 **每個訪客都下載一份 17.2KB 的 `index.html`，其中 11.2KB（65%）是寫給我們自己看的註解——而那些註解要求的載入順序，另有 20+ 條測項在真的把關** — S（純文字搬移，零執行期風險）— 來源：**#82 實作輪的首屏預算實測**（為了讓 #82 進得了門，本輪已先回收 531 bytes 的失效註解，見下）
    - **一句話**：這是站上**最便宜的一筆首屏空間**——它一行程式碼都不是，卻和 `slot.js` 一樣按位元組計費。
    - **機械事實**：`index.html` 17,215 bytes（LF 正規化），`<!-- -->` 共 **35 則／11,225 bytes＝65%**；其中 **>400B 的有 11 則／7,515 bytes**。同一份載入順序，`prototype/tests/` 內另有 `platform/reports-load-order`／`platform/rbac-load-order`／`platform/lazy-no-unsatisfiable-shared-dep`／`platform/first-screen-budget` 等 **20+ 條常駐鎖在機械把關**（測項不出貨＝零首屏成本）。
    - **本輪已先做的一小塊（可當樣本）**：`index.html:118-121` 自己寫著「#101 之後『置於 selftest 之後＝載入即可 register 自身測項』這句話不再是擺放的理由」，而**下面有 6 則註記仍逐字複述那個已失效的理由**。本輪把那 6 句刪掉（各則的**真依賴一字未動**）⇒ **回收 531 bytes**，而 #82 正是靠這 531 bytes 才進得了 1,600KB 的門。
    - **範圍**：把 >400B 註記的**敘事/歷史/事故脈絡**搬進 `docs/first-screen-load-order.md`（不出貨），`index.html` 每則**只留祈使句那一行**（例：`必須排在 betlog.js 之後，否則靜默少一張報表（見 docs/…#reports）`）。預估回收 **5–6KB**。
    - ⚠️ **為什麼標 ⬜待批准而不是自動做**：這是**拿文件的就近性換位元組**。本專案有明確的反例史——那些註解正是「#66／#101／#106／#115 同一個坑」被反覆踩到後寫下的，搬遠了確實有「下一個人改順序時看不到」的風險。**緩解**：每則保留祈使句 + 錨點連結，且該順序全部已有常駐鎖（改壞了會紅，不是靠人看見）。**請船長裁決要不要用這個交換。**
    - ⚠️ **必寫成測項的不變量**：(a) `index.html` 的 `<script src>` **清單與順序逐位不變**（純註解搬移的機械證明）；(b) 每一則被搬走的註記，其 `docs/` 目標段落**必須存在且被連結**（反向鎖擋「搬走了就沒了」）；(c) 首屏 bytes **必須真的變小**（否則這張卡沒落地）。

185. ✅完成（2026-09-15 平台軌 **20:00 窗**·commit `0ebca87`·新鎖 1・改寫 1·**負向擾動 16/16 CAUGHT**·node 407 → **408 全綠**·`prototype/src` 一位元組未碰·**首屏逐位未動**·`sw.js` 不 bump） **第二把「程式碼 vs 註解/字串」的尺，有跟今天修掉的那把一模一樣的盲點——而它是 10 條常駐鎖用來證明「這行字真的會被求值」的那把** — S–M（只動 `prototype/tests/`＝**首屏逐位未動**）— 來源：**（09-12 14:00 窗）修 `nonCodeMask` 時的連帶普查**
    - **一句話**：我們今天證明了「一條正則字面量可以讓一把尺看不見 89.3% 的檔案」，然後發現**隔壁還有一把尺、同一個盲點、而且正在被 10 條鎖使用**。
    - **機械事實（可複跑）**：`prototype/tests/checks-platform.js:9273` 的 `stripStringLiterals()` 同樣只認 `' " \``、**不認正則字面量**。以「被剝掉的區段有沒有跨實體換行」當錯位訊號實測：`stripComments()` 之後仍有 **142 段跨行**，落在 **4 支檔**——`core/betlog.js`（`:52` 逐字就是今天那條 `var NEEDS_QUOTE = /[",\n]/;`）、`core/reports.js`（`:70` 同一條）、以及 `prototype/games/slot-engine/poc-spin/app/lib/` 下的 `pixi.min.js`／`spine-pixi-v8.min.js`（09-11 `34475a2` 隨路線 C PoC 進 HEAD 的 vendor 檔）。
    - **為什麼比今天那條更值得修**：`stripStringLiterals` 的**存在理由**就是 CLAUDE.md §4 形狀⑦(e)——擋 `var s = "if (HL.liveStats) HL.liveStats.record(";` 這種「字面在檔內、求值沒發生」。它錯位時會把真實程式碼**整段丟掉** ⇒ 凡是「**這支檔不得出現 X**」型的斷言，在 `betlog.js`／`reports.js` 上**會靜默通過**。而 `:9420`／`:9518`（#181 的 URL 路由能力普查）掃的正是**全 `src/`**，兩支檔都在射程內。
    - **修法（照今天的形制，一次做完兩件）**：(a) 把今天落地的正則感知＋換行收束**抽成單一出口**（候選：`registry-probe.js` 已匯出 `nonCodeMask`，`checks-platform.js` 直接引用它，不得再自寫第三份——比照 `i18n-key-scan.js`／`catalog-freshness.js` 的「判準抽成一支檔、禁止自寫規則」紀律）；(b) `checks-games.js` 內另有 **4 份** `stripComments`／`stripStringLiterals` 複本，一併收斂或明載為何不收斂。
    - ⚠️ **必寫成測項的不變量**：(a) 沿用今天那條的結構主斷言——**任何被剝掉的區段都不得跨實體換行**，射程擴到 `prototype/games/`；(b) **零回歸**：收斂前後，10 個消費站點的讀數必須逐位相同（差異就是今天這個 bug 的受害者，要逐筆列出來）；(c) 反向錨——vendor `.min.js` 要嘛進射程要嘛明列豁免，**不得靠「剛好沒掃到」**。
    - **阻塞事實**：無首屏位元組、無 preview 需求（全在 `tests/`）⇒ **不卡 #118**。唯一風險是收斂當下可能讓某些既有鎖的讀數改變（那正是本卡要逐筆列出的東西），故建議獨立一輪做。

    - ✅ **落地（2026-09-15 平台軌·20:00 窗）**：
      · ⭐ **清點數比卡上寫的多五倍**：卡寫「本檔 5 份＋`checks-games.js` 4 份」，實測是 **26 份**——
        `checks-platform.js`：具名 5（`stripComments` **×2**〔同層同名宣告兩次、**後者勝 ⇒ 前者從一開始就是死碼**〕／`noComments`／`stripCommentsKeepLines`／`stripStringLiterals`）
        ＋ **行內 10 處**（測項體裡各自寫的 `var strip = function (s) {…}`／`var code = raw.replace(…)`）
        ＋ **2 個自帶引號狀態機的括號配對器**（`fnBody`／`extractRegisters`——`fnBody` 是本檔最常用的取材工具）；
        `checks-games.js`：具名 5 ＋ **行內 21 處**。⇒ 卡上的清單是**名字**的清單，不是**形狀**的清單。
      · **收斂**：全部改向 `registry-probe.js` 的 `nonCodeMask` 求值。刻意寫成 `regProbe.nonCodeMask(…)`（每次查屬性），
        好讓鎖的「活見證者」能在執行期把它換成哨兵。**CSS 剝註解是唯一豁免**（CSS 沒有行註解、沒有正則字面量 ⇒ 正則是正確工具），逐行釘住、每檔最多一份。
      · ⭐ **零回歸的證明方式與卡上要求的一致（逐檔比對，不是「跑一下沒紅」）**：128 支 src 檔中 **9 支**的
        ...
        `core/betlog.js` +4,722（**54.0%**）／`core/offline.js` +1,584／`core/config.js` +278／`core/progress-src.js` +8／`data/games-loader.js` +8／`core/edge.js` +3。
        **相反方向零筆** ⇒ 這次收斂只會讓斷言變嚴、不會放寬。兩個失明入口各不相同：
        ① 正則移除註解會把**字串裡的 `//`**（`u.indexOf("//")`、`"https://…"`）連同後面的程式碼吃到行尾；
        ② 字元狀態機不認正則字面量 ⇒ `/[",\n]/` 開出幻影字串後整份錯位。
      · ⚠️ **口徑刻意分兩種，而這是實測逼出來的**：`checks-games.js` 有多條**距離型**斷言（`/A[\s\S]{0,120}B/`）。
        把註解換成等長空白（保留行號的那種口徑）會讓距離變長 ⇒ `games/bounty/member-mine-board-inert`／`games/dice-duel/staged-reveal` **當場轉紅**。
        ⇒ 該檔沿用原本的密度（註解整段丟掉、連同同一行前置空白）。**尺只有一把（同一份 `nonCodeMask`），刻度有兩種。**
    - 🔒 新鎖 `platform/code-mask-single-ruler`（(a) 兩支測項檔都不得再出現那兩種形狀＋必須真的呼叫單一出口／(b) 用事故那一行當合成輸入證明**比舊版嚴**、同時不得退化成「什麼都不剝」／(b3) 行號不得位移／
      **(c) 活見證者**＝把匯出的 `nonCodeMask` 換成哨兵，三個函式的輸出必須跟著變（少了它，一份「自己重寫、今天恰好同值」的影子實作會讓 (a)(b) 全綠）／(c2) 哨兵必須還原／(d) `checks-games.js` 的別名數量雙向等式）。
      改寫 `platform/code-mask-regex-aware`：結構主斷言射程 **`src/` → `src/` ＋ `prototype/games/`**（09-11 `34475a2` 已把一整個目錄帶進 HEAD ⇒ 量程外再怎麼錯位，「零違規」都讀起來完全正常），
      vendor bundle **明列豁免**並由雙向錨守（檔要真的在、且要真的帶反引號——ES2015+ 的 template literal 合法跨行 ⇒ 「ES5 不跨行」這條前提對它們**本來就不成立**，不是「我們不想掃」）。
    - 🧪 **負向擾動 16/16 CAUGHT**（P1 註解剝除改回正則版／P2 字串剝除改回字元狀態機／P3b 真的自己重寫一把遮罩／P4 把 `nonCodeMask` 捕捉成區域變數〔活見證者換不掉〕／P5 不保留換行／P6 不剝字串內容／P7 什麼都不剝／
      P8 射程拿掉 `prototype/games`／P9 豁免指向不存在的檔／P10 拿自家 ES5 檔當豁免／P11、P11b `checks-games` 改回本地版・別名數被動手腳／P12 CSS 豁免被當成新的複製來源／P13 哨兵不還原／P14 `fnBody`、P15 `extractRegisters` 改回不認正則的括號配對）。
      ⚠️ **P3 第一版是壞擾動，不是漏鎖**（同 09-15 遊戲軌 #36 的 P6）：我讓「影子實作」內部還是呼叫 `regProbe.nonCodeMask` ⇒ 行為根本沒變，鎖當然綠。改成真的重寫狀態機才是真的造出第二份真相 ⇒ CAUGHT。
    - 🚨 ⭐ **本輪最值得記的一件事，出在擾動工具本身（新的一種空綠，而且它偽裝成 CAUGHT）**：
      第一批擾動跑到一半被外層 timeout 砍掉，**它的「還原」那一步沒跑到** ⇒ P12 的變異被留在 `checks-platform.js` 裡。
      第二批 harness 啟動時把**那個已被污染的檔**讀成 `orig`，於是它的基線**本來就是紅的**，而每一條擾動照樣印出 `CAUGHT`
      （因為判定只看「本鎖有沒有紅」，沒問「它是不是本來就紅」）。**七條 CAUGHT 全部不可信，而輸出看起來完全正常。**
      ⇒ 修法：harness 第一件事先跑一次基線，**基線非綠就中止**；復原後逐位比對並印出 `restored identical=true`。重跑後 7/7 才是真的 CAUGHT。
      ⇒ 這正是本專案最常見的形狀跑到**驗證工具**上的一次，已開成 **#201**（把擾動 harness 變成有保險的常駐工具，不要每輪手刻）。

> 🤖 **以下由自我進化引擎「平台軌」自動開卡**（2026-09-13 **08:00 窗** · 來源：`intel/db/platform-modules.json` **資安** 分類全審〔9 模組，該分類 `last_audited` 09-07＝全八分類最舊〕＋ **Cybet 第二輪到期深挖**〔`next_due` 09-13 到期，原本要做的是「第二輪零增量 ⇒ 判 saturated 並汰除」，**結果不是零增量**〕）。全自動模式下標 🟦已批准待做。

187. ✅完成（2026-09-13 平台軌 20:00 窗·**第一波＝紅利流水單注上限**；第二波已改開成 #191，見卡末） **我們的紅利條款有三個軸（流水倍數／可用範圍／壽命），而對手的同一張條款頁上有五個——少掉的那兩個（紅利期間的單注上限、贏額/提領上限）剛好是 §11 說「真金前必須重調」的那一類，而且它們是同一個 entry 物件上的第四、第五個欄位** — S–M（沿用 `badd()` 「授予當下求值一次、之後只讀」的既有紀律；**吃 eager 位元組，落地前必先量**）
    - **來源**：Cybet 2026-09-13 第二輪深挖（一手 `guidebook.cybet.com` 自家手冊 ＋ bettingnews／99bitcoins／cryptoslate 交叉）。⚠️ **這一輪原本的劇本是「第二輪零增量 ⇒ 判 saturated ⇒ 汰除」**（08-14 的 `saturation_watch` 逐字這樣寫）。實際重查後**增量不為零**，而多出來的東西不是新機制、是**同一批既有機制的條款欄位**——08-14 那輪把 Cybet 的紅利只當「有哪些紅利」讀，沒有問「這張條款上有哪幾個維度」。⇒ 與 2026-08-16「維度清單漏掉玩家保護」、09-12「族群級盲點（BitStarz）」同形，只是這次漏的是**同一份 dossier 裡的一個欄位層**。
    - **對手實況（逐項）**：40× 流水／**7 天期限**／**第三方 slot 貢獻 100%、自家 Originals 只算 5%**／**紅利期間單注上限 $5**／**紅利可提領上限＝紅利額的 200% 且封頂 $2,000**、免費旋轉贏額封頂 $50／跑完流水後**總提領上限＝初始（存款＋紅利）的 10×，超出部分自動沒收**。
    - **逐項對照 ApexWin（實測，非推論）**：流水倍數 ⇒ **已有**（#74 `HL.sla.bonusReqFor`，且依段位查表，比對手的扁平 40× 更細）；期限 ⇒ **已有**（#71 `HL.bonusTtl`）；**貢獻率 ⇒ 已有**（#89 `HL.wagerScope.weightFor(sc, game)` 逐遊戲權重＝對手那條「Originals 只算 5%」的同一個軸，**這是一次正面確認我們的容器選對了**）；**單注上限 ⇒ 全站零命中**；**贏額/提領上限 ⇒ 全站零命中**（`grep -rn "winCap|贏額上限|maxWin" prototype/src` 只命中四款 slot 自己的**派彩數學上限** `CFG.maxWin`，那是遊戲內的機率模型參數，**不是紅利條款**——這兩者同名不同物，開卡時別把它們當成同一件事）。
    - **出口形狀（容器先於內容·此為本卡重點）**：兩個軸都是 `mkEntry()` 產出的**同一個 entry 物件上的選用欄位**，沿用 `sc`（#89）／`exp`（#71）已經走過兩次的同一條紀律——**只在授予當下求值一次寫進 entry，之後只讀不重算**（降段/改表不追溯加重既有紅利），且**未宣告時不寫該欄位** ⇒ 舊存檔與未宣告來源在 localStorage 裡是**逐位相同的物件**，零回歸靠「欄位不存在」而不是靠比對。值由既有的站別感知出口供應（`HL.sla` 或 `HL.econCfg` 註冊一筆），**不得在 `progress.js` 裡寫死數字**。
    - ⚠️ **落地時最容易做錯的兩件事**：① 把「單注上限」做成**擋下注**——那會撞上 `HL.rg` 的閘而長出第二個下注前置點（#158 正在收斂的就是這件事）；正確做法是**不擋，只判定該注是否計入流水**（比照 `weightFor` 回 0 的既有語義）。② 把「贏額上限」做成**扣餘額**——沒收既有餘額會撞 #71 立下的信任紅線（已轉為可領取的錢不得被回頭作廢）；正確做法是**封頂該筆紅利可轉出的金額**，並在條款面寫出來。
    - **阻塞事實（開卡當下即量，不留給實作輪重新發現）**：`core/progress.js`／`core/service-level.js` 皆在 `index.html` 首屏 eager 清單 ⇒ 吃 `platform/first-screen-budget`。**本輪收尾餘裕僅 33 bytes**（見 #188）⇒ **本卡在 #188 之前實質不可落地**。
    - **驗收（headless 可證）**：① 未宣告兩軸的紅利，其 entry 與 `HL.bonus.status()` 輸出**逐欄與改版前相同**；② 宣告上限後，超過上限的注**只是不計流水**、餘額與派彩逐位不變；③ 贏額上限只影響「可轉出」的計算，`unlocked` 既有值不被回頭改寫；④ 兩軸的值都問得到站別（demo/live 各一份），不得是載入期純量三元式（#97 的既有紀律）。
    - ─────────── **落地實況（2026-09-13 平台軌 20:00 窗·第一波）** ───────────
    - **做法**：**不新增容器**——`HL.sla` 的維度註冊表本來就站別×段位感知，直接加第六個維度 `bonus-max-bet`（`kind:"info"`；`cap` 型會被 `evaluate()` 拉進**提款閘**）。entry 多一個選用欄位 `mb`，與 `req`(#74)／`sc`(#89)／`exp`(#71) 共用同一條紀律：授予當下求值一次、之後只讀、**查無政策就不寫欄位**（零回歸靠「欄位不存在」而非比對）。併筆加 `sameMb` 守衛。揭露面四處**全部是既有容器自動長出來的**（SLA 面板逐列＋矩陣／領取中心該筆一行／首次被擋一則通知／`HL.econCfg` 自動收錄），**零新表面**。
    - **數值**：假站全段位「不限」（§11／#74(c)：demo 不得比現況更嚴）；真站 200/250/300/400/500（桌遊籌碼面額 `[10,50,100,500,1000]` ⇒ 會真的咬到，不是裝飾性的）。
    - ⭐ **一個非顯然的決定：`NOCAP = 1e9` ＝ ∞ 的有限數值編碼。** 假站若寫 `0`，`HL.econCfg` 的「真站不得比假站寬鬆」是逐位數值比較 ⇒ 會把 `live(200) > demo(0)` 誤判成違規；若寫非數字，`cmpPairs()` 會**整個維度跳過**＝靜默失去看守。保序的有限編碼讓兩個看守都留著。（`core/bonus-ttl.js` 當年踩到同一題〔`0 ＝ 永不到期 ＝ 最寬鬆卻數值最小`〕，它選的是「放棄通則、自己補一條測項守」。）
    - ⭐ **本專案第一次對 `progress.js` 做行為級驗證**：它開檔第一行就取 `HL.dom.el`、沒有 node 契約 ⇒ 歷來只能靜態掃。本輪改用 **vm 沙箱實跑真的 `progress.js`**（只需 5 個 stub 命名空間）。理由：逐字釘 `if (e.mb > 0 && bet > e.mb)` 擋不住 `if (false && …)`，更擋不住把 `bet` 換成 `w`（後者只有特定連鎖情境才分岔＝最容易活下來的那種缺陷）。⇒ 已開 **#192** 把這個配方推廣到 `progress.js` 另外四條只靠源碼掃的金錢不變量。
    - **鎖 2**（node 376 → **378 全綠**）：`platform/bonus-max-bet-never-blocks`（行為級·(A) 正向對照／(B) 不擋不倒扣不碰餘額／(C) 不靜默且只提醒一次／(D) 比原始 bet 不比剩餘 w／(E) FIFO／(F) 零回歸逐欄全等／(G) 併筆雙向／(H) 無 `HL.sla` 時退化成不限）＋ `platform/bonus-max-bet-policy`（政策面·含「不得與 `HL.rg` 的 `bet-single` 同名」反錨）。**負向擾動 17/17 CAUGHT**。
    - ⭐ **兩條首批 MISSED，而且是同一個形狀：沙箱每次都把環境餵齊，於是「環境不齊時的退化分支」沒有見證者。** (a) **P8**——把求值出口的 `: 0` 改成 `: 300`（取不到 `HL.sla` 時偷偷套上限）**整組全綠**，而那個分支**在真實開機序上走得到**（`index.html` 裡 `progress.js` 排第 80、`service-level.js` 排第 169）。補 (H) 後抓到。(b) **P17**——補上的 (H) **自己第一版也是空的**：它傳 `maxBet=0`，使「sla 不存在」與「sla 回 0」**同形** ⇒ 把沙箱的 `noSla` 選項改成 no-op 照樣全綠。改傳 300 讓兩者可分辨才真的釘住。⇒ CLAUDE.md §4 形狀⑦ 的新一種：**沙箱的「布景」本身也要有見證者，否則斷言認的是布景、不是行為。**
    - **成本與 funding**：本卡第一波即使把長註解**全部**搬進 `intel/` 仍需 **~2,400 bytes**（中文註解一字 3 bytes），而進場餘裕只有 **1,932** ⇒ **不合身**。由同一支檔、本輪 `擴充性` 分類輪替審出來的**零讀取欄位**支付：`service-level.js` 維度規格的 `hint`（六段說明、**818 bytes**、`src/` 與 `tests/` 讀取者皆 0、`HL.sla.dims()` 亦無呼叫者）＝T34 家族第二例。六段原文全數保存於 `intel/bonus-max-bet-2026-09-13.md` §7。收尾餘裕 **369 bytes**、sw v296→**v297**。
    - **全文**（∞ 編碼的三種寫法對照、五個選用欄位的紀律表、沙箱配方、第二波為何不做）→ [intel/bonus-max-bet-2026-09-13.md](intel/bonus-max-bet-2026-09-13.md)
    - 🚧 **第二波（贏額／可轉出上限）＝本輪刻意不做，已改開成 #191**。理由（實測，非推論）：它在我們的模型裡**沒有忠實對應**——本站沒有「用紅利的錢去玩」這回事（紅利先進獎金錢包、打完流水才變成能用的錢）。最近的類比是單筆紅利的可轉出上限，但寫成**倍數**（對手是 200%）⇒ `bOnWager` 達標時 `freed += e.amt`，倍數 ≥1 **恆不生效＝空綠容器**；寫成**全域絕對金額** ⇒ 會一視同仁套到全部 22 個送幣來源，而 `core/tournament.js` 首獎是 `POOL = 1,000,000 × SPLIT[0]` 的**一次 `badd`** ⇒ 任何合理上限都會砍掉九成，**那不是條款是沒收**。⇒ 前置是「一張以 `source` 為 key 的紅利條款登記表」，不是再加一個維度。

188. ✅完成（2026-09-13 平台軌 14:00 窗·第一波） **首屏預算是全站三軌共用的硬牆，而牆的另一邊有 15.5KB 是「92 支 eager 檔各自抄一份的同一行字」——先把其中最大且無阻的那 1.8KB 收回來** — M（純機械替換＋更新一條既有鎖的白名單；**零行為變更、淨回收位元組**）
    - **為什麼現在開**：本輪（#178 第一波）收尾後首屏餘裕只剩 **33 bytes**，而 09-12 遊戲軌才剛把它從 289 推到 2,072（`CARD_DEFAULTS`）。兩天之內三軌把 2,000 bytes 用光，而佇列上至少 **12 張卡**的阻塞事實逐字寫著「吃 eager 位元組／卡在首屏」（#87／#93／#132／#133／#134／#137／#138／#139／#141／#150／#154／#157／#158／#186／本輪新開的 #187…）。⇒ **這不是某一軌的問題，是三軌共用的瓶頸**，而它的下一筆額度不必等 #118（拆 `slot.js`）。
    - **實測（可複跑；口徑＝92 支 eager JS，同檔內重複只算一次，跳過註解行、長度 ≥40 字元）**：逐檔重複的樣板前 18 名合計 **15,479 bytes**。前七名：
      · `function t(k, d) { d = d || k; return HL.i18n ? HL.i18n.t(k, d) : d; }` — **27 檔／1,820B**
      · `var isNode = typeof module !== "undefined" && module.exports;` — 25 檔／1,464B
      · `})(typeof window !== "undefined" ? window : globalThis);` — 26 檔／1,400B
      · `else (HL._selftestQ = HL._selftestQ || []).push(registerTests);` — 23 檔／1,386B
      · `try { registerTests(require("./selftest.js")); } catch (e) {}` — 22 檔／1,281B
      · `var HL = isNode ? null : (global.HL = global.HL || {});` — 23 檔／1,210B
      · `if (HL.shell && HL.shell.refreshChrome) HL.shell.refreshChrome();` — 19 檔／1,170B
    - **⚠️ 據實分級：這 15.5KB 不是全部可回收的。** 第 2／3／6 名是**無打包工具**這個架構選擇的直接代價（IIFE 收尾、node 契約前導），沒有 build step 就收不回來——**別把它們寫進預期回收量**。第 4／5 名（selftest 三行樣板，合計約 3.9KB）理論上可收，但每支檔的 `require("./selftest.js")` 是**相對於該檔的路徑**，收斂需要先解決這件事 ⇒ 列為第二波、本卡不承諾。
    - **第一波＝只做第 1 名（27 檔／1,820B，換成共用出口後每檔約 20B ⇒ 淨回收約 1,300B）**。
    - **阻塞事實（開卡當下即查，不留給實作輪重新發現）**：常駐鎖 **`platform/i18n-no-duplicated-default-arg`** 的不變量 B 逐字要求「凡有單引數 `t("中文")` 呼叫的檔，其 local helper 必須是 `T_HELPER_OK_SHAPES` 白名單裡的**逐字形狀之一**」，而白名單自己還有一條「每個形狀都要有真實使用者、不得腐爛」的反錨。⇒ **這不是繞過它，是要把新形狀登記進 `tests/i18n-key-scan.js` 的白名單**，並確認舊形狀退場後那條反錨不會因為「零使用者」而轉紅。⚠️ 那條鎖之所以存在，是因為舊形 `function t(k, d) { … : d; }` 被單引數呼叫時**無 i18n 的分支會回 `undefined`＝畫面空白** ⇒ **新的共用出口必須自己補得出預設值**（`d = d || k`），這是本卡唯一不可退讓的行為不變量。
    - **驗收**：① `node prototype/tests/run.js` 全綠且**測項數不變**（本卡不加鎖，只更新既有鎖的白名單）；② 首屏讀數**確實下降**且 script 數不變；③ 27 支檔各挑一條單引數 `t("中文")`，在**無 `HL.i18n`** 的情境下回的是那句中文、不是 `undefined`（這條要在 node 直接打純函式，不看 live 資料——live 資料裡 `HL.i18n` 永遠在，這條性質**沒有見證者**）。
    - **✅ 第一波落地（2026-09-13 平台軌 14:00 窗）**：共用出口 `HL.tt` 立在 `core/dom.js`（`index.html` **第一支本地 script**；出口不能放 `core/i18n.js`，它自己排在 :83、晚於 cashback(:77)/challenges(:81)＝DEBT T27 記的那個陷阱）。28 支 eager 檔的 local helper 一律換成 `var t = HL && HL.tt;`，**位置刻意上移到 `var HL = …` 的下一行**——別名是 `var` 不是函式宣告、沒有提升，留在原位（有 10 支原本寫在第 111–599 行）會讓載入期早於它的呼叫拿到 `undefined`。`HL &&` 那六個位元組是為 12 支 `isNode ? null : …` 的檔付的：它們在 node 端 `HL` 為 `null`，而舊 helper 在 node 從來沒被呼叫過（一呼叫就會踩 `HL.i18n`）⇒ 語意逐位相同。
    - **實測**：首屏餘裕 **1,484 → 2,745 bytes（+1,261）**，script 數 93 不變；`node prototype/tests/run.js` **373 全綠且測項數不變**（符合驗收①②）。
    - **⚠️ 落地時多做的一件事（驗收③的實作）**：白名單新增 `var t = HL && HL.tt;` 之後，不變量 B 只認得「引用得到出口」，**證明不了「補得出預設值」**——那條邏輯已經搬進 `HL.tt`，而白名單一個字都沒看過它（§4 形狀⑦ 的新一種：**認引用不認性質**）。⇒ 在**同一條鎖內**（不新增測項）補兩組斷言：(i) 把 `core/dom.js` 在假 window 上真的跑一次、直接打 `HL.tt`（無 i18n 回那句中文／兩引數採用呼叫端預設／有 i18n 時單引數必須把 `(k, k)` 交給引擎）；(ii) **載入序**——`index.html` 裡 `core/dom.js` 必須排在每一個引用者之前，並附分母錨（別名使用者 ≥25、script 解析 ≥50）防它對空集合宣告通過。
    - **順帶修好的東西**：`tests/checks-platform.js` 新增共用 `domHL()`，6 個以 shim 載入真檔的鎖改由它供應 `HL.tt`（**刻意跑真檔取值，不在每個 shim 手抄一份**——手抄的那份會與真出口各自漂移，而漂移的症狀正是本專案 §4 那一族）。
    - **負向擾動 8/8 CAUGHT**（P1 拿掉 `HL.tt`／P2 不補預設值／P3 單引數不把 `(k,k)` 交給引擎／P4 `dom.js` 在 `index.html` 移到 cashback 之後／P5 某檔換回舊形壞變體／P6 白名單撤銷登記／P7 28 支全改回舊形＝別名分母塌成 0／P8 script 尺被改窄）。**P4 另單獨複驗過歸屬**：8 條轉紅裡確實含本條，且訊息逐字指名 6 支排在 `dom.js(#49)` 之前的檔。
    - **刻意沒做（據實）**：① `views/casino.js`／`views/tournament.js` 兩支**延遲載入**檔留在舊形——它們不吃首屏位元組，而留著它們正好讓白名單「每個形狀都要有真實使用者」那條反錨對 `T_HELPER_SAFE` 仍然成立（改完會變成 0 個使用者＝死條目，得連白名單一起刪）。② **第二波（selftest 三行樣板約 3.9KB）本輪未動**，卡上原本就寫明它需要先解決 `require("./selftest.js")` 的相對路徑問題。

186. 🟦已批准待做 **我們有兩個活動會告訴玩家「你上一期拿了什麼」，而它們各存一份、各渲染一份、彼此不認識——第三個活動要有結果面，就得再抄第三份，而抄漏一份的症狀正好是「畫面看起來完全正常」** — S（新增 `core/event-results.js` 登記簿 ＋ 兩個既有活動各接一行 ＋ 一個統一面板；**吃首屏 eager 位元組，落地前先量餘裕**）｜來源：**platform-modules 台帳「活動」分類輪替**〈活動收尾面／往期成績與得獎公示〉`partial` 的 evidence **自 2026-09-04 起就逐字寫著這張卡的出口形狀**（連續三輪未開＝#167 同輪避免雙開、後兩輪維度不在此格）× **BitStarz（本輪新入庫·全庫第 37 筆）**：Slot Wars／Table Wars **兩條並行週賽各自有每週一次的結果公示**
    - **機械事實（可複跑）**：`core/raffle.js` 的 `KEY_H = HL_RAFFLE_HIST`（上限 12 筆）與 `core/tournament.js` 的 `KEY_H = HL_TOURNEY_HIST`（上限 8 筆）是**兩份各自獨立的清單 ＋ 兩份各自獨立的 render 迴圈**；全庫**沒有任何登記簿**把「哪些活動有往期結果」收成一處。`rain`／`世界活動 hero`／`季票`／`公會週榜` 則一個結果面都沒有。
    - **出口形狀（容器先於內容）**：`HL.eventResults.register({ id, label, history() })` ＋ 一個統一的「往期活動」面板；各活動只負責供 `history()`，面板與排序由容器管。比照 `HL.reports`（#109）／`HL.dock`／`HL.cashier`（#82）的既有形制——**容器零內建、註冊即出現**。
    - 🚧 **反重複界線**：
      · **不是 #167 的雙胞胎**——#167 是「讓錦標賽**這一個**活動的賽果被讀到」（它的驗收已達成且有鎖）；本卡是「讓**第 N 個**活動不必再抄一份」。#167 落地後這個缺口才看得見，不是它的重做。
      · **不是 #176 的雙胞胎**——#176 是**條款面**（加入之前被告知什麼，維度 17），本卡是**收尾面**（跑完之後長什麼樣，維度 10）。台帳刻意分成兩個模組，正是為了讓「時間軸綠、結果面空白」不再對指標免疫。
      · **不是 #109 報表註冊表的雙胞胎**——#109 是**營運/匯出**面（誰要一份 CSV），本卡是**玩家**面（我上期第幾名）。兩者受眾與出口都不同；但本卡**應沿用 #109 的登記簿形制**，不得長出第三種寫法。
    - ⚠️ **落地時最容易做錯的一件事**：把它做成「讀兩個既有 `KEY_H` 的聚合器」。那是第三份真相——容器必須**向各活動求值**（`history()`），否則第三個活動加進來時，聚合器又要改一次。
    - **驗收（headless 可證）**：① 容器零內建（拿掉兩個註冊者 ⇒ 面板具名空態，不是壞掉）；② 兩個既有活動的往期清單**逐筆等於**它們各自 `history()` 的回值（不得在容器側私藏副本）；③ 新增第三個註冊者＝一行，面板與既有兩者逐位不變；④ 真站/假站的站別隔離沿用 `HL.site.ns()`，容器側不得再手寫第三份站別判斷（承 #157 `drivable()` 同一條紀律）。
    - **阻塞事實**：新增 `core/event-results.js` 會進 `index.html` 首屏清單 ⇒ 吃 eager 位元組。本輪收尾餘裕 **980 bytes**，一支小型登記簿約需 700–1,000 bytes ⇒ **落地前必須先量、必要時自籌**（比照 #179 用註解壓縮自籌）。


- [x] **#189 · 遊戲樣式隨遊戲延遲載入（容器原本只搬了一半）** — ✅ **2026-09-13 遊戲軌 10:00 窗落地**
    - **病根**：`#80`/`#110` 把遊戲**程式**搬離首屏，**樣式卻整包留在 `components.css`**。實測各遊戲專用樣式合計約 **33.5KB**（單款 slot 典型 3.1–3.3KB）⇒ 玩家一次都沒點開的遊戲，樣式照樣在開站時下載；而首屏讀數看起來一切正常。
    - **它擋住的是整條管線**：進場首屏餘裕僅 **33 bytes**，一款新 slot 要 ~3,650（CSS 3.3K + manifest/rtp/edge 三張註冊表 ~350）⇒ **任何新遊戲都上不了架**。4 筆 `specd` 卡在管線頂端、本軌自 08-21 起 23 天沒有新遊戲——逐輪記的阻塞理由是「需可靠 preview」（09-12 已證否），**真正的硬牆是首屏預算**。
    - **落地**：`core/lazy-load.js` 的 `load(src)` 依副檔名分派（`.css` → `<link rel=stylesheet>`），**刻意走同一個出口**＝同一份載入態表、併發只注入一次、失敗語意一致；`data/lazy-games.js` 一列可帶 `css:`，**程式與樣式都到齊才換手**（否則真 render 會在樣式抵達前先畫一次）。樣式失敗不讓遊戲變「載入失敗」（退化＝無樣式但可玩）。
    - **結果**：首屏餘裕 **33 → 2,229 bytes**，同時多了一整款遊戲。首款採用者＝`abyssal-surge`；並把 `gem-storm` 樣式逐位元組遷出作為既有款的首個見證者（線上複驗：6×5=30 格、grid 6 欄 5 列、token 仍解析得到）。
    - **鎖** `platform/lazy-css-rides-with-game`（檔案存在／**行為級**都到齊才換手／不得在首屏留第二份）·負向擾動 **13/13 CAUGHT**。⚠️ 首批 11/13：探針原本結算「佇列最前面那個」＝認位置（§4 形狀⑦變體），且沒驗樣式是否**以 `<link>` 身分**注入 ⇒ 改依身分結算 script + 檢查節點型別後才全中。
    - 完整設計與沿革 → [intel/lazy-game-container-2026-09-13.md](intel/lazy-game-container-2026-09-13.md)。

- [x] **#190 · 把其餘 25 款的專用樣式遷出首屏（#189 的收割）** — ✅**完成**（2026-09-14 遊戲軌 10:00 窗 · commit `deed285`+`1f38bf3` · 新鎖 1・改寫 2 · 負向擾動 **12/12 CAUGHT**）
    - **結果**：**17 個清單列／19 個獨有前綴**遷出首屏。`components.css` **232,367 → 183,450 bytes（−48,917／−21%）**；首屏餘裕 **322 → 48,907 bytes**。1,825 條規則中 **1,824 條逐位元組不變**，唯一改動＝`.ax-inst__bet, .ax-inst__num, .ax-limbo__target` 逗號清單拆分（前兩者是 instant 共用外殼、必須留在首屏）。`@keyframes` 一律不搬（已知陷阱②）。
    - **刻意沒搬的三個**（都不是遺漏）：`instant-crash-mines.js`（無獨有前綴）、`table-baccarat.js`（`ax-bacc` 與輪盤共用）、**`.ax-tbl__`**（`core/table.js` 畫的桌遊引擎外殼，六款共用）⇒ **「可安全搬遷者」已做完**。
    - **⭐ 已知陷阱①被實際踩到，而且比卡上寫的更嚴重**：卡上寫「鎖的 (c) 已用『該前綴在其他 view 一次都沒出現』當判準，別繞過它」——**而那個判準本身就是錯的**：它只掃 `src/views/`，而 `.ax-tbl__` 住在 `core/table.js`。跟著鎖走會把共用外殼搬進輪盤私有檔＝**另五款桌遊當場沒有籌碼列，而鎖全綠**。已修：(c) 改掃全 `src/`（剥註解，否則 `core/ui.js` 一行註解會讓 `ax-dice` 被誤判成共用＝fail-open）＋新增 (c2)(c3)。
    - **⭐ 新發現的第二類風險（卡上未曾預料）：CSS 勝負關係會被搬家翻轉**。延遲樣式檔在 `components.css` **之後**載入 ⇒ 同特異度時「後寫者勝」會反過來。`.ax-pump__go`（舊檔 159 行）原本被同一顆按鈕上的 `.ax-btn-primary`（464 行）蓋過＝**死碼**；搬家後它反過來贏。處置＝保住原渲染結果（零視覺改變）、刪掉死規則；新鎖 `games/lazy-css-no-cascade-flip`（修好後全庫無見證者 ⇒ 以**合成輸入**打純函式）。
    - **驗收（卡上要求的那一條，已做）**：遷移前先在**線上舊版**逐元素抓 41 個 computed style 指紋（25 款、**連抓兩輪取交集**剔除天然不穩元素），push 後對**線上新版**同法再抓並逐元素比對：**比對 3,336 個元素、0 個樣式改變**。另以 `el.matches()` 在真 DOM 裡掃同特異度對撞作**第二條獨立路徑**，兩條各自只抓到**同一條**（pump）。開機時遊戲樣式檔載入數＝**0**，開一款才來一支且只來那一支（實測 money-wheel → pirots）。
    - **射程邊界（據實）**：指紋量的是**預設渲染狀態**，hover／disabled／is-win 這類**狀態樣式**不在射程內（新鎖是靜態掃描，涵蓋得到這些規則，算補位）；「好不好看」仍記 **UNVERIFIED**。
    - **獎品明確**：`components.css` 現 232KB，依 class 前綴歸戶的各遊戲專用樣式仍約 **30KB** 在首屏（`.ax-slot` 5,483／`.ax-chx` 4,974／`.ax-mw` 4,921／`.ax-sb` 4,703／`.ax-ab` 4,402／`.ax-bacc` 4,393／`.ax-dt` 4,211／`.ax-rou` 4,013／`.ax-toad` 3,132／`.ax-pir` 3,103／`.ax-dbn` 3,062…）。容器已經在（#189），每款只是「加一個 `css:` 欄位 + 搬一個區塊」。
    - **為什麼本輪刻意不一起做**：那是 25 款的**視覺回歸面**，而本輪沒有能證明「搬完長得一樣」的量具——headless 驗不到版面，線上複驗一次只能走幾款。gem-storm 是刻意選的單一見證者（前綴獨立、區塊連續、已線上複驗）。
    - **⚠️ 已知的兩個陷阱**（搬之前先讀）：① **共用前綴不可搬**（如 `.ax-inst__` 被多支 view 共用）——鎖 `platform/lazy-css-rides-with-game` 的 (c) 已用「該前綴在其他 view 一次都沒出現」當判準，別繞過它；② **共用 keyframes**：`ax-dbn-win`／`ax-pir-pop` 被四款 slot 共用且定義在首屏，搬動它們要四款一起處理（gem-storm 的私有檔現在就仰賴這兩個仍留在首屏的動畫）。
    - **驗收**：每遷一款 → 該款前綴不得再出現在 `components.css`（鎖自動守）＋ 線上走一次玩家路徑量 `getComputedStyle`（遷移前 `block/none` → 遷移後回到原本的 grid/欄數），**並附一款未遷移的做正向對照**。

> 🤖 **以下由自我進化引擎「平台軌」自動開卡**（2026-09-13 **20:00 窗** · 來源：#187 第一波落地過程中**當場量到**的兩件事，皆為實測非推論）。全自動模式下標 🟦已批准待做。

191. 🟦已批准待做 **紅利條款的第五個軸（可轉出上限）需要的不是「再加一個維度」，而是一張以「送幣來源」為 key 的條款登記表——因為全域套用會把錦標賽首獎從 30 萬砍成 5 千** — M（新登記表＋接 `badd()` 一處；**吃 eager 位元組**）— 來源：#187 第一波落地時對第二波的可行性實測
    - **一句話**：#187 的前四個選用欄位裡有三個是「逐段位」的（`req`／`mb`）、一個是「逐來源」的（`exp`，#71 `HL.bonusTtl`）。第五個軸**必須是逐來源**，而我們現在只有 `bonusTtl` 一張來源表、且它的語意是壽命。
    - **為什麼不能照 #187 的做法再加一個 `HL.sla` 維度**（實測，非推論）：
      · 寫成**倍數**（對手 Cybet 是「紅利額的 200%」）⇒ 本站 `bOnWager` 達標時 `freed += e.amt`，倍數 ≥1 **恆不生效** ⇒ 一個永遠不會紅的空綠容器（CLAUDE.md §4「這個容器有沒有人真的用過？沒人用的話，是什麼在擋？」）。
      · 寫成**全域絕對金額** ⇒ 一視同仁套到全部 22 個 `HL.bonus.add` 來源。`core/tournament.js` 的 `POOL = 1,000,000`、首獎 `prizeFor(1)` 是**一次 `badd`** ⇒ 任何合理的上限都會砍掉九成以上。**那不是條款，是沒收**，且會撞 #71 立下的信任紅線。
    - **出口形狀**：比照 `HL.bonusTtl.register({ id: <source 字串>, days, daysLive })` 的形制開一張**紅利條款表**（`id` 同樣直接用 `badd(n,{source})` 的 source 字串＝不另立第二套 key），欄位為「可轉出上限」（絕對金額，兩站別各一）。entry 上是第五個選用欄位 `wc`，**授予當下求值一次、查無政策不寫欄位**（與 `req`／`sc`／`exp`／`mb` 同一條紀律）。
    - ⚠️ **落地時的兩條紅線**：① 上限只封頂「這筆紅利能轉出多少」，**不得回頭扣 `unlocked`**（#71 的信任紅線：已轉為可領取的錢不得被回頭作廢）；② 被封頂而未發放的差額必須 `HL.ledger.record("bonus_void", …)` **回沖帳本**＋發通知，**不得靜默蒸發**（沿 #71 逾期作廢的既有處置）。
    - ⚠️ **需要船長看過再做的部分**：「哪些來源該被封頂」是產品決策不是工程決策——錦標賽／公會週榜／黃金之城這類「贏來的」獎金，與儲值配比的 Reload 紅利，性質完全不同。建議落地時**先只註冊 `Reload 週期紅利` 一筆當單一見證者**，其餘等裁決。
    - **阻塞事實（開卡當下即量，不留給實作輪重新發現）**：`core/progress.js` 與新表皆 eager ⇒ 吃 `platform/first-screen-budget`。**本輪收尾餘裕僅 369 bytes**，而一張新的來源登記表（比照 `bonus-ttl.js` 的純函式區規模、註解全搬 intel）估 **1.5–2.5KB** ⇒ **本卡在下一筆首屏額度進帳之前不可落地**。候選額度來源：#190（其餘 25 款遊戲樣式約 30KB）、#188 第二波（selftest 三行樣板約 3.9KB）。

192. ✅完成（2026-09-14 平台軌 08:00 窗 · 新鎖 4 · 負向擾動 **25/25 CAUGHT** · 首屏逐位未動）**`core/progress.js` 有四條「金錢語意」的不變量只靠逐字 grep 在守，而本輪剛證明它跑得進 vm 沙箱——把那四條升級成行為級** — S–M（**只動 `prototype/tests/`＝首屏逐位未動**）— 來源：#187 第一波立鎖時的發現
    - **一句話**：我們一直以為 `progress.js` 只能靜態掃（它開檔第一行就取 `HL.dom.el`、沒有 node 契約、`require` 會直接爆），本輪發現**它只需要 5 個 stub 命名空間**（`dom`／`ui`／`state`／`notify`／`sla`）就能在 vm 裡實跑，而 `HL.bonus` 的 `add`／`onWager`／`claim`／`status` 全部可直接操作、`localStorage` 只是一個 20 行的假 `lsGet`／`lsSet`。
    - **現成配方**：`prototype/tests/checks-platform.js` 的 `bonusSandbox()`（#187 本輪新增）。
    - **該升級的四條**（全部是「錢」的語意，目前皆為源碼掃或無測）：
      · `platform/bonus-ttl-cannot-touch-unlocked`（#71 信任紅線：TTL 不得作廢已轉為可領取的錢）——現在靠「`bSweep` 函式體內不得出現 `unlocked` 這個字」。
      · **#74 `req` 凍結**（降段不得追溯加重既有紅利的門檻）——目前**沒有任何測項直接驗這件事**，只有 `sla/bonus-wager-frozen` 的架構掃。
      · **#89 範圍權重**（`weightFor` 回 0 時不得推進、不得倒扣、不得跳過頭筆）——純函式那半有測，**`bOnWager` 裡的 FIFO 消耗換算那半沒有**。
      · **#71 `exp` 併筆守衛**（壽命不同不得併筆）——同上，`sameExp` 只有源碼在。
    - ⚠️ **必寫的不變量**：每一條都要配一個**正向對照**（先證明沙箱量得到「正常情況下會發生的事」），否則「沒發生」的斷言在沙箱壞掉時恆真——這正是 #187 (A) 存在的理由。
    - ⚠️ **沙箱的布景本身也要有見證者**（#187 負向擾動 P17 的教訓）：任何 `opts.xxx` 選項若在「開」與「關」兩種情況下斷言結果同形，那個選項被改成 no-op 也不會紅＝斷言認的是布景不是行為。
    - **阻塞事實**：無首屏位元組、無 preview 需求（全在 `tests/`）⇒ **不卡首屏預算**，任何一輪都可做。
    - ✅ **落地（2026-09-14 08:00 窗）**：`bonusSandbox()` 擴為可控時鐘（`tick()`／`now()`）＋ 真依賴注入（`opts.ttl`／`opts.scope`）＋ `HL.ledger` 觀測點 ＋ `setWagerMult()`；新鎖 4 —— `platform/bonus-unlocked-survives-expiry`（#71 信任紅線）／`platform/bonus-req-frozen-at-grant`（**#74**）／`platform/bonus-scope-weight-fifo`（#89 的 FIFO 消耗換算）／`platform/bonus-exp-merge-guard-behavior`（#71 (d)）。四條**都不取代**既有源碼掃鎖：源碼掃守「結構上做不到」，行為級守「實際上沒發生」。node **380→384 全綠**；首屏 **322 bytes 餘裕＝與進場逐位相同**；`sw.js` 不 bump（`grep -c "tests/" prototype/sw.js` ＝ 0）。
    - ⭐ **落地時推翻卡上一個隱含假設：依賴不能用 `require()` 掛進沙箱。** `wager-scope.js`／`bonus-ttl.js` 都有 `module.exports`，最順手就是 require 當布景——但 `wager-scope.js` 的 `resolverOf()` 在 **node 分支下永遠取不到 `HL.games`** ⇒ `registryReady` 恆 false ⇒ 權重表那條路一律 **fail-open 回 1**。首版即如此，「部分權重」情境跑出 `prog=300`（fail-open）而非真值 `200`，**而斷言是綠的**（因為斷言是照跑出來的數字寫的）。改成**在同一個 vm 裡跑它們的瀏覽器分支**才量到真值。⇒ **「有 node 契約」≠「node 契約下的行為等於瀏覽器行為」**；檔內有 `isNode` 分岔者，沙箱必須指定走哪一支並說得出理由。
    - **⑥ 擾動清單反過來補了一條漏掉的斷言**：原 (B) 只走「這一注就達標」那支 ⇒ 把 `eff = floor(w×wt)` 改成 `eff = w` 在 (B) 裡量不出來（消耗換算仍由 `ceil(need/wt)` 決定）。補 **(B3)「未達標那一支」** 才釘得住（對應 P13）。⇒ **擾動清單要在斷言寫完之後再列一次，它會指出斷言的盲區。**
    - **負向擾動 25/25 CAUGHT**：P1–P5 信任紅線（回頭扣 unlocked／sweep 被短路／claim 不歸零／不回沖帳本／靜默蒸發）·P6–P9 req 凍結（載入重算／併筆整筆重算 90→160／求值不走 `HL.sla`／進度歸零）·P10–P15 範圍權重 ×FIFO（倒扣／跳過頭筆／少扣 `w-=need`／`eff=w`／權重短路成恆 1／誤寫餘額）·P16–P20 壽命併筆（`sameExp` 恆真／條件被短路／`undefined≠0`／併筆全關／順手改尾筆到期日）·**P21–P24 布景見證者**（`opts.ttl`／`tick()`／`HL.games` stub／`setWagerMult` 各自改 no-op 都必須有人紅）·P25 純函式側 `bonusTtl.sweep`。
    - ⚠️ **擾動過程中量具自己被咬了一口，值得記**：第一趟把腳本包在 `timeout 590` 裡，P16 當場被砍 ⇒ 子行程 stdout 只剩半截、一個 `❌` 都沒有 ⇒ 腳本**把「這一趟沒跑完」印成「MISSED（全綠）」**，差一點據此寫下「抓到一條空綠鎖」。修法＝以 `run.js` 收尾必印的統計行當**存活證明**，收不到就判 `ABORT`。⇒ **一把只會數失敗的尺，分不出「沒有失敗」與「沒有量到」**（§4 形狀⑦ 的量具版）。

> 🤖 **以下由自我進化引擎「平台軌」自動開卡**（2026-09-14 **08:00 窗** · 來源：本輪審「前端UI/UX」時，**同一份手抄讀數被證實自 09-08 寫下當天就有兩筆是錯的**，且各被複製了十份）。全自動模式下標 🟦已批准待做。

193. ✅完成（2026-09-14 平台軌 **20:00 窗**·commit 見下·新鎖 1·**負向擾動 13/13 CAUGHT**） **台帳的共用探針讀數是手抄進 JSON 的，於是一筆抄錯會變成十筆抄錯、存活六天、而且沒有任何東西會發現——因為抄的是數字，對的卻是形容詞** — S–M（全在 `intel/tools/`＝**零首屏位元組、無 preview 需求**）— 來源：platform-modules 台帳〈前端UI/UX〉分類輪替審（2026-09-14 08:00 窗實測）
    - **一句話**：分類輪替每輪會跑一組固定的「共用探針電池」（`HL.nav` 命中數／`isPF`／`data-theme`／`settingsModal(`／`ui.comingSoon(`／斷點 distinct…），但**讀數是人眼看 grep 輸出再打進 evidence 字串的**，而那段字串**被逐字複製進該分類全部 10 格** ⇒ 抄錯一次＝錯十份，且讀任何單一格的人分不出那一行是**為這一格量的**還是**繼承來的**。
    - **本輪抓到的兩筆（皆已回到 09-08 的 commit `548fef8` 用同一把尺重算證實「不是漂移，是當天就錯」）**：
      · `ui.comingSoon(` 記 **27**（app-shell 25／game-frame 1／lobby 1），**當日實際 28**——漏記 `core/responsible.js` 那一筆；而那一筆是**註解**不是呼叫點 ⇒ 正確口徑是「原始命中 28／實際呼叫點 27」。**不更正的話，下一輪量到 28 會誤報成「又多了一個死巷入口」。**
      · `HL.gameAxes` 記「出口 **13** 個齊備」，vm 實跑 `Object.keys()` 求值**實測 12**。
    - ⭐ **根因不是粗心，是「形容詞沒有反向」**：「**齊備**」後面接的那個數字**沒有任何測項、任何複核在對**，所以它可以錯一整輪而永遠不會被碰到。這是 §4 形狀⑦「斷言認的是概念還是寫法」的台帳版本——**台帳上的形容詞若沒有一個可求值的量跟著，它記的只是寫下那天的心情**。
    - **出口形狀（容器先於內容，比照 `intel/tools/ledger-card-sweep.js` 的既有形制）**：`intel/tools/ledger-probe.js` ＝一張**可註冊的探針表** `{ id, category, label, run() }`，`node intel/tools/ledger-probe.js` 印出逐筆讀數；台帳 evidence 從此**引用工具輸出**而不是手抄。新增一條探針＝表上加一列。
    - **必寫的不變量（每一條都要配負向擾動）**：
      · **(a) 探針回的是值，不是形容詞**：`run()` 必須回可求值的量（數字／字串／`{raw, effective, note}` 三元組），回 `undefined` 或拋例外**必須大聲失敗**（fail-closed），不得靜默從報表上消失——否則這張表會用「少印一行」的方式說謊，而那正是本卡要治的病的升級版。
      · **(b) 兩段式口徑是一等公民**：`isPF`（原始 9／去重 6）與 `ui.comingSoon(`（原始 28／呼叫點 27）證明「原始命中」與「有效數」**本來就是兩個量**。探針必須能同時表達兩者，否則下一個人又會把其中一個當成另一個。
      · **(c) 可對任意 git ref 重跑**（`--ref <sha>`）：本輪之所以能證明「這是漏記不是漂移」，靠的就是回到 `548fef8` 用同一把尺重算。**沒有這個能力，「當天就錯」與「後來變了」在台帳上完全同形**——而兩者該做的事完全相反（前者改記法、後者查改動）。
      · **(d) 它不是第二份真相**：工具只印讀數，**不判 present/partial/weak/absent**。狀態判定留在人這一側（台帳的價值就在那個判斷）。
      · **(e) 正向對照**：報表必須至少有一條探針在**故意改壞來源**時當場變值——否則「全部零漂移」可能只是工具沒讀到檔（比照 `ledger-card-sweep` 的樣本量錨與 `catalog-freshness` 的 (I) 加總守恆）。
    - **阻塞事實（開卡當下即量）**：產出全在 `intel/tools/`（不被 GitHub Pages 服務、不進 `prototype/`）⇒ **首屏零位元組、不吃 `platform/first-screen-budget`**，任何一輪都可做。⚠️ 唯一的取捨：`intel/` 沒有 `prototype/tests/run.js` 那樣的常駐閘 ⇒ 本卡的不變量要嘛寫成工具自帶的 `--selftest`，要嘛在 `prototype/tests/` 立一條鎖去跑它；**擇一，但不得兩者皆無**（否則這張治「沒人在對」的卡自己也沒人在對）。
    - **反重複界線**：不是 `ledger-card-sweep.js` 的雙胞胎——那支問「**evidence 裡的缺口敘述是不是已經被我們自己的卡關閉了**」（跨 BACKLOG × 台帳的一致性），本卡問「**evidence 裡的那個數字是不是真的**」（台帳 × 原始碼的一致性）。兩支共用 `intel/tools/` 的同一種形制，但射程不重疊。
    - ---
    - **【落地紀錄 · 2026-09-14 20:00 窗】** `intel/tools/ledger-probe.js`＝可註冊的探針表（`{id, category, label, scope, run(ctx), witness}`），**22 條探針**（後台 13＋前端UI/UX 9）。`--category` 篩分類、`--ref <sha>` 對任意 git ref 重算、`--json`、`--selftest`、`--witness`。五條不變量全部做成可執行的：
        - **(a)** `run()` 必須回 `{raw, effective}`；拋例外／回 undefined 一律轉成**該列印 ❌ 且 exit code 1**，**報表列數恆等於登記數**（不得用「少印一行」消失）。`register()` 對缺 run／缺 witness／缺量程／撞 id 的 spec 一律當場拋。
        - **(b)** 每一列都同時印**原始命中／有效數**；並指名**兩個活見證者**（`ui/coming-soon-sites` 28/27、`backend/econ-cfg-registrars` 15/14，差額各是一筆註解）讓「剝註解」那一層自己有紅線。
        - **(c)** 探針**只准經由 ctx 讀檔**（selftest 掃探針區禁止 `fs`）⇒ `--ref` 真的換得掉來源。實測回到 09-08 的 `548fef8`：`ui.comingSoon(` **28/27**、`HL.gameAxes` **12** ⇒ **獨立複現 08:00 窗那兩筆「當天就錯」的判定**。
        - **(d)** 工具**不讀 `platform-modules.json`、不印 present/partial/weak/absent**（兩條都是 selftest 斷言）。
        - **(e)** **每一條探針都必須帶活見證者**：改壞來源時 effective 必須當場變、且**不得改成報錯**。首跑當場打掉 **10 條空見證者**（`isPFX` 仍含 `isPF`／見證者落在被排除的檔／改的是 raw 那一半／量檔名的探針改內容沒用…）。
    - **⭐ 新增 `scope`（量程）為必填**：每一列都印「這把尺看得到哪些檔」。直接來自本分類自己的事故——「營運儀表板寫入面恆為 2、連十二輪零漂移」為真，但那把尺只掃 `views/ops-dashboard.js`，真正的營運開關在 `core/demo-tools.js`（§4 形狀⑦-(h)）。
    - **鎖** `platform/ledger-probe-fail-closed`（`tests/checks-platform.js`·node fast·**不出貨＝零首屏**）：跑工具的 selftest ＋ 四道反空綠錨（探針數 ≥15／分類數 ≥2／每條都自陳量程／五個不變量家族都要在場）。⚠️ **它刻意不照既有慣例「require 失敗→skip」**：`register()` 是 fail-closed 的，載入失敗**正是最該紅的時候**——檔不存在才 skip，檔在而載不起來一律 fail（負向擾動 P12b 實證：拆掉這道保險後，把工具整支弄壞會變成綠底的「略過 1」）。
    - **負向擾動 13/13 CAUGHT**：P1 兩道 witness 閘全拆／P2 見證者 no-op／P3 見證者改成把探針弄壞／P4 runOne 吞 throw／P5 加一條沒量程的探針（載入即拋）／P6 探針回 undefined／P7 探針區直接用 fs／P8 刪掉整個 (b) 家族／P9 兩段式口徑退化／P10 探針回狀態判定／P11 overlay 的 list() 退化／P12 工具載不起來／P13 探針數砍到 15 以下。
    - **⭐ 擾動開出來的兩個真洞（都是我自己寫的，靜態全綠時完全看不見）**：① **P1 首版 MISSED**——拿掉必填欄位清單裡的 `witness` 不會紅，因為**下一行 `spec.witness.mutate` 會先 TypeError**＝性質靠另一條路碰巧成立；兩道閘一起拆才露出來。② **P9 首版 MISSED**——把 `hits()` 的 code 直接回 raw（＝註解又被算成呼叫點，正是 08-31 把 7 讀成 8 的那個錯），而 (b) **照樣全綠**，因為兩段式的見證者還有別條路（檔名型探針）。⇒ 補 **(b2) 指名活見證者 ＋ (b3) 合成輸入打純函式**。**這兩個都是「防空心的保險架在沒有漏掉的那一段量程裡」的第四、第五次重演。**
    - **當輪即被使用**：本輪審「後台」十格的讀數**全部由本工具求值**，並當場抓到**兩筆離開定值七～十二輪的讀數**（延遲載入清單 20→22、放置區入口 1→2，兩筆都是真的長大）——舊寫法下它們會被抄成同一個數字再活好幾輪。
    - **射程邊界（據實）**：本輪只遷了**兩個分類**（後台 13 條＋前端UI/UX 9 條）。其餘六個分類（擴充性／資料／金流／功能／活動／資安）的讀數**今天仍是手抄的**，同一種病仍在 ⇒ 已開 **#194**。

> 🤖 **以下由自我進化引擎「平台軌」自動開卡**（2026-09-14 **20:00 窗** · 來源：#193 落地後量到的射程邊界）。全自動模式下標 🟦已批准待做。

194. ✅完成（2026-09-15 平台軌 **08:00 窗**·commit 見下·探針 **22 → 46 條**、分類 **2 → 8**（台帳輪替的全部分類）·新錨 1（逐名分類）·**負向擾動 14/14 CAUGHT**·node **400 全綠**·`prototype/src` **一位元組未碰**·`sw` 不 bump） **治好了兩個分類的手抄讀數，另外六個分類今天還在抄** — S（純 `intel/tools/`＝**零首屏位元組、無 preview 需求**；每個分類 3–6 條探針）— 來源：platform-modules 台帳〈後台〉分類輪替審（2026-09-14 20:00 窗·#193 落地當下即量）
    - **一句話**：#193 把「共用探針電池」變成可求值、可對任意 git ref 重跑的量，但**只遷了後台與前端UI/UX**。其餘六個分類的 evidence 裡仍是「人眼看 grep 輸出再打進 JSON 的數字」，而那正是 09-08 那兩筆錯讀數活了六天的成因——**病沒有變，只是有兩個分類先被治好了**。
    - **出口形狀**：不需要新容器，`intel/tools/ledger-probe.js` 的表上加列即可（`{id, category, label, scope, run, witness}`）。**每加一條都必須帶活見證者與非空量程**，`register()` 會 fail-closed 拒收，鎖 `platform/ledger-probe-fail-closed` 的錨（探針數 ≥15、分類數 ≥2）會隨之水漲船高。
    - **每個分類先挑那條「被引用最多輪、而且從沒人複核過」的讀數**（那是最可能已經錯了的一筆）。已知候選：〈擴充性〉`registry-gaps` 的 unproven 基線／〈資料〉`betlog` CAP 與 `activity` KEEP_DAYS 的保留上界／〈金流〉`TXN_KINDS` 型別數與 `HL.sla` 呼叫點／〈功能〉成就種子筆數（08-26 已因口徑錯過一次：naive 21／權威 19）／〈活動〉`promoCal` 品項數／〈資安〉`HL.rbac` 消費者數（#182 之前恆為 1）。
    - ⚠️ **開卡當下即量的兩個陷阱**（別讓實作輪重新發現）：① 口徑有兩種——`grep -o`（次數）與 `grep -rn`（行數）在同一個關鍵詞上可以差一倍（`isPF` 12 vs 9），探針必須把用的是哪一種寫進 note；② **見證者最容易寫成空的**：#193 首跑 22 條裡有 **10 條**的見證者無效（改名後仍含原字串／落在被排除的檔／改到的是 raw 那一半／量檔名的探針改內容沒用）。**先跑 `--witness` 看它真的翻值，再寫進台帳。**

195. ✅完成（2026-09-14 遊戲軌 **22:00 窗**·commit 見下·**第 28 款遊戲上架**·新鎖 **10**·負向擾動 **23/23（P8／P15 是我自己的斷言有洞、修完才抓到；P19 由 deep 的 buyin-rtp 抓到）**·`sw` v312→**v313**） **管線頂端那筆 specd 候選（BGaming「Space Knight Merge Up 2」）終於進建置——而動手才發現它宣稱的 9 級階梯，照著規格寫出來是**走不到**的：20,000 局內 T6 以上零次、炸彈零顆，而所有數值測項都會是綠的** — L（新增 `views/slot-star-forge.js` ＋ `styles/game-star-forge.css`〔**隨遊戲延遲載入·首屏零位元組**〕＋ 六處註冊 ＋ 三語 ＋ 10 條鎖）— 來源：2026-09-14 10:00 窗 #190 解除首屏封鎖後的明文交棒「下一輪就可以開始出新遊戲了，首選 `space-knight`（已有完整 fidelity_spec 與 MC 可行性證明）」
    - **一句話**：**星鑄 Star Forge ⚒️**（6×6 cluster-merge、符號 9 級 level-up、位置乘數 ×2→×128、過熱炸彈、三種買入）。帶進平台的新維度＝**符號等級演進**——既有 27 款沒有任何一款讓「符號變成另一個符號」。
    - **⭐ 本輪最貴的發現（全文見 [intel/star-forge-tier-ladder-2026-09-14.md](intel/star-forge-tier-ladder-2026-09-14.md)）**：照 spec 的「固定門檻 4」寫出來，逐階質量 ÷4 且「4 個要正交相鄰」在高階幾乎不可能 ⇒ 實測 20,000 局內升出 T5 **412 次**、**T6 以上 0 次、💣 0 顆、T9 0 次** ⇒ `PAY[7..10]`、炸彈、頂階**全部是寫在賠付表與說明面裡、而程式永遠走不到的裝飾**。而**這個狀態下 RTP／最大贏額／無 NaN／連鎖收斂全部會綠**，畫面也完全正常（玩家只會覺得「這款好像永遠只爬到第五階」，而那正好可以被解釋成高波動）。
      · 修法三件缺一不可：**成群門檻隨階級遞減**（4/4/4/3/3/2/2/2/1）＋**免費遊戲中 T≥5 符號「不冷卻」跨轉黏著**＋**把 💣 移到 base 局真的到得了的那一階**（canonical T8 → 我們 T6）。順帶讓玩法更清楚：**基礎局第 6 階過熱炸開把你推進熔爐，第 7 階以上只在熔爐裡煉得出來。**
      · **常駐鎖 `games/star-forge/tier-ladder-reachable` 不驗數值，它驗「每一階有沒有被真的升出來過」。** ⇒ 通則：凡「階梯／等級／收集／解鎖」型機制，都要問一次「最高那幾級程式真的走得到嗎」並把答案做成鎖。
    - **RTP 定版方法（本款端到端 SD＝**10.80**〔400 萬局實測〕⇒ 直接 MC 需約 **1,800 萬局**才收斂到 ±0.5pp）**：改走**分層估計**——`RTP = base局期望 + 觸發率 × E[免費整段]`，三個量各自獨立量測再以誤差傳播合成。實測 base **8,000 萬局×2 種子**、免費段 **400 萬段×2 種子** ⇒ CI95 **±0.125pp**（同預算下比直接 MC 緊 3–4 倍；免費段的樣本本來就得為第 14 項單獨量 ⇒ 不是額外成本）。校準後**用全新種子獨立再量一次**（base 2,000 萬局 ＋ 免費段 100 萬段）＝ **96.4774% ± 0.251pp**。校準鈕只有一個（`CFG.payScale`），而「RTP 對它嚴格線性」這個前提**已常駐化成鎖** `games/star-forge/rtp-is-linear-in-payscale`（同種子跑兩次比值、零雜訊；免費段另留 0.5% 容差給 10000× 上限的非線性）。
    - **三種買入各自獨立過保真閘第 14 項**：價格由**單一求值** `buyPrice(b) = round(該路徑實測 EV ÷ 宣告 RTP)` 同時驅動按鈕文字與扣款。**標準鍛造 90×**（EV 87.06 ⇒ 96.73%）／**雙倍熔爐 138×**（每格起始 ×2·EV 132.84 ⇒ 96.26%）／**熔爐全開 587×**（31 轉＋起始 ×2＋首轉預置引信·EV 566.75 ⇒ 96.55%）
    - **⭐ 我自己寫的鎖被自己的測試打回來兩次，兩次都是老毛病的新變體**：
      · **(a) 斷言被「它要檢查的那個東西的定義處」自己餵飽**：`buy-price-single-source` 第一版「全檔 `buyPrice(b)` 恰好 1 次」當場紅在 3 次，其中一次是 `function buyPrice(b){…}` **函式自己的定義處** ⇒ 那條斷言量的根本不是 render。修法＝把量程收到 render 區間，並加**反向錨**「定義處必須落在量程之外」。
      · **(b) 一把平台軌的尺把散文判成控件，開出 correct code 的假紅**：`platform/game-axes-pace-rubric` 用 `/兌現|cash ?out/` 判「有沒有局中兌現控制」而**不剝註解** ⇒ 我在註解寫「頂階結晶兌現」就被判成 stepwise。修法＝**先剝註解再測**，**刻意不剝字串字面量**（這把尺要找的正是按鈕上那行 UI 文案，剝掉就量空了）——這是 §4 形狀⑦-(e)「連字串一起剝」**不能無腦套到每一把尺**的反例。反向見證者＝該鎖自己的「stepwise 款數 === 7」；實測全 36 支 view 中**只有 star-forge 一支**判定改變、7 款 stepwise 全部照樣命中。
    - **演出層：兩個「先把結果說出口」的坑**（其一正是 emerald-sprite 上週才踩過的）：① retrigger 的公告與 HUD 轉數上修**不能排在盤面落定之前**（`sp.planned` 已含本轉加的轉數）⇒ 抽成純函式 `fsHudSpins(sp, landed)`，鎖 `retrigger-announced-after-witness` **行為與順序兩層都驗**；② 單轉平均 **22 拍**、一段 13–31 轉 ⇒ 上千拍，改為「一個 `d` 管整段」使總時長有硬上界；`paceOf` 刻意住在**數學區**（放 render 區＝node 走不到＝寫著可驗實際驗不到）。
    - **刻意偏離 canonical（據實列）**：宣告 RTP 96.5%（媒體列 98%，沿用 spec 2026-08-06 的決定）／💣 觸發階 T8→**T6**／T9 由「成群」改為**單顆即結晶兌現**（兩顆 T9 同盤在 10^8 局尺度到不了）／三買入價由 EV 求值而非 canonical 的 100/250/500（硬寫 canonical 價＝Dead By Noon 那種暗虧）／成群門檻由固定 4 改為隨階級遞減。
    - **⚠️ 同輪順帶更正一個全站在用的讀數**：CLAUDE.md §10 記「首屏餘裕 48,907 bytes」，實測 HEAD **只有 11,281**——差的 35,362 是同日前景 #175／#181 兩輪加的 2 支 eager 檔（`core/offline.js` 6,610B、`core/route.js` 6,285B）與既有檔增長，那兩輪沒回頭更新該行。本輪已改為實測值並附重算方法（本款自己只吃 816B）。
    - **據實未驗**：保真閘第 **10b/11b（觀感層）UNVERIFIED**——`rAF` 與 CSS transition 在排程輪只在截圖那一刻推進一格（見 `intel/scheduled-visual-verification-2026-09-12.md`）⇒「合併升階的進化動畫好不好看」量不到；「單轉 22 拍會不會膩」也量不到。
    - ---

> 🤖 **以下由自我進化引擎「平台軌」自動開卡**（2026-09-15 **08:00 窗** · 來源：`intel/db/platform-modules.json` **金流** 分類全審〔7 模組，09-12＝與功能/活動並列全庫最舊〕＋ #194 落地當下量到的兩件事，皆為實測非推論）。全自動模式下標 🟦已批准待做。

196. 🟦已批准待做 **交易有三個型別，只有一個有生命週期——而那個生命週期不是型別的屬性，是縫在提款那一條路上的** — S–M（容器優先；出口在 `layout/app-shell.js`＝首屏 eager ⇒ **動工前當場重跑首屏那把尺**，不要引用任何寫下來的餘裕值）— 來源：platform-modules 台帳〈金流／交易狀態面〉2026-09-15 全審，探針 `cashflow/txn-kind-states` 實測 **型別 3／帶 states 者 0**
    - **一句話**：`TXN_KINDS`（`app-shell.js:103`）三個型別 `deposit`／`withdraw`／`p2p_out` 的欄位是 `{label, sign, tone, ic}`——**沒有任何一個欄位在說「這一筆會經過哪些狀態」**。#174 做對了提款那一條路（`row.eta` ＋ 算出來的 `txnStatus(t, now)`），但它是**寫在提款分支裡的 if**，不是型別宣告的東西 ⇒ 儲值與轉贈至今沒有任何生命週期，**而沒有 `eta` 的列一律回 `done`**（含 #174 之前的舊紀錄與會員模式的伺服器記帳）。
    - **出口形狀（容器先於內容）**：`TXN_KINDS` 每個型別可宣告 `states: [{id, label, ttlFrom}]`（未宣告＝今日行為逐位相同＝零回歸）；`txnStatus()` 改為**向型別求值**而不是問「有沒有 eta」。第一個真正的內容是把 #174 已經做出來的提款時序**搬進 `withdraw` 的宣告**（純搬移、零行為變更），儲值/轉贈**本輪不填**——容器先做出來，內容等真的有事件來源再填。
...
198. ⚬待批准 **暗影儀式的儀式條有五級，而第四、第五級在自然遊玩下走不到——因為它每一注歸零** — M（純 `views/slot.js`，但它是**首屏 eager 檔**⇒ 動工前當場重量）
    - **一句話**：`st.bar` 在每一注基礎旋轉開頭被歸零（`slot.js` 的 `spin()` 與 CORE 的 `_fresh`，兩份一致）⇒ 五級儀式實際上是「**單一注內的爬升**」，
      而不是跨注的長期進度。要在同一注累積到 Lv5 需要的點數實測機率為 **0**（250,000 注 0 次；**重平衡前也是 0**，這不是 09-15 那輪弄壞的）。
    - **為什麼這是問題而不是「高波動」**：賠付表過去逐字寫「Lv.5 進入 Cursed Spins（5×5 僅 M+H）」⇒ 對玩家而言那是一個**承諾**，
      而它只能用 96× 買入達成。（10:00 窗已把文案改成據實的「需由購買功能進入」——**那是止血，不是修好**。）
      這正是星鑄 2026-09-14 記進交接檔的那條通則：凡「階梯／等級／收集／解鎖」型機制，都要問一次「最高那幾級程式真的走得到嗎」。
      區別在於：星鑄是**建置當下**拓到的，這一款是**上線十幾個月後才被量到**。
    - **三個可走的方向（需裁決，不要預設選第一個）**：
      · **(a) 讓儀式條跨注累積**（真正的 meta 進度）——手感最好，但這是**最大的一個改動**：RTP 要全部重算（長期進度會把期望值集中到尾部），
        且得處理「進度存哪裡」（localStorage？跨站別？會員模式要不要進伺服器？）。
      · **(b) 保留單注爬升，但把上半段的門檻阿優化到真的走得到**（比照星鑄的修法：高階降低門檻）——
        10:00 窗已把階梯改成前貴後便宜（[34,44,33,27,21]），**但還不夠**；實測顯示要讓 Lv5 達到 ~1/20,000 的量級，
        門檻要降到使 **total RTP 飛到 380% 以上**（實測：`thresh=[20,30,25,20,15]` ⇒ Lv5 0.0044%、total 382.6%）——
        因為高等級本身太值錢（L→H 替換 + 池裡只剩 M+H）。⇒ (b) 必須**同時調降高等級的價值**，不是只降門檻。
      · **(c) 接受現狀，把 Cursed 明確定位成「購買專屬特色」**（很多商業 slot 真的這樣做）——成本最低，但要連儀式條的 UI 一起重設計（
        現在它畫了五格而玩家永遠只看得到前兩格在動）。
    - **不變量（落地時要同時立鎖）**：① 不論選哪條，**宣告 RTP、兩個買入自身 RTP 三者必須重跑並重新落入 ±0.5pp**（本輪建立的
      `rtp-measure` 硬斷言會直接把這件事變成紅燈）；② `ritual-ladder-reach-is-measured` 的斷言要跟著從「Lv4/Lv5 由買入」
      改成「自然也走得到」（選 (a)/(b) 時）；③ **`HL.slotEngine` 是第二個消費者**（Slots Battle 的 `fgboard.js`），池一改對戰分數分佈跟著改。
    - ⚠️ **實作輪可以直接拿走的現成工具**：`CFG.weights`（本輪新抽出）使輪帶組成可由 node 外部就地掃參，
      不必每改一次就改檔；本輪的探索器／網格搜尋／階梯量測程式均已驗證可用（寫法見本卡 commit 的 gate_log）。

199. ⚬待批准 **競技場自建房的錢完全不在帳本上——2% 平台開房費是平台收入，卻一毛錢都沒進 GGR** — S–M
（純 `views/arena.js` 接線 ➕ 可能要在 `core/ledger.js` 新增一個型別；**arena.js 是首屏 eager 檔 ⇒ 動工前當場重量**）
    — 來源：2026-09-15 前景修「真站開房押金有進無出」（`4bc8abd`）時順手量到，**刻意不在那張卡的射程內**。
    - **機械事實**：`grep -c "HL.ledger" prototype/src/views/arena.js` ⇒ **0**。而同一支檔在**三處**動玩家餘額：
      · `createBounty`：`balance - (deposit + openFee)`（開房）
      · `endMyRoom`：`balance + prizePool`（賞金房結束）與 `balance + net`（對押房結束）
    - **為什麼這是真的缺口而不是潔癖**：CLAUDE.md §4 逐字寫著「**任何新送幣/新金流務必在授予當下 `HL.ledger.record(...)`**」，
      而帳本的插樁點名單（`liveStats.record`／`HL.bonus.add`／faucet／簽到／rakeback／JP／`pushDemoTxn`／jackpot onBet）**沒有自建房**。
      ⇒ `HL.opsBoard` 的 GGR/NGR/RTP 看不到這一整個金流表面，而儀表板畫面一切正常——同 #182 的發現方式
      （「一個零漂移的讀數，量的是隔壁那支檔」）。
    - **型別怎麼對應是這張卡真正要想的事**（不要隨便挪用現有型別）：
      · **開房費 2%** 是不歸還的平台收入（rake）——現有型別裡**沒有 rake**；記成 `bet` 會把它算進流水而扭曲 RTP。
      · **押金**是 escrow（出去會回來），不是損益；記成 `bet`/`win` 會把每一間房的進出都當成流水。
      ⇒ 新增型別會動到 `derived()` 的 GGR/NGR/RTP 數學與儀表板的規則健檢，**這就是它該自己一張卡的理由**；
      半做（隨便挑一個型別先接上）比不做糟——那會讓儀表板從「沒算進來」變成「算錯了」。
    - **驗收**：(a) 三個餘額變動點各自在**授予當下**記帳（不得在領取端記＝重複計）；
      (b) 開一間房再讓它到期，`HL.ledger.derived()` 的淨現金流變動恰等於 **−openFee**（零和外加 rake）；
      (c) 假站的模擬挑戰（`simBounty`／`simVsslot`）**不得**進帳本——那是假玩家的錢，進了就是把假活動算成營運收入；
      (d) 常駐鎖：`views/arena.js` 的每一個 `HL.state.set({ balance` 周圍必須有對應的 `HL.ledger.record`（反向普查，同 #180 的形制）。

> 🤖 **以下由自我進化引擎「平台軌」自動開卡**（2026-09-15 **20:00 窗** · 來源：`intel/db/platform-modules.json` **功能** 分類全審〔18 模組，09-12＝全庫最舊〕＋ #185 實作輪當場量到的一件事）。全自動模式下標 🟦已批准待做。

200. 🟦已批准待做 **通知有四種身分，而它只活到「進門那一瞬間」——存下來的通知不帶身分，於是分類軸與偏好都沒有東西可以掛** — S（`core/notify.js` 是**首屏 eager 檔**⇒ 動工前跑 `node intel/tools/ledger-probe.js --category 擴充性` 當場重量；本卡淨增約 10 bytes）— 來源：platform-modules 台帳〈功能／通知中心：分類軸與玩家偏好〉2026-09-15 全審
    - **一句話**：`core/notify.js` 的 `add()` 先 `var k = kindOf(n)` 拿去過 #178 的自我排除閘，接著 `o.list.unshift({ id, ic, title, text, t, read })`——**`k` 沒有被寫進去**。
      型別求值了、用過了、然後丟掉；存下來的那一則通知**沒有身分**。
    - **為什麼這是缺口而不是潔癖**：① 通知中心無法依類分群/過濾，玩家也沒有逐類開關（那正是本模組 partial 的理由，#128 的上層卡）；
      ② `HL.notify.kindOf` **已經匯出**（`:103`），而對**任何一則存下來的通知**求值都會回 `comms`（欄位不在 ⇒ fail-closed 退化）——
      今天沒有人踩到，只因為全庫**零個外部消費者**。⇒ 這是一個「現在是對的、第一個使用者出現時才會錯」的出口，正是 CLAUDE.md §4「修一半而看不出來」的形狀。
    - **出口形狀（容器先於內容，本卡刻意只做容器）**：`add()` 把 `kind: k` 寫進存下來的物件。**今天零行為變更**——沒有任何表面讀它。
      分群 UI、逐類偏好、抑制策略的回溯適用，全部留給 #128；本卡只負責讓那些東西**有東西可掛**。
    - **不變量（落地時要同時立鎖）**：① 存下來的每一則通知都帶 `kind`，且其值 ∈ `NKIND`（沒自陳的一律落 `comms`＝與閘同一份 `kindOf`，**不得在寫入處另寫一份分類**）；
      ② **入口閘的行為逐位不變**——`passes(k)` 仍在 `unshift` 之前、`rg` 仍硬性放行（CLAUDE.md §4 那條「錯了就無法補救的一格」）；
      ③ **舊資料相容**：`load()` 讀到沒有 `kind` 的既有通知不得崩、不得被過濾掉（玩家的歷史通知不能因為升級而消失）；
      ④ 反向錨——`kindOf` 對「存下來的通知」與「進門的那一則」必須給出**同一個答案**（現況實測**不成立**，那就是本卡要修的東西）。
    - ⚠️ **前置事實（別讓實作輪重新發現）**：`blocked` 計數（被抑制的則數）目前**只有總數、沒有分類**；本卡不擴充它，但寫 `kind` 之後它就有條件變成逐類計數 ⇒ 留給 #128。

201. ✅完成（2026-09-16 平台軌·**08:00 窗**·commit `058f962`·新鎖 1·**負向擾動 15/15 CAUGHT**·**並在真檔案系統上把那次事故重演了一遍**·零首屏位元組） **每一輪都手刻一次負向擾動 harness，而它沒有保險：被中斷就把變異留在 repo 裡，下一輪的「基線」是紅的，而每一條擾動照樣印 CAUGHT** — S（純 `intel/tools/`＝**零首屏位元組、零 `prototype/src`**）— 來源：**#185 實作輪當場踩到（2026-09-15 20:00 窗），不是推論**
    - **發生了什麼**：第一批擾動跑到 P11 時被外層 timeout 砍掉 ⇒ 它的「還原」那一步沒跑到，**P12 的變異留在 `prototype/tests/checks-platform.js` 裡**。
      第二批 harness 啟動時把那個被污染的檔讀成 `orig`，**基線本來就是紅的**；而它的判定是「跑完之後本鎖有沒有紅」⇒ **七條擾動全部印出 `CAUGHT`，七條全部不可信**，輸出看起來完全正常。
    - **為什麼值得一張卡**：負向擾動是本專案**證明鎖不是空綠**的唯一手段（CLAUDE.md §10.1 第 2 條），而它本身是每輪手刻的一次性腳本。
      一個「會說謊的驗證工具」比沒有工具更糟——它讓一輪的結論、卡片、journal、交接檔全部建立在假讀數上。本輪是**工具自己**成為了 §4 形狀的受害者。
    - **出口形狀**：`intel/tools/perturb.js`——收一份 `{目標檔, [ {名稱, mutate(orig)->text} … ], 判定用的鎖 id }`，並強制三件事：
      ① **開跑前先量基線**，非全綠就中止（連跑都不跑）；② **每一條擾動前後都把目標檔還原並逐位比對**，比對不過就中止；
      ③ 結束時印 `restored identical=true/false`，**false 就是紅**。另加 `--case` 只跑一條（本輪被 timeout 砍就是因為一次跑十四條）。
    - **不變量（落地時要同時立鎖）**：① 拿一條**已知會被抓到**的擾動與一條**已知不會**的擾動當 fixture，工具必須分別回 CAUGHT／MISSED（否則它可能恆真或恆假）；
      ② **把基線弄髒**（合成輸入：先寫入一個會讓該鎖轉紅的變異）⇒ 工具必須**中止並說出原因**，而不是照跑（這條是本輪事故的直接回歸測）；
      ③ 中途拋例外時目標檔仍必須還原（`finally`，比照 `code-mask-single-ruler` 的哨兵還原那條）。
    - ⚠️ **刻意不做**：不把它接進 `prototype/tests/run.js`——擾動是**寫鎖當下**的工具，不是每次自我檢測都要跑的東西（跑一輪擾動＝跑 N 次完整測試套組）。
    - **落地（2026-09-16 · `intel/tools/perturb.js`）**：卡面 ①②③ 全數兌現，另加兩條落地時才發現必要的：
      · ⭐ **④ 崩潰殘留哨兵 `.perturb-state.json`（.gitignore）**——**卡面那三條加起來仍然接不住本次事故**：
        ③ 的 `finally` 只在「拋例外」時有效，而**外層 timeout 是把整個行程砍掉，`finally` 根本不會跑**。
        ⇒ 動第一個位元組之前先把原檔寫進哨兵；下一次啟動若哨兵還在且檔案對不上，**先還原、大聲說出來**，再量基線。
        **這張卡自己差一點就成為「修一半」的一員**（CLAUDE.md §4）。
      · **⑤ 空轉擾動當場判 `NO-OP`**——來源是 09-15 16:00 窗 money-wheel 的 P6：擾動改了位元組但**沒改到行為**，
        當時被誤讀成「漏鎖」。靜默記成 MISSED 的後果是**有人會去放寬一條其實沒問題的鎖**。
      · `--case` 擴成三種寫法（`3` ／ `1-5,9` ／ 名稱片段），因為事故當天正是「一次跑十四條 × 每條一輪完整套組」撞上 timeout。
      · 順手拆掉一份**第二真相**：`ok` 原本並上 `missed.length === 0 && noop.length === 0`，而那與 `every(verdict==='CAUGHT')` 是同一件事
        （擾動 P9 第一版因此是**壞擾動而非漏鎖**——它改了位元組卻改不到行為，正是 ⑤ 要治的那個形狀，這次由我自己踩到）。
    - **新鎖 `platform/perturb-harness-has-a-fuse`**（node **410 → 411** 全綠·2.9ms）：用**注入的假驗證器 + 記憶體 io** 打本工具的控制流
      （擾動一輪＝跑 N 次完整套組，那不該發生在每次自我檢測裡）。(A) 雙向 fixture／(B) 髒基線**連 `mutate` 都不得被呼叫到**（數呼叫次數，不看旗標）／
      (C) 例外時**當場**還原（攔 io 寫入序，因為「跑完是乾淨的」被收尾那次還原擦掉了、證明不了任何事）／(D) 崩潰殘留**先還原再量基線**（順序就是這條的全部）／
      (E) NO-OP／(F) 還原失敗視同紅＋印那一行＋**哨兵要留著**／(I) `--case` 真的篩得動。
      **(G)(H) 是反 fail-open 的錨**（§4 形狀⑦(i)）：上面全部走注入的 `verify`，只證明控制流對、**沒證明生產路徑也是 fail-closed**
      ⇒ (G) 餵一份解析不出任何一項的 stdout，`defaultVerify` 必須**拋錯**（否則那個 `fail:0` 會讓每一條擾動都印 MISSED）；
      (H) 把 `parseRun` 認得的形狀**釘回 `run.js` 真的在印的那一行**。
    - 🧪 **負向擾動 15/15 CAUGHT**（用它自己跑，分四批 `--case`）。三條擾動第一版是**壞擾動**（錨點字面量寫錯／改了位元組沒改到行為），
      **而工具當場把它們報成 `ERROR`／`MISSED` 而不是 CAUGHT** ⇒ ⑤ 這條在落地當天就付了一次費用。
    - 🌐 **在真檔案系統上把事故重演了一遍**（不是只在記憶體裡）：寫哨兵 → 把 `perturb.js` 留在變異態（＝被 SIGKILL）→ 啟動新一輪
      ⇒ 自癒 1 檔、基線回到全綠、擾動照常得出 CAUGHT、目標檔**逐位還原**、哨兵清乾淨。

> 🤖 **以下由自我進化引擎「平台軌」自動開卡**（2026-09-16 **08:00 窗** · 來源：`intel/db/platform-modules.json` **活動** 分類全審〔9 模組，`last_audited` 09-12＝現存最舊〕本輪**新開取材維度 23「活動的到達面」**首輪執行·機械實測非推論）。全自動模式下標 🟦已批准待做。

202. 🟦已批准待做 **十四個活動都算得出「還剩多久結束」，而那個數字從來沒有離開過活動面板——玩家不打開它，就永遠不知道有活動** — S–M（`core/promo-cal.js` 是**首屏 eager 檔** ⇒ 動工前跑 `node intel/tools/ledger-probe.js --category 擴充性` **當場重量**，別抄任何文件裡的數字）— 來源：platform-modules 台帳〈活動／促銷框架〉2026-09-16 全審（維度 23 首輪）
    - **機械事實（全可複跑）**：`HL.promoCal` 是全庫**唯一**算得出每個活動 `startsIn`／`endsIn` 的地方（`evalSpec` 逐 spec 求相位 `live｜upcoming｜always｜ended`；**7 個內建 spec ＋ 7 支外部註冊檔＝14 則**），而 `grep -c "HL.notify" prototype/src/core/promo-cal.js` ＝ **0**。
    - ⭐ **那個語意在程式裡出現過，但只被拿去排序**：`list()` 的 `(a.endsIn) - (b.endsIn)` 旁邊逐字寫著 `// 快結束的先提醒`——**它做的事是排序，不是提醒**。玩家要先打開日曆，才看得到「快結束」這件事。
    - **逐筆讀完活動側全部 12 筆 `HL.notify.add`**（season ×2／raffle ×2／tournament ×2／challenges ×3／rakeboost ×1／referral ×1／onboarding ×1）：**12/12 都是「已經發生的事」**（開獎了／結算了／你達成了／你領到了／你輸入了邀請碼）。`開始了`／`快結束了`／`你還沒加入`**一筆都沒有**。
    - ⚠️ **與 `optIn`（#52）疊起來才看得出嚴重性**：需要主動加入的活動，加入鈕**只存在於活動日曆面板**（`promo-cal.js:216-217`）⇒ **玩家不開日曆就永遠不會加入，而沒有任何東西會讓他去開日曆**。活動於是對他等於不存在——**而畫面正常、測項全綠、帳本也正常**（CLAUDE.md §4「修一半而看不出來」的活動版）。
    - **出口形狀（容器優先，別在每個活動各寫一遍）**：`promoCal` 的 spec 增選用欄位 `remind: { at: ["start", "endingIn"], leadMs: 86400000 }`（**未宣告者行為完全不變**＝比照 `optIn`／`audience` 的零回歸做法），由**日曆一處**在 `evalSpec` 求相位時派送到 `HL.notify`；每個活動每期每種提醒**至多一次**（冪等鍵存站別命名空間 `HL.site.ns()`）。
    - **依賴**：**#200**（通知身分沒被存下來）。本卡派出去的提醒天生屬於 `kind:"promo"` 這一類，而現在的通知**存下來就不帶身分** ⇒ 先做 #200，這一批提醒才掛得住分類軸與玩家偏好；否則玩家關不掉它，而「關不掉的提醒」比沒有提醒更糟。
    - **與既有卡的分工（去重紀律）**：#186 管**賽後那一次公示**、#176 管**條款**、#136／#149／#157 管**遊戲內掛件**（門開在玩家正在看的畫面上）；本卡管**賽前與賽中有沒有人來敲門**，是**平台層**的那一個出口。四者可獨立成立或獨立失敗。
    - **誠實邊界**：只做**站內到達**（通知中心）。推播／email 需後端與同意管理，屬 `CONTROL.avoid`，不開真接入。
    - **不變量（落地時要同時立鎖）**：① 未宣告 `remind` 的 spec **逐欄零回歸**；② 同一期同一種提醒**不得送第二次**（用合成時鐘推過整個窗，數送出筆數）；③ **真站不得比假站更吵**（比照 §4 假活動閘的紀律）；④ ⭐ **活見證者**——把某個 spec 的 `endAt` 往前挪，那一則提醒**必須跟著提前送出**（否則提醒時刻是另外編的一套，而畫面完全正常＝本卡診斷出的那個形狀又長回來）。

## 分析師日誌（最新 3 則；歷史見 [BACKLOG-archive.md](BACKLOG-archive.md)）
- **2026-09-16（平台軌 · **08:00 窗** · **#201 當輪落地＝證明鎖不空綠的那個工具，自己終於有了保險** ＋ 台帳輪替審「**活動**」9 模組全審 ＋ **新開取材維度 23「活動的到達面」** ＋ 開卡 **#202** · claim `p-081051-c70a` · commit `058f962`）
    - **① 閘門/進場**：三開關皆 true；`build_lock: false`（維護軌 09-16 00:00 窗 00:50 釋放）→ claim `p-081051-c70a`（commit `a8b0255` **當下即提交**）→ 停頓做非寫入讀取後重讀確認 token 仍在。`last_platform_run_at` 距今 **10.6h** < 24h＝**非 catchup**；`lead_track: games` 容許讓路，但本輪有真工作故不讓路。船長「待處理」區**需裁決 0 項、無指派給本軌的指令**。`db/platforms.json` **37 筆追蹤中、逾期 0**（最早 rainbet 09-18）⇒ **本輪不做平台深挖，據實記，該檔一位元組未動**。`ledger-card-sweep` **雙向 0 筆告警**（上一輪修好的動詞表首次在乾淨狀態下複跑）。
    - **② #201 落地**：`intel/tools/perturb.js`。卡面三條（先量基線／逐位還原比對／印 `restored identical=`）全數兌現。
    - **⭐ ③ 落地當下發現卡面那三條接不住它自己要治的那次事故**：`finally` 只在**拋例外**時有效，而**外層 timeout 是把整個行程砍掉**——`finally` 根本不會跑。⇒ 加第 ④ 條：**崩潰殘留哨兵**（動第一個位元組前先寫，下一次啟動先還原、大聲說出來，再量基線）。**這張卡自己差一點就成為「修一半」的一員**。
    - **⭐ ④ 第 ⑤ 條在落地當天就付了一次費用**：`NO-OP`（擾動改了位元組但沒改到行為）不是理論——本輪 15 條擾動裡，**三條的第一版是壞擾動**（兩條錨點字面量寫錯、一條 P9 拆掉的是一段**冗餘**條件），而工具當場把它們報成 `ERROR`／`MISSED` 而不是 CAUGHT。若沒有這一條，P9 會被讀成「漏鎖」，然後**有人會去放寬一條其實沒問題的鎖**。順手把那段冗餘拆掉（`ok` 不再並上 `missed.length===0 && noop.length===0`——那是 `every(CAUGHT)` 的第二份真相）。
    - **⑤ 新鎖 `platform/perturb-harness-has-a-fuse`**（node **410 → 411** 全綠·2.9ms）：注入假驗證器 + 記憶體 io 打控制流。(B) 刻意**數 `mutate` 的呼叫次數**而不是看 `aborted` 旗標（旗標可以最後才補上）；(C) 刻意**攔 io 的寫入序**，因為「跑完之後檔是乾淨的」會被收尾那次還原擦掉、**證明不了任何事**；(F) 的 fixture 刻意讓變異與原值**等長**，好讓「逐位比對被偷換成長度比」照樣紅。**(G)(H) 是反 fail-open 的錨**：前面全部走注入的 `verify`，只證明控制流對、沒證明生產路徑也是 fail-closed。
    - **🧪 ⑥ 負向擾動 15/15 CAUGHT**（用它自己跑，`--case` 分四批——事故當天正是一次跑十四條才撞上 timeout）。**並在真檔案系統上把事故重演一遍**：寫哨兵 → 把 `perturb.js` 留在變異態 → 啟動新一輪 ⇒ 自癒 1 檔、基線回到全綠、擾動照常 CAUGHT、目標檔**逐位還原**、哨兵清乾淨。
    - **⑦ 台帳輪替「活動」9 模組全審 · 九格狀態全維持，但缺口是新的**：本輪不問「活動長什麼樣」，改問**新開的維度 23「玩家怎麼知道它開始了」**。⭐ `HL.promoCal` 是全庫**唯一**算得出 `startsIn`／`endsIn` 的地方（7 內建 spec ＋ 7 外部註冊檔＝14 則），而它的 `HL.notify` 命中 **0**；那個語意在程式裡只出現過一次，是排序旁邊逐字寫的 `// 快結束的先提醒`——**它做的事是排序**。逐筆讀完活動側 **12 筆**通知：**12/12 都是「已經發生的事」**。與 `optIn` 疊起來 ⇒ **玩家不開日曆就永遠不會加入，而沒有任何東西會讓他去開日曆**。⇒ **開卡 #202**（依賴 #200）。
    - **⭐ ⑧ 為什麼這一格能藏這麼久**：維度 17 問**加入之前**（條款）、維度 10 問**結束之後**（收尾）——**正中間那一段沒有任何維度在看**，而那正是活動真正在跑的那一段。⇒ 記一條可重複使用的自問：**我這一輪新算出來的那個量，是真的被送到玩家面前了，還是只是被拿去排序？**（已寫進 `sourcing-methods.md` 維度 23 的「紀律」段）
    - **⑨ 驗證與收尾**：`node prototype/tests/run.js` **410 → 411 全綠**。本輪 `prototype/` **只動 `tests/`** ⇒ `sw.js` **不 bump**（v319，同 08-30／09-12／09-15／09-16 先例）；**首屏逐位未動**（`intel/tools/` 與 `tests/` 都不在 `index.html` 上）。`.gitignore` 加一行（哨兵不進版控）。
    - **下輪（09-16 14:00）建議**：台帳輪替到 **資安**（9 模組·`last_audited` 09-13＝屆時最舊）；實作首選 **#200**（僅約 10 bytes eager，且它是 **#202 的前置**——先做完 #200，#202 派出去的提醒才掛得住分類軸與玩家偏好），動工前務必 `node intel/tools/ledger-probe.js --category 擴充性` **當場重量**。
- **2026-09-15（平台軌 · **20:00 窗** · **#185 當輪落地＝「程式碼 vs 註解/字串/正則」的判準收斂成一把尺，而清點數是卡上寫的五倍** ＋ 台帳輪替審「**功能**」18 模組全審〔一格改判 `partial→present`、一格的機械事實**反了**〕 ＋ **修好 `ledger-card-sweep` 的動詞表**〔它看不見「認領」，而本輪就是靠它才發現台帳高報了一個十天前已關閉的缺口〕 ＋ 開卡 **#200／#201** · claim `p-201035-5f88`·心跳 20:10→20:26→21:18→收尾·**全部讀自系統時鐘 `date -Is`**·進場鎖乾淨 false〔遊戲軌 16:00 窗 17:16 釋放〕·**未奪鎖**·dark 5.7h<24h 非 catchup·`lead_track=games` 容許讓路但本輪有真工作故不讓路·node 407→**408 全綠**·**負向擾動 16/16 CAUGHT**·`prototype/src` **一位元組未碰**·**首屏逐位未動**·`sw.js` **不 bump**（v317·`grep -c "tests/" prototype/sw.js` ＝ 0）
    - **① 閘門/進場**：三開關皆 true；`build_lock: false` → claim `p-201035-5f88`（commit `ee43387` **當下即提交**）→ 停頓做非寫入讀取後重讀確認 token 仍在。船長「待處理」區**無指派給本軌的新指令**（P3／P6 為舊件，P6 本軌已三度建議關閉）。
    - **② 本輪未做平台深挖（據實）**：`db/platforms.json` **36 筆追蹤中、逾期 0**（最早 rainbet 09-18、其次 bet365／jackpotter／rollbit 09-20）。為了有事做而重掃媒體＝引擎明文禁止的空心跳，該檔一位元組未動。
    - **③ #185 落地**：卡寫「本檔 5 份＋`checks-games.js` 4 份」，**實測 26 份**——`checks-platform.js` 具名 5（其中 `stripComments` **同層同名宣告兩次 ⇒ 前者從一開始就是死碼**）＋行內 10 ＋**兩個自帶引號狀態機的括號配對器**（`fnBody`／`extractRegisters`）；`checks-games.js` 具名 5 ＋行內 21。全數收斂到 `registry-probe.js` 的 `nonCodeMask`，CSS 剝註解為唯一豁免（逐行釘住、每檔最多一份）。
    - **⭐ ④ 零回歸是「逐檔比對」證出來的，不是「跑一下沒紅」**：128 支 src 檔中 **9 支**輸出不同，**九支全部同一個方向＝舊尺把真程式碼丟掉**：`core/reports.js` **+18,546（該檔 62.9% 失明）**／`chicken.js` +10,441／`i18n/en.js` +6,536／`core/betlog.js` +4,722（54.0%）／`offline.js` +1,584／`config.js` +278／`progress-src.js` +8／`games-loader.js` +8／`edge.js` +3。相反方向**零筆** ⇒ 只會讓斷言變嚴。兩個入口不同：①正則移除註解會吃掉**字串裡的 `//`**（`u.indexOf("//")`、`"https://…"`）②字元狀態機不認正則 ⇒ `/[",\n]/` 之後整份錯位。
    - **⑤ 口徑刻意分兩種，而這是實測逼出來的**：`checks-games.js` 有多條**距離型**斷言（`/A[\s\S]{0,120}B/`），把註解換成等長空白會讓距離變長 ⇒ `games/bounty/member-mine-board-inert`／`games/dice-duel/staged-reveal` 當場轉紅。⇒ 該檔沿用原密度。**尺只有一把（同一份 `nonCodeMask`），刻度有兩種。**
    - **🚨 ⭐ ⑥ 本輪最值得記的一件事出在擾動工具本身——一種新的空綠，而且它偽裝成 CAUGHT**：第一批擾動被外層 timeout 砍掉、**還原那一步沒跑到** ⇒ P12 的變異留在 `checks-platform.js` 裡；第二批 harness 把那個被污染的檔讀成 `orig`，**基線本來就是紅的**，而判定只問「本鎖有沒有紅」⇒ **七條全部印 CAUGHT、七條全部不可信**，輸出完全正常。修法＝harness 先量基線、非綠即中止＋復原後逐位比對並印 `restored identical=true`；重跑後 7/7 才是真的。⇒ 已開成 **#201**（把擾動 harness 變成有保險的常駐工具，別再每輪手刻）。
    - **⑦ 台帳輪替「功能」18 模組全審 · 一格改判、一格的機械事實反了**：
      · **〈PWA/離線〉partial → present**：舊 evidence 逐字寫「`PRECACHE` 仍恰 4 筆、可執行資產 0 筆 ⇒ 離線拿得到殼、拿不到任何程式碼」，而 **#175 已於 09-14 落地**（`sw.js` install 當下解析 `index.html`＋逐筆容錯快取 **94 支 script ＋ 3 支 css**，`SHELL_SEED` 那 4 筆**已經不是檔案清單、是解析的起點**）。**台帳高報了一個自己十天前已關閉的缺口。**
      · **〈通知中心：分類軸與玩家偏好〉維持 partial，但機械事實反了**：舊寫「`kind`/`category`/`channel` 在 `notify.js` **0 命中**」，實測 **分類軸已經有了**（#178 第三波的 `NKIND` 四種身分＋`kindOf`＋`passes`）。⭐ **新量到的才是重點**：`add()` 求值 `k = kindOf(n)` 過閘後，`unshift` 的物件**沒有寫 `kind`** ⇒ 型別只活在進門那一瞬間；`HL.notify.kindOf` 雖已匯出，對**存下來的**通知一律回 `comms`——今天沒人踩到只因零個外部消費者 ⇒ 開卡 **#200**（容器優先）。
      · 其餘十六格狀態全部維持，但**兩筆連八輪的定值離開了，而且都是真的長大**：`HL.games.register(` **24/21 → 27/24 支檔**（三款新 slot）、`lazy-games` MANIFEST **20 → 26**；`HL.support.register(` **11/10 → 12/11**（`core/offline.js` 隨 #175 上架）；`views/` 命中 `HL.fair` **34/25 → 37/28**（比例未退步）；`notify.js` **74 → 104 行**。零漂移的仍有：`FANOUT_ROSTER` 20、`addWager` 2 呼叫點、`guild.js` 295 行、`HL.auth.` 73/20、`shop` 外部註冊者 0（連九輪）、成就牆外部註冊者 4。
    - **⭐ ⑧ 修好一把尺，而它漏掉的正好是本輪唯一的真缺口**：`ledger-card-sweep` 的引用偵測是**列舉動詞**（開卡／掛在／併入／底下追蹤…），evidence 寫「已由 **#175** 認領」時它**一筆也看不見**——全庫實測 **12 筆**這種寫法。補上「認領／認養／接手／承接／負責」後當場冒出 3 筆告警，逐筆讀完**2 筆是誤報**（「依 #82 先例（同一筆**未認領**記載三次即應升格）」被讀成「#82 認領」）⇒ 再加否定形排除，最終 **1 筆真告警＝〈PWA/離線〉#175**，回填後雙向 0。**列舉式的判準既會漏也會多，每補一格都要記下理由**（§4 形狀⑦-(h) 第三次重演）。
    - **⑨ 驗證與收尾**：`node prototype/tests/run.js` **407 → 408 全綠**；`--deep --group games` 另跑一次全過。本輪 `prototype/` 只動 `tests/`（`grep -c "tests/" prototype/sw.js` ＝ 0 ⇒ 沿 #135 判準 **不 bump**，停在 v317），**首屏逐位未動**（進場實測餘裕 **1,227 bytes**，`node intel/tools/ledger-probe.js --category 擴充性`）。零 UI 表面、零 `src/` 改動 ⇒ 無線上複驗需求。收尾清 `build_lock=false`。
    - **下輪（09-16 08:00）建議**：台帳輪替到 **活動**（9 模組·09-12＝現存最舊，且本輪修好的動詞表在那一格有兩筆 `#82` 引用要順手複核）；實作首選 **#201**（零首屏、純 `intel/tools/`、本輪剛親身踩過它要治的事故、配方現成），次選 **#200**（僅約 10 bytes eager，但動工前仍請當場重量——餘裕只剩 1,227）。
- **2026-09-15（平台軌 · **08:00 窗** · **#194 當輪落地＝把「可求值的台帳讀數」從 2 個分類擴到全部 8 個** ＋ 台帳輪替審「**金流**」7 模組全審 ＋ **三筆手抄讀數當場證明過期（其中一筆是昨天才寫下的）** ＋ 開卡 **#196／#197** · claim `p-081019-d3c7`·心跳 08:10→08:28→08:34→收尾·進場鎖乾淨 false〔遊戲軌 22:00 窗 00:18 釋放〕·**未奪鎖**·dark 11.0h<24h 非 catchup·`lead_track: games` 但本輪有前手明文交棒的實作卡故不讓路·node **400 全綠**·**負向擾動 14/14**·`prototype/src` **一位元組未碰**·`sw` 不 bump〔測項與 intel/ 都不出貨〕）**
    - **① #194 落地**：`intel/tools/ledger-probe.js` 探針 **22 → 46 條**、分類 **2 → 8**＝台帳輪替走的**全部八個分類**。新增 24 條（金流 9／功能 3／活動 3／資安 3／資料 3／擴充性 3），每一條都自陳量程、帶**活見證者**（`--witness` 46/46 逐條翻值、零條靠「把探針弄壞」過關）。鎖 `platform/ledger-probe-fail-closed` 的錨同步上調（探針 ≥15→**≥40**、分類 ≥2→**≥8**），並新增一條**逐名分類**的反向錨：`LEDGER_CATS` 八個名字**每一個都必須有尺**。
    - **⭐ ② 為什麼要逐名，而不是只把數字調高**：只調 ≥40／≥8 的話，**刪掉〈資安〉全部探針、在別的分類補幾條**就能把兩個數字都補回來，而那一格會靜默退回「evidence 裡手抄的數字」。負向擾動 **P1** 就是打這一點——把〈金流〉整組改名成〈金流X〉：**探針數 46 不變、分類數 8 不變**，只有逐名那條會紅。
    - **⭐ ③ 本輪最貴的一筆：〈交易狀態面〉昨天（09-14 20:00 窗）才寫下的讀數，今天就已經過期。** 原文「`HL.sla.valueOf` 從 1 個呼叫點變成 2 個」，實測 **3**（`app-shell.js` 的 `etaSuffix()`／`pushDemoTxn()`／提款送出各一）。它**被寫下時是對的** ⇒ 這不是誰粗心，是那個數字沒有任何東西在對它——正是 #194 的立卡理由。另兩筆同型：〈提款審核佇列〉的「raw 4 筆」實測 **8**（判定不受影響，八筆全是註解/字串 ⇒ effective 恆 0，**absent 的正確狀態是 effective 不動、不是 raw 不動**）；〈錢包/帳本〉的「五處消費 `HL.mock.currencies`」實測 **8**（#183 開卡後三天又多了 3 處）。
    - **④ 台帳輪替「金流」7 模組全審·七格狀態全部維持**（收銀台 partial／錢包帳本 present／提款審核佇列 absent〔續 `CONTROL.avoid`〕／P2P partial／紅利流水 partial／資金分倉 absent／交易狀態面 partial）。**本輪起這七格的每一句數字敘述都對應到一條探針**（9 條，逐格全覆蓋），下一輪可直接 `node intel/tools/ledger-probe.js --category 金流` 重跑而不是再抄一次。
    - **⑤ 據實移除一個會誤導人的數字**：〈資金分倉〉evidence 引用的「首屏收尾餘裕 289B」已被 09-13／09-14 連續改寫兩代（#189／#190 −48.9KB、前景 #175／#181 +2 支 eager 檔）⇒ 改成**條件表述**（動工前當場重跑那把尺），並把「首屏餘裕本身沒有出口、只能手抄」開成 **#197**。實測本輪餘裕 **10,465 bytes／94 支 script**（用的就是 `platform/first-screen-budget` 自己的 `firstScreenMeasure`，放進 vm 取用＝零複製）。
    - **⑥ 順帶釘正一個「兩個數都對」的口徑差**：`index.html` 有 **95** 個 `<script src=`，而首屏那把尺數 **94**——差的是 `cdn.jsdelivr.net` 的 supabase（外部 CDN、不吃我們的位元組）。CLAUDE.md §10 的 95 與本輪的 94 **不是漂移**，是兩種口徑；已寫進 #197 讓實作輪不必再困惑一次。
    - **⑦ 本輪未做平台深挖（據實）**：`db/platforms.json` **37 筆逾期 0**（最早 rainbet 09-18）。仍照 SKILL 第 1 步取材一次（WebSearch ×1 多訊號刷新），結果**零筆入庫且是刻意的**——Rainbet 仍是流量冠軍（已在庫 tier-1）、BetPanda／Roobet／1xBit／Cybet 全在庫、榜單型新名字（Coins.Game／Pickwin／Bet25）無獨立訊號；且清單 37 筆已超過「精煉 ≤ 約 32」的指引、09-12 本軌自己寫過「下一輪應先汰除而非再加」。結論寫進 `_sourcing_negative_log`（第 7 筆），`platforms[]` 一位元組未碰。
    - **⑧ 引擎健檢**：三軌 `last_*_run_at` platform 09-14T21:09（11.0h）／games 09-15T00:1x／maintain 09-14T13:31＝**無一落後 >24h**；`build_lock` 進場乾淨 false、本輪 claim 帶心跳且**三筆心跳全部讀自系統時鐘**（`date -Is`，回應維護軌 E15 與遊戲軌 09-14 ⑥ 的提報）；`ledger-card-sweep` 正向/反向告警**皆 0 筆**。
