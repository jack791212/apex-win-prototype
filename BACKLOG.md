...
- **2026-09-08（平台軌 · **20:00 窗** · 台帳輪替審「**前端UI/UX**」9→**10** 模組（新增一格）＋**新開取材維度 21**＋開卡 **#181**＋當輪落地承諾側修法與**一條新鎖** · claim `p-201200-c7e2`·心跳 20:12→21:5x·進場鎖乾淨 false·**未奪鎖**）**
    ...
    - **③ 取材（到期 0 筆·主題式·維度 21 首輪）**：最近到期為 **09-09**（leovegas／rainbet）⇒ **未提前取湊數**、`platforms.json` 一位元組未動。**一手直取 1 筆**（Track360 深連結詞條）＋**二手 1 筆**（PWA 返回鍵·w3c/manifest #693），已逐段標明。
    - **④ ⭐ 本輪的發現（台帳盲點第 16 例）**：`HL.router.go` 只做 `HL.state.set(patch)` + `renderApp()`，**完全不碰 URL** ⇒ `pushState`／`replaceState`／`location.hash`／`popstate`／`hashchange` **全庫命中 0**，而去處有 **~36 個**（`router.go` 34＋`goGame` 1／VIEWS 12／`games.register` 24）＝**36 個去處、0 個地址**。URL 只當**入站**用；唯一 URL 建構子 `origin+pathname` 有 **3 個消費者**、組出同一字串＝**大廳大門**。⇒ **6 顆分享鈕**的訊息**點名了遊戲**而連結回不到那裡；`display: standalone` ＋ history 零條目 ⇒ **裝成 App 後第一次按返回就離開**；`views/arena.js` 建房表單更逐字賣過「僅分享連結可加入」。
    - **⑤ ⭐ 為什麼所有既有測項都是綠的（§4 家族新實例）**：沒有任何一條鎖問過「URL」這個字——承諾住在**字串**裡、能力住在**不存在的程式碼**裡，兩邊都不在任何斷言的射程內。更難堪的是：**同一張表單的隔壁三列（Shared／Team／Sponsored）早就被前手輪次逐一改成「示意」並寫下理由**，**修隔壁那一列的那一輪沒有回頭看這一列**。
    - **⑥ 落地**：兩句話改成只講實際發生的事＋**i18n 三語同步**（原本一條都不在字典裡）⇒ ⭐ **首屏淨還回 12 bytes（157→169）**，連續兩輪「動了 eager 檔而餘裕變大」。新鎖 **`platform/url-as-address-census`**（住 `tests/`＝零首屏位元組）：量程錨（**逐寫法 8 條合成探針**＋大門建構子集合逐一相等）／主不變量＋**反向錨**（長出路由就主動轉紅逼回填）／承諾閘（白名單只收「邀請連結」且必須有命中）／大門型分享出口棘輪釘定 **6**／三句誠實替代句三語齊備。`node` 354→**355 全綠**；`sw` v278→**v279**。
    - **⑦ ⭐ 負向擾動兩批，兩條 MISSED 都是「我的鎖只有部分射程」**：首批 **13/15**。(P11) 量程錨只餵**一段混合探針**、只斷言 `write > 0` ⇒ 砍掉 `pushState` 那一支後 `location.hash =` 照樣命中、**正向全綠而 pushState 隱形**＝**一條合起來的正向對照，只證明了它其中一支還活著**（改為逐寫法 8 條探針）。(P15) 只改一支語言包**不被任何既有鎖抓到**——i18n 五面棘輪量的是 `t("中文")` 慣用法，**`text:` 硬寫的字串不在其射程內**（補 (e)，只覆蓋本輪三句、不越界宣稱 #141）。修完第二批 **7/7 CAUGHT**。
    - **⑧ 已知限制**：`preview_start` 在排程輪被環境拒絕 ⇒ **無任何 preview 目視**，兩句話與其英文／簡中譯文的實際渲染屬 **UNVERIFIED**。**#181 本體未落地**：宿主 `main.js`／`core/ui.js` 皆 eager（**第十張**堵在 #118／#169 上的卡），且缺陷住在 §9 headless 配方**刻意繞過**的那一段（掛載／換頁）⇒ **必須排在有 preview 的輪，與 #93／#169／#175 同批**。
