...
- **2026-09-01（平台軌 · **20:00 窗** · 建置輪＝台帳輪替審「**活動**」7 模組＋查獲並當輪修完「**真站的活動日曆宣告了一個真站上永遠不會發生的活動**」＋立常駐鎖 `platform/promo-cal-hides-undrivable`＋開卡 #157 · claim `p-201230-9f2c` · `sw` v256→v257 · 首屏 **+118 bytes**）**
    - **① 進場**：`build_lock` 乾淨 `false`（遊戲軌 09-01 16:00 窗 `g-160613-742d` 已於 commit `a29e359` 釋放）→ claim `p-201230-9f2c` → **當下即單檔 commit `af3b921`**（§10 紀律：只存在工作區的 claim 等於沒 claim）→ 停頓做非寫入讀取後重讀確認 token 仍在·**未奪鎖**。dark **4.8h**（`last_platform_run_at` 09-01T15:20）<24h ⇒ 非 catchup；`lead_track=games` 下本可讓路，但 14:00 窗 ⑪ 明文指派本窗「台帳輪替＝活動（7 模組）」⇒ **做而不讓路＝跨輪指派連續第十八輪**。船長待處理 [P-FS]／[ENG-1] 續掛，無新指令。`ledger-card-sweep.js` 進場＋收尾**雙向皆 0 筆**告警。
    - **② 取材**：`platforms.json` 本日**零到期**（0/36，最近 09-02 thrill／mega-dice）⇒ 不深挖、`platforms_researched` **不加**（據實不虛報）。依 SKILL 第 5 步 escape① 擴大取材，本輪取到**一條欄位級可對照的形制**：Stake 的 rain 資格是「**近 7 天流水 $3,500** ＋ 最近在聊天室活躍」——**流水資格閘**才是 rain 在經濟上站得住的原因。ApexWin 的 rain 只有「近 10 分鐘發過言」、零流水門檻。這條訊號本身不生缺口（假站刻意慷慨），但它**釘死了一件事**：真站版 rain 不能只是「把閘拿掉」，見 §11 待辦「rain 長尾送幣真站微調」。
    - **③ ⭐ 真產出＝台帳審「活動」時撿到的缺陷，形狀是「閘只裝了一半，而裝的是不會被看見的那一半」**。
      · **機械事實（可複跑）**：`HL.rain.tick()` 全庫**唯一呼叫點**是 `layout/chat.js` 的 `startAuto()`，而它坐在 `if (HL.site && HL.site.isLive()) return;` **之後**（同一道閘一次關掉假聊天訊息與 RainBot 自動紅包雨，chat.js 行內註解也明寫）；`core/rain.js` 全檔 `HL.site` 命中 **0**、自己不排任何計時器 ⇒ **真站上紅包雨永遠不會發生**。
      · **另一半沒裝**：`core/promo-cal.js` 的 rain spec 上架條件只寫 `avail: !!HL.rain`＝**模組載入了就上架、從不問站別** ⇒ 真站玩家在 #49 活動日曆看得到「🌧️ 聊天室灑幣 · 常設 · **在聊天室活躍即可分得**」，點下去開聊天室、橫幅永遠空白、一場雨都不會來。**日曆照常排序、node 全綠、console 零錯誤。**
      · **為什麼這是缺陷而不是「還沒做」**：`promo-cal.js` 自己的註解（#107 受眾閘，:121）寫著「**不合格者一律不出現而不是灰掉：灰掉等於預告一個玩家拿不到的獎**」。本檔已經有這條原則，rain 只是沒被納入。⇒ CLAUDE.md §4「修一半而看不出來」家族。
    - **④ 本輪特別去查了「這是不是通例」，答案是否定的——所以棘輪敢把值釘死**：同檔另外六個活動**不是**同一形狀。`raffle.js:37-38` 的 isLive 閘只把 botTickets/players 歸零、`tournament.js:61` 的 `seedBots()` 只回 `[]`——**機制本身在真站照跑**（真實押注照樣計分）；happyhour／luckyspin／season／safetynet 根本沒有站別閘。**rain 是唯一「整台機器都在閘後面」的**。⇒ 新鎖的差集斷言直接釘成 `["rain"]`，多一個或少一個都轉紅。
    - **⑤ 修法（刻意選最小的那個，並據實記為權宜）**：`avail` 改為 `!!HL.rain && !(HL.site && HL.site.isLive())`。**正確的修法是述詞化**（見 #157），但那要動 `evalSpec()` ⇒ 首屏餘裕不夠（見 ⑧）。**刻意不做的兩件事**：(a) 不改成「灰掉並標示真站不供應」——`promo-cal.js` 自己的原則就是不灰掉；(b) 不給真站接一個「單機版 rain」——真站沒有假聊天室，那會變成一個沒有社群的免費水龍頭，且缺 Stake 那道流水資格閘（見 ②）。
    - **⑥ 常駐鎖 `platform/promo-cal-hides-undrivable`（零出貨位元組·shim 實跑真檔）**：以最小 shim 把**真的** `rain.js` + `promo-cal.js` 載進來、只切 `HL.site` 這一顆旋鈕，比對兩個站別的 `list()`。守**雙向**：① 真站不得上架 rain；② 假站必須照常上架（不得矯枉過正）；③ **差集棘輪**＝「demo 有、live 沒有」的集合必須**恰好**是 `["rain"]`；④ **反向源碼錨**＝驅動點必須仍在 chat.js 的 isLive 早退之後（哪天真站接上真驅動，①③ 就變錯的 ⇒ 這條先紅，逼人回來重看日曆這側）；④-b rain.js 不得自己排計時器；⑤ **空心尺反錨**＝對照組 spec（avail 恆真）必須在**兩個站別都上架**，否則「live 查不到 rain」可能只是因為 `list()` 在 live 恆空；⑥ 零回歸＝另六個 spec 的 avail 不得含站別判斷。
    - **⑦ 驗證**：node **323→324 全綠**。**負向擾動 7/7 CAUGHT**（進場基線先驗為 324/324 綠、快照落磁碟、每例還原後與快照**逐位相同**、收尾複跑 324/0）。**P1（還原修法前的 avail＝重現缺陷本體）／P2（兩站都關掉＝矯枉過正）／P7（把 tick 呼叫整個拿掉）＝僅本鎖轉紅**＝具鑑別力。P3／P4／P5／P6 各多一個 `platform/first-screen-budget` 連坐——**原因可解釋且本身就是一則讀數**：那四例的擾動都往 eager 檔加了位元組，而落地後餘裕只剩 8 bytes ⇒ 任何加字都會撞到 M6 門檻。⚠️ 依 09-01 14:00 窗訂下的第三道防線，**整套擾動期間工作區無第二個寫者**（intel/ 與 prototype/ 一律等擾動跑完才動）。
    - **⑧ [P-FS] 據實**：落地後首屏 **1638392／1638400 bytes、91 支 script ⇒ 餘裕 126 → 8 bytes**（本輪淨 **+118**：`promo-cal.js` 的判斷式 +34、行內註解 +84）。**這是連續多輪以來第一次餘裕被真的花掉**，且花在一個「玩家看得見的錯誤宣告」上，我認為值得。⇒ **下一輪起，任何動首屏的改動都不可能了**（8 bytes 連一個中文字都放不下），#118 從「建議」變成**事實上的前置**。
    - **⑨ 台帳「活動」7 模組**全部回填 `last_audited=09-01`：`季票` present（`season.js:26-27` 兩標量連七輪逐位相同）／`促銷框架` partial（`registry-gaps.js` 權威口徑外部註冊者 **7**，連三輪無新註冊者；**本輪修法動的是這一格的檔**）／`世界活動 hero` present（真站兩道歸零閘在位、`isLive` 命中 2；**本輪記下它與 rain 的差別＝「歸零 ≠ 下架」不可互相類推**）／`抽獎·Raffle·Rain` present（**本輪缺陷的那一格**）／`推薦聯盟` partial（22,263／17,794 bytes 逐位相同）／`新手引導` present（**其 avail 本來就在問「這則活動現在成不成立」＝#157 的第二個範本**）／`遊戲內活動掛件` absent ★（#136／#149 續掛，**本輪為它多寫了一條驗收：掛件不得再手寫第三份站別判斷**）。
    - **⑩ 下輪**：台帳輪替＝**資安（8 模組·08-30 最舊）**；`platforms.json` **09-02 兩站到期**（thrill／mega-dice）⇒ 下輪有真調研可做；**#153 仍順延**（連續第三輪：本輪 `max_implement_per_run: 1` 已用在 rain 這一格，且一輪只徹底做一件）。
