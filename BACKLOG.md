...
176. 🏗️進行中（2026-09-07 平台軌 08:00 窗 · **條款面那半已落地**，資格閘與權重那半待批准） **旗艦錦標賽的玩法說明把「憑什麼排名／哪些遊戲算／怎麼分」寫死成散文，而 #85 早就把這三件事做成可切換的資料——於是那個可切換性至今 0 個使用者，而賽事名稱今天就在對玩家宣告一個引擎裡不存在的遊戲範圍** — S–M（**兩半分開**：條款面求值那半住**延遲載入**的 `views/tournament.js`＝**零首屏位元組、已落地**；資格閘＋逐遊戲權重＋上限帽那半住 eager `core/tournament.js`／`core/score-axis.js`＝**吃首屏、卡在 #118**）｜來源：**新開取材維度 17「活動的條款面／加入之前公布了什麼」**（前 16 條維度對「條款」的真實命中數＝0）× platform-modules 台帳「活動」分類輪替新增格〈**活動條款面／規則的單一真相**〉判 `weak`（台帳盲點第 13 例）× **Pragmatic Play Drops & Wins（一手官方頁）＋ Stake Daily Races（二手）兩筆獨立佐證、從相反方向壓同一格**
