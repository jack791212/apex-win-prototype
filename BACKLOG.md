...
- **2026-09-05（平台軌 · **20:00 窗** · 台帳輪替審「**前端UI/UX**」8→**9** 模組（新增一格）＋到期複查 tier-3 兩站 GoKong／Zonko ＋開卡 **#171** ＋一條**零首屏位元組**的止血鎖 · claim `p-201015-b3d7` · `node` 339→**340 全綠** · 負向擾動 **6/6 CAUGHT** · 首屏 **1,638,181／餘裕 219B 全程未動** · **sw 不 bump**）**
    - **① 進場**：`build_lock` 乾淨 `false`（遊戲軌 09-05 16:00 `g-160530-4c8a` 已於 `5661e34` 釋放）→ claim `p-201015-b3d7` → **當下即單檔 commit `4da32a8`** → 停頓後重讀確認 token 仍在＝claim 成立·**未奪鎖**。`last_platform_run_at` 09-05T14:55 ⇒ dark **5.2h < 24h**＝非 catchup；`lead_track: games` 本可讓路，但前手 14:00 窗**明文指派本輪三項**（台帳輪替→前端UI/UX／到期餘 3 筆／續作） ⇒ 不讓路。船長「待處理」無平台軌新指派（#118／#160／#169／#93 皆待裁決，依 §7 未動）。
    - **② 進場量測**：`ledger-card-sweep` 正／反向皆 **0 筆**；`node` 進場 **339 全綠**；首屏 **1,638,181／1,638,400＝餘裕 219B**。⚠️ **順手更正一個被連續兩輪抄過去的數字**：09-05 12:00 與 14:00 兩則都記「**91 scripts**」，但首屏鎖 `staticScripts()` 的正則要求 `src="\.`（本地相對路徑）⇒ **它量到的是 90 支**；`index.html` 的 91 行 `<script src=` 裡有一支是 `cdn.jsdelivr.net` 的 Supabase **外部 CDN**（不計入首屏位元組、也不在鎖的量程內）。位元組讀數兩輪逐位相符、無誤；**只有「支數」把外部那一支算進去了**。⇒ 本輪起記為 **90 本地 + 1 外部 CDN**。
    - **③ 取材（到期 3 筆取 priority 前二）**：`gokong(58)` ／ `zonko(57)` 深挖，到期餘 **1 筆**（`kaasino` 55）。⚠️ **限制照實記**：`WebFetch` 對 clashofslots **403**，本輪唯一直取成功者＝**casinolandia（200）**；Zonko 側**全為 WebSearch 二手摘要**（未直取官網），兩份 dossier 已逐句標明來源強度。**新事實**：GoKong 遊戲庫 30,000+、slot 分群軸明載 **RTP／波動／主題／功能** 四軸、搜尋列同時命中遊戲/供應商/分類、**11 語 ＋ 幣別選項並列為「Player Customization」帳戶層偏好**；Zonko **營運方 Mamba Limited**、遊戲 500+→**670+**、**VIP 7 級 Blue→Diamond·進度貨幣 Stars 可由 missions/gameplay/promotions 取得**、3,000 GC 在 **email 驗證前**即發。
    - **④ ⭐ 本輪最重要的發現：又是取材與台帳輪替從兩邊指到同一格，而那一格本來不存在（台帳盲點第 9 例，與 14:00 窗同形）**。GoKong 把「**Player Customization**」寫成獨立段落（語言＋**幣別選項**），交叉 **Stake 的站層專頁 `/settings/preferences`**（**把餘額顯示成本地法幣就設在那裡**、25 種法幣）⇒ 兩邊都在回答「**這個站把玩家偏好放在哪裡**」，而**第 1–12 條取材維度沒有一條會問這件事**（第 7 條問多語系形制、第 11 條問帳戶安全自助面、第 5 條問責任博弈工具）⇒ **新開維度 13「玩家偏好的安放處」**。
    - **⑤ ⭐ 回頭量自己，照出的是「做了，但放錯地方」**：`core/game-settings.js` 的 `HL.gset` ＝全站唯一偏好持久層，`DEF` 恰 **4 條**；**編輯面全庫只有一份**（`grep -l "HL.gset.set("` ＝ `views/game-frame.js`），掛在**遊戲外框 ⚙**（`:96`）與 **PiP ⚙**（`:149`）——**兩顆都在遊戲裡**。而 4 條裡有 **2 條的生效範圍是整個站**：`fiatView` 被 `core/dom.js` 的 `money()` 消費＝**全站金額格式化器（實測 67 支檔／384 個呼叫點）**、改完 `HL.app.refresh()` 全站重繪；`anim` 被掛成 `<html>.ax-anim-off` **全域 kill-switch**（`components.css:2985`，與 `prefers-reduced-motion` 同一套規則）並由 `core/reveal.js` 讀取。⇒ **玩家要在大廳關動效或換金額顯示幣別，得先隨便進一款遊戲。**
    - **⑥ ⭐ 但這張卡真正的價值不在那兩條設定，而在容器**：佇列裡**已經有三張卡各自需要「一個放偏好的地方」**——**#128**（通知分類軸與玩家偏好，卡體自陳「入口位置**待 #118／#93**」）、**#148**（外觀/主題模式，`data-theme` 開機寫上去、CSS 零讀者〔本輪逐項複跑仍為 2 筆寫入／`[data-theme` 0／`prefers-color-scheme` 0〕）、本輪的 **#171** ——**而沒有任何一張卡擁有那個容器**。⇒ 三個功能各自等一個沒人負責的貨架，正是本軌「容器先於內容」要攔的形狀。開卡 **#171**（⬜待批准·M），切法＝`HL.prefs` 登記簿（`scope:'site'|'game'`）＋兩個渲染面共用同一份登記簿，#128／#148 只需各自 `register` 一條。
    - **⑦ 台帳（輪替＝前端UI/UX，8→9）**：8 筆全部逐項複核後回填 `last_audited`（零漂移：`views/casino.js` 仍是唯一大廳來源／`app-shell.js` 仍是唯一導覽殼／`src/i18n/` 仍兩支／`HL.gameAxes` 出口 10 個齊備／`data-theme` 仍只寫不讀）；**新增一格「玩家偏好面（站層設定出口 × 偏好登記簿）」判 `weak`**——**刻意不判 absent**：偏好本身在、持久化在、跨遊戲生效在、`<html>` kill-switch 也真的接上了，**缺的是容器與可達性**。
    - **⑧ 兩筆「不因外部佐證而改判」的據實記錄（避免把佐證誤讀成缺口）**：① **分群軸**——GoKong 的四軸是我方 partial 的**外部佐證**，但我方 `HL.gameAxes` 容器齊備、缺的是 RTP 那一軸的**資料**（#94／#98 家族，阻塞事實在案），**不開新卡**。② **搜尋**——GoKong 評測稱其搜尋『more advanced than most』，但我方 `views/casino.js:47-78` 的乾草堆**已含 title／provider／author ＋「這款遊戲落在哪些玩家看得見的入口」的名字**、且自陳「全站唯一的搜尋述詞」並已有常駐鎖守著 ⇒ **這一點我方並不落後**；#163 判 partial 的理由是**零結果出口與最近搜尋**，本輪不改判。
    - **⑨ 本輪落地：一條零首屏位元組的止血鎖**（住 `prototype/tests/checks-platform.js`，不在 `index.html` ⇒ **sw 不 bump**）`platform/gset-preference-surface`：(a) 旗標清單以 **`DEF` 物件本身**為錨（不是「檔案裡提到過的字」）；(b) 每個旗標都必須有編輯列＝**`game-settings.js` 檔頭自立的「避免死 flag」紀律第一次有機械出口**；(c) **反向**：每個被當 key 傳進去的字面值都必須是真旗標——`set(k,v)` 對未知 key **靜默 return**，所以打錯字的那一列**畫得出來、按得下去、什麼都不會發生**且 console 全靜默（§4「修一半而看不出來」家族）；(d) 編輯面**唯一一份**；(e) **棘輪：站層生效卻只在遊戲內可編輯 ≤2**（現況 2）；(f) 兩道防空心（抽取器至少 4 個 key、消費端掃描至少 10 筆）。
    - **⑩ 驗證/成本**：`node` 339→**340 全綠**；負向擾動 **6/6 CAUGHT**（P1 DEF 加第 5 旗標無編輯列／P2 `row("anim")` 打成 `row("anmi")`／P3 移除 hotkeys 那一列／P4 另一支檔第二份編輯面／P5 讓 game-scoped 的 `fast` 變成站層生效／P6 破壞 `DEF` 錨把尺弄空），**每例僅本鎖轉紅（失敗恰 1、他鎖 0）·BASELINE=0·RESTORED=0**。首屏 **1,638,181 全程未動（餘裕 219B）**、本地 script 90 支不變。
    - **⑪ 刻意不做（不半修）**：站層偏好面**本輪不落地**——入口必須落在 `layout/app-shell.js`（首屏 eager、餘裕 219B），且它與 **#93／#118／#169** 是同一個瓶頸；面本身可以是零成本延遲 route，**入口不行** ⇒ 全部寫進 #171 的阻塞事實區，不在無人值守輪硬擠。
    - **⑫ 收尾**：`platforms_researched` +2、`platform_cards_opened` **+1（#171）**、`platform_cards_implemented` +0（本輪落地的是止血鎖，非卡）；`consecutive_idle_rounds` 維持 **0**（真產出）。他軌/前景孤兒 `games/registry.json` M／`styles/tokens.css` M／`games/slot-engine/` ??／`Game assets/*`＝依 §7 **一位元組未碰**。**下輪（平台 08:00）**：台帳輪替→**後台**（8 分類循環回頭）；到期餘 1 筆（`kaasino`）；**現成、無需再調研的開卡對象＝Zonko 的「有邊界的 FTUE 多日軌」**（07-02 首記、連續三輪確認成立卻連續三輪讓路，已在 dossier 標明）。