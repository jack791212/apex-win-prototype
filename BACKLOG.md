...
185. 🟦已批准待做 **第二把「程式碼 vs 註解/字串」的尺，有跟今天修掉的那把一模一樣的盲點——而它是 10 條常駐鎖用來證明「這行字真的會被求值」的那把** — S–M（只動 `prototype/tests/`＝**首屏逐位未動**）— 來源：**本輪（09-12 14:00 窗）修 `nonCodeMask` 時的連帶普查**
...
- **2026-09-14（平台軌 · **08:00 窗** · **#192 當輪落地＝四條金錢不變量升級成行為級** ＋ 台帳輪替審「**前端UI/UX**」10 模組全審 ＋ **更正兩筆自 09-08 起就錯、且各被複製十份的手抄讀數** ＋ 開卡 **#193** · claim `p-081215-b3e7`·心跳 08:12→08:38→08:55→09:20→收尾·進場鎖乾淨 false〔維護軌 00:00 窗釋放〕·**未奪鎖**·dark 11.1h<24h 非 catchup·`lead_track: games` 但前手 20:00 窗明文指派故不讓路）**
    - **① 閘門/進場**：三開關皆 true；`build_lock: false` → claim `p-081215-b3e7`（commit `98a0c69` **當下即提交**）→ 停頓做非寫入讀取後重讀確認 token 仍在。前手 20:00 窗的明文指派是「台帳輪替到**前端UI/UX**、實作首選 **#192**（不吃首屏、配方現成）」⇒ **兩項全數執行＝跨輪指派連續第 N 輪**。
    - **② 本輪未做平台調研，據實記**：`db/platforms.json` **37 筆逾期 0**（最早 rainbet 09-18）。為了有事做而重掃媒體正是 `ban_busywork_heartbeat` 明文禁止的空心跳，該檔一位元組未動。`ledger-card-sweep` 正向 1 筆告警（〈資安／帳戶安全自助中心〉引用 #188）**沿用前輪的誤報判定**（該引用是當阻塞事實、不是關閉缺口），反向 0。
    - **③ #192 落地：四條只靠逐字 grep 在守的「錢」的不變量，升級成 vm 沙箱實跑 `core/progress.js`**。新鎖 4：`platform/bonus-unlocked-survives-expiry`（#71 信任紅線）／`platform/bonus-req-frozen-at-grant`（**#74，在此之前沒有任何測項直接驗**）／`platform/bonus-scope-weight-fifo`（#89 的 FIFO 消耗換算那一半）／`platform/bonus-exp-merge-guard-behavior`（#71 (d)）。四條**都不取代**既有的源碼掃鎖——源碼掃守「這件事結構上做不到」，行為級守「這件事實際上沒有發生」。
    - **⭐ ④ 沙箱不能用 `require()` 掛那兩支依賴，這是本輪最貴的一個實測發現。** `wager-scope.js`／`bonus-ttl.js` 兩支都有 `module.exports` 的 node 契約，**最順手的寫法就是 require 進來當布景**——而那會讓整組斷言量錯分支：`wager-scope.js` 的 `resolverOf()` 在 node 分支下**永遠取不到 `HL.games`** ⇒ `registryReady` 恆 false ⇒ 權重表那條路一律 **fail-open 回 1**（那是刻意的設計：查不到登錄表不鎖玩家的錢）。首版就是這樣寫的，「部分權重」情境實測跑出 `prog=300`（fail-open 那條路）而不是正確的 `200`，**而斷言是綠的**——因為我當時是照著跑出來的數字寫斷言的。改成**在同一個 vm 裡跑它們的瀏覽器分支**之後才量到真值。⇒ **「有 node 契約」不等於「node 契約下的行為等於瀏覽器行為」**；凡是檔內有 `isNode` 分岔的，沙箱要指定走哪一支，並且要能說出為什麼。
    - **⑤ 可分辨的指紋比釘算式穩**：#74 的併筆路徑斷言釘在 `req === 90`（＝`10 + 8×10`，只為新增那份算）而不是釘算式本身；整筆重算會是 `20×8 = 160`。#89 的消耗換算釘在「次筆拿到多少」這個下游可觀測量：正確 200／少扣 300／換算寫反 350，三種錯法各有不同數字。
    - **⑥ 設計擾動時當場補了一條漏掉的斷言**：原本 (B) 只走「這一注就達標」那條分支 ⇒ 把 `eff = floor(w×wt)` 改成 `eff = w`（權重根本沒乘進去）**在 (B) 裡量不出來**（消耗換算仍由 `ceil(need/wt)` 決定）。補上 (B3)「未達標那一支」才釘得住，對應 **P13**。⇒ **擾動清單要在斷言寫完之後再列一次，它會反過來指出斷言的盲區。**
    - **⑦ 負向擾動 25/25 CAUGHT**（P1–P5 信任紅線／P6–P9 req 凍結／P10–P15 範圍權重 × FIFO／P16–P20 壽命併筆守衛／P21–P24 **布景見證者**〔`opts.ttl`／`tick()`／`HL.games` stub／`setWagerMult` 各自改成 no-op 都必須有人紅〕／P25 純函式側 `bonusTtl.sweep`）。
    - **⭐ ⑧ 擾動跑到一半，這把尺自己被咬了一口——而咬它的是「沒有紅的」與「根本沒跑完」同形。** 第一趟我把腳本包在 `timeout 590` 裡，它在 P16 當場被砍：子行程的 stdout 只剩半截、一個 `❌` 都沒有 ⇒ 腳本掃不到失敗 ⇒ **把「這一趟沒跑完」印成了「MISSED（全綠）」**，而且工作區被留在擾動態。差一點就據此寫下「抓到一條空綠鎖」。修法＝**以 `run.js` 收尾必印的統計行當存活證明**，收不到就判 `ABORT` 而不是 MISSED；重跑 P16–P25 全數 CAUGHT。⇒ 這是 §4 形狀⑦「斷言認的是概念還是寫法」的**量具版**：**一把只會數失敗的尺，分不出「沒有失敗」與「沒有量到」。**
    - **⑨ 台帳輪替「前端UI/UX」10 模組全審（全庫最舊＝09-08）**：共用探針電池機械複跑**逐項零漂移、十格狀態全部維持**。但**更正兩筆 09-08 寫下當天就不對、並被逐字抄進十格的讀數**：① `ui.comingSoon(` 記 27、**當日實際 28**（漏記 `core/responsible.js`；回到 `548fef8` 用同一條 grep 重算證實＝**漏記不是漂移**），且第 28 筆是**註解**不是呼叫點 ⇒ 正確口徑「原始 28／呼叫點 27」；② `HL.gameAxes` 記「出口 **13** 個齊備」、vm 實跑 `Object.keys` **實測 12**。**⭐ 第二筆的教訓比數字重要：「齊備」是個沒有反向的形容詞**，後面接的數字沒有任何測項、任何複核在對，所以它可以錯一整輪而永遠不會被碰到。
    - **⭐ ⑩ 而「這兩筆錯誤各被抄了十份」本身就是缺陷形狀**：共用電池的那段字串被逐字複製進該分類全部 10 格 ⇒ 一筆抄錯＝十筆抄錯，且讀任何單一格的人**分不出那一行是為這一格量的還是繼承來的**。⇒ 本輪起共用電池只在〈大廳/遊戲牆〉記一次全文、其餘九格回指；並開卡 **#193**（把電池變成 `intel/tools/ledger-probe.js`，讀數由工具求值、台帳只引用，且必須能 `--ref <sha>` 重跑——**沒有那個能力，「當天就錯」與「後來變了」在台帳上完全同形，而兩者該做的事相反**）。
    - **⑪ i18n 那一格換了一把尺（舊探針對它結構上是瞎的）**：舊電池問「`src/i18n/` 有幾支檔」——那個數字自 #100 拆檔後**不會再變**；而這一格真正的表面六天內被改了 **12 次**（全 `src/` 最高）⇒ **零漂移量的是隔壁那支檔**（記憶〈讀數量錯對象〉）。改記可重跑的覆蓋率實測：呼叫面 502 呼叫點／365 整節點鍵、DOM 綁定面 649／526，**缺漏 EN 0 ＋ zh-Hans 0**；字典 en 1,669／zh-Hans 1,452 條 ⇒ 十二次改動一次都沒掉出 #119 的棘輪。維持 `partial` 的理由不變（缺的是 #141 執行期登記簿，不是字典覆蓋率）。
    - **⑫ 驗證與收尾**：`node prototype/tests/run.js` **380 → 384 全綠**。**首屏 1,638,078 bytes／93 支 script、餘裕 322 bytes＝與進場逐位相同**（#192 全在 `prototype/tests/`）。**`sw.js` 不 bump**（`grep -c "tests/" prototype/sw.js` ＝ **0** ＝測項不出貨，沿 #135 既有判準），停在 v299。**排程輪拿不到 dev server**，而本卡零 UI 表面、零 `src/` 改動 ⇒ 無線上複驗需求。收尾清 `build_lock=false`。
    - **下輪（14:00）建議**：台帳輪替到 **後台**（次舊＝09-11）；實作首選 **#193**（零首屏位元組、本輪剛量到兩個見證者、且能讓往後每一輪的台帳讀數不再靠手抄）。次選 #191，但它吃 1.5–2.5KB eager 而**餘裕只有 322 bytes** ⇒ 在 #190／#188 第二波進帳之前仍不可落地。
