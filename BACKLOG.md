...
- **2026-09-06（平台軌 · **08:00 窗** · 台帳輪替審「**後台**」8→**9** 模組（新增一格）＋到期複查 tier-3 兩站 Kaasino／Card Crush ＋**新開取材維度 14**＋開卡 **#172／#173**＋一條零首屏位元組的止血鎖）**
    - **① 進場**：`build_lock` 乾淨 `false`（維護軌 09-06 00:00 `m-001430-a7c3` 已於 `f6f311a` 釋放）→ claim `p-081015-b7a2`（`941bb05`）→ **未奪鎖**。`last_platform_run_at` 09-05T21:05 ⇒ dark **11.1h < 24h＝非 catchup**；`lead_track: games` 本可讓路，但 **09-05 20:00 窗留有明文指派**（「下一個平台輪若無新指派，Zonko 的有邊界 FTUE 軌是無需再調研就能開的卡」）⇒ **做而不讓路＝跨輪指派連續第十七輪**。
    - **② 進場量測**：`ledger-card-sweep` 正／反向皆 **0 筆**；`node` 進場 **341 全綠**；首屏 **1,638,181／1,638,400＝餘裕 219B、本地 script 90 支**（與 09-05 兩輪**逐位相符**＝第十二個「逐位未動」數據點）。
    - **③ 取材（到期 2 筆全取）**：`kaasino(55)`／`card-crush(63)` 深挖，取後到期歸零。⚠️ **限制據實兩記**：`WebFetch` 直取 `kaasino.com` **連線被拒（ECONNREFUSED）**、`tribuna.com` **403** ⇒ **Kaasino 那份全是二手摘要**；Card Crush 那份 `sweepskings` **直取成功**（一手評測）。兩份 dossier 已逐句標明來源強度。
    - **④ ⭐ 本輪最重要的發現＝我們把「客服層級」做成了 VIP 賣點，而全站沒有任何一個地方聯絡得到人（台帳盲點第 10 例）**：兩個到期站從**相反方向**指到同一格——**Card Crush** 把人工通道當**義務**（自我排除與 KYC 都要寄信給客服）、**Kaasino** 把它當**獎賞**（Prime 頂階＝專屬客戶經理）。回頭量自己：`core/service-level.js` 的 `#63` 已把「客服層級」註冊成分階權益維度（`id:"support-level"`、`fmt` 逐字產出 標準客服／優先客服／**專屬客戶經理**，且經 `HL.econCfg` 進了經濟旋鈕描述表，全檔 **9 處**客服字樣），`core/responsible.js:922` 也寫「**客服也無法代為解除**」——**而 `mailto:` 全 `src` 0 命中、無工單／聯絡表單／請求佇列**；唯一問答表面 **AI Luna**（`ai-concierge.js:38`）在 KB 與 `HL.support` 都答不出來時，把玩家**送回說明中心**＝他剛剛沒找到答案的地方（**同一個迴圈、0 個外部出口**）。⇒ **承諾在、通道不在**，CLAUDE.md §4 家族的新變體。開卡 **#173**、新開**取材維度 14**。
    - **⑤ ⭐ 這一格為什麼特別值錢：它是「後台」八格裡唯一一個純前端現在就能做的**。該分類 `會員管理`／`營運自動化規則` 判 absent 的真正前置**都是後端權威 + 角色閘**（§11，已連十輪維持不開卡）；而人工通道**兩端都在我們手上**——玩家端＝一個送得出請求的出口，營運端＝`HL.opsBoard`（唯一營運面，寫入面**恰 2 處**、**無收件匣**）。更關鍵的是它**餵飽了那個一直沒有 action 可執行的營運面**：`營運自動化規則` 連四輪的不開卡理由逐字是「先做規則引擎會做出一個**沒有 action 可執行**的容器」——**請求佇列正是第一個真的 action 來源**。
    - **⑥ 履行跨輪指派：#172 Zonko 有邊界 FTUE 軌正式立卡**（2026-07-02 首記 → 08-06／09-05 兩度複驗 → **連三輪讓路**）。機械事實：`rewards.js` `LADDER` **30 天遞增**＋`MILESTONES {8,15,22,30}`＝**永久 streak**（新手與第 400 天的玩家看到同一條）；`onboarding.js` `WINDOW_MS = 6h`＝**首日轉化鉤子、非多日軌**。⭐ 依 SKILL「開卡前掃來源 evidence 裡的阻塞句式」，**三條阻塞已逐條抄進卡**：(a) `onboarding/rewards/rakeboost` **三支全是首屏 eager**、餘裕 219B ⇒ **必須排在退還位元組的卡之後**；(b) 帳齡唯一真相＝`HL.rakeboost.newcomerTs()`（`#107` 已向它求值，檔頭明寫「多一份帳齡＝兩個新手期定義會分岔」）**不准再刻第二個錨**；(c) 前 8 天與階梯**重疊**（第 8 天恰是第一個里程碑尖峰 3,000）⇒ **交棒語意必須寫進驗收**，別留給實作輪。
    - **⑦ 台帳（輪替＝後台，8→9）**：8 筆逐項複核後回填 `last_audited`，**九項讀數全部逐位未動**——儀表板唯讀性 `onClick|onInput|onChange` 恰 **2**（連十一輪）／`HL.econCfg` in views **13**（連八輪）／`registry-gaps` econCfg external **14**·promoCal **7**·games **24**（皆相同）／`mock-data` 模組級 var **24**（連五輪）／`lazy-games` MANIFEST **20**（連六輪）／`ls src/views | grep -icE "admin|cms|backoffice"` **0**（連十一輪）／`jurisdiction` 外部讀者 **0**（連十輪）／`HL.opsAuto|HL.rules|playerTag|assignTag|HL.tags` **0**（連五輪）。**新增第 9 格「玩家請求佇列／人工通道 Player Request Queue」判 `absent`**（不是 weak——玩家端出口、請求狀態、營運端收件匣**三樣全缺**）。
    - **⑧ 兩筆刻意不開卡的據實記錄（避免把佐證誤讀成缺口）**：① Card Crush「**Live table games 不參與彩池**」＝`#78`（彩池資格軸）之外的**第二個軸：類別排除**，同來源模組、同出口形狀 ⇒ **併入 #78 的來源引用、不另開卡**。② Card Crush 的**牌組陣位（Affinity 依位置生效）**＝我方 `arena` 是單一分數軸（`HL.battleMode`）所無，但「收藏卡牌對戰」整條軸線本身尚未立卡 ⇒ 屬更大的一張，**本輪不切小卡**。另：Kaasino 的招牌 split-screen **本輪六份評測 0 命中**——**未被否定、但也未被複驗**，已在 `#69` 標註來源強度而非撤回。
    - **⑨ 本輪落地：一條零首屏位元組的止血鎖**（住 `prototype/tests/checks-platform.js`，**不在 `index.html` ⇒ sw 不 bump、首屏逐位未動**）：`platform/human-channel-promise-ratchet` —— (a) **防空心/正向對照**：`support-level` 維度與「專屬客戶經理」標籤必須真的還在，否則棘輪該**退役**而不是空綠；(b) **棘輪**：通道落地前「提到客服卻無出口」的檔 **≤2**（實測 `service-level.js`／`responsible.js`，語言包**刻意排除**＝譯文不是承諾本身）；(c) **反向**：通道一落地就必須把基準回填為 0，且基準高於實測時**主動要求收緊**（承 T54「過期的哨比沒有哨更誤導」）；(d) **第二個消費者**：AI Luna 的無命中死路**恰 1 份**，通道落地時必須同時改它，否則就是修一半。
    - **⑩ 驗證/成本**：`node` 341→**342 全綠**；負向擾動 **5/5 CAUGHT**（P1 第三支檔開始提客服／P2 `support-level` 被改名／P3 通道落地但基準沒回填／P4 死路變兩份／P5 承諾字樣被抽乾使掃描器空轉），**基線 0 失敗、還原後 0 失敗**；P1–P4 **僅本鎖轉紅**，P5 另連坐 `platform/i18n-key-ratchet`（**正確連坐**：該擾動確實改寫了 UI 字串）。首屏 **1,638,181 全程未動**、本地 script **90**、**`sw` 不 bump**（只動 `prototype/tests/`）。
    - **⑪ 刻意不做（不半修）**：**#173 的通道本輪不落地**——入口必須落在 `layout/app-shell.js`（首屏 eager、餘裕 219B），與 `#93／#118／#128／#148／#171` 同一個瓶頸；且營運端收件匣是 `HL.opsBoard` 的**第 3 個寫入面**，需 preview 目視 ⇒ 不宜無人值守。**#172 同理**（三支宿主全是首屏 eager）。兩張都據實標明阻塞而不硬塞。
    - **⑫ 收尾**：`platforms_researched` +2、`platform_cards_opened` **+2（#172／#173）**、`platform_cards_implemented` +0（本輪落地的是止血鎖，非卡）；`consecutive_idle_rounds` 維持 0（本輪有真工作）。