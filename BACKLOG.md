...
- **2026-09-05（平台軌 · **08:00 窗** · 建置輪＝台帳輪替審「**資料**」（5→**6** 模組·新增一格）＋**新開取材維度 12「玩家的自我統計面」**＋深挖 tier-1 到期兩站（Stake／BC.Game）＋查獲並當輪修完「**#65 把誤導的標籤從面板拿掉了，卻留在會匯出 CSV 的那張報表上**」＋立常駐鎖 `platform/vip-xp-label-single-truth`＋開 #170（⬜待批准）· claim `p-081018-3940`·進場鎖乾淨 false **未奪鎖**·dark 10.7h 非 catchup · 首屏 1638159→**1638181（餘裕 241→219B）** · `node` 337→**338 全綠** · 負向擾動 **6/6 CAUGHT** · sw v269→**v270**）**
    - **① 取材（維度 12 新開·同家族第十種實例）**：`grep -lie "my stats|player stats|統計頁|個人統計|自我統計|stats page|profile stats|勝率|win rate|統計儀表|統計面板"` 在 **36 份 dossier 命中 1 份**，逐筆讀完＝`stake-us.md:32` 的「Dice 設 98% **勝率**」＝遊戲中獎機率設定的**同形詞**（與第 9 條那 5 筆 "bet slip" 完全同型）⇒ **真實命中 0/36**；對照組 `VIP|cashback|rakeback` **36/36**。
    - **② ⭐ 維度 12 與維度 9 是兩件事，而維度 9 會讓人以為問過了**：9 問**可攜性**（拿不拿得出去），12 問**可理解性**（看不看得懂自己）。首輪執行即取到兩站**相反的一半**：**Stake**＝`my-bets/archive` 逐期 JSON 給得完整、**站上零分析面**，閱讀被 StakeStats／StakeArchiveDownloader／Stake Analyzer／Spindex（盈虧・月 ROI・逐幣別）一整圈第三方接走；**BC.Game**＝Transactions/Bill/Game history 三分頁 ＋ **內建圖表化分析** ＋ 依時間區間/遊戲型別出自訂報表，**原生匯出來源未載**。⇒ **沒有一家兩者都有**。
    - **③ 台帳（輪替＝資料，5→6）**：`ledger-card-sweep` 正/反向皆 **0 筆**。5 筆回填 `last_audited`；**新增一格「玩家自我統計面」判 `weak`**（刻意不判 absent：6 欄終身統計每一注都在累積、站別隔離、且 `HL.reports` 容器已在——缺的是**出口 + 面 + 口徑宣告**，不是資料）。
    - **④ ⭐ 本輪真正的查獲（審資料時撞上的）**：`live-stats.js:56` 每一注餵 `HL_ACHIEVE` 的 **6 欄終身統計**，但 `achievements.js:179` 的公開 API **不含 `stats()`**，`status()` 只把每欄折成 `prog`（0–1）⇒ **玩家看得到 25 條進度條、讀不到任何一個數字**；唯一叫「統計」的 📈 面板是 `var data = fresh()`＝**重載歸零**（面板自陳「本瀏覽器工作階段」＝誠實）。⇒ 開 **#170**。
    - **⑤ ⭐ 而且順手照出一個現成的「兩份真相」（本輪同窗修完）**：`HL.vip.status().wager` 是 **edge 加權**（`HL.edge.weighted` 1.00×–1.80×）且自 **#65** 起**可含儲值/簽到來源**的**經驗值**。#65 已據此把 VIP 面板標籤改成「累積 VIP 經驗」＋`xpNum()`（純數字），檔內註解甚至寫明「原標籤會讓玩家以為只算押注」——**但只改了那一個表面**。`reports.js:501` 的「進度快照」（`aud:"player"`、**會匯出 CSV**）仍是 `push("VIP 累積押注", money(v.wager))`：**退回被否決的講法，還多套一個 NT$ 把經驗值講成錢**；而徽章門檻用的 `HL_ACHIEVE.wagered` 才是**原始押注額** ⇒ 同一段人生兩個「累積押注」，差距最高 80%。**量程證據**：全庫顯示 `vip.status().wager` 的表面**恰 2 個**，其餘 `.wager` 消費者皆非顯示（`rakeboost.js:438` 布林／`referral.js:98` 門檻——而 referral 的**畫面**標籤早就寫對了）⇒ **落後的只有報表這一個表面**＝§4「第二個消費者」型態。
    - **⑥ 修法/成本**：`HL.vip` 匯出 `xpNum`（格式化器單一出口）＋報表改 `push("累積 VIP 經驗", HL.vip.xpNum(v.wager))`。**順帶修好 i18n**：舊標籤「VIP 累積押注」**不在任何字典裡**（EN/zh-Hans 玩家原樣看見中文），新標籤兩本字典都已有 ⇒ **零字典改動、覆蓋率淨增一格**。首屏 **+22B**（241→**219B**）。
    - **⑦ 驗證**：`node` 337→**338 全綠**；負向擾動 **6/6 CAUGHT**（P1 標籤退回／P2 值退回 money()／P3 撤掉 `xpNum` 匯出／P4 報表自寫 toLocaleString／P5 刪掉第二個消費者／P6 面板那側改回帶「押注」）——**每例僅本鎖轉紅（totalFails=1）·BASELINE=0·RESTORED=0**。行為級那條刻意注入**標記式** `xpNum`（不是把實作抄一份），值須逐字等於標記 ⇒ 同時證明「走了該出口」與「沒再套 money」，且不自我指涉。
    - **⑧ 本輪刻意不修的一項（不半修）**：`距下一段位`／面板的`再押注 …` 兩處都用 `money(v.toNext)` 呈現同一個經驗值——但**兩個表面是一致的**（不是漂移），屬「口徑本身要不要改」的獨立決策，已寫進 #170 卡體。
    - **⑨ 下輪（平台 14:00）**：台帳輪替→**擴充性**；到期平台餘 **5 筆**（legendz/gokong/capyspin/zonko/kaasino·皆 tier-3）；最高價值續作仍＝**等 #169 裁決**（#170 的 ②③ 亦受首屏 219B 閘限，但 ① 唯讀出口極小可先落地供 #138 引用）。
