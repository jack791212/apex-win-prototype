...
- **2026-09-06（平台軌 · **20:00 窗** · 台帳輪替審「**功能**」18 模組 · **PWA/離線 present→partial**（8 輪來第一次不是只看版號）＋**新開取材維度 16「裝機與離線」**＋開卡 **#175** ＋一條零首屏位元組的止血鎖 · claim `p-201530-7b1e`）**
    - **① 進場**：`build_lock` 乾淨 `false`（遊戲軌 09-06 16:00 `g-160600-2046` 已於 `99d9e6d` 釋放）→ claim `p-201530-7b1e`（commit `7f7f1b1`，**當下就 commit**）、**未奪鎖**。`last_platform_run_at` 09-06T14:32 ⇒ dark **5.6h < 24h＝非 catchup**；`lead_track: games` **本可讓路**，但本輪有真研究工作（台帳輪替 + 新維度）⇒ **做而不讓路**。
    - **② 進場量測**：`ledger-card-sweep` 正／反向皆 **0 筆**；`node prototype/tests/run.js` **343 全綠**；首屏 **1,638,181／1,638,400＝餘裕 219B、本地 script 90**（與 09-05 14:00／20:00、09-06 08:00／14:00 **逐位相符＝第十三個未動的數據點**）。
    - **③ 取材（本輪到期 0 筆）**：`platforms.json` 36 筆**無一到期**（最近到期 09-08 的 stake-us／roobet），故不做「提前取」湊數；改依 SKILL 第 1 步「每輪重新取材」對**本輪台帳分類（功能）**做主題式取材：`stake.us`（Originals Challenge：每週指定一款 Originals + 目標倍數，5,000 SC 由全體達標者均分；Sea Shanty 等**單款遊戲專屬挑戰**）／`roobet`（Roo's Specials：在**指定遊戲**找特定倍數解鎖獎池；點使用者名字可看其下注數/歷史流水並打賞）／**Slotegrator 2026-07-17 為 Casino Builder 推出 PWA 功能**（B2B 平台商把「PWA 安裝＋SW 快取遊戲資產＋**安裝提示時機可控**（首儲後才跳）」當成平台能力賣）。⚠️ **限制據實**：三筆皆 `WebSearch` 二手摘要，未直取官網。
    - **④ ⭐ 本輪發現（台帳盲點第 12 例 · 而這一例的新意是「稽核指標就是破壞行為」）**：`sw.js` 檔頭逐字寫「離線可用」「讓**首次離線**也能開」、`index.html:8` 寫「可安裝 + 離線載入」、#14 打 **✅**、台帳這一格判 **present 共 8 輪**——而 `PRECACHE` 恰 **4 筆、可執行資產 0 筆**，`<div id="app">` **是空的**、整個介面由 **90 script + 3 css ＝ 93 個 code 檔**在執行期畫出。⇒ **離線首次啟動＝白畫面**，缺檔走 `Response("",{status:504})`＝**靜默**。**更關鍵**：`activate` 刪掉所有 `key !== CACHE` 的快取 ⇒ **每次 bump 清空離線語料**，而台帳 8 輪的 evidence **量的全是 bump 次數**（「六天 27 次」「兩日 53 次」）並以此證明「紀律持續運作」；實測近 7 日 `sw.js` **24 次** commit＝**一天清空 3.4 次**。
    - **⑤ 誠實邊界（不把發現講得比事實大）**：network-first + 執行期快取是**真的**（線上回訪過的檔會被快取 ⇒ 第二次之後、下次 bump 之前離線可開）；且**玩家可見表面沒有這個承諾**——全庫唯一玩家看得見的 `離線` 字樣是 `core/meta.js:156`「進度離線保留」＝**持久性**主張（localStorage 為真，不在射程）。⇒ 這是**台帳高報自己**，不是對玩家說謊；價值在於它讓一個「純前端零後端就能做到」的真能力被當成已完成 8 輪。
    - **⑥ 台帳輪替（功能 · 18 模組 · 09-04 為全庫最舊）**：**17 格逐位未動**（`HL.games.register(` 24 呼叫點／21 檔、lazy MANIFEST 20 檔、`registry.json` HEAD 1 筆、第三方聚合程式碼 0 命中、views 34 檔中 25 檔命中 `HL.fair`、註冊表 10 個 unproven 0 —— 皆與 09-01／09-04 逐位相同）；**唯一改判＝「PWA/離線 Service Worker」present → `partial`**（可安裝為真、SW 為真、執行期快取為真；**缺的是離線骨架裡的程式碼與安裝時機控制**）。
    - **⑦ 順帶回填 #149 的第六、七條對手佐證（不開第二張卡）**：Stake.us Originals Challenge（每週輪替**指定一款**遊戲）與 Roobet Roo's Specials（在**指定遊戲**找倍數）與 #149「挑戰引擎收得到 `game` 卻從不讀」是同一件事的正面形制。**依去重規則不開新卡**——來源模組同一格、出口形狀同一種；#149 已列五站共識，本輪加註兩站。
    - **⑧ 本輪落地物＝一條零首屏位元組的止血鎖**（住 `prototype/tests/checks-platform.js`）：`platform/offline-shell-precache-ratchet`。(a) **防空綠/錨**（PRECACHE 解析得到、殼的 code 檔 ≥50、`#app` 內容為空）；(b) **棘輪「不得半修」**（PRECACHE 的可執行資產只能是 **0 或 ≥93**——半份 precache 會離線 boot 一半、其餘 504 靜默，比完全沒有更難診斷）；(c) **反向錨之一**（一旦放 code 進去，`addAll(PRECACHE).catch(function(){})` 這個**整批原子失敗又被吞掉**的寫法必須先消失）＋錨住「每次 bump 清空語料」的那行 `activate`；(d) **反向錨之二**（能力到位前，玩家可見字串不得長出「離線可用」類**可用性**承諾——承 #173／#174 的承諾面棘輪同一紀律）。
    - **⑨ 刻意不做（不半修）**：**#175 本輪不落地**——SW 的 install/activate/離線行為 **headless 驗不到**（§9：面板隱藏不合成影格、SW 生命週期需真瀏覽器，且改 sw.js 後 preview 會被舊 SW 餵舊檔）⇒ 與 #93 同類，需一個**有 preview 的輪次**。⚠️ 但要記下一個好消息：**sw.js 不在 `index.html` 的 script 清單裡 ⇒ 改它不吃首屏 219B ⇒ #175 是目前佇列裡少數不堵在 #118 上的卡**。**零 `prototype/src` 改動 ⇒ 首屏讀數逐位未動、`sw.js` `CACHE` 不 bump**（本輪只動 `tests/`，不出貨）。
    - **⑩ 驗證**：`node prototype/tests/run.js` **343 → 344 全綠**（+1 常駐鎖）；**負向擾動 5/5 CAUGHT**（P1 PRECACHE 加 1 支 js＝半修／P2 補滿 93 筆但保留 silent addAll／P3 `#app` 塞靜態內容／P4 在 `core/faucet.js` 加玩家可見「離線也能玩」／P5 把 `PRECACHE` 改名破壞錨），**每例失敗恰 1 項且就是本鎖**（`onlyMine=true`）、BASELINE 0、還原後 0。
    - **⑪ 收尾**：`platforms_researched` +2、`platform_cards_opened` **+1（#175）**、`platform_cards_implemented` +0；`consecutive_idle_rounds` 維持 **0**（真工作）。`build_lock` 清回 `false`。
