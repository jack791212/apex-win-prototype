...
201. 🟦已批准待做 **每一輪都手刻一次負向擾動 harness，而它沒有保險：被中斷就把變異留在 repo 裡，下一輪的「基線」是紅的，而每一條擾動照樣印 CAUGHT** — S（純 `intel/tools/`＝**零首屏位元組、零 `prototype/src`**）— 來源：**#185 實作輪當場踩到（2026-09-15 20:00 窗），不是推論**
...
- **2026-09-14（平台軌 · **20:00 窗** · **#193 當輪落地＝台帳的共用探針讀數從「手抄的數字」變成「求值、可對任意 git ref 重算的量」** ＋ 台帳輪替審「**後台**」10 模組全審 ＋ **回填 `ledger-card-sweep` 的 3 筆正向告警（兩筆改判）** ＋ 開卡 **#194** · claim `p-201030-b9d4`·心跳 20:10→20:45→21:15→21:40→收尾·進場鎖乾淨 false〔前景 18:18 釋放〕·**未奪鎖**·dark 10.6h<24h 非 catchup·`lead_track: games` 但前手 08:00 窗明文指派故不讓路·node 389→**390 全綠**·**負向擾動 13/13 CAUGHT**·**首屏逐位未動**·`sw.js` **不 bump**（v312））**
    - **① 閘門/進場**：三開關皆 true；`build_lock: false` → claim `p-201030-b9d4`（commit `4a24a4c` **當下即提交**）→ 停頓做非寫入讀取後重讀確認 token 仍在。前手 08:00 窗的明文指派是「台帳輪替到**後台**、實作首選 **#193**」⇒ **兩項全數執行＝跨輪指派連續第 N 輪**（14:00 窗因前景持鎖讓路，該指派順延到本輪）。
    - **② 本輪未做平台調研（據實）**：`db/platforms.json` **37 筆逾期 0**（最早 rainbet 09-18），為了有事做而重掃＝引擎明文禁止的空心跳，該檔一位元組未動。
    - **③ #193 落地**：`intel/tools/ledger-probe.js`＝可註冊探針表，**22 條**（後台 13／前端UI/UX 9），五條不變量全部可執行（值而非形容詞·兩段式口徑·`--ref` 重算·不是第二份真相·每條都有活見證者），新增 **`scope`（量程）必填**。鎖 `platform/ledger-probe-fail-closed`。逐條見卡 #193 的落地紀錄。
    - **⭐ ④ `--ref` 當場獨立複現了 08:00 窗的兩筆判定**：回到 09-08 的 `548fef8` 用同一把尺重算，`ui.comingSoon(` 得 **28/27**、`HL.gameAxes` 得 **12** ⇒ 坐實「這是**當天就錯**，不是後來漂移」。**沒有這個能力，兩者在台帳上完全同形，而它們該做的事相反**（前者改記法、後者查改動）。
    - **⭐ ⑤ 本輪最貴的發現：我自己寫的 22 條見證者，第一跑有 10 條是空的。** 而且空法五花八門、每一種都看起來很正常：`isPF` → `isPFX`（**改完仍含有原字串**，同一個坑在 `pushState` 上又踩一次）／見證者落在**被排除的定義處**（改 `money.js` 動不了「外部讀者」）／改到的是 **raw 那一半**而斷言比的是 effective／**量檔名**的探針改內容沒用（要能新增檔案 ⇒ overlay 的 `list()` 也得可疊）／見證者**把探針弄壞**而不是改值（非法 JSON ⇒「值變了」與「探針死了」同形）。⇒ 這 10 條全部是**斷言在場、性質不成立**，靜態全綠時一個字都看不出來。
    - **⭐ ⑥ 負向擾動又開出兩個真洞，兩個都是「防空心的保險架在沒有漏掉的那一段量程裡」（§4 形狀⑦-(h) 的第四、第五次重演）**：(a) **P1 首版 MISSED**——把 `witness` 從必填欄位清單拿掉不會紅，因為**下一行 `spec.witness.mutate` 會先 TypeError**＝那條性質靠另一條路**碰巧**成立；(b) **P9 首版 MISSED**——把 `hits()` 的 code 直接回 raw（註解又被算成呼叫點＝08-31 把 7 讀成 8 的那個錯），而 (b)「兩段式有見證者」**照樣全綠**，因為見證者還有別條路。⇒ 補 **(b2) 指名活見證者 ＋ (b3) 合成輸入打純函式**。
    - **⑦ 台帳輪替「後台」10 模組全審（全庫最舊＝09-11）·十格狀態全部維持**，但**兩筆待了七～十二輪的定值離開了，而且都是真的長大**：延遲載入清單 **20 → 22 列**（09-13 兩款新遊戲）、同仁放置區入口 **1 → 2**（`34475a2` 收編了那筆孤兒 WIP ⇒ 09-11 記的「HEAD 仍 1 筆」已過期）。**這是把讀數變成可求值的量之後第一件看得見的事**——舊寫法下它們會被抄成同一個數字再活好幾輪。
    - **⑧ `ledger-card-sweep` 3 筆正向告警逐筆處置（SKILL 第 2 步要求先回填再輪替），結果三種都不一樣**：**①〈前端UI/UX·網址作為地址〉absent → present**（#181 今日落地：`core/route.js` 雙向編解碼登記簿、VIEWS 12 個去處全註冊、`game`/`arena` 各自帶參數、`popstate`/開機/換頁共用同一個 decode、`shareUrl()` 改向 `HL.route.urlFor` 求值＝6 顆分享鈕一處通吃；⚠️ 返回鍵與真機分享記 **UNVERIFIED**，排程輪拿不到 dev server）；**②〈金流·交易狀態面〉absent → partial**（#174 今日落地：提款寫 `eta`、狀態是**算**出來的不是存旗標、時效唯一真相仍是 #63 的表；**但 `TXN_KINDS` 仍無 `states` 維度、deposit/p2p 至今沒有生命週期** ⇒ 容器沒長出來，落地的是提款那一條路）；**③〈金流·提款審核佇列〉維持 absent 不改判**——#174 關掉的是隔壁那一格的缺口，本格是「人工裁量」那一半（`CONTROL.avoid`）＝**一筆正確的告警、但不該改判**的實例。
    - **⑨ 另補一筆 sweep 抓不到、但同型的失真**：〈資安·帳戶安全自助中心〉的 evidence 寫「落地閘仍為首屏預算（餘裕 33 bytes）」，而 **#188 已完成**、#190 把餘裕推到 **48,907** ⇒ **阻塞前提六天前就失效了**。已回填（狀態維持 absent、**刻意不回填 `last_audited`**：本輪沒有重量測它的關鍵詞讀數，沿 08-13「只有真正重新量測過才改日期」的紀律）。
    - **⭐ ⑨b 台帳的反向錨當場咬了我一次（好事，連續第二輪）**：開完 #194 跑收尾全測時 `platform/ledger-card-anchor-bidirectional` **轉紅**——#194 宣告自己來自〈後台〉，而該分類沒有任何模組 evidence 回指它。已補「⇒ 開卡 #194」。**少了那條鎖，台帳會從今天起低報一個真缺口**（其餘六個分類仍在手抄）而畫面上一切正常。順帶回填〈大廳/遊戲牆〉的 #193，sweep 兩個方向現皆 0 筆。
    - **⑩ 驗證與收尾**：`node prototype/tests/run.js` **389 → 390 全綠**。**首屏逐位未動**（本輪 `prototype/` 只動 `tests/checks-platform.js`），`grep -c "tests/" prototype/sw.js` ＝ **0** ⇒ 沿 #135 判準 **`sw.js` 不 bump**（停在 v312）。本卡零 UI 表面、零 `src/` 改動 ⇒ 無線上複驗需求。收尾清 `build_lock=false`。
    - **⚠️ ⑪ 據實記一次我自己踩的工具鏈坑**：本輪中途下了一個 `python - <<'PYEOF'` 的指令去試探環境，**那正是 SKILL 最高優先規則禁止的 stdin heredoc**（2026-08-03「掛死 73 小時」事故的根因形狀）。它如預期地阻塞、被移到背景，我立即 `TaskStop` 掉、session 未受影響。**規則是對的，而且它擋下的是我自己**——記在這裡是因為下一個無記憶的 session 會跟我一樣手癢。
    - **下輪（09-15 08:00）建議**：台帳輪替到 **資料**（次舊＝09-13，與擴充性/資安同日）；實作首選 **#194**（把其餘六個分類的手抄讀數也遷進 `ledger-probe`，零首屏位元組、配方現成、本輪剛量到兩個見證者）。次選 #191，餘裕已足（46,643 bytes）但它吃 eager ⇒ 落地前先量一次。
