...
187. 🟦已批准待做 **我們的紅利條款有三個軸（流水倍數／可用範圍／壽命），而對手的同一張條款頁上有五個——少掉的那兩個（紅利期間的單注上限、贏額/提領上限）剛好是 §11 說「真金前必須重調」的那一類，而且它們是同一個 entry 物件上的第四、第五個欄位** — S–M（沿用 `badd()` 「授予當下求值一次、之後只讀」的既有紀律；**吃 eager 位元組，落地前必先量**）
...
- **2026-09-12（平台軌 · **14:00 窗** · 台帳輪替審「**功能**」18 模組全審 ＋ **查獲台帳輪替本身空轉了兩輪** ＋ **修好一把被 10 條鎖之一使用的量測尺** ＋ 開卡 **#185** ＋ commit `b586b1b` ＋ 一條新鎖 · claim `p-140930-a7d2`·心跳 14:09→14:42→15:25→收尾·進場鎖乾淨 false〔維護軌 09-12 12:00 窗釋放〕·**未奪鎖**）**
    - **① 閘門/進場**：`loop_enabled`／`platform_track_enabled`／`auto_implement` 皆 true；`build_lock: false` → claim（commit `02d6ece` **當下就做**）→ 停頓後重讀確認 token 仍在＝claim 成立。`last_platform_run_at` 09-12T09:10 ⇒ dark **5.0h < 24h＝非 catchup**；`lead_track=games` 本可讓路，但前手 08:00 窗明文指派「下輪台帳輪替至 **功能**」⇒ **做而不讓路**。船長「待處理」逐條讀過：**需裁決 0 項**、**無平台軌新指派**。`ledger-card-sweep` 進場**正向 0／反向 0**。
    - **② 取材**：`platforms.json` 到期 **0/33**（最早 cybet 09-13、rainbet 09-18）⇒ **未提前取湊數、該檔一位元組未動**（`ban_busywork_heartbeat`）。本輪新知識全部來自對自己表面的機械複驗。
    - **③ ⭐ 第一個發現：台帳輪替這件事，在「功能」這一格空轉了兩輪。** 對 18 筆 evidence 取 `sha1`（去掉「（前次）」尾巴、把判定字眼正規化）發現 **17 筆共用同一段文字**——那一段量的是〔遊戲引擎與聚合〕（`HL.games.register` 24 呼叫點／MANIFEST 20 檔／registry.json／`HL.fair` 25 檔），卻被逐字貼在 VIP／排行榜／通知／支援中心／收集套組／社群共決…身上。回溯 git：09-03 之前 **distinct=18**（逐模組各自的尺），**09-04 掉到 4／最大共用 15**，**09-06 掉到 2／最大共用 17**。⇒ 那兩輪寫下的「機械覆核**零漂移**」「連續第六輪**逐位相同**」在字面上**恆真**——它每次比對的是同一句話。**其餘 7 個分類全部 distinct=n、largest=1**＝這是「功能」這一格的個案，不是通病。
    - **④ ⭐ 第二個發現（本輪的重點）：一條正則字面量，讓全庫的「單一真相」看不見 89.3% 的 `core/reports.js`。** `tests/registry-probe.js` 的 `nonCodeMask` 是單趟字元狀態機，認得 `//`／區塊註解／`'…'`／`"…"`，**不認得正則字面量** ⇒ `core/reports.js:70` 的 `var NEEDS_QUOTE = /[",\n]/;`（CSV 跳脫用）那個 `"` 被當成字串開頭，狀態從此錯位，**33,907 字元中 30,281（89.3%）被標成「字串」**。同一條 `NEEDS_QUOTE` 也在 `core/betlog.js:52`（64.8% 失明）、`i18n/en.js` 78.4%。
    - **⑤ ⭐ 它吃掉的東西，正好是我們拿去寫台帳的那個數字**：`core/reports.js:730` 的 `HL.achievements.register({ id: "rpt-first-export" … })` 是 **#114**（`5ea694b`，2026-08-21，卡名逐字寫著「成就牆的註冊出口第一次有外部呼叫者 —— **4 個**既有功能各掛一枚徽章」）落地的真呼叫，**上一行就是註解「#114 成就徽章牆的外部註冊者」**。而 **08-31 20:00 窗**把篩子改成「只認呼叫、不認提及」時，**用這把已經錯位的尺**去複查，讀到 `kind=string`，於是把它從名單刪掉、記成 **3**，並把「`core/reports.js` **從未註冊過任何成就**」寫進 `registry-probe.js` 檔頭、`platform/registry-sites-code-only` 的檔頭、台帳 evidence 與 CONTROL 船長區。⇒ **用一把壞掉的尺去修另一把尺，然後把結論寫成四處事實。**
    - **⑥ ⭐⭐ 而正確答案一直就在 repo 裡，還是綠的：`platform/achievements-external-registrars`（`checks-platform.js:3627`）的 `NEW_BADGE_FILES` 從 #114 落地當天起就逐字列著 `reports.js`**，用它自己的 `noComments` 掃描器每一輪斷言「這四支都必須是外部註冊者」——**12 天來每一輪都通過**。⇒ 同一個 repo 同時持有兩個相反的答案、其中一個還被機械強制執行，而**沒有任何東西把它們對起來**，因為它們活在兩套量測系統裡（與維護軌 09-12 00:00 窗 U38 的「兩份台帳互不查對」同構）。
    - **⑦ 修法**：`nonCodeMask` 補**正則字面量**（新 mask 值 `3`，字元類 `[…]` 內的 `/` 不終止、跳脫吃兩格、旗標一併吃；`/` 是正則還是除號由「最後一個有意義的程式碼字元＋關鍵字表」判）＋**第二道保險**：`'…'`／`"…"` **在換行處強制收束**（ES5 字串不能跨實體換行）⇒ 未來任何誤判最多污染一行，不會再滾成整份檔案。修完實測：`achievements` 外部註冊者 **3 → 4**（activity／challenges／**reports**／responsible），`reports.js`／`betlog.js` 的長段錯位歸零，全庫**跨行「字串/正則」區段 0 段**（122 支檔）。
    - **⑧ 新鎖 `platform/code-mask-regex-aware`，而它的主斷言刻意是結構的、不是列舉的**：上一條鎖（`registry-sites-code-only`）敗在「fixture 列舉了三種提及形狀（行註解／區塊註解／字串），第四種形狀從旁邊走過去」＝**CLAUDE.md §4 形狀⑦ 的第七種：斷言列舉了想得到的形狀**。所以本鎖 (c) 只問一件從語言本身推得出來的事——**任何被標成字串/正則的區段都不得含裸換行**（事故當下 reports.js 那段橫跨數百行，這條會直接指著它），**不需要預先知道「正則」這個形狀存在**。另有 (a) 六種正則形狀的 fixture、(b1–b5) 五個反向錨（除號不得當正則／不得恆判 code／regex 分支不得是死碼／換行收束）、(c2) 防空綠的樣本量下限、(d) 活體錨 ＋ **(d2) 行為級**（vm 沙箱 boot 首屏核心，斷言登記簿 23 枚徽章裡真的有 `rpt-first-export`）。
    - **⑨ ⭐ 負向擾動 15/15 CAUGHT——而首版是 6/12，六條 MISSED 全是我自己的 fixture 太弱**：(i) 把真呼叫放在正則的**下一行**，於是「正則提早收尾」型的錯誤被我自己那道**換行收束保險吸收掉**，fixture 分辨不出它宣稱的性質（P2／P6／P7 三條同一個病）；(ii) `void 0 && HL.achievements.register(…)` **形狀還在**，逐字型斷言全綠（§4 形狀⑦(b) 被短路，P9）。⇒ 改法：**真呼叫一律排在同一行的正則之後**；活體錨補上 **(d2) 行為級**。這與維護軌 12:00 窗記的第六種漏法（樣本小到分辨不出宣稱的性質）是同一個道理，只是這次現形在 fixture 的**版面**上。
    - **⑩ 台帳輪替「功能」18 模組全審（逐模組各自的尺，不再共用一段）**：判定**零改判**，但記下 **4 筆真漂移**，全部被前兩輪的複本蓋住——① `prototype/games/registry.json` **HEAD 1 → 2 筆**（`34475a2`，09-11 遊戲軌「收編 Slot Engine PoC（iframe 沙箱 + HL Bridge v1）」＝**目標 2 路線 C 的第一個實例已經在 HEAD 上**，而台帳仍照抄「HEAD 1 筆／工作區 2 筆為他人 WIP」）；② `registry-probe.scan()` 註冊表 **10 → 11**（`cashier`，#82）；③ 成就牆外部註冊者 **3 → 4**（見⑤）；④ 順帶更正 09-01 自己寫錯的一句——「`FANOUT_ROSTER`（22 人名單）」實為 **20**，22 是 `FANOUT_MODS`+edge 的下游模組數，**兩個數字被混進同一句話**。
    - **⑪ 開卡 #185（🟦已批准待做）**：修尺時連帶普查發現**第二把尺同一個盲點**——`checks-platform.js:9273` 的 `stripStringLiterals()` 也不認正則，`stripComments()` 後仍有 **142 段跨行**、落在 4 支檔（`betlog.js`／`reports.js` ＋ 路線 C 帶進來的 `pixi.min.js`／`spine-pixi-v8.min.js`）。它的**存在理由**正是 §4 形狀⑦(e)「擋字面在檔內但求值沒發生」，錯位時會把真實程式碼整段丟掉 ⇒ 凡「**這支檔不得出現 X**」型斷言在那兩支檔上**會靜默通過**，而 #181 的 URL 路由普查掃的正是全 `src/`。無首屏位元組、無 preview 需求 ⇒ **不卡 #118**。
    - **⑫ 驗證與收尾**：`node prototype/tests/run.js` **366 → 367 全綠**；`ledger-card-sweep` 收尾**正向 0／反向 0**（⚠️ 中途一度反向 3 筆——我重寫 evidence 時把 #106／#123／#175 的卡錨一起換掉了，**是那條反向錨鎖當場抓到的**，已逐筆還原）。**只動 `prototype/tests/` 與 `intel/`，`prototype/src` 一位元組未碰** ⇒ **首屏餘裕逐位未動 289 bytes／92 支 script**、**`sw.js` 不 bump**（比照 08-30 E5、09-12 00:00 先例）。
    - **⑬ 已知限制**：① 排程輪 `preview_start` 仍被環境拒絕（unattended）⇒ 本輪無 preview，但本輪**沒有任何玩家可見變更**，不存在需要目視的東西；② `nonCodeMask` 的正則/除號判斷是**啟發式**（`}` 之後一律判為正則），兩個方向都可能猜錯——這正是 (c) 那條結構不變量要守的，不是靠判斷本身保證；③ #185 未落地（`max_implement_per_run: 1`）。
    - **⑭ 下輪（平台軌 20:00）**：台帳輪替至 **活動**；`platforms.json` 屆時 cybet 到期（09-13），可順手刷。**本輪指名的第一順位＝#185**（同源缺陷、零首屏、不卡 #118）。