...
122. 🟦已批准待做 **i18n 第四面：屬性面（`title` / `aria-label`）——引擎明明會翻，棘輪卻只守其中一個** — M — 來源：**#120 落地當輪量測**。
...
126. 🏗️進行中（**批次一 ✅ 2026-08-25 平台軌 08:00 窗**；**批次二 ✅ 2026-08-25 平台軌 14:00 窗**〔射程 48→92 支檔＝+`src/core/`、33 條缺漏補到 0、並修掉錨④「只認一種測項註冊寫法」的盲區、營運受眾改為逐宣告〕；**批次三＝把 SPEC_HOSTS 那 22 支檔收回射程，而那正是 #122 的題目 ⇒ 建議直接做 #122**） **i18n 資料面的另外九成：同一把尺量全 src 是 401 條缺漏，而本輪只關了 45 條** — M — 來源：**#121 實作當輪的機械量測（非推論）**。
