...
- **2026-09-06（平台軌 · **14:00 窗** · 台帳輪替審「**金流**」6→**7** 模組（新增一格）＋到期複查 tier-1/2 兩站 bet365／Rollbit ＋**新開取材維度 15**＋開卡 **#174**＋一條零首屏位元組的止血鎖）**
    - **① 進場**：`build_lock` 乾淨 `false`（維護軌 09-06 12:00 `m-120946-a1f8` 已於 `70779ae` 釋放）→ claim `p-141200-c4e1`（commit `0b75734`）、**未奪鎖**。`last_platform_run_at` 09-06T09:05 ⇒ dark **5.1h < 24h**＝非 catchup。`lead_track: games`，但本輪有真研究工作（兩站到期在即）⇒ 依 SKILL 第 0 步「可讓路」非「必讓路」，**做而不讓**。
    - **② 進場量測**：`ledger-card-sweep` 正／反向皆 **0 筆**（台帳無「卡已落地卻沒回填」的失真）；`node prototype/tests/run.js` 進場 **342 全綠**；首屏 **1,638,181／1,638,400 bytes（餘裕 219B）、本地 90 支 script**＝與 09-06 08:00 窗**逐位相符**（本輪零 `prototype/src` 改動，讀數不該動，確認未動）。
    - **③ 取材（次日到期的 tier-1/2 兩筆提前取，取後 `next_due` 推到 09-20）**：`bet365(85, tier-1)`／`rollbit(76, tier-2)`。⚠️ **限制據實**：`help.rollbit.com` **403**、`aceodds.com` 直取**失敗（無輸出）**、`bet365.com` 官方 banking 頁**未嘗試**（歷輪皆 403 且需地區/登入）⇒ **兩份 dossier 的本輪段落全為二手**（一手直取成功的是 racingpost 與 thespike.gg 兩篇評測），已逐段標明；但**兩站的關鍵事實彼此獨立互證**同一組結論。
    - **④ ⭐ 本輪最重要的發現＝我們先做出一個承諾，再用自己的下一個動作當場否證它（台帳盲點第 11 例，與今早 08:00 窗同一天、同一個形狀）**：兩個到期站從**相反方向**指到同一格——**bet365**（受監管法幣）從**事前**答：逐**方式**的時效公開且差異化（Trustly/Apple Pay Instant、e-wallet 1–4h、卡 1–4h＋銀行 1–3 天），狀態逐字 **Pending／Processing／Completed** 在帳戶頁 Bank 區；**Rollbit**（crypto、主打即時）從**事後**答：一個**專屬 Withdrawals 頁**查狀態＋鏈上 TXID，時效綁**資產**（SOL/LTC 分鐘級 vs BTC/ETH 尖峰 60 分）。回頭量自己：`core/service-level.js:148` 的 `wd-sla-hours` 被 `app-shell.js:287-288` 印成「**預計到帳時間 48 小時內**」，而 `:322` 在**同一個 tick** 扣餘額、`pushDemoTxn("withdraw")`、toast「**已提款**」——**那 48 小時從來沒有發生過**；`walletTxns` 逐筆恰 `{kind, amount, bal, ts}`（**沒有狀態欄**）、交易語境 `pending|processing|審核中|處理中` **實質 0 命中**、`HL.sla.valueOf(` 全庫**恰 1 個呼叫點**＝那個印字的面板本身。⇒ 開卡 **#174**、新開**取材維度 15**、台帳「金流」新增一格判 `absent`。
    - **⑤ ⭐ 一個必須寫下來的更正（它讓這個缺口隱形了九輪）**：台帳「提款審核佇列」自 08-26 起把「交易狀態詞彙缺席」與自己**歸為同一個根**，而該格因 `CONTROL.avoid` 連九輪不開卡 ⇒ **狀態可見性跟著被一起 defer**。本輪拆開：(a)「**送去給人看**」＝人工裁量＝需牌照/後端/角色閘＝**該格判斷成立、維持 avoid、維持 absent 不改判**；(b)「**送出→依時效自動到帳→狀態可見**」＝**純時序、零人工、零牌照**，而且 `core/responsible.js:211-227`（限額調升冷卻的 `{value, pending:{value, at}}` ＋到期自動生效）**早就是在庫的同形制先例**。⇒ **「需牌照」只擋人工裁量那一半，不擋時序與狀態的可見性**——已寫進維度 15 的紀律與該格 evidence。
    - **⑥ 台帳輪替（金流 · 09-03 為全庫最舊）**：六格逐一機械複驗，**五格逐位未動**（收銀台 partial 連十輪、提款審核佇列 absent、玩家間交易 partial 連九輪、獎金/流水 partial、資金分倉 absent 零命中）；`錢包/帳本` 維持 `present` 但新記一條與覆蓋率正交的事實（`walletTxns` 四欄形狀 ⇒ 帳本在**資料形狀上**就無法表達「錢在路上」，非渲染沒印）。**新增第 7 格**〈交易狀態面／到帳事件〉判 `absent`（**不是 weak**：狀態、到帳事件、承諾的消費者**三樣全缺**）。
    - **⑦ 順帶回填 #82 的第二條設計依據（不開第二張卡）**：兩站互證「**時效/限額/費用是通道的屬性**」（bet365 綁方式、Rollbit 綁資產），而我方唯一的時效來源綁的是 VIP 段位、`FIAT_METHODS`/`CRYPTO_COINS` 逐欄實測只有 `{ic,n}`／`{code,net,ic}`。已寫進收銀台格 evidence；**不開卡**——出口同在首屏 `app-shell.js`，與 #82 同一個瓶頸，開了也是第九張堵在同一處的卡。
    - **⑧ 本輪落地物＝一條零首屏位元組的止血鎖**（住 `prototype/tests/`）：`platform/withdraw-eta-promise-ratchet`。(a) **正向對照/防空綠**：承諾必須真的還在（維度存在、`kind:"info"`、兩站別時效值全 > 0、面板確實印它）——承諾若消失，本棘輪該退役而不是繼續空綠；(b) **棘輪**：機制落地前，`wd-sla-hours` 的**非顯示消費者維持 0**、承諾面檔數不准再長；(c) **反向錨**：`walletTxns` 寫入端欄位一旦長出 `status`/`eta`＝機制開始落地 ⇒ 本鎖**主動轉紅**，逼迫回填收緊（承 T54「過期的哨比沒有哨更誤導」）。**負向擾動 5/5 CAUGHT**（見下）。
    - **⑨ 刻意不做（不半修）**：**#174 本輪不落地**——`pushDemoTxn`／`txnRow`／提款鈕**全在 `layout/app-shell.js`**（首屏 eager、餘裕 **219 bytes**），與 `#93／#118／#128／#148／#169／#171／#172／#173` 同一個瓶頸，**這是第八張堵在此處的卡**。據實標明阻塞而不硬塞。
    - **⑩ 驗證**：`node prototype/tests/run.js` **342 → 343 全綠**（+1 常駐鎖）；**負向擾動 5/5 CAUGHT**（P1 拿掉 `wd-sla-hours` 註冊／P2 把時效值改成 0／P3 移除面板那行「預計到帳時間」／P4 偽造第二個非顯示消費者／P5 在 `pushDemoTxn` 塞 `status` 欄；每例僅本鎖轉紅、還原回 343）。**零 `prototype/src` 改動 ⇒ 首屏讀數不動、`sw.js` `CACHE` 版號不 bump**（依 §10.1 第 3 條：bump 的條件是「動 `prototype/`〔執行期資產〕」，`prototype/tests/` 不被瀏覽器載入）。
    - **⑪ 收尾**：`platforms_researched` +2、`platform_cards_opened` **+1（#174）**、`platform_cards_implemented` +0（本輪落地的是止血鎖，非卡）；`consecutive_idle_rounds` 維持 0（本輪有真工作）。`build_lock` 清回 `false`。
