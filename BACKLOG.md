...
193. 🟦已批准待做 **台帳的共用探針讀數是手抄進 JSON 的，於是一筆抄錯會變成十筆抄錯、存活六天、而且沒有任何東西會發現——因為抄的是數字，對的卻是形容詞** — S–M（全在 `intel/tools/`＝**零首屏位元組、無 preview 需求**）— 來源：platform-modules 台帳〈前端UI/UX〉分類輪替審（2026-09-14 08:00 窗實測）
...
- **2026-09-13（平台軌 · **08:00 窗** · 台帳輪替審「**資安**」9 模組全審 ＋ **cybet 第二輪深挖推翻了自己的劇本** ＋ **#178 第一波當輪落地** ＋ 改寫鎖 1 ＋ 開卡 **#187／#188**）**
    - **① 閘門/進場**：三開關皆 true；`build_lock: false` → claim `p-081230-a7f1`（commit `6887e71` **當下即提交**）→ 停頓後重讀確認 token 仍在。`lead_track: games` 本可讓路，但前輪 20:00 窗明文指派兩項（台帳輪替至資安／cybet 09-13 到期判定）⇒ **做而不讓路**。`ledger-card-sweep` 正向 0／反向 0。
    - **② ⭐ 一個到期的觀察對象，推翻了我們自己三週前寫好的處置。** cybet 的 `saturation_watch` 逐字寫著「第二輪零增量 ⇒ 判 saturated ⇒ 屆時可直接汰除」。照做的結果是**增量不為零**——而它**沒有長出任何新機制**。差別只在鏡頭：08-14 問「這個站有哪些留存機制」（逐項答完、全部已覆蓋）；本輪問「**它那張條款頁上有哪幾個維度**」，同一批來源、同一個站，多出兩個真缺口：**紅利期間單注上限**（$5/spin）與**贏額/提領上限**（紅利額 200% 封頂 $2,000／免費旋轉封頂 $50／跑完流水後總提領封頂＝初始 10×、超出自動沒收）。一手來源＝該站自家手冊 `guidebook.cybet.com`。
      · 逐項對照：流水倍數 ✅（#74 依段位查表，比它的扁平 40× 更細）／期限 ✅（#71）／**逐遊戲貢獻率 ✅（#89 `HL.wagerScope`）＝它那條「自家 Originals 只算 5%」的同一個軸，一次正面確認容器選對了**／單注上限 ❌／贏額·提領上限 ❌ ⇒ 開卡 **#187**、`saturation_watch` 1/2 → **0/2 歸零、不汰除**。
      · ⚠️ 開卡時差點踩的**同名不同物**：`maxWin` 在本站只命中四款 slot 的 `CFG.maxWin`＝**遊戲內派彩數學上限**（進 RTP 模型），與「紅利條款的贏額上限」是兩件事。
      · ⭐ 這與 08-16「維度清單漏掉玩家保護」、09-12「族群級盲點（BitStarz）」**同形，但層級更小**：前兩次漏的是一整個表面／一整個族群，這次漏的是**同一份 dossier 內部的一個欄位層**。
    - **③ 實作＝#178 第一波（⬜待批准 → 🏗️進行中）**：`HL.rg` 長出 `suppressed(kind)` 述詞 ＋ `SUP_BY_PAUSE` **策略表（資料，不是寫死的條件式）**；閘接在送幣的**單一出口** `HL.bonus.add` 且**排在寫入之前** ⇒ 一處覆蓋 **17 檔 19 個送幣點**；容器外第 18 點 `core/faucet.js` 的閘放在 **`eligible()` 而非 `claim()`** ⇒ 藥丸與領取鈕一併收起、不留「按了沒反應」的鈕；不靜默吞掉＝通知中心紀錄 ＋ 當場 toast ＋ 三語（語言包非首屏 ⇒ i18n 零首屏位元組）。
      · ⭐ **本輪最想留下的一句**：`SUP_BY_PAUSE.cool = {}`＝**冷靜期刻意不抑制送幣**，因為它的承諾面逐字只說「期間將暫停下注，時間到自動解除」。**射程超過承諾，與射程不足於承諾，是同一種病**——兩者都讓畫面上的字與行為對不上。本庫過去只記過後者。
    - **④ 鎖＝改寫既有 `platform/rg-pause-scope-census`，不放寬**（§10.1）。⭐ **它自己先把我的修法判紅了**——(d) 那條「暫停狀態的消費者恆為 1 筆、而且不是閘」正是設計來要求「有人回來看」的。改寫：(b) 基準 17 逐位保留、語意換成「全部委託給單一出口」；(d) 消費者 1 → 3 筆；(e) faucet 條改釘「閘必須在 `eligible()` 裡」；**新增 (f)**（委託對象必須真的問：逐字守衛形狀＋剝字串版＋閘須排在 `save(KEY_B` 之前＋抑制分支須留通知與 toast）與 **(g) 行為級**（抽出真正的策略表與述詞、注入假 `status()` 直接跑）。**測項數不變＝改寫錨群非加鎖**，node 370 全綠。
    - **⑤ 負向擾動 18/18 CAUGHT——首批 16/18，而 MISSED 的那條是我自己的擾動寫錯了。** P18 要證「新增一個沒有閘的送幣表面會被抓到」，我卻把新呼叫加進**本來就在名單上的** `core/luckyspin.js` ⇒ 兩個計數都不會變、**擾動沒造出差異**（它被別的鎖抓到，但不是本鎖）。改打一支不在名單上且非 eager 的檔（`views/instant-keno.js`）後 CAUGHT。⇒ 08-18「沒抓到與沒擾動同形」的新變形：**射程對了不代表擾動有效——它可能打在同一個桶裡。**
    - **⑥ 台帳（資安 9/9）**：〈暫停狀態的全站射程〉weak → **partial**（關的是業界五面中的第 2＋4 面；行銷通訊與招攬 CTA 仍全開 ⇒ 不是 present）。另更正兩把尺：〈真/假站軸〉84→**85 行**，逐版追到 `core/demo-tools.js`＝**09-11 自家 #182 加的閘**（⭐ 09-04 立的「逐位相同只有真的重跑過才能寫」這次真的擋住了一次抄襲）；〈RBAC〉的量測只看 `docs/*.sql` ⇒ **對前端半邊結構性失明**（#182 讓 `HL.rbac` 消費者 1→2、站上真的有一條操作軌跡了，而 evidence 仍寫「零稽核軌跡」），判定維持 partial 但量測法兩側並列。另對〈風控/反詐欺〉那把「連十四輪 0」的尺記一筆存疑：它只認五個英文識別字，本站新長出來的防線一個都不會命中它。
    - **⑦ ⚠️ 首屏 +947 bytes，餘裕 980 → 33**（92 支不變）⇒ 開卡 **#188**：實測 92 支 eager 檔逐檔重複樣板前 18 名合計 **15,479B**，第一波＝27 檔各抄一份的 `t()` helper（1,820B，淨回收約 1,300B）；**據實分級**其中約 4KB 是「無打包工具」的架構代價、收不回來。
    - **⑧ 收尾**：node **370 全綠**·sw v291→**v292**·`STATE` 三個計數各 +1／+2／+1、`consecutive_idle_rounds` 維持 0·逐檔 add·`build_lock` 清回 `false`。commit **`b684f60`**。**✅ 線上複驗（push 後當場做）**：未暫停 Δ+500（正向對照）／冷靜期 **不抑制**且錢照發 Δ+500（刻意的射程界線在線上成立）／自我排除 **Δ 0** 且通知中心留下 `🔒 帳戶暫停期間不發放獎勵` ×3／toast 中英各自由引擎真的翻出來／faucet `eligible` true→false 而 `low`／`offCooldown` 逐位不變（變化可歸因於閘）。**據實**：`comms`／`cta` 兩類目前零消費者 ⇒ 招攬文案仍對已排除玩家顯示（第二波）。**下輪（14:00）**：台帳輪替至 **資料**；當日無平台到期（下一個 rainbet 09-18）⇒ 可做廣度取材，**或直接吃 #188**。
