...
- **2026-09-02（平台軌 · **14:00 窗** · 建置輪＝台帳輪替審「**資料**」5 模組＋查獲並當輪修完「**報表讀不出來時，最外層那句話是「成功」**」＋開 #159/#160·claim `p-140930-7c1d`·心跳 14:09→15:22·進場鎖乾淨 false 未奪鎖）**
    - **① 進場**：`build_lock` 乾淨 `false`（維護軌 09-02 12:00 窗 `m-120900-8e2c` 已於 commit `3240218` 釋放）→ claim `p-140930-7c1d` → **當下即單檔 commit（`acf0b84`）** → 重讀確認 token 仍在＝claim 成立、未奪鎖。`last_platform_run_at` 09-02T09:42（dark 4.5h < 24h ⇒ 非 catchup）；`lead_track: games` 允許讓路，但本輪有真工作故不讓路。
    - **② 台帳自查**：`ledger-card-sweep` 正向 0 筆／反向 0 筆告警（進場與收尾雙向皆 0）。
    - **③ ⭐ 本輪的缺陷：失敗被壓成一個「看起來很正常的合法狀態」。** `core/reports.js` 的 `rowsOf` 用 `catch (e) { return []; }` 把「`rows()` 拋錯」壓成與「真的沒有資料」**完全同形** ⇒ ① 中心頁寫「這張報表目前沒有資料。」（玩家有 500 局注單也一樣）；② `csvOf` 回**只有表頭**的字串，而它**非空** ⇒ `download()` 的 `if (!text) return false` 判不出來、**真的寫出一個空檔**；③ toast 報「**已匯出 CSV**」。catch 不留痕、**console 全乾淨**。⇒ §4「修一半而看不出來」家族的新形狀：**「沒有資料」是每張報表的日常，所以錯誤偽裝成它就完全隱形**。
    - **④ ⭐ 第二個消費者是 i18n 棘輪替我抓到的（本輪最該記住的一點）**：修完中心頁後 `platform/i18n-key-ratchet` 轉紅、指向 `core/betlog.js:247`——注單面板**也**呼叫同一個 `HL.reports.download`，失敗訊息還在指定原因（「瀏覽器不支援」）。⇒ **若只修中心頁，就是字面上的「修一半」，而且沒有任何一條「匯出正確性」的鎖會抓到**。已一併修，並在新鎖裡加**量程錨**（比照 08-02「尺的量程漏了一段」）：直接數 `HL.reports.download` 的消費者、要求 ≥2 且每個消費者都必須帶那句不指定原因的失敗訊息。
    - **⑤ 修法**：`rowsOf(id, f, ctx, out)` 第四參數逐次回報 `out.err`（不留黏著狀態＝不製造第二份真相）；`csvOf` 在 err 時連表頭都不給 ⇒ download 必然寫不出檔；中心頁把兩態分開講（新增「這張報表讀取時出錯…」，且「報表列數」失敗時顯示 `—` 而非照報 0）；兩個消費者的失敗 toast 一律**不指定原因**（`匯出失敗（未寫出檔案）`）。
    - **⑥ 收緊既有鎖（§10.1 紀律）**：`reports/rows-failure-is-empty` 原本斷言「拋錯的報表**仍應給得出表頭**」——**那句斷言把缺陷寫成了規格**。已改為「連表頭都不給」+ 反向錨（真的沒有資料仍給表頭 ⇒ 兩態輸出必須不同）；守的不變量沒變（一張壞不炸全站）。新增常駐鎖 `platform/reports-error-not-empty`（行為級走 shim 真檔真註冊 + 源碼級守 UI/訊息/量程）。node **326→327 全綠**，**負向擾動 11/11 CAUGHT**（含 P8「一律報錯」的假修法、P9 量程縮回只認 reports.js、P11 只修一半）。
    - **⑦ 首屏逐位不變（1638386B/餘裕 14B·90 支 script）**：修法成本由**就地誠實回收**支付——`reports.js` 的 12+3 個 `t("X","X")` 折疊回收 629B，reports.js 淨 +6B、betlog.js 淨 −6B ⇒ **首屏淨 0**。sw v260→v261。
    - **⑧ ⭐ 順手做的全首屏量測，結果比本輪的修法更重要**：首屏 90 支 eager script 中「兩引數逐字相同」的 `t()` 呼叫點 **479 個、散 28 支檔、合計 14,659 bytes（14.3KB）**——**是現有餘裕（14B）的 1000 倍以上**，且純字面折疊、不搬檔、**headless 可驗**。⇒ 開卡 **#160**，並在 CONTROL 已回應區據實回覆船長：**#118 不再是唯一解法**。（界線：`k ≠ d` 的合法 fallback 有 17 個，受 #129 棘輪管，不得動。）
    - **⑨ 取材（刻意配對本輪分類）**：新增 sourcing 常規維度 **9「玩家資料自主權／可攜性」**——機械實測 36 份 dossier 真實命中數 **0**（3 份寬鬆命中逐筆讀完全是 "bet slip" 同形詞）；首次取到會改設計的形制＝**Stake 沒有「一份對帳單」**（Deposits／Withdrawals 分頁各一顆 Export CSV 要匯兩次自己併、注單是另一套 JSON archive）⇒ 修正 #151 兩處設計假設。另**一手複核更正本庫一筆 tier-1 名次口徑**：rainbet 原記「SimilarWeb Casinos 類別第 1」，一手頁（2026-07）實為**第 7**，正確讀法是「crypto 子集第 1」；並記下該榜**混入支付商與社交遊戲**的口徑陷阱。**不新增平台**（overdue 0/36）。
    - **⑩ 台帳「資料」5 模組**全部回填 `last_audited=09-02`：`報表與匯出` present（缺陷已修＋新記檔名身分缺口→#159）／`資料·分析` partial（08-31 敘述複驗不漂移＋補「已有的那一半，其讀不出來是靜默的」）／`玩家注單` partial（08-31 下修理由不變＋本輪修掉面板的假成功）／`本機存檔台帳` absent（新增「匯出過的檔算一個出口」→寫進 #159）／`期間軸與對帳單` absent（**開卡以來第一次有外部對照組**）。
    - **⑪ 下輪**：台帳輪替＝**擴充性（8 模組·08-31 最舊）**；`platforms.json` **09-03 兩站到期**（dorados／spinblitz，皆 tier-3）⇒ 下輪有真到期票。**建議優先序**：若船長批准 **#160** 則先做它（一輪解開首屏飢荒），否則續走「配對分類的取材 + 當輪修一條缺陷」的模式。
