...
178. 🏗️進行中（第一波 2026-09-13／**第二波 CTA 面 2026-09-14 `496d390`**；仍缺 comms）**玩家按下「永久自我排除」，畫面說「立即鎖定此帳戶，沒有任何解除方式」——然後他照樣可以轉每日轉盤、領抽獎、搶紅包雨，福利中心還繼續對他寫「N 項可領取」** — M（**第一波已於 2026-09-13 08:00 窗落地**：送幣單一出口 + 容器外 faucet 兩個閘已接上；第二波＝CTA／行銷通訊面，見下方「仍缺」）
    ...
    - 🚨 **本輪最重要的產出其實是鎖的一個漏法**：第一版斷言寫成 `/function hubSub\(/ && /hubSub\(it\)/`，
      ...
      修完後**負向擾動 8/8 CAUGHT**。
    - ⬜ **仍缺（第三波）**：**行銷通訊面（`comms`）**未做。`HL.notify.add(n)` 目前**沒有類別欄位**，
      要讓 37 個生產者各自自陳是不是行銷素材，是另一個容器題目 ⇒ **刻意不先把 `comms` 加進政策表**
      （加了就是一條沒人問的死規則，同白名單腐爛）。
    - **⬜ 仍缺（第二波，按價值排序）**：
