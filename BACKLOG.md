...
194. 🟦已批准待做 **治好了兩個分類的手抄讀數，另外六個分類今天還在抄** — S（純 `intel/tools/`＝**零首屏位元組、無 preview 需求**；每個分類 3–6 條探針）— 來源：platform-modules 台帳〈後台〉分類輪替審（2026-09-14 20:00 窗·#193 落地當下即量）
...
- **2026-09-13（平台軌 · **20:00 窗** · 台帳輪替審「**擴充性**」10 模組全審〔一格改判〕 ＋ **#187 第一波當輪落地** ＋ 新鎖 2〔含本專案第一條 `progress.js` 行為級鎖〕 ＋ 開卡 **#191／#192** · claim `p-201015-c5d9`·心跳 20:10→20:52→收尾·進場鎖乾淨 false·**未奪鎖**·dark 4.8h<24h 非 catchup）**
    - **① 閘門/進場**：三開關皆 true；`build_lock: false` → claim `p-201015-c5d9`（commit `e3e3aab` **當下即提交**）→ 停頓後重讀確認 token 仍在。`lead_track: games` 本可讓路，但前手 14:00 窗明文指派「台帳輪替到擴充性／實作首選 #187（其阻塞事實逐字寫著『在 #188 之前實質不可落地』，而 #188 第一波當天落地）」⇒ **做而不讓路**。
    - **② 本輪沒有做平台調研，據實記**：`db/platforms.json` 37 筆**逾期 0**（最近一筆 rainbet 09-18 才到期）。為了有事做而重掃媒體正是引擎明文禁止的空心跳。
    - **③ `ledger-card-sweep`**：正向 1 筆待人工確認、**判定為誤報**——〈資安／帳戶安全自助中心〉的 evidence 提到 `#188`，但那是引用它當**阻塞事實**（首屏預算），不是「這張卡關閉了這個缺口」。工具是關鍵詞啟發式，這正是它已知的誤報形狀。反向 0。
    - **④ ⭐ 首屏預算的匯率，比我們一直以為的貴一倍以上。** #187 第一波即使把長註解**全部**搬進 `intel/`（CLAUDE.md §10 的既有做法），仍要 **~2,400 bytes**——因為**中文註解一個字 3 bytes**，而本專案的設計說明幾乎全是中文。進場餘裕 1,932 ⇒ **它不合身**。⇒ 記一條可複用的估算法：**一張 S–M 卡的真實首屏成本應以 2–3KB 估，不是幾百 bytes**；「把註解搬去 intel」不是節流手段，是**必要條件**。
    - **⑤ funding 從哪來：本輪審「擴充性」時，在**要動的那同一支檔**裡查獲一個零讀取欄位。** `core/service-level.js` 的維度規格帶一個 `hint`，六個維度各寫了一段說明（合計 **818 bytes**），而 `grep -rn "\.hint"` 在 `src/`（對 sla）與 `tests/` **命中皆為 0**、`HL.sla.dims()` 也沒有任何呼叫者 ⇒ **寫入者有、讀取者零**，每個玩家每次開站都下載、沒有一個像素用到它。屬 T34（`HL.rg` hint 死欄位）同一家族的第二例，而**這一個更貴**。移除後剛好付得起 #187。六段原文全數保存於 `intel/bonus-max-bet-2026-09-13.md` §7。⚠️ **據實記張力**：刪它是「清死欄位」還是「把該接上的東西刪了」，取決於未來要不要讓 `HL.sla.open()` 顯示逐列說明——真要接時補回來是一次 `register()` 的事，但那一刻要重新付這 818 bytes。已寫進 CONTROL 讓船長知道。
    - **⑥ #187 第一波落地＝不新增容器**：`HL.sla` 的維度註冊表本來就站別×段位感知 ⇒ 直接加第六個維度 `bonus-max-bet`，**四個表面零改動**（服務水準面板逐列＋矩陣、`HL.econCfg` 自動收錄、逐段位單調性看守、站別方向看守）。entry 多一個選用欄位 `mb`，與 `req`(#74)／`sc`(#89)／`exp`(#71) 同一條紀律。語義刻意是**不擋下注**（擋是 `HL.rg` 的 `bet-single`，那是玩家自設閘）——只決定該注算不算流水，沿 #89 `weightFor` 回 0 的既有處置。
    - **⑦ 一個非顯然的決定：`NOCAP = 1e9` ＝ ∞ 的有限數值編碼。** 假站必須是「不限」（demo 不得比現況更嚴），但「不限」要寫進一張**逐位數值比較**的表：寫 `0` ⇒ `HL.econCfg` 會把 `live(200) > demo(0)` 誤判成違規（每次開儀表板一條假警報）；寫非數字 ⇒ `cmpPairs()` **整個維度跳過**＝**靜默失去看守**（比假警報糟）。保序有限編碼讓兩個看守都留著。`core/bonus-ttl.js` 當年踩同一題，選的是「放棄通則、自己補守」——兩種都行，但後者少一個「靠記得」的環節。
    - **⑧ ⭐ 本專案第一次對 `core/progress.js` 做行為級驗證。** 它開檔第一行就取 `HL.dom.el`、沒有 node 契約 ⇒ 歷來所有針對它的鎖都只能逐字 grep。本輪發現**它只需要 5 個 stub 命名空間**就跑得進 vm，於是 `platform/bonus-max-bet-never-blocks` 是構造情境**實跑真的 progress.js**。為什麼非這樣不可：逐字釘 `if (e.mb > 0 && bet > e.mb)` 擋不住 `if (false && …)`，更擋不住把 `bet` 換成 `w`——**後者只有特定連鎖情境才分岔**（頭筆只差 250、第二筆上限 300、單注 500），是最容易活下來的那種缺陷。⇒ 已開 **#192** 把配方推廣到另外四條金錢不變量（其中 **#74 `req` 凍結目前沒有任何測項直接驗**）。
    - **⑨ ⭐ 負向擾動 17/17，而首批兩條 MISSED 是同一個形狀：沙箱每次都把環境餵齊，於是「環境不齊時的退化分支」沒有見證者。**
      · **P8**：把求值出口的 `: 0` 改成 `: 300`（取不到 `HL.sla` 時偷偷套上限）⇒ **整組全綠**。而那個分支在真實開機序上**走得到**（`index.html` 裡 `progress.js` 排第 80、`service-level.js` 排第 169）。補 (H)〔無 sla 的沙箱〕後抓到。
      · **P17**：補上的 **(H) 自己第一版也是空的**——它傳 `maxBet=0`，使「sla 不存在」與「sla 回 0」**同形** ⇒ 把沙箱的 `noSla` 選項改成 no-op 照樣全綠。改傳 300 讓兩者可分辨才真的釘住。
      ⇒ CLAUDE.md §4 形狀⑦ 的新一種：**沙箱的「布景」本身也要有見證者，否則斷言認的是布景、不是行為。**
    - **⑩ 台帳輪替「擴充性」10 模組全審**：一格改判 **〈第二註冊路徑（放置區／Dev Kit）與中央掛鉤的接線〉`weak` → `partial`**——它 09-08 立格時的兩條依據（「放置區命中 0/1」「交給外部作者的三份契約對那個掛鉤命中皆為 0」）**本輪實測兩條都已不成立**（2 款 2/2 命中 `HL.liveStats.record`、四份契約 1/3/3/5 處皆非零、且已由 #180 的常駐鎖看守）＝「缺口被我們自己的卡關閉了、台帳沒回填」的又一例。**刻意不判 present**：路線圖的 C（iframe 沙箱）與 D（後端註冊＋審核）皆未做 ⇒ 放置區遊戲目前與平台同 origin、同全域 `window.HL`，一支同仁的遊戲可以讀寫 `HL.state`／`HL.bonus` 的任何東西——那是 partial 的天花板。其餘 9 格機械覆核零漂移（`HL.dock.register` 3 處／2 檔、`HL.ui` 19 個出口、`featureFlag|HL.flags` 0 命中、`liveStats.register|on(` 0 命中、`HL.release` 37 處、`HUB_GROUPS` 的 `when|order|enabled` 仍 0、`views/casino.js` 仍逐行硬寫），10/10 回填 `last_audited`。
    - **⑪ 台帳的反向錨當場咬了我一次（好事）**：我在重寫〈首屏載入架構〉evidence 時，把舊文裡「⇒ 開卡 #164」那一句連同摘要一起截掉了，`platform/ledger-card-anchor-bidirectional` 立刻轉紅並指名 `#164[✅完成]←「擴充性／金流」`。⇒ **重寫 evidence 時要保住卡號回指**，已補一行歷史錨並註明勿刪。
    - **⑫ 驗證與收尾**：`node prototype/tests/run.js` **376 → 378 全綠**；`sw` v296→**v297**；首屏 **1,599.64KB／92 支 script**，餘裕 1,932 → **369 bytes**（本輪淨支出 1,563＝#187 的 ~2,381 減去 `hint` 回收的 818）。⚠️ **餘裕 369 bytes ⇒ 下一輪任何 eager 改動都會撞 `platform/first-screen-budget`**；#191 已標明「在下一筆首屏額度進帳之前不可落地」，#192 則完全不吃首屏。收尾清 `build_lock=false`。