...
192. 🟦已批准待做 **`core/progress.js` 有四條「金錢語意」的不變量只靠逐字 grep 在守，而本輪剛證明它跑得進 vm 沙箱——把那四條升級成行為級** — S–M（**只動 `prototype/tests/`＝首屏逐位未動**）— 來源：#187 第一波立鎖時的發現
...
- **2026-09-12（平台軌 · **20:00 窗** · 台帳輪替審「**活動**」9 模組全審 ＋ **#176B 資格閘/權重當輪落地並解除 #118 阻塞** ＋ 新鎖 1 改寫 1 ＋ 開卡 **#186** ＋ **新入庫 BitStarz（全庫第 37 筆）** · claim `p-201000-c9f4`·心跳 20:10→20:50→21:20·進場鎖乾淨 false·**未奪鎖**·dark 4.5h<24h 非 catchup）**
    - **① 閘門/進場**：`loop_enabled`／`platform_track_enabled`／`auto_implement` 皆 true；`build_lock: false` → claim（commit `124f853` **當下即提交**，承 §10 快照的紀律）。`lead_track: games` 但**本輪有真工作故不讓路**。
    - **② 台帳自我查核**：`node intel/tools/ledger-card-sweep.js` ⇒ **正向 0 筆、反向 0 筆**待人工確認（第二輪保持乾淨）。另照 14:00 窗的教訓對**全八個分類**重跑 evidence `sha1` 去重：**8/8 皆 distinct**（活動 9/9），上一輪修掉的「17 筆同一段複本」沒有回來。
    - **③ 取材**：`platforms.json` 到期 **0/36**（最早 cybet 09-13）⇒ 不提前刷湊數。改做**廣度取材**，於是撞到本輪第一個發現 ⇩
    - **④ ⭐ 第一個發現：我們的觀察庫裡沒有 BitStarz——而它在 2026 年的多家榜單上是 crypto casino 第一。** 36 筆裡一筆都沒有，自 2014 年營運、500+ 幣種。回頭看取材方法，`db/sourcing-methods.md` 長期偏向 **Stake 類「原生遊戲＋social」那一支軌道**（Stake／Stake.us／Rollbit／Roobet／Shuffle／BC.Game…），於是**傳統型大型 crypto casino 整支沒進過視野**。⇒ 這與 2026-08-16「維度清單漏掉玩家保護、35 份 dossier 命中 0」是同一種形狀，只是這次漏的不是**維度**而是**族群**。**建議加進取材紀律的一句話**：除了問「這份維度清單漏了什麼」，再問「**這份名單偏向哪一種平台，因此系統性地看不見哪一種**」。已入庫＋建 dossier [intel/platforms/bitstarz.md](intel/platforms/bitstarz.md)。⚠️ 清單現 **37 筆**已超過 SKILL「精煉 ≤ 約 32」的指引 ⇒ **下一輪應先汰除**（cybet 09-13 到期即可做第二輪零增量判定）。
    - **⑤ ⭐ 它帶回來的素材恰好全落在本輪台帳輪替的那一格（活動）**：Slot Wars（週 $5,000）與 Table Wars（週 $10,000·**獎金無流水**·付獎深 40）是**兩條並行、分品類、週期性**的賽；Level Up Adventure 是**個人獎池、跟自己的進度條比**；而「每一則促銷都有完整、公開、**每次版式相同**的條款」是台帳〈活動條款面〉的**第三筆獨立佐證**（前兩筆＝Pragmatic Drops & Wins／Stake Daily Races）。**刻意只開一張卡**：多期並存要的是更大的重構、Level Up 與我方 #46 季票同型（不是新軸）、無流水獎金是經濟參數且真站已刻意收緊（§11）。
    - **⑥ ⭐ 第二個發現（本輪的重點）：那個「卡在 #118」的半張卡，其實已經不卡了——而它卡住的期間，賽事名稱一直在對玩家說謊。** #176 的 B 半（資格閘＋權重）自 09-07 起標著「吃首屏、卡在 #118」；遊戲軌 09-12 16:00 窗把餘裕從 289 還原到 **2,072 bytes** ⇒ **阻塞解除**。本輪落地：
      · **資格閘就是權重 0**——Pragmatic 的「eligible games」與 Stake 的「live dealer 約 50%」是同一條軸的兩端；分開做會立刻長出兩份真相。`core/tournament.js` 新增 `weigh()` 為唯一出口，向 **#89 `HL.wagerScope`** 求值，本檔不自刻權重表。**`HL.wagerScope` 的消費者 1 → 2**（上一輪把「全庫只有 1 個消費者」記成缺口，本輪還了它一個）。
      · **`NAMES` 由字串陣列改成 `{ n, s }` 描述子**——名字與它的範圍住同一筆 ⇒ 「名字說 Slots、引擎沒在濾」**在結構上寫不出來**。
      · **`scoreAxis` 第四軸 `sumMult`**（倍數總和，對標 Drops & Wins 逐字的 sum of win multipliers）。
      · 條款面：`SCOPE_RULE` 逐 preset 一句 ＋「合格遊戲」KV（值向 `wagerScope.labelOf` 求值）＋ `status().scope` 單一出口；i18n 各 +7 條。
    - **⑦ ⭐ 對 CLAUDE.md §4 形狀⑤（可切換性被條款面反鎖）的一個正面回答**：#176 診斷的是「`#85` 的可切換性至今 0 個使用者」。本輪**沒有**去找地方呼叫 `startNew({...})`，而是把 `scope` 接進**預設名稱表** ⇒ 它不需要任何呼叫點就有三個真實使用者。⚠️ 據實界定：`axis`／`groupBy` **依舊是 0 個顯式使用者**（`startNew` 帶 spec 的呼叫點仍為 0）。⇒ **要讓一個容器有人用，最可靠的做法是把它接進預設路徑，而不是等人來呼叫它。**
    - **⑧ ⭐ 新鎖刻意做成行為級，因為舊的那條反向錨示範了形狀⑦(a)**：`platform/tournament-terms-single-truth` 的 (d) 舊版用 `/eligibleGames|allowedGames|onlyGames|scopeOf|qualif/` 抓資格閘——而本輪真的長出來的閘叫 `scopeWeight`／`weigh`，**一個字都沒被抓到**（它認的是命名，不是概念）。改掃 `scopeWeight(` 只是換一個會被改名的識別字，而且 `if (false && scopeWeight(…))` 一樣掃得到。⇒ 新鎖 **`platform/tournament-scope-is-the-gate`** 在 node vm 裡拿**真的** score-axis ＋ **真的** wager-scope 把 `core/tournament.js` 跑起來，用「同一批注、換一個 scope、分數必須不同」證明閘真的在濾；(d) 改成守三件形狀上的事（名字有範圍詞⇒必須帶 s／s 必須是真的 preset／s 必須有條款句）。
    - **⑨ ⭐ 鎖自己抓到我寫錯的不變量**：(e) 首版寫成 `名字有範圍詞 === 有 scope` 的**雙條件**，於是把「深夜極速賽＋standard 權重」判成缺陷。但**「有 scope 就必須寫進名字」不是不變量**——Stake Daily Races 的逐遊戲權重也不在名字裡，它在條款裡。⇒ 拆成方向相反的兩條：(e-i) 名字不得說謊、(e-ii) 範圍不得默默生效。**這是形狀⑦ 的反面：斷言比概念更強，一樣是壞的。**
    - **⑩ ⭐ 負向擾動逼出一個既有的射程漏洞**：P11「把 zh-Hans 的『倍數總和』整條刪掉」**連兩次 MISSED**。追下去發現——玩家在條款面看到的兩行 KV，值分別來自 `score-axis.js` 的 `axis.label` 與 `wager-scope.js` 的 `preset.label`，而**這兩支檔都在 `i18n-key-scan.js` 的 `SPEC_HOSTS` 清單上**，`platform/i18n-data-ratchet` 對它們**整支跳過**（合理：它們託管測項夾具）。⇒ 本輪用該掃描器自己的 `testSpecRegions()` 挖掉測項區塊後，只認「測項區塊以外的 label 欄」把這幾條救回來。**另外一件值得記的**：第一版用 `needsHans()` 當 zh-Hans 的守衛，實測它對「數／總／贏／額」四個字都學不到映射（一致性規則寧可漏判）⇒ `needsHans("倍數總和")===false`＝**一條永遠不會開火的斷言**。改用雙向棘輪（沒有 zh-Hans 條目的 label 恰 1 條＝刻意的「有效押注」繁簡同形），P11′′′ 與反向的 P20 都會紅。
    - **⑪ 負向擾動 20/20 CAUGHT**（P2 恆回 1／P3 倍數軸跟著縮放／P4 移除 sumMult 軸／P5 拿掉軸句／P6 拿掉範圍句／P7 名字宣告範圍卻無 s／P8 s 拼錯／P9 status 不交出 scope／P10 en 拿掉範圍句／P11′′′ zh-Hans 拿掉軸標籤／P12 rulesModal 不再讀 SCOPE_RULE／P13 權重寫死成第二份真相／P14 sumMult 不再擋 bet<=0／P15 自訂 name 默默繼承範圍／P16 把閘短路成 `if (false && …)`／P17 en 拿掉 wagerScope label／P19 preset 改名／P20 補一條繁簡同形的無用條目／P21 新增一條軸卻不補字典／P1′′ 兩道資格閘全拆）。⚠️ **另有 3 例是我自己 harness 的無效擾動、1 例錨點打錯字**，據實記下：P1／P1′ 只拆了兩道 `if (!ctx) return;` 的**第一道**（第二道照擋 ⇒ 性質未被破壞，不是漏抓）；P18 想打壞 label 抽取器，但 `\s*` 本來就吃換行 ⇒ 擾動無效；P11／P11′ 把簡體「数」打成繁體「數」，錨點自然打不中。**「沒抓到」與「沒擾動」同形——這條 08-18 的教訓本輪又應驗了四次。**
    - **⑫ 台帳輪替「活動」9 模組全審**：一格改判 **〈活動條款面／規則的單一真相〉`weak` → `partial`**（#176B 落地）；〈活動收尾面〉維持 `partial` 並**開卡 #186**（它的 evidence 自 09-04 起就逐字寫著這張卡的出口形狀，連續三輪未開）；〈遊戲內活動掛件〉維持 `absent` 但**更新阻塞事實**——#136 原卡寫「必須排在 #118 之後（餘裕 63 bytes）」，而本輪收尾餘裕 980 bytes ⇒ **首屏不再是它的阻塞點，真正的前置仍是 #149**。其餘 6 格一行帶過、9/9 回填 `last_audited`。
    - **⑬ 驗證與收尾**：`node prototype/tests/run.js` **368 → 369 全綠**；`sw` v288→**v289**；⚠️ **首屏餘裕 2,072 → 980 bytes**（92 支不變）——本輪花掉 1,092 bytes eager，而那正是遊戲軌 16:00 窗剛還回來的額度。按遊戲軌新的邊際成本（~90 bytes/款），**剩下的餘裕仍夠上架約 10 款遊戲**；但 #186 的登記簿約需 700–1,000 bytes，**落地前必須先量**。
    - **⑭ 已知限制**：排程輪 `preview_start` **第 11 次**被環境拒絕（unattended session）⇒ **無任何目視**。本輪改動含玩家可見面（規則 modal 多一行 KV、多一句範圍規則、三個賽事名稱改字），這些**只有 headless 證明**（DOM 結構未渲染、三語未目視）⇒ 標記 **UNVERIFIED**，請船長在真機開一次「🏆 錦標賽玩法」確認版面。
    - **⑮ 下輪（平台軌 09-13 08:00）**：台帳輪替至 **資安**；`platforms.json` 屆時 **cybet 到期**（09-13）＝它 `saturation_watch` 要求的「第二輪零增量」判定窗，可順手做並考慮汰除（清單 37 筆已超指引）。本輪指名的第二把尺（#185）仍在佇列、不卡首屏。
