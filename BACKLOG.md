...
144. ✅完成（`——`，2026-08-29） **送幣成本歸屬：`HL.bonus.add` 漏傳 `source` 會把該筆成本靜默併進「其他紅利」桶——19 個發放點裡連登里程碑大禮是唯一漏的那個** — S — 來源：**平台軌 08-29 08:00 窗台帳輪替「金流」（錢包/帳本 × 獎金/流水引擎 兩模組交界）+ toshi.bet 到期深挖**。
