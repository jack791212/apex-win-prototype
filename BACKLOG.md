...
178. ⬜待批准 **玩家按下「永久自我排除」，畫面說「立即鎖定此帳戶，沒有任何解除方式」——然後他照樣可以轉每日轉盤、領抽獎、搶紅包雨，福利中心還繼續對他寫「N 項可領取」** — M（送幣側接線動 `HL.bonus.add` 與各獎勵入口＝**全在首屏 eager**，餘裕 72 bytes ⇒ **卡在 #118／#169**；本輪已先落地零位元組的止血鎖）
...
- **2026-09-12（平台軌 · **08:00 窗** · 台帳輪替審「**金流**」7 模組全審 ＋ 開卡 **#183／#184** ＋ **實作 #82 並當輪完整落地** ＋ 一條新鎖 · claim `p-080915-c3e8`·心跳 08:09→08:52→收尾·進場鎖乾淨 false〔維護軌 09-12 00:00 窗釋放〕·**未奪鎖**）**
    - **① 閘門/進場**：`loop_enabled`／`platform_track_enabled`／`auto_implement` 皆 true；`build_lock: false` → claim（commit **當下就做**）→ 停頓後重讀確認 token 仍在＝claim 成立。`last_platform_run_at` 09-11T21:20 ⇒ dark **10.8h < 24h**＝**非 catchup**；`lead_track=games` 本可讓路，但前手 09-11 20:00 窗明文指派「下輪台帳輪替至 **金流**」⇒ **做而不讓路＝跨輪指派連續第十七輪**。船長「待處理」逐條讀過：**需裁決 0 項**、**無平台軌新指派**。
    - **② 台帳自我查核**：`node intel/tools/ledger-card-sweep.js` **正向 0／反向 0 告警**。
    - **③ 取材**：`platforms.json` 到期 **0/33**（最早一批 rainbet 09-18）⇒ **未提前取湊數、`platforms.json` 一位元組未動**（`ban_busywork_heartbeat`）。本輪的新知識全部來自**對自己表面的機械複驗**，見 ④。
    - **④ ⭐ 本輪的發現（「口徑量錯對象」家族的第二例，而且這次是我們自己的尺只量到一半）**：金流「支付/收銀台」這一格的機械複驗**連十輪**都是 `FIAT_METHODS`(`app-shell.js:91`)／`CRYPTO_COINS`(:92) 兩個常數陣列＝**儲值側**；而 **`renderWd()` 另外自帶第三份清單**——硬寫 `[["fiat","法幣（銀行）"],["crypto","加密貨幣"]]` ＋ 硬寫 placeholder `提款地址（USDT-TRC20）` ⇒ **玩家儲值進得了 BTC／ETH，卻只提得出 USDT-TRC20**，而「哪些通道能提款」在資料上根本無法表達。**十輪「逐位未動」每一輪都是真的、也每一輪都不完整**——與 09-12 00:00 維護軌 U38（審計空紅）同構＝**射程／量測窗排除了真相所在的位置**，只是這次現形在台帳的尺上。
    - **⑤ 落地 #82（當輪完整落地，非半修）**：`core/cashier.js`＝`HL.cashier` 通道註冊表（容器零內建／fail-closed／**預設全關**／唯讀純值副本／`flows`·`regions`·`currencies`·`networks` 述詞），`FIAT_METHODS`／`CRYPTO_COINS` **從 app-shell.js 消失**，**儲值側與提款側同時**改讀同一張表。零回歸以 node 重播首批註冊逐項比對證明（儲值兩列**逐位相同**）；提款側為**刻意的行為改變**（`fiat` 只剩銀行、`crypto` 由一種變三種）。額度依卡**委派 #70 `HL.rg`／#63 `HL.sla`**，`min/max` 只做述詞、首批一筆未填。
    - **⑥ 鎖與擾動**：新鎖 **`platform/cashier-registry`**（(A) 行為半**真的 require 進來跑**容器；(B) 形狀半釘住 UI 端的兩個消費者），node 362→**363 全綠**。⭐ **負向擾動 20/20 CAUGHT，但首批是 15/16**——MISSED 的 (P10) 又是我自己的鎖：(B4) 首版只要求「`renderWd` 體內出現 `HL.cashier.`」，把 kind 切換列改回硬寫之後**巢狀在 `drawForm` 裡的幣別查詢仍在**、斷言照樣命中＝**正向全綠而第二份清單長回來了**（§4 形狀⑦(c) **巢狀洩漏第二次應驗**）。改成兩份清單各釘一條＋反向擋住「kind 字串陣列字面量」，P10／P10b 雙雙 CAUGHT。（另有一條 (B4) 首版寫在 `stripStringLiterals` 版上而要打的字面正是字串 ⇒ **永遠紅**，一併更正——剝字串是雙面刃，斷言要打字串內容時只能掃 `stripComments` 版。）
    - **⑦ ⚠️ 首屏代價，據實寫在最前面**：新檔 2,502B ＋ `app-shell.js` 淨增 ＋ `index.html` 一行，扣掉本輪回收的 **531B 失效註解** ⇒ **餘裕 3,808 → 289 bytes**、scripts 91→**92**。**#160 在 09-11 剛解開的額度，到本輪已被三筆支出用掉九成以上** ⇒ 任何 eager 改動再度堵死。已開 **#184**（`index.html` 17.2KB 裡有 **11.2KB＝65%** 是註解，而同一份順序另有 20+ 條常駐鎖在真的把關；標 ⬜待批准，因為它拿文件就近性換位元組，請船長裁決），**#118 仍是結構解法**。
    - **⑧ 另開 #183（🟦已批准待做）**：實作 #82 時在同一支檔裡撞到——站上有**三份互不相認的幣別**：`money.currencies()`（宣告「各模式允許的幣別」卻**零消費者**）／`mock.currencies`（**五處**真消費、**從不問 money-mode** ⇒ 休閒模式下錢包照樣列 7 種加密幣）／`HL.cashier` 通道幣別（**USDT 儲值進得來，錢包裡沒有那一格**；LTC/TRX/XRP 有餘額格卻沒有任何儲值通道）。⇒ **「宣告了卻沒有消費者」家族第 3 筆**（前兩筆＝`wd-sla-hours`／`HL.rbac`）。
    - **⑨ 已知限制**：排程輪 `preview_start` **被環境拒絕**（unattended session）⇒ **無 preview 目視**，收銀台/提款面的實際渲染與三語譯文屬 **UNVERIFIED**。i18n 側順帶補了 `信用卡/超商代碼/銀行轉帳/提款地址` 四鍵；⚠️ **補 `銀行轉帳: 银行转账` 之後，棘輪的繁→簡字集變完備，當場揪出一條潛伏的真缺漏 `白銀`**（該掃描器自述「字集不完備 ⇒ 會低估」，本輪實證）。
    - **⑩ 收尾**：`sw` v285→**v286**；`STATE`：`platform_cards_opened` +2、`platform_cards_implemented` +1、`platforms_researched` **不加**（本輪零新平台調研）、`consecutive_idle_rounds` 維持 **0**。`build_lock` 清回 `false`。**下輪（平台軌 14:00）**：台帳輪替至 **功能**（09-06＝現全庫最舊）；`platforms.json` 下一批到期 rainbet 09-18。