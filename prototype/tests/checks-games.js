...
(function () {
  ...
  // ── 贏分分級 + 淨額 roll-up 鎖（修 game-feel #2 flat-feedback-no-tiering·龍虎鬥側）─────────
  //   與 games/sicbo/tiered-feedback 同一把尺（家族鎖）。舊版：結算只寫一行綠字＋單一 ax-green，
  //   同花和 51× 與龍/虎 2× 視覺重量相同、淨額一次跳到終值＝玩家無法感知「這注中得很大」。
  //   守：① 分級純函式門檻/單調/除零守衛＋與真 returnsOf 賠付對齊（suited 51×→epic、tie 9×→big、龍/虎 2×→普通）；
  //   ② roll-up 純函式分步/末步精確/單調；③ 源碼結構（贏支才路由 data-tier、淨額走 rollupValueAt、無殘留 money(r.net)）。
  selftest.register({
    id: "games/dragon-tiger/tiered-feedback", group: "games", env: "node", tier: "fast",
    title: "dragon-tiger：贏分分級（epic≥50×/mega≥15×/big≥5×·對齊 returnsOf：同花和 51×→epic/和 9×→big/龍虎 2×→普通）＋淨額分步 roll-up（末步精確·單調）＝修 game-feel #2 flat-feedback-no-tiering",
    run: function (t) {
      if (!mod || typeof mod.winTier !== "function" || typeof mod.winMult !== "function"
        || typeof mod.rollupSteps !== "function" || typeof mod.rollupStepMs !== "function" || typeof mod.rollupValueAt !== "function") {
        t.skip("模組未載入或未匯出分級/roll-up 純函式（table-dragon-tiger.js winTier/winMult/rollupSteps/rollupStepMs/rollupValueAt）"); return;
      }
      // ① 分級門檻邊界（staked=100；x=payout/staked）
      t.ok(mod.winTier(499, 100) === "", "x=4.99 應為普通贏（無分級），實 " + mod.winTier(499, 100));
      t.ok(mod.winTier(500, 100) === "big", "x=5 應為 big，實 " + mod.winTier(500, 100));
      ...
      t.ok(bad === 0, "winTier 非單調（payout 增大時分級倒退 " + bad + " 次）");
      // 與真實 returnsOf 賠付對齊（驗的即玩的）：單注 staked=100
      var suitedTie = mod.returnsOf({ winner: "tie", suited: true });   // 同花和 = 51×
      t.ok(mod.winTier(100 * suitedTie.suited, 100) === "epic", "同花和(51×) 應觸發 epic，實 " + mod.winTier(100 * suitedTie.suited, 100));
      var tieHit = mod.returnsOf({ winner: "tie", suited: false });     // 和 = 9×
      t.ok(mod.winTier(100 * tieHit.tie, 100) === "big", "和(9×) 應觸發 big，實 " + mod.winTier(100 * tieHit.tie, 100));
      var dragonWin = mod.returnsOf({ winner: "dragon", suited: false }); // 龍贏 = 2×
      t.ok(mod.winTier(100 * dragonWin.dragon, 100) === "", "龍/虎贏(2×) 應為普通贏（無分級）＝#2 修前後對照");
      // ② roll-up 純函式
      var N = mod.rollupSteps();
      t.ok(N >= 6, "roll-up 步數 " + N + " < 6＝幾乎一次跳號（退回無 roll-up）");
      t.ok(N === mod.ROLLUP_STEPS, "rollupSteps() 與 ROLLUP_STEPS 常數不一致");
