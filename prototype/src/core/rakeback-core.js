 ...
 *   基準＝#50 EDGE 表 23 款的平均莊優 CALIB_MEAN_EDGE_PCT = 2.0586%。
...
(function (global) {
  ...
  var EDGE_PCT = {
    demo: [0.223321, 0.355490, 0.487661, 0.620740, 0.797573],
    live: [0.043752, 0.065628, 0.087504, 0.110293, 0.132169]
  };

  /* 校準基準＝edge 表的**款數平均莊優**。這個常數不是裝飾：`rakeback/cost-neutral-*`
   * 兩鎖用「平均莊優 × 本表比例」對照舊制成本，**edge 表一長，平均就漂，兩鎖就會紅**。
   * 2026-09-13（#189·abyssal-surge 上架，23→24 款）：平均 2.0586 → 2.117833，本表兩列同乘 0.97203116。
   * 2026-09-13（emerald-sprite 上架，24→25 款）：平均 2.117833 → 2.181920，兩列再同乘 0.97062816
   *   （＝舊平均/新平均）⇒ **玩家實得返水逐位維持改表前的水準**
   * 2026-09-14（star-forge 上架，25→26 款）：平均 2.181920 → 2.232615，兩列再同乘 0.97729344。
   *   （這是重跑校準，不是放寬鎖；比例本身被等比縮小，成本中性因此才守得住）。
 * 2026-09-15（shadow-ritual 重平衡後登記 edge 2.93，26→27 款）：平均 2.232615 → 2.258444，兩列再同乘 0.98856317。
 *   ⚠️ 這一次不是新遊戲上架，是**一款一直在線的舊遊戲第一次有了可登記的 RTP**（DEBT S-slot-rtp 結案）。
 *   同一條規則照樣適用：EDGE 表的款數一變，均勻分布下的平均莊優就變，校準必須跟著重跑。 */
  var CALIB_MEAN_EDGE_PCT = 2.258444;  // 校準基準（#50 EDGE 表 27 款平均），改表時需重跑校準
