...
(function (global) {
  ...
  /* #106 說明中心（#72 容器）：推薦制有**三件玩家一定會問、而答案並不直覺**的事——
   *   ① 獎勵不是註冊就發，是**分階釋放**（好友玩到門檻才放一階）；
   *   ② 自己貼碼給自己**不算**（歸因層直接擋）；
   *   ③ 真站看得到邀請碼卻**領不到獎**——因為純前端沒有跨裝置通道可以見證「誰把誰帶進來」。
   * 第 ③ 點若不寫在說明中心，玩家只會看到一個永遠是空的好友清單而無從理解。
   * 各階數字一律由 `tiers()`（站別感知）當場求值，不手抄。 */
  if (HL.support && HL.support.register) {
    HL.support.register({
      id: "bonus/referral", cat: "bonus", order: 20,
      title: "推薦好友的獎勵怎麼發？",
      keys: ["推薦", "邀請", "好友", "邀請碼", "referral", "invite", "分階"],
      body: function () {
        var ts = tiers() || [];
        var rows = ts.map(function (x, i) {
          return "第 " + (i + 1) + " 階（好友累積 " + x.need + " 點成長進度）：你 +" + x.ref + "、好友 +" + x.ee;
        });
        var head = ts.length
          ? "推薦獎勵**分階釋放**，不是註冊就發——" + rows.join("；") + "。"
            + "門檻算的是好友的**累積成長進度（成本加權後的有效押注）**，不是名目押注金額；同一位好友的同一階只會發一次。"
          : "推薦階梯尚未載入。";
        var self = "自我推薦（把自己的碼填給自己）在歸因當下就會被擋下，不會產生任何獎勵。"
                 + "每個帳號只能有一位推薦人，且**寫入後不可更改**。";
        var live = "";
        if (!rewardsEnabled()) {
          live = "⚠️ 真站目前**不發放**推薦獎勵：純前端沒有任何跨裝置通道，你的裝置無法證明「某位新玩家是你帶進來的」，"
               + "而若照著客端自行聲稱就發獎，等於任何人都能宣稱自己帶進一百個人＝無限印幣。"
               + "因此本站選擇據實不供應，但**歸因照記**——接上伺服器見證者的那天，既有紀錄可以直接補發。"
               + "邀請碼與連結照樣可以分享。";
        }
        return [head, self, live].filter(Boolean).join(" ");
      },
      action: { label: "開啟推薦面板", run: function () { open(); } }
    });
  }
})(window);
