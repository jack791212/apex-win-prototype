...
(function (global) {
  ...
  /* #176 規則面的單一真相：「憑什麼排名」與「怎麼分」以前是四句散文，而 #85 已經把計分軸與
   *   分組做成**可切換的資料**（`startNew({ axis, groupBy })`）⇒ 換一次賽制，同一個 modal 裡
   *   上一行 KV 會誠實地說「最高倍數」、下一行散文卻仍說「有效押注即累積積分」＝自我矛盾。
   *   （這也正是那個可切換性至今在全庫 0 個呼叫點的原因之一：沒人敢換。）
   * 修法＝依軸/分組**選一條整句片語**，而不是串接數值。
   *   ⚠️ P3 契約：翻譯只發生在「整個文字節點等於一條 key」⇒ 這些句子必須整句成節點、不得串接
   *   當下數值（否則英文/简中會原樣顯示繁中）。付獎曲線那句同理維持整句，其百分比與
   *   `HL.tournament.SPLIT` 的一致性改由常駐鎖 `platform/tournament-terms-single-truth` 逐項驗算
   *   ——手抄的數字不再是「沒人看的第二份真相」，而是**被鎖住的**第二份真相。
   * 新增一條計分軸卻忘了補句子時，同一條鎖會轉紅（不會靜默沿用流水軸的說法）。 */
  var AXIS_RULE = {
