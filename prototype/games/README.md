...
### 1-3　完成後交付（重要：只交 game.js）

測好後，**只要把 `game.js` 傳給平台方**（有圖再附一個 `assets/` 資料夾）——
`index.html` / `hl-stub.js` / `styles.css` 是測試台，**不用回傳**。
同時告訴平台方兩個標籤：你的 **暱稱**、想要的 **遊戲代號**（英數與 `-`，例 `space-dice`）。

> 換句話說：你交回的**不是**一包 dev-kit 資料夾，而是「**一支 `game.js`（＋選用的 `assets/`）＋ 暱稱 ＋ 遊戲代號**」。平台方會幫你把它放進 `games/<暱稱>/<代號>/`。

---

## 2. 平台方：怎麼上架（白話 3 步）

1. 用同仁給的 **暱稱 / 遊戲代號** 建一個資料夾，把他那支 `game.js` 放進去（有 `assets/` 就一起）：
   `prototype/games/<暱稱>/<遊戲代號>/game.js`
